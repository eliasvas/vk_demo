...
void CFG::build_immediate_dominators() {
  // Traverse the post-order in reverse and build up the immediate dominator
  // tree.
  ...
  for (auto i = post_order.size(); i; i--) {
    ...
    if (pred.empty()) // This is for the entry block, but we've already set up
                      // the dominators.
      continue;
    ...
    for (auto &edge : pred) {
      if (immediate_dominators[block]) {
        ...
        immediate_dominators[block] =
            find_common_dominator(immediate_dominators[block], edge);
      } else
...
bool CFG::is_back_edge(uint32_t to) const {
  // We have a back edge if the visit order is set with the temporary magic
  // value 0.
...
bool CFG::has_visited_forward_edge(uint32_t to) const {
  // If > 0, we have visited the edge already, and this is not a back edge
  // branch.
  auto itr = visit_order.find(to);
  return itr != end(visit_order) && itr->second.get() > 0;
}
...
bool CFG::post_order_visit(uint32_t block_id) {
  ...
  // This is needed to avoid annoying cases with do { ... } while(false) loops
  // often generated by inliners.
  // To the CFG, this is linear control flow, but we risk picking the do/while
  // scope as our dominating block.
  // This makes sure that if we are accessing a variable outside the do/while,
  // we choose the loop header as dominator.
  // We could use has_visited_forward_edge, but this break code-gen where the
  // merge block is unreachable in the CFG.

  // Make a point out of visiting merge target first. This is to make sure that
  // post visit order outside the loop
  // is lower than inside the loop, which is going to be key for some traversal
  // algorithms like post-dominance analysis.
  // For selection constructs true/false blocks will end up visiting the merge
  // block directly and it works out fine,
  // but for loops, only the header might end up actually branching to merge
  // block.
  if (block.merge == SPIRBlock::MergeLoop &&
      post_order_visit(block.merge_block))
  ...
  // This is needed to avoid cases where an inner branch dominates the outer
  // branch.
  // This can happen if one of the branches exit early, e.g.:
  // if (cond) { ...; break; } else { var = 100 } use_var(var);
  // We can use the variable without a Phi since there is only one possible
  // parent here.
  // However, in this case, we need to hoist out the inner variable to outside
  // the branch.
  // Use same strategy as loops.
  if (block.merge == SPIRBlock::MergeSelection &&
      post_order_visit(block.next_block)) {
    // If there is only one preceding edge to the merge block and it's not
    // ourselves, we need a fixup.
    // Add a fake branch so any dominator in either the if (), or else () block,
    // or a lone case statement
    // will be hoisted out to outside the selection merge.
    // If size > 1, the variable will be automatically hoisted, so we should not
    // mess with it.
    // The exception here is switch blocks, where we can have multiple edges to
    // merge block,
    // all coming from same scope, so be more conservative in this case.
    // Adding fake branches unconditionally breaks parameter preservation
    // analysis,
    ...
    if (pred_itr != end(preceding_edges)) {
      ...
      if (block.terminator == SPIRBlock::MultiSelect &&
          num_succeeding_edges == 1) {
        // Multiple branches can come from the same scope due to "break;", so we
        // need to assume that all branches
        // come from same case scope in worst case, even if there are multiple
        // preceding edges.
        // If we have more than one succeeding edge from the block header, it
        // should be impossible
        // to have a dominator be inside the block.
        // Only case this can go wrong is if we have 2 or more edges from block
        // header and
        // 2 or more edges to merge block, and still have dominator be inside a
        // case label.
        if (!pred.empty())
          add_branch(block_id, block.next_block);
      } else {
      ...
      // If the merge block does not have any preceding edges, i.e. unreachable,
      // hallucinate it.
      // We're going to do code-gen for it, and domination analysis requires
      // that we have at least one preceding edge.
      add_branch(block_id, block.next_block);
    }
  }

  // Then visit ourselves. Start counting at one, to let 0 be a magic value for
  // testing back vs. crossing edges.
...
uint32_t CFG::find_loop_dominator(uint32_t block_id) const {
  while (block_id != SPIRBlock::NoDominator) {
    ...
    // Only consider a loop dominator if we are branching from inside a block to
    // a loop header.
    // NOTE: In the CFG we forced an edge from header to merge block always to
    // support variable scopes properly.
    for (auto &pred : itr->second) {
      auto &pred_block = compiler.get<SPIRBlock>(pred);
      if (pred_block.merge == SPIRBlock::MergeLoop &&
          pred_block.merge_block == ID(block_id)) {
        pred_block_id = pred;
        ignore_loop_header = true;
        break;
      } else if (pred_block.merge == SPIRBlock::MergeSelection &&
                 pred_block.next_block == ID(block_id)) {
        pred_block_id = pred;
        break;
      }
    }

    // No merge block means we can just pick any edge. Loop headers dominate the
    // inner loop, so any path we
...
bool CFG::node_terminates_control_flow_in_sub_graph(BlockID from,
                                                    BlockID to) const {
  // Walk backwards, starting from "to" block.
  // Only follow pred edges if they have a 1:1 relationship, or a merge
  // relationship.
  // If we cannot find a path to "from", we must assume that to is inside
  // control flow in some way.
  ...
  while (to != from) {
    ...
    if ((dom.merge == SPIRBlock::MergeSelection && dom.next_block == to) ||
        ...
        (dom.terminator == SPIRBlock::Select && dom.true_block == to &&
         false_path_ignore) ||
        (dom.terminator == SPIRBlock::Select && dom.false_block == to &&
         true_path_ignore)) {
      // Allow walking selection constructs if the other branch reaches out of a
      // loop construct.
...
void DominatorBuilder::lift_continue_block_dominator() {
  // It is possible for a continue block to be the dominator of a variable is
  // only accessed inside the while block of a do-while loop.
  // We cannot safely declare variables inside a continue block, so move any
  // variable declared
  // in a continue block to the entry block to simplify.
  // It makes very little sense for a continue block to ever be a dominator, so
  // fall back to the simplest
  // solution.
  ...
  // If we are branching to a block with a higher post-order traversal index
  // (continue blocks), we have a problem
  // since we cannot create sensible GLSL code for this, fallback to entry
  // block.
