...
// After compilation, it is possible to query whether or not this location was
// used.
// If vecsize is nonzero, it must be greater than or equal to the vecsize
...
// Matches the binding index of a MSL resource for a binding within a descriptor
// set.
// Taken together, the stage, desc_set and binding combine to form a reference
// to a resource
// descriptor used in a particular shading stage. The count field indicates the
// number of
// resources consumed by this binding, if the binding represents an array of
// resources.
// If the resource array is a run-time-sized array, which are legal in GLSL or
// SPIR-V, this value
// will be used to declare the array size in MSL, which does not support
// run-time-sized arrays.
// If pad_argument_buffer_resources is enabled, the base_type and count values
// are used to
// specify the base type and array size of the resource in the argument buffer,
// if that resource
// is not defined and used by the shader. With pad_argument_buffer_resources
// enabled, this
// information will be used to pad the argument buffer structure, in order to
// align that
// structure consistently for all uses, across all shaders, of the descriptor
// set represented
// by the arugment buffer. If pad_argument_buffer_resources is disabled,
// base_type does not
// need to be populated, and if the resource is also not a run-time sized array,
// the count
// field does not need to be populated.
// If using MSL 2.0 argument buffers, the descriptor set is not marked as a
// discrete descriptor set,
// and (for iOS only) the resource is not a storage image (sampled != 2), the
// binding reference we
// remap to will become an [[id(N)]] attribute within the "descriptor set"
// argument buffer structure.
// For resources which are bound in the "classic" MSL 1.0 way or discrete
// descriptors, the remap will
// become a [[buffer(N)]], [[texture(N)]] or [[sampler(N)]] depending on the
// resource types used.
...
struct MSLConstexprSampler {
  ...
  MSLSamplerYCbCrModelConversion ycbcr_model =
  ...
  bool swizzle_is_identity() const {
    return (swizzle[0] == MSL_COMPONENT_SWIZZLE_IDENTITY &&
            swizzle[1] == MSL_COMPONENT_SWIZZLE_IDENTITY &&
            swizzle[2] == MSL_COMPONENT_SWIZZLE_IDENTITY &&
            swizzle[3] == MSL_COMPONENT_SWIZZLE_IDENTITY);
  }
  bool swizzle_has_one_or_zero() const {
    return (swizzle[0] == MSL_COMPONENT_SWIZZLE_ZERO ||
            swizzle[0] == MSL_COMPONENT_SWIZZLE_ONE ||
            swizzle[1] == MSL_COMPONENT_SWIZZLE_ZERO ||
            swizzle[1] == MSL_COMPONENT_SWIZZLE_ONE ||
            swizzle[2] == MSL_COMPONENT_SWIZZLE_ZERO ||
            swizzle[2] == MSL_COMPONENT_SWIZZLE_ONE ||
            swizzle[3] == MSL_COMPONENT_SWIZZLE_ZERO ||
            swizzle[3] == MSL_COMPONENT_SWIZZLE_ONE);
  }
};

// Special constant used in a MSLResourceBinding desc_set
// element to indicate the bindings for the push constants.
// Kinda deprecated. Just use ResourceBindingPushConstant{DescriptorSet,Binding}
// directly.
...
// Special constant used in a MSLResourceBinding binding
// element to indicate the buffer binding for buffer size buffers to support
// OpArrayLength.
...
// This buffer binding should be kept as small as possible as all automatic
...
public:
  ...
  struct Options {
    ...
    uint32_t texel_buffer_texture_width =
        4096; // Width of 2D Metal textures used as 1D texel buffers
    ...
    // Ensures vertex and instance indices start at zero. This reflects the
    // behavior of HLSL with SV_VertexID and SV_InstanceID.
    bool enable_base_index_zero = false;

    // Fragment output in MSL must have at least as many components as the
    // render pass.
    // Add support to explicit pad out components.
    bool pad_fragment_output_components = false;

    // Specifies whether the iOS target version supports the [[base_vertex]] and
    // [[base_instance]] attributes.
    ...
    // Forces all resources which are part of an argument buffer to be
    // considered active.
    // This ensures ABI compatibility between shaders where some resources might
    // be unused,
    // and would otherwise declare a different IAB.
    bool force_active_argument_buffer_resources = false;

    // Aligns each resource in an argument buffer to its assigned index value,
    // id(N),
    // by adding synthetic padding members in the argument buffer struct for any
    // resources
    // in the argument buffer that are not defined and used by the shader. This
    // allows
    // the shader to index into the correct argument in a descriptor set
    // argument buffer
    // that is shared across shaders, where not all resources in the argument
    // buffer are
    // defined in each shader. For this to work, an MSLResourceBinding must be
    // provided for
    // all descriptors in any descriptor set held in an argument buffer in the
    // shader, and
    // that MSLResourceBinding must have the basetype and count members
    // populated correctly.
    // The implementation here assumes any inline blocks in the argument buffer
    // is provided
    // in a Metal buffer, and doesn't take into consideration inline blocks that
    // are
    // optionally embedded directly into the argument buffer via
    // add_inline_uniform_block().
    bool pad_argument_buffer_resources = false;

    // Forces the use of plain arrays, which works around certain driver bugs on
    // certain versions
    // of Intel Macbooks. See
    // https://github.com/KhronosGroup/SPIRV-Cross/issues/1210.
    // May reduce performance in scenarios where arrays are copied around as
    // value-types.
    ...
    // In a tessellation control shader, assume that more than one patch can be
    // processed in a
    // single workgroup. This requires changes to the way the InvocationId and
    // PrimitiveId
    // builtins are processed, but should result in more efficient usage of the
    // GPU.
    bool multi_patch_workgroup = false;

    // If set, a vertex shader will be compiled as part of a tessellation
    // pipeline.
    // It will be translated as a compute kernel, so it can use the global
    // invocation ID
    // to index the output buffer.
    bool vertex_for_tessellation = false;

    // Assume that SubpassData images have multiple layers. Layered input
    // attachments
    // are addressed relative to the Layer output from the vertex pipeline. This
    // option
    // has no effect with multiview, since all input attachments are assumed to
    // be layered
    // and will be addressed using the current ViewIndex.
    bool arrayed_subpass_input = false;

    // Whether to use SIMD-group or quadgroup functions to implement group
    // nnon-uniform
    // operations. Some GPUs on iOS do not support the SIMD-group functions,
    // only the
    // quadgroup functions.
    bool ios_use_simdgroup_functions = false;

    // If set, the subgroup size will be assumed to be one, and subgroup-related
    // builtins and operations will be emitted accordingly. This mode is
    // intended to
    // be used by MoltenVK on hardware/software configurations which do not
    // provide
    // sufficient support for subgroups.
    bool emulate_subgroups = false;

    // If nonzero, a fixed subgroup size to assume. Metal, similarly to
    // VK_EXT_subgroup_size_control,
    // allows the SIMD-group size (aka thread execution width) to vary depending
    // on
    // register usage and requirements. In certain circumstances--for example, a
    // pipeline
    // in MoltenVK without
    // VK_PIPELINE_SHADER_STAGE_CREATE_ALLOW_VARYING_SUBGROUP_SIZE_BIT_EXT--
    // this is undesirable. This fixes the value of the SubgroupSize builtin,
    // instead of
    // mapping it to the Metal builtin [[thread_execution_width]]. If the thread
    // execution width is reduced, the extra invocations will appear to be
    // inactive.
    // If zero, the SubgroupSize will be allowed to vary, and the builtin will
    // be mapped
    ...
    // The type of index in the index buffer, if present. For a compute shader,
    // Metal
    // requires specifying the indexing at pipeline creation, rather than at
    // draw time
    // as with graphics pipelines. This means we must create three different
    // pipelines,
    // for no indexing, 16-bit indices, and 32-bit indices. Each requires
    // different
    // handling for the gl_VertexIndex builtin. We may as well, then, create
    // three
    // different shaders for these three scenarios.
    IndexType vertex_index_type = IndexType::None;

    // If set, a dummy [[sample_id]] input is added to a fragment shader if none
    // is present.
    // This will force the shader to run at sample rate, assuming Metal does not
    // optimize
    ...
    void set_msl_version(uint32_t major, uint32_t minor = 0,
                         uint32_t patch = 0) {
      msl_version = make_msl_version(major, minor, patch);
    }

    bool supports_msl_version(uint32_t major, uint32_t minor = 0,
                              uint32_t patch = 0) const {
      return msl_version >= make_msl_version(major, minor, patch);
    }

    static uint32_t make_msl_version(uint32_t major, uint32_t minor = 0,
                                     uint32_t patch = 0) {
  ...
  bool get_is_rasterization_disabled() const {
    return is_rasterization_disabled &&
           (get_entry_point().model == spv::ExecutionModelVertex ||
            get_entry_point().model == spv::ExecutionModelTessellationControl ||
            get_entry_point().model ==
  ...
  // If shader inputs are provided, is_msl_shader_input_used() will return true
  // after
  ...
  // is_msl_resource_binding_used() will return true after calling ::compile()
  // if
  // the set/binding combination was used by the MSL code.
  void add_msl_resource_binding(const MSLResourceBinding &resource);

  // desc_set and binding are the SPIR-V descriptor set and binding of a buffer
  // resource
  // in this shader. index is the index within the dynamic offset buffer to use.
  // This
  // function marks that resource as using a dynamic offset
  // (VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC
  // or VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC). This function only has any
  // effect if argument buffers
  // are enabled. If so, the buffer will have its address adjusted at the
  // beginning of the shader with
  // an offset taken from the dynamic offset buffer.
  void add_dynamic_buffer(uint32_t desc_set, uint32_t binding, uint32_t index);

  // desc_set and binding are the SPIR-V descriptor set and binding of a buffer
  // resource
  // in this shader. This function marks that resource as an inline uniform
  // block
  // (VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK_EXT). This function only has any
  // effect if argument buffers
  // are enabled. If so, the buffer block will be directly embedded into the
  // argument
  ...
  // When using MSL argument buffers, we can force "classic" MSL 1.0 binding
  // schemes for certain descriptor sets.
  // This corresponds to VK_KHR_push_descriptor in Vulkan.
  void add_discrete_descriptor_set(uint32_t desc_set);

  // If an argument buffer is large enough, it may need to be in the device
  // storage space rather than
  // constant. Opt-in to this behavior here on a per set basis.
  void set_argument_buffer_device_address_space(uint32_t desc_set,
                                                bool device_storage);

  // Query after compilation is done. This allows you to check if an input
  // location was used by the shader.
  bool is_msl_shader_input_used(uint32_t location);

  // If not using add_msl_shader_input, it's possible
  // that certain builtin attributes need to be automatically assigned
  // locations.
  // This is typical for tessellation builtin inputs such as tess levels,
  // gl_Position, etc.
  // This returns k_unknown_location if the location was explicitly assigned
  // with
  // add_msl_shader_input or the builtin is not used, otherwise returns N in
  // [[attribute(N)]].
  uint32_t get_automatic_builtin_input_location(spv::BuiltIn builtin) const;

  // NOTE: Only resources which are remapped using add_msl_resource_binding will
  // be reported here.
  // Constexpr samplers are always assumed to be emitted.
  // No specific MSLResourceBinding remapping is required for constexpr samplers
  // as long as they are remapped
  // by remap_constexpr_sampler(_by_binding).
  bool is_msl_resource_binding_used(spv::ExecutionModel model, uint32_t set,
                                    uint32_t binding) const;

  // This must only be called after a successful call to CompilerMSL::compile().
  // For a variable resource ID obtained through reflection API, report the
  // automatically assigned resource index.
  ...
  // If the resource was a combined image sampler, report the image binding
  // here,
  // use the _secondary version of this call to query the sampler half of the
  // resource.
  // If no binding exists, uint32_t(-1) is returned.
  uint32_t get_automatic_msl_resource_binding(uint32_t id) const;

  // Same as get_automatic_msl_resource_binding, but should only be used for
  // combined image samplers, in which case the
  // sampler's binding is returned instead. For any other resource type, -1 is
  // returned.
  ...
  // Same as get_automatic_msl_resource_binding, but should only be used for
  // combined image samplers for multiplanar images,
  // in which case the second plane's binding is returned instead. For any other
  // resource type, -1 is returned.
  uint32_t get_automatic_msl_resource_binding_tertiary(uint32_t id) const;

  // Same as get_automatic_msl_resource_binding, but should only be used for
  // combined image samplers for triplanar images,
  // in which case the third plane's binding is returned instead. For any other
  // resource type, -1 is returned.
  ...
  // The sampler will not consume a binding, but be declared in the entry point
  // as a constexpr sampler.
  // This can be used on both combined image/samplers (sampler2D) or standalone
  // samplers.
  // The remapped sampler must not be an array of samplers.
  // Prefer remap_constexpr_sampler_by_binding unless you're also doing
  // reflection anyways.
  void remap_constexpr_sampler(VariableID id,
                               const MSLConstexprSampler &sampler);

  // Same as remap_constexpr_sampler, except you provide set/binding, rather
  // than variable ID.
  // Remaps based on ID take priority over set/binding remaps.
  void remap_constexpr_sampler_by_binding(uint32_t desc_set, uint32_t binding,
                                          const MSLConstexprSampler &sampler);

  // If using CompilerMSL::Options::pad_fragment_output_components, override the
  // number of components we expect
  // to use for a particular location. The default is 4 if number of components
  // is not overridden.
...
protected:
  ...
  enum SPVFuncImpl {
    ...
    // Unfortunately, we cannot use recursive templates in the MSL compiler
    // properly,
    ...
    SPVFuncImplUnsafeArray, // Allow Metal to use the array<T> template to make
                            // arrays a value type
    SPVFuncImplInverse4x4,
    SPVFuncImplInverse3x3,
    SPVFuncImplInverse2x2,
    // It is very important that this come before *Swizzle and
    // ChromaReconstruct*, to ensure it's
    // emitted before them.
  ...
  void emit_binary_unord_op(uint32_t result_type, uint32_t result_id,
  ...
  void emit_spv_amd_shader_trinary_minmax_op(uint32_t result_type,
                                             uint32_t result_id, uint32_t op,
                                             const uint32_t *args,
                                             uint32_t count) override;
  void emit_header() override;
  void emit_function_prototype(SPIRFunction &func,
                               const Bitset &return_flags) override;
  void emit_sampled_image_op(uint32_t result_type, uint32_t result_id,
                             uint32_t image_id, uint32_t samp_id) override;
  void emit_subgroup_op(const Instruction &i) override;
  std::string
  to_texture_op(const Instruction &i, bool sparse, bool *forward,
                SmallVector<uint32_t> &inherited_expressions) override;
  void emit_fixup() override;
  std::string to_struct_member(const SPIRType &type, uint32_t member_type_id,
                               uint32_t index,
                               const std::string &qualifier = "");
  void emit_struct_member(const SPIRType &type, uint32_t member_type_id,
                          uint32_t index, const std::string &qualifier = "",
  ...
  bool
  variable_decl_is_remapped_storage(const SPIRVariable &variable,
                                    spv::StorageClass storage) const override;

  // GCC workaround of lambdas calling protected functions (for older GCC
  // versions)
  std::string variable_decl(const SPIRType &type, const std::string &name,
  ...
  std::string to_func_call_arg(const SPIRFunction::Parameter &arg,
                               uint32_t id) override;
  std::string to_name(uint32_t id, bool allow_alias = true) const override;
  std::string
  to_function_name(const TextureFunctionNameArguments &args) override;
  std::string to_function_args(const TextureFunctionArguments &args,
                               bool *p_forward) override;
  std::string to_initializer_expression(const SPIRVariable &var) override;
  std::string to_zero_initialized_expression(uint32_t type_id) override;

  std::string unpack_expression_type(std::string expr_str, const SPIRType &type,
                                     uint32_t physical_type_id, bool is_packed,
                                     bool row_major) override;

  // Returns true for BuiltInSampleMask because gl_SampleMask[] is an array in
  // SPIR-V, but [[sample_mask]] is a scalar in Metal.
  bool builtin_translates_to_nonarray(spv::BuiltIn builtin) const override;

  std::string bitcast_glsl_op(const SPIRType &result_type,
                              const SPIRType &argument_type) override;
  bool emit_complex_bitcast(uint32_t result_id, uint32_t id,
                            uint32_t op0) override;
  bool skip_argument(uint32_t id) const override;
  std::string to_member_reference(uint32_t base, const SPIRType &type,
                                  uint32_t index, bool ptr_chain) override;
  ...
  bool member_is_non_native_row_major_matrix(const SPIRType &type,
  ...
  uint32_t add_interface_block_pointer(uint32_t ib_var_id,
  ...
  void emit_local_masked_variable(const SPIRVariable &masked_var,
                                  bool strip_array);
  void add_variable_to_interface_block(spv::StorageClass storage,
                                       const std::string &ib_var_ref,
                                       SPIRType &ib_type, SPIRVariable &var,
                                       InterfaceBlockMeta &meta);
  void add_composite_variable_to_interface_block(spv::StorageClass storage,
                                                 const std::string &ib_var_ref,
                                                 SPIRType &ib_type,
                                                 SPIRVariable &var,
                                                 InterfaceBlockMeta &meta);
  void add_plain_variable_to_interface_block(spv::StorageClass storage,
                                             const std::string &ib_var_ref,
                                             SPIRType &ib_type,
                                             SPIRVariable &var,
                                             InterfaceBlockMeta &meta);
  bool add_component_variable_to_interface_block(spv::StorageClass storage,
                                                 const std::string &ib_var_ref,
                                                 SPIRVariable &var,
                                                 const SPIRType &type,
                                                 InterfaceBlockMeta &meta);
  void add_plain_member_variable_to_interface_block(
      ...
      InterfaceBlockMeta &meta);
  void add_composite_member_variable_to_interface_block(
      spv::StorageClass storage, const std::string &ib_var_ref,
      SPIRType &ib_type, SPIRVariable &var, uint32_t index,
      InterfaceBlockMeta &meta);
  void add_tess_level_input_to_interface_block(const std::string &ib_var_ref,
                                               SPIRType &ib_type,
                                               SPIRVariable &var);

  void fix_up_interface_member_indices(spv::StorageClass storage,
                                       uint32_t ib_type_id);
  ...
  std::string member_location_attribute_qualifier(const SPIRType &type,
                                                  uint32_t index);
  std::string argument_decl(const SPIRFunction::Parameter &arg);
  std::string round_fp_tex_coords(std::string tex_coords, bool coord_is_fp);
  uint32_t get_metal_resource_index(SPIRVariable &var,
                                    SPIRType::BaseType basetype,
                                    uint32_t plane = 0);
  uint32_t get_member_location(uint32_t type_id, uint32_t index,
                               uint32_t *comp = nullptr) const;
  uint32_t get_or_allocate_builtin_input_member_location(
      spv::BuiltIn builtin, uint32_t type_id, uint32_t index,
      uint32_t *comp = nullptr);

  uint32_t get_physical_tess_level_array_size(spv::BuiltIn builtin) const;

  // MSL packing rules. These compute the effective packing rules as observed by
  // the MSL compiler in the MSL output.
  // These values can change depending on various extended decorations which
  // control packing rules.
  // We need to make these rules match up with SPIR-V declared rules.
  uint32_t get_declared_type_size_msl(const SPIRType &type, bool packed,
                                      bool row_major) const;
  uint32_t get_declared_type_array_stride_msl(const SPIRType &type, bool packed,
                                              bool row_major) const;
  uint32_t get_declared_type_matrix_stride_msl(const SPIRType &type,
                                               bool packed,
                                               bool row_major) const;
  uint32_t get_declared_type_alignment_msl(const SPIRType &type, bool packed,
                                           bool row_major) const;

  uint32_t get_declared_struct_member_size_msl(const SPIRType &struct_type,
                                               uint32_t index) const;
  uint32_t
  get_declared_struct_member_array_stride_msl(const SPIRType &struct_type,
                                              uint32_t index) const;
  uint32_t
  get_declared_struct_member_matrix_stride_msl(const SPIRType &struct_type,
                                               uint32_t index) const;
  uint32_t get_declared_struct_member_alignment_msl(const SPIRType &struct_type,
                                                    uint32_t index) const;

  uint32_t get_declared_input_size_msl(const SPIRType &struct_type,
                                       uint32_t index) const;
  uint32_t get_declared_input_array_stride_msl(const SPIRType &struct_type,
                                               uint32_t index) const;
  uint32_t get_declared_input_matrix_stride_msl(const SPIRType &struct_type,
                                                uint32_t index) const;
  uint32_t get_declared_input_alignment_msl(const SPIRType &struct_type,
                                            uint32_t index) const;

  const SPIRType &get_physical_member_type(const SPIRType &struct_type,
                                           uint32_t index) const;
  SPIRType get_presumed_input_type(const SPIRType &struct_type,
                                   uint32_t index) const;

  uint32_t get_declared_struct_size_msl(const SPIRType &struct_type,
  ...
  bool validate_member_packing_rules_msl(const SPIRType &type,
                                         uint32_t index) const;
  ...
  void emit_atomic_func_op(uint32_t result_type, uint32_t result_id,
                           const char *op, uint32_t mem_order_1,
                           uint32_t mem_order_2, bool has_mem_order_2,
                           uint32_t op0, uint32_t op1 = 0,
                           bool op1_is_pointer = false,
                           bool op1_is_literal = false, uint32_t op2 = 0);
  const char *get_memory_order(uint32_t spv_mem_sem);
  void add_pragma_line(const std::string &line);
  void add_typedef_line(const std::string &line);
  void emit_barrier(uint32_t id_exe_scope, uint32_t id_mem_scope,
                    uint32_t id_mem_sem);
  ...
  void cast_to_variable_store(uint32_t target_id, std::string &expr,
                              const SPIRType &expr_type) override;
  void cast_from_variable_load(uint32_t source_id, std::string &expr,
                               const SPIRType &expr_type) override;
  void emit_store_statement(uint32_t lhs_expression,
                            uint32_t rhs_expression) override;

  void analyze_sampled_image_usage();

  bool access_chain_needs_stage_io_builtin_translation(uint32_t base) override;
  void prepare_access_chain_for_scalar_access(std::string &expr,
                                              const SPIRType &type,
                                              spv::StorageClass storage,
                                              bool &is_packed) override;
  void fix_up_interpolant_access_chain(const uint32_t *ops, uint32_t length);
  bool emit_tessellation_access_chain(const uint32_t *ops, uint32_t length);
  bool emit_tessellation_io_load(uint32_t result_type, uint32_t id,
                                 uint32_t ptr);
  bool is_out_of_bounds_tessellation_level(uint32_t id_lhs);

  void ensure_builtin(spv::StorageClass storage, spv::BuiltIn builtin);

  void mark_implicit_builtin(spv::StorageClass storage, spv::BuiltIn builtin,
                             uint32_t id);
  ...
  std::unordered_map<StageSetBinding, std::pair<MSLResourceBinding, bool>,
                     InternalHasher> resource_bindings;
  std::unordered_map<StageSetBinding, uint32_t, InternalHasher>
  ...
  bool builtin_declaration =
      false; // Handle HLSL-style 0-based vertex/instance index.

  bool is_using_builtin_array =
      false; // Force the use of C style array declaration.
  ...
  std::unordered_map<SetBindingPair, MSLConstexprSampler, InternalHasher>
  ...
  std::unordered_set<uint32_t>
      atomic_image_vars; // Emulate texture2D atomic operations
  std::unordered_set<uint32_t> pull_model_inputs;

  // Must be ordered since array is in a specific order.
  std::map<SetBindingPair, std::pair<uint32_t, uint32_t>>
  ...
  MSLResourceBinding &get_argument_buffer_resource(uint32_t desc_set,
                                                   uint32_t arg_idx);
  void add_argument_buffer_padding_buffer_type(SPIRType &struct_type,
                                               uint32_t &mbr_idx,
                                               uint32_t &arg_buff_index,
                                               MSLResourceBinding &rez_bind);
  void add_argument_buffer_padding_image_type(SPIRType &struct_type,
                                              uint32_t &mbr_idx,
                                              uint32_t &arg_buff_index,
                                              MSLResourceBinding &rez_bind);
  void add_argument_buffer_padding_sampler_type(SPIRType &struct_type,
                                                uint32_t &mbr_idx,
                                                uint32_t &arg_buff_index,
                                                MSLResourceBinding &rez_bind);
  void add_argument_buffer_padding_type(uint32_t mbr_type_id,
  ...
  uint32_t
  build_extended_vector_type(uint32_t type_id, uint32_t components,
                             SPIRType::BaseType basetype = SPIRType::Unknown);
  ...
  struct OpCodePreprocessor : OpcodeHandler {
    ...
    CompilerMSL::SPVFuncImpl get_spv_func_impl(spv::Op opcode,
                                               const uint32_t *args);
    ...
    std::unordered_map<uint32_t, uint32_t>
        image_pointers; // Emulate texture2D atomic operations
    bool suppress_missing_prototypes = false;
  ...
  struct SampledImageScanner : OpcodeHandler {
    SampledImageScanner(CompilerMSL &compiler_) : compiler(compiler_) {}
  ...
  // Sorts the members of a SPIRType and associated Meta info based on a
  // settable sorting
  // aspect, which defines which aspect of the struct members will be used to
  // sort them.
  // Regardless of the sorting aspect, built-in members always appear at the end
  // of the struct.
  struct MemberSorter {
