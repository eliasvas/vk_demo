...
// - Project B links against SPIRV-Cross statically (might be a different
// version).
// This leads to a conflict with extremely bizarre results.
// By overriding the namespace in one of the project builds, we can work around
// this.
...
public:
  ...
  template <typename Op> void for_each_bit(const Op &op) const {
    ...
    // but hitting this path should happen extremely rarely, so having this slow
    // path is fine.
...
private:
  ...
  // so optimize for this case. Bits spilling outside 64 go into a slower data
  // structure.
...
static inline std::string convert_to_string(int32_t value) {
  // INT_MIN is ... special on some backends. If we use a decimal literal, and
  // negate it, we
...
static inline std::string convert_to_string(int64_t value,
                                            const std::string &int64_type,
                                            bool long_long_literal_suffix) {
  // INT64_MIN is ... special on some backends.
  // If we use a decimal literal, and negate it, we might overflow the
  // representable numbers.
  // To workaround it, emit int(0x80000000) instead.
  if (value == std::numeric_limits<int64_t>::min())
    return join(int64_type, "(0x8000000000000000u",
                (long_long_literal_suffix ? "ll" : "l"), ")");
  else
...
public:
  ...
  // As long as we block implicit conversion between TypedID<A> and TypedID<B>
  // we're good.
...
// This type is only used by backends which need to access the combined image
// and sampler IDs separately after
...
struct SPIRCombinedImageSampler : IVariant {
  ...
  SPIRCombinedImageSampler(TypeID type_, VariableID image_, VariableID sampler_)
      : combined_type(type_), image(image_), sampler(sampler_) {}
...
struct SPIRConstantOp : IVariant {
  ...
  SPIRConstantOp(TypeID result_type, spv::Op op, const uint32_t *args,
                 uint32_t length)
      : opcode(op), basetype(result_type) {
...
struct SPIRType : IVariant {
  ...
  // Array elements can be either specialization constants or specialization
  // ops.
  ...
  // Structs can be declared multiple times if they are used as part of
  // interface blocks.
  ...
  // Allows the backend to traverse how a complex type is built up during access
  // chains.
...
// SPIREntryPoint is not a variant since its IDs are used to decorate
// OpFunction,
// so in order to avoid conflicts, we can't stick them in the ids array.
struct SPIREntryPoint {
  SPIREntryPoint(FunctionID self_, spv::ExecutionModel execution_model,
                 const std::string &entry_name)
      : self(self_), name(entry_name), orig_name(entry_name),
  ...
  struct WorkgroupSize {
    ...
    uint32_t constant = 0; // Workgroup size can be expressed as a
                           // constant/spec-constant instead.
  } workgroup_size;
...
struct SPIRExpression : IVariant {
  ...
  SPIRExpression(std::string expr, TypeID expression_type_, bool immutable_)
      : expression(move(expr)), expression_type(expression_type_),
        immutable(immutable_) {}

  // If non-zero, prepend expression with to_expression(base_expression).
  // Used in amortizing multiple calls to to_expression()
  // where in certain cases that would quickly force a temporary when not
  // needed.
  ...
  // Used by access chain Store and Load since we read multiple expressions in
  // this case.
  SmallVector<ID> implied_read_expressions;

  // The expression was emitted at a certain scope. Lets us track when an
  // expression read means multiple reads.
...
struct SPIRBlock : IVariant {
  ...
  struct Phi {
    ...
    BlockID
        parent; // If we're in from_block and want to branch into this block ...
    VariableID
        function_variable; // to this function-global "phi" variable first.
  };

  // Before entering this block flush out local variables to magical "phi"
  // variables.
  ...
  // Declare these temporaries, but only conditionally if this block turns out
  // to be
  ...
  // Do we need a ladder variable to defer breaking out of a loop construct
  // after a switch block?
  bool need_ladder_break = false;

  // If marked, we have explicitly handled Phi from this block, so skip any
  // flushes related to that on a branch.
  // Used to handle an edge case with switch and case-label fallthrough where
  // fall-through writes to Phi.
  ...
  // so before branching anywhere we need to make sure that we declare these
  // variables.
  SmallVector<VariableID> dominated_variables;

  // These are variables which should be declared in a for loop header, if we
  // fail to use a classic for-loop,
  // we remove these variables, and fall back to regular variables outside the
  // loop.
  SmallVector<VariableID> loop_variables;

  // Some expressions are control-flow dependent, i.e. any instruction which
  // relies on derivatives or
...
struct SPIRFunction : IVariant {
  ...
  void add_parameter(TypeID parameter_type, ID id,
  ...
  // Mostly used for lowering internal data structures onto flattened
  // structures.
  // Need to defer this, because they might rely on things which change during
  // compilation.
  // Intentionally not a small vector, this one is rare, and std::function can
  // be large.
  Vector<std::function<void()>> fixup_hooks_out;

  // Hooks to be run when the function begins.
  // Mostly used for populating internal data structures from flattened
  // structures.
  // Need to defer this, because they might rely on things which change during
  // compilation.
  // Intentionally not a small vector, this one is rare, and std::function can
  ...
  // On function entry, make sure to copy a constant array into thread addr
  // space to work around
  // the case where we are passing a constant array by value to a function on
  // backends which do not
...
struct SPIRAccessChain : IVariant {
  ...
  SPIRAccessChain(TypeID basetype_, spv::StorageClass storage_,
                  std::string base_, std::string dynamic_index_,
                  int32_t static_index_)
      : basetype(basetype_), storage(storage_), base(std::move(base_)),
        dynamic_index(std::move(dynamic_index_)), static_index(static_index_) {}

  // The access chain represents an offset into a buffer.
  // Some backends need more complicated handling of access chains to be able to
  // use buffers, like HLSL
  // which has no usable buffer type ala GLSL SSBOs.
  // StructuredBuffer is too limited, so our only option is to deal with
  // ByteAddressBuffer which works with raw addresses.
...
struct SPIRVariable : IVariant {
  ...
  SPIRVariable(TypeID basetype_, spv::StorageClass storage_,
               ID initializer_ = 0, VariableID basevariable_ = 0)
  ...
  // Temporaries which can remain forwarded as long as this variable is not
  // modified.
...
struct SPIRConstant : IVariant {
  ...
  struct ConstantVector {
    Constant r[4];
    // If != 0, this element is a specialization constant, and we should keep
    // track of it as such.
    ID id[4];
    uint32_t vecsize = 1;
  ...
  struct ConstantMatrix {
    ConstantVector c[4];
    // If != 0, this column is a specialization constant, and we should keep
    // track of it as such.
    ID id[4];
    uint32_t columns = 1;
  };
  ...
  SPIRConstant(TypeID constant_type_, const uint32_t *elements,
  ...
  SPIRConstant(TypeID constant_type_,
  ...
  // If this constant is a specialization constant (i.e. created with
  // OpSpecConstant*).
  bool specialization = false;
  // If this constant is used as an array length which creates specialization
  // restrictions on some backends.
  ...
  // Non-Vulkan GLSL, HLSL and sometimes MSL emits defines for each
...
public:
  ...
  // If this is called inside ::compile(), we invalidate any references we took
  // higher in the stack.
...
//#define SPIRV_CROSS_COPY_CONSTRUCTOR_SANITIZE
  ...
  T *allocate_and_set(Types new_type, Ts &&... ts) {
    T *val = static_cast<ObjectPool<T> &>(*group->pools[new_type])
                 .allocate(std::forward<Ts>(ts)...);
    set(val, new_type);
    return val;
  }
...
template <typename T> T &variant_get(Variant &var) { return var.get<T>(); }
...
T &variant_set(Variant &var, P &&... args) {
  auto *ptr = var.allocate_and_set<T>(static_cast<Types>(T::type),
                                      std::forward<P>(args)...);
  return *ptr;
}
...
enum ExtendedDecorations {
  // Marks if a buffer block is re-packed, i.e. member declaration might be
  // subject to PhysicalTypeID remapping and padding.
  SPIRVCrossDecorationBufferBlockRepacked = 0,

  // A type in a buffer block might be declared with a different physical type
  // than the logical type.
  // If this is not set, PhysicalTypeID == the SPIR-V type as declared.
  SPIRVCrossDecorationPhysicalTypeID,

  // Marks if the physical type is to be declared with tight packing rules, i.e.
  // packed_floatN on MSL and friends.
  // If this is set, PhysicalTypeID might also be set. It can be set to same as
  // logical type if all we're doing
  // is converting float3 to packed_float3 for example.
  // If this is marked on a struct, it means the struct itself must use only
  // Packed types for all its members.
  ...
  // Used for decorations like resource indices for samplers when part of
  // combined image samplers.
  // A variable might need to hold two resource indices in this case.
  SPIRVCrossDecorationResourceIndexSecondary,
  // Used for resource indices for multiplanar images when part of combined
  // image samplers.
  ...
  // Apply to a variable in the Input storage class; marks it as holding the
  // base group passed to vkCmdDispatchBase(),
  // or the base vertex and instance indices passed to vkCmdDrawIndexed().
  // In MSL, this is used to adjust the WorkgroupId and GlobalInvocationId
  ...
  // Apply to a variable that is a function parameter; marks it as being a
  // "dynamic"
  // combined image-sampler. In MSL, this is used when a function parameter
  // might hold
  ...
  // size of the stage
  // input grid.
  // In MSL, this is used to hold the vertex and instance counts in a
  // tessellation pipeline
  // vertex shader.
  SPIRVCrossDecorationBuiltInStageInputSize,

  // Apply to any access chain of a tessellation I/O variable; stores the type
  // of the sub-object
  // that was chained to, as recorded in the input variable itself. This is used
  // in case the pointer
  // is itself used as the base of an access chain, to calculate the original
  // type of the sub-object
  // chained to, in case a swizzle needs to be applied. This should not happen
  // normally with valid
  // SPIR-V, but the MSL backend can change the type of input variables,
  // necessitating the
  // addition of swizzles to keep the generated code compiling.
  SPIRVCrossDecorationTessIOOriginalInputTypeID,

  // Apply to any access chain of an interface variable used with pull-model
  // interpolation, where the variable is a
  // vector but the resulting pointer is a scalar; stores the component index
  // that is to be accessed by the chain.
  // This is used when emitting calls to interpolation functions on the chain in
  // MSL: in this case, the component
  // must be applied to the result, since pull-model interpolants in MSL cannot
  // be swizzled directly, but the
...
// name_of_type is the textual name of the type which will be used in the code
// unless written to by the callback.
using VariableTypeRemapCallback =
    std::function<void(const SPIRType &type, const std::string &var_name,
...
static inline bool type_is_floating_point(const SPIRType &type) {
  return type.basetype == SPIRType::Half || type.basetype == SPIRType::Float ||
         type.basetype == SPIRType::Double;
}

static inline bool type_is_integral(const SPIRType &type) {
  return type.basetype == SPIRType::SByte || type.basetype == SPIRType::UByte ||
         type.basetype == SPIRType::Short ||
         type.basetype == SPIRType::UShort || type.basetype == SPIRType::Int ||
         type.basetype == SPIRType::UInt || type.basetype == SPIRType::Int64 ||
...
// Returns true if an arithmetic operation does not change behavior depending on
// signedness.
...
struct SetBindingPair {
  ...
  inline bool operator<(const SetBindingPair &other) const {
    return desc_set < other.desc_set ||
           (desc_set == other.desc_set && binding < other.binding);
  }
};
...
struct LocationComponentPair {
  ...
  inline bool operator<(const LocationComponentPair &other) const {
    return location < other.location ||
           (location == other.location && component < other.component);
  }
};
...
struct StageSetBinding {
  ...
  inline bool operator==(const StageSetBinding &other) const {
    return model == other.model && desc_set == other.desc_set &&
