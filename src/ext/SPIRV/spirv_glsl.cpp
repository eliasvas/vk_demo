...
enum ExtraSubExpressionType {
  // Create masks above any legal ID range to allow multiple address spaces into
  // the extra_sub_expressions map.
...
static BufferPackingStandard
packing_to_substruct_packing(BufferPackingStandard packing) {
  switch (packing) {
  case BufferPackingStd140EnhancedLayout:
...
// We'll rely on fixing it up ourselves in the rare case we have a
// comma-as-decimal locale
// rather than setting locales ourselves. Settings locales in a safe and
// isolated way is rather
// tricky.
#ifdef _WIN32
  ...
  // nl_langinfo is not supported on this platform, fall back to the worse
  // alternative.
  const struct lconv *conv = localeconv();
  if (conv && conv->decimal_point)
    current_locale_radix_character = *conv->decimal_point;
...
const char *CompilerGLSL::vector_swizzle(int vecsize, int index) {
  static const char *const swizzle[4][4] = {
    ...
    // This works around a GCC 9 bug, see details in
    // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=90947.
    // This array ends up being compiled as all nullptrs, tripping the
    // assertions below.
    {"", nullptr, nullptr, "$"},
#else
...
void CompilerGLSL::reset() {
  // We do some speculative optimizations which should pretty much always work
  // out,
  ...
  // Ensure that we declare phi-variable copies even if the original declaration
  // isn't deferred
  ...
  ir.for_each_typed_id<SPIRVariable>(
      [&](uint32_t, SPIRVariable &var) { var.dependees.clear(); });
...
void CompilerGLSL::remap_ext_framebuffer_fetch(uint32_t input_attachment_index,
                                               uint32_t color_location,
                                               bool coherent) {
  subpass_to_framebuffer_fetch_attachment.push_back(
      {input_attachment_index, color_location});
...
bool CompilerGLSL::location_is_framebuffer_fetch(uint32_t location) const {
  return std::find_if(begin(inout_color_attachments),
...
bool CompilerGLSL::location_is_non_coherent_framebuffer_fetch(
...
void CompilerGLSL::find_static_extensions() {
  ir.for_each_typed_id<SPIRType>([&](uint32_t, const SPIRType &type) {
    ...
    } else if (type.basetype == SPIRType::Int64 ||
    ...
    } else if (type.basetype == SPIRType::SByte ||
  ...
  case ExecutionModelGeometry:
    ...
    if (execution.flags.get(ExecutionModeInvocations) &&
  ...
  case ExecutionModelCallableKHR:
    ...
    if (options.es || options.version < 460)
      SPIRV_CROSS_THROW("Ray tracing shaders require non-es profile with "
                        "version 460 or above.");
    if (!options.vulkan_semantics)
      SPIRV_CROSS_THROW("Ray tracing requires Vulkan semantics.");

    // Need to figure out if we should target KHR or NV extension based on
    // capabilities.
  ...
  if (!pls_inputs.empty() || !pls_outputs.empty()) {
    if (execution.model != ExecutionModelFragment)
      SPIRV_CROSS_THROW("Can only use GL_EXT_shader_pixel_local_storage in "
                        "fragment shaders.");
  ...
  if (ir.addressing_model == AddressingModelPhysicalStorageBuffer64EXT) {
    ...
    SPIRV_CROSS_THROW("Only Logical and PhysicalStorageBuffer64EXT addressing "
                      "models are supported.");
  }

  // Check for nonuniform qualifier and passthrough.
  // Instead of looping over all decorations to find this, just look at
  // capabilities.
  for (auto &cap : ir.declared_capabilities) {
    switch (cap) {
    ...
    case CapabilityMultiView:
      if (options.vulkan_semantics)
        ...
        if (options.ovr_multiview_view_count == 0)
          SPIRV_CROSS_THROW("ovr_multiview_view_count must be non-zero when "
                            "using GL_OVR_multiview2.");
...
void CompilerGLSL::ray_tracing_khr_fixup_locations() {
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
    ...
    if (var.storage != StorageClassRayPayloadKHR &&
        var.storage != StorageClassCallableDataKHR &&
        var.storage != StorageClassIncomingRayPayloadKHR &&
...
string CompilerGLSL::compile() {
  ...
  if (!options.vulkan_semantics) {
    // only NV_gpu_shader5 supports divergent indexing on OpenGL, and it does so
  ...
  backend.support_precise_qualifier = (!options.es && options.version >= 400) ||
                                      (options.es && options.version >= 320);
  ...
  // Find all such instances and make sure we can cast the pointers to a
  // synthesized block type.
  if (ir.addressing_model == AddressingModelPhysicalStorageBuffer64EXT)
  ...
  if (interlocked_is_complex) {
    statement("void main()");
    begin_scope();
    statement("// Interlocks were used in a way not compatible with GLSL, this "
              "is very slow.");
...
std::string CompilerGLSL::get_partial_source() { return buffer.str(); }

void CompilerGLSL::build_workgroup_size(SmallVector<string> &arguments,
  ...
  if (wg_x.id) {
    if (options.vulkan_semantics)
      ...
      arguments.push_back(
          join("local_size_x = ",
               get<SPIRConstant>(wg_x.id).specialization_constant_macro_name));
  } else
  ...
  if (wg_y.id) {
    if (options.vulkan_semantics)
      ...
      arguments.push_back(
          join("local_size_y = ",
               get<SPIRConstant>(wg_y.id).specialization_constant_macro_name));
  } else
  ...
  if (wg_z.id) {
    if (options.vulkan_semantics)
      ...
      arguments.push_back(
          join("local_size_z = ",
               get<SPIRConstant>(wg_z.id).specialization_constant_macro_name));
  } else
    arguments.push_back(join("local_size_z = ", execution.workgroup_size.z));
}

void CompilerGLSL::request_subgroup_feature(
    ShaderSubgroupSupportHelper::Feature feature) {
  if (options.vulkan_semantics) {
    auto khr_extension =
        ShaderSubgroupSupportHelper::get_KHR_extension_for_feature(feature);
    require_extension_internal(
        ShaderSubgroupSupportHelper::get_extension_name(khr_extension));
  } else {
...
void CompilerGLSL::emit_header() {
  ...
  statement("#version ", options.version,
            options.es && options.version > 100 ? " es" : "");
  ...
  bool interlock_used =
      execution.flags.get(ExecutionModePixelInterlockOrderedEXT) ||
  ...
  for (auto &ext : forced_extensions) {
    if (ext == "GL_EXT_shader_explicit_arithmetic_types_float16") {
      // Special case, this extension has a potential fallback to another vendor
      // extension in normal GLSL.
      ...
      if (!options.vulkan_semantics) {
        ...
        statement("#extension GL_EXT_shader_explicit_arithmetic_types_float16 "
                  ": require");
      }
      statement("#else");
      ...
      if (options.vulkan_semantics)
        statement("#extension GL_EXT_shader_explicit_arithmetic_types_int16 : "
                  "require");
      else {
  ...
  if (!options.vulkan_semantics) {
    ...
    for (uint32_t feature_index = 0; feature_index < Supp::FeatureCount;
         feature_index++) {
      ...
      for (auto &ext : exts) {
        ...
        const char *extra_predicate =
            Supp::get_extra_required_extension_predicate(ext);
      ...
      if (!Supp::can_feature_be_implemented_without_extensions(feature)) {
        statement("#else");
        statement("#error No extensions available to emulate requested "
                  "subgroup feature.");
      }
  ...
  case ExecutionModelGLCompute: {
    if (execution.workgroup_size.constant != 0) {
      ...
      // If there are any spec constants on legacy GLSL, defer declaration, we
      // need to set up macro
      // declarations before we can emit the work group size.
      if (options.vulkan_semantics ||
          ((wg_x.id == ConstantID(0)) && (wg_y.id == ConstantID(0)) &&
           (wg_z.id == ConstantID(0))))
...
void CompilerGLSL::emit_struct(SPIRType &type) {
  ...
  // If the type master is packed however, we can no longer assume that the
  // struct declaration will be redundant.
  if (type.type_alias != TypeID(0) &&
      !has_extended_decoration(type.type_alias,
...
string CompilerGLSL::layout_for_member(const SPIRType &type, uint32_t index) {
  ...
  // This is a bit problematic because in SPIR-V decorations are applied on the
  // struct types directly.
  // This is not supported on GLSL, so we have to make the assumption that if a
  // struct within our buffer block struct
  // has a decoration, it was originally caused by a top-level layout()
  // qualifier in GLSL.
  //
  ...
  // to
  //
  // struct Foo { mat4 matrix; }; // GLSL doesn't support any layout shenanigans
  // in raw struct declarations.
  // buffer UBO { layout(row_major) Foo foo; }; // Apply the layout on
  // top-level.
  ...
  if (dec.decoration_flags.get(DecorationLocation) &&
      can_use_io_location(type.storage, true))
  ...
  if (dec.decoration_flags.get(DecorationComponent) &&
    ...
    if (!options.es) {
      if (options.version < 440 && options.version >= 140)
        ...
        SPIRV_CROSS_THROW("Component decoration is not supported in targets "
                          "below GLSL 1.40.");
  ...
  // SPIRVCrossDecorationPacked is set by layout_for_variable earlier to mark
  // that we need to emit offset qualifiers.
  ...
  else if (type.storage == StorageClassOutput &&
           dec.decoration_flags.get(DecorationOffset))
...
uint32_t CompilerGLSL::type_to_packed_base_size(const SPIRType &type,
...
uint32_t CompilerGLSL::type_to_packed_alignment(const SPIRType &type,
                                                const Bitset &flags,
  ...
  if (type.storage == StorageClassPhysicalStorageBufferEXT) {
    ...
    if (ir.addressing_model == AddressingModelPhysicalStorageBuffer64EXT) {
      ...
      SPIRV_CROSS_THROW("AddressingModelPhysicalStorageBuffer64EXT must be "
                        "used for PhysicalStorageBufferEXT.");
  }
  ...
  if (!type.array.empty()) {
    ...
    return max(minimum_alignment,
               type_to_packed_alignment(*tmp, flags, packing));
  }
  ...
  if (type.basetype == SPIRType::Struct) {
    ...
    for (uint32_t i = 0; i < type.member_types.size(); i++) {
      ...
      alignment =
          max(alignment,
              type_to_packed_alignment(get<SPIRType>(type.member_types[i]),
                                       member_flags, packing));
    }
    ...
    // Alignment requirement for scalar block layout is always the alignment for
    // the most basic component.
    if (packing_is_scalar(packing))
      return base_alignment;

    // Vectors are *not* aligned in HLSL, but there's an extra rule where
    // vectors cannot straddle
...
uint32_t CompilerGLSL::type_to_packed_array_stride(
    const SPIRType &type, const Bitset &flags, BufferPackingStandard packing) {
...
uint32_t CompilerGLSL::type_to_packed_size(const SPIRType &type,
                                           const Bitset &flags,
                                           BufferPackingStandard packing) {
  if (!type.array.empty()) {
    uint32_t packed_size = to_array_size_literal(type) *
                           type_to_packed_array_stride(type, flags, packing);

    // For arrays of vectors and matrices in HLSL, the last element has a size
    // which depends on its vector size,
  ...
  if (type.basetype == SPIRType::Struct) {
    ...
    for (uint32_t i = 0; i < type.member_types.size(); i++) {
      ...
      uint32_t packed_alignment =
          type_to_packed_alignment(member_type, member_flags, packing);
      uint32_t alignment = max(packed_alignment, pad_alignment);

      // The next member following a struct member is aligned to the base
      // alignment of the struct that came before.
      // GL 4.5 spec, 7.6.2.2.
    ...
    if (packing_is_scalar(packing)) {
      ...
      // For matrices in HLSL, the last element has a size which depends on its
      // vector size,
      // so that it is possible to pack other vectors into the last element.
...
bool CompilerGLSL::buffer_is_packing_standard(const SPIRType &type,
                                              BufferPackingStandard packing,
                                              uint32_t *failed_validation_index,
                                              uint32_t start_offset,
                                              uint32_t end_offset) {
  // This is very tricky and error prone, but try to be exhaustive and correct
  // here.
  // SPIR-V doesn't directly say if we're using std430 or std140.
  // SPIR-V communicates this using Offset and ArrayStride decorations (which is
  // what really matters),
  // so we have to try to infer whether or not the original GLSL source was
  // std140 or std430 based on this information.
  // We do not have to consider shared or packed since these layouts are not
  // allowed in Vulkan SPIR-V (they are useless anyways, and custom offsets
  // would do the same thing).
  //
  // It is almost certain that we're using std430, but it gets tricky with
  // arrays in particular.
  // We will assume std430, but infer std140 if we can prove the struct is not
  // compliant with std430.
  //
  // The only two differences between std140 and std430 are related to padding
  // alignment/array stride
  ...
  bool is_top_level_block = has_decoration(type.self, DecorationBlock) ||
                            has_decoration(type.self, DecorationBufferBlock);
  ...
  for (uint32_t i = 0; i < type.member_types.size(); i++) {
    ...
    uint32_t packed_alignment =
        type_to_packed_alignment(memb_type, member_flags, packing);

    // This is a rather dirty workaround to deal with some cases of
    // OpSpecConstantOp used as array size, e.g:
    // layout(constant_id = 0) const int s = 10;
    // const int S = s + 5; // SpecConstantOp
    // buffer Foo { int data[S]; }; // <-- Very hard for us to deduce a fixed
    // value here,
    // we would need full implementation of compile-time constant folding. :(
    // If we are the last member of a struct, there might be cases where the
    // actual size of that member is irrelevant
    // for our analysis (e.g. unsized arrays).
    // This lets us simply ignore that there are spec constant op sized arrays
    // in our buffers.
    // Querying size of this member will fail, so just don't call it unless we
    // have to.
    //
    // This is likely "best effort" we can support without going into
    // unacceptably complicated workarounds.
    bool member_can_be_unsized = is_top_level_block &&
                                 size_t(i + 1) == type.member_types.size() &&
                                 !memb_type.array.empty();

    uint32_t packed_size = 0;
    if (!member_can_be_unsized || packing_is_hlsl(packing))
      packed_size = type_to_packed_size(memb_type, member_flags, packing);

    // We only need to care about this if we have non-array types which can
    // straddle the vec4 boundary.
    if (packing_is_hlsl(packing)) {
      // If a member straddles across a vec4 boundary, alignment is actually
      // vec4.
    ...
    // Field is not in the specified range anymore and we can ignore any further
    // fields.
    ...
    if (actual_offset >= start_offset) {
      ...
      // For EnhancedLayout variants, we have the flexibility to choose our own
      // offsets.
      if (!packing_has_flexible_offset(packing)) {
        if (actual_offset !=
            offset) // This cannot be the packing we're looking for.
        {
        ...
        // We still need to verify that alignment rules are observed, even if we
        // have explicit offset.
        if (failed_validation_index)
          *failed_validation_index = i;
        return false;
      }
      ...
      // We cannot use enhanced layouts on substructs, so they better be up to
      // spec.
...
bool CompilerGLSL::can_use_io_location(StorageClass storage, bool block) {
  // Location specifiers are must have in SPIR-V, but they aren't really
  // supported in earlier versions of GLSL.
  ...
  if ((get_execution_model() != ExecutionModelVertex &&
    ...
    if (!options.es && options.version < minimum_desktop_version &&
        !options.separate_shader_objects)
      return false;
...
string CompilerGLSL::layout_for_variable(const SPIRVariable &var) {
  ...
  if (options.vulkan_semantics) {
    if (flags.get(DecorationInputAttachmentIndex))
      attr.push_back(
          join("input_attachment_index = ",
               get_decoration(var.self, DecorationInputAttachmentIndex)));
  }

  bool is_block = has_decoration(type.self, DecorationBlock);
  if (flags.get(DecorationLocation) &&
      can_use_io_location(var.storage, is_block)) {
    ...
    if (!combined_decoration.get(DecorationLocation))
      attr.push_back(
          join("location = ", get_decoration(var.self, DecorationLocation)));
  }
  ...
  if (is_block && var.storage == StorageClassOutput) {
    // For blocks, there is a restriction where xfb_stride/xfb_buffer must only
    // be declared on the block itself,
    // since all members must match the same xfb_buffer. The only thing we will
    // declare for members of the block
    // is the xfb_offset.
    ...
    for (uint32_t i = 0; i < member_count; i++) {
      if (has_member_decoration(type.self, i, DecorationStream)) {
        uint32_t member_geom_stream =
            get_member_decoration(type.self, i, DecorationStream);
      ...
      if (has_member_decoration(type.self, i, DecorationXfbStride)) {
        uint32_t stride =
            get_member_decoration(type.self, i, DecorationXfbStride);
    ...
    if (have_geom_stream) {
      ...
      attr.push_back(
          join("stream = ", get_decoration(var.self, DecorationStream)));
    }
    ...
    if (flags.get(DecorationXfbBuffer) && flags.get(DecorationXfbStride) &&
      ...
      attr.push_back(
          join("xfb_buffer = ", get_decoration(var.self, DecorationXfbBuffer)));
      attr.push_back(
          join("xfb_stride = ", get_decoration(var.self, DecorationXfbStride)));
      attr.push_back(
          join("xfb_offset = ", get_decoration(var.self, DecorationOffset)));
      uses_enhanced_layouts = true;
    }
  ...
  if (flags.get(DecorationComponent) &&
    ...
    uses_enhanced_layouts = true;
    attr.push_back(
        join("component = ", get_decoration(var.self, DecorationComponent)));
  }
  ...
  if (uses_enhanced_layouts) {
    if (!options.es) {
      if (options.version < 440 && options.version >= 140)
        ...
        SPIRV_CROSS_THROW("GL_ARB_enhanced_layouts is not supported in targets "
                          "below GLSL 1.40.");
  ...
  if (var.storage != StorageClassPushConstant &&
    ...
    if (flags.get(DecorationDescriptorSet) && options.vulkan_semantics)
      attr.push_back(
          join("set = ", get_decoration(var.self, DecorationDescriptorSet)));
  }

  bool push_constant_block =
      options.vulkan_semantics && var.storage == StorageClassPushConstant;
  bool ssbo_block = var.storage == StorageClassStorageBuffer ||
                    var.storage == StorageClassShaderRecordBufferKHR ||
                    (var.storage == StorageClassUniform &&
                     typeflags.get(DecorationBufferBlock));
  bool emulated_ubo = var.storage == StorageClassPushConstant &&
                      options.emit_push_constant_as_uniform_buffer;
  bool ubo_block =
      var.storage == StorageClassUniform && typeflags.get(DecorationBlock);

  // GL 3.0/GLSL 1.30 is not considered legacy, but it doesn't have UBOs ...
  bool can_use_buffer_blocks = (options.es && options.version >= 300) ||
                               (!options.es && options.version >= 140);
  ...
  if (can_use_binding && flags.get(DecorationBinding))
    attr.push_back(
        join("binding = ", get_decoration(var.self, DecorationBinding)));

  if (var.storage != StorageClassOutput && flags.get(DecorationOffset))
    attr.push_back(
        join("offset = ", get_decoration(var.self, DecorationOffset)));

  // Instead of adding explicit offsets for every element here, just assume
  // we're using std140 or std430.
  // If SPIR-V does not comply with either layout, we cannot really work around
  // it.
...
string CompilerGLSL::buffer_to_packing_standard(
    const SPIRType &type, bool support_std430_without_scalar_layout) {
  if (support_std430_without_scalar_layout &&
    ...
    return "std140";
           ...
           buffer_is_packing_standard(type, BufferPackingScalar)) {
    require_extension_internal("GL_EXT_scalar_block_layout");
    return "scalar";
  } else if (support_std430_without_scalar_layout &&
             buffer_is_packing_standard(type,
                                        BufferPackingStd430EnhancedLayout)) {
    if (options.es && !options.vulkan_semantics)
      SPIRV_CROSS_THROW("Push constant block cannot be expressed as neither "
                        "std430 nor std140. ES-targets do "
    ...
    return "std430";
  } else if (buffer_is_packing_standard(type,
                                        BufferPackingStd140EnhancedLayout)) {
    // Fallback time. We might be able to use the ARB_enhanced_layouts to deal
    // with this difference,
    // however, we can only use layout(offset) on the block itself, not any
    // substructs, so the substructs better be the appropriate layout.
    // Enhanced layouts seem to always work in Vulkan GLSL, so no need for
    // extensions there.
    if (options.es && !options.vulkan_semantics)
      SPIRV_CROSS_THROW("Push constant block cannot be expressed as neither "
                        "std430 nor std140. ES-targets do "
    ...
    SPIRV_CROSS_THROW("Buffer block cannot be expressed as any of std430, "
                      "std140, scalar, even with enhanced "
                      "layouts. You can try flattening this block to support a "
...
void CompilerGLSL::emit_push_constant_block_glsl(const SPIRVariable &var) {
  // OpenGL has no concept of push constant blocks, implement it as a uniform
  // struct.
  auto &type = get<SPIRType>(var.basetype);
  ...
  // We're emitting the push constant block as a regular struct, so disable the
  // block qualifier temporarily.
  // Otherwise, we will end up emitting layout() qualifiers on naked structs
...
void CompilerGLSL::emit_buffer_reference_block(uint32_t type_id,
                                               bool forward_declaration) {
  auto &type = get<SPIRType>(type_id);
  string buffer_name;

  if (forward_declaration) {
    // Block names should never alias, but from HLSL input they kind of can
    // because block types are reused for UAVs ...
    // Allow aliased name since we might be declaring the block twice. Once with
    // buffer reference (forward declared) and one proper declaration.
    ...
    // Make sure we get something unique for both global name scope and block
    // name scope.
  ...
  if (!forward_declaration) {
    ...
    if (type.basetype == SPIRType::Struct) {
      ...
      statement("layout(", merge(attributes), ")", decorations, " buffer ",
                buffer_name);
    } else if (alignment)
      statement("layout(buffer_reference, buffer_reference_align = ", alignment,
                ") buffer ", buffer_name);
    else
    ...
    if (type.basetype == SPIRType::Struct) {
      ...
      statement(type_to_glsl(pointee_type), " value",
                type_to_array_glsl(pointee_type), ";");
    }
...
void CompilerGLSL::emit_buffer_block_native(const SPIRVariable &var) {
  ...
  // Make sure we get something unique for both global name scope and block name
  // scope.
  // See GLSL 4.5 spec: section 4.3.9 for details.
  add_variable(block_namespace, resource_names, buffer_name);

  // If for some reason buffer_name is an illegal name, make a final fallback to
  // a workaround name.
  // This cannot conflict with anything else, so we're safe now.
  // We cannot reuse this fallback name in neither global scope (blocked by
  // block_names) nor block name scope.
  ...
  statement(layout_for_variable(var), is_coherent ? "coherent " : "",
            is_restrict ? "restrict " : "", is_writeonly ? "writeonly " : "",
            is_readonly ? "readonly " : "", ssbo ? "buffer " : "uniform ",
            buffer_name);
...
void CompilerGLSL::emit_buffer_block_flattened(const SPIRVariable &var) {
  ...
  if (get_common_basic_type(type, basic_type)) {
    ...
    if (basic_type != SPIRType::Float && basic_type != SPIRType::Int &&
        basic_type != SPIRType::UInt)
      SPIRV_CROSS_THROW(
          "Basic types in a flattened UBO must be float, int or uint.");

    auto flags = ir.get_buffer_block_flags(var);
    statement("uniform ", flags_to_qualifiers_glsl(tmp, flags),
              type_to_glsl(tmp), " ", buffer_name, "[", buffer_size, "];");
  } else
...
const char *CompilerGLSL::to_storage_qualifiers_glsl(const SPIRVariable &var) {
  ...
  if (var.storage == StorageClassInput || var.storage == StorageClassOutput) {
    if (is_legacy() && execution.model == ExecutionModelVertex)
      ...
      return "varying "; // Fragment outputs are renamed so they never hit this
                         // case.
...
void CompilerGLSL::emit_flattened_io_block_member(
    const std::string &basename, const SPIRType &type, const char *qual,
  ...
  // We're overriding struct member names, so ensure we do so on the primary
  // type.
  if (parent_type->type_alias)
    parent_type = &get<SPIRType>(parent_type->type_alias);

  // Sanitize underscores because joining the two identifiers might create more
  // than 1 underscore in a row,
  // which is not allowed.
  ParsedIR::sanitize_underscores(flattened_name);

  uint32_t last_index = indices.back();

  // Pass in the varying qualifier here so it will appear in the correct
  // declaration order.
  // Replace member name while emitting it so it encodes both struct name and
  // member name.
...
void CompilerGLSL::emit_flattened_io_block_struct(
    const std::string &basename, const SPIRType &type, const char *qual,
    const SmallVector<uint32_t> &indices) {
  ...
  for (uint32_t i = 0; i < uint32_t(member_type->member_types.size()); i++) {
    sub_indices.back() = i;
    if (get<SPIRType>(member_type->member_types[i]).basetype ==
        SPIRType::Struct)
...
void CompilerGLSL::emit_flattened_io_block(const SPIRVariable &var,
                                           const char *qual) {
  auto &var_type = get<SPIRType>(var.basetype);
  if (!var_type.array.empty())
    SPIRV_CROSS_THROW("Array of varying structs cannot be flattened to "
                      "legacy-compatible varyings.");

  // Emit flattened types based on the type alias. Normally, we are never
  // supposed to emit
  // struct declarations for aliased types.
  auto &type =
      var_type.type_alias ? get<SPIRType>(var_type.type_alias) : var_type;
...
void CompilerGLSL::emit_interface_block(const SPIRVariable &var) {
  ...
  if (block) {
    ...
    if (options.force_flattened_io_blocks ||
        (options.es && options.version < 310) ||
        (!options.es && options.version < 150)) {
      // I/O blocks on ES require version 310 with Android Extension Pack
      // extensions, or core version 320.
      // On desktop, I/O blocks were introduced with geometry shaders in GL 3.2
      // (GLSL 150).
      ...
      if (options.es && options.version < 320) {
        ...
        if (!has_extension("GL_EXT_geometry_shader") &&
            !has_extension("GL_EXT_tessellation_shader"))
      ...
      // If for some reason buffer_name is an illegal name, make a final
      // fallback to a workaround name.
      // This cannot conflict with anything else, so we're safe now.
      if (block_name.empty())
    ...
    // To support this, flatten the struct into separate varyings instead.
    if (type.basetype == SPIRType::Struct &&
        (options.force_flattened_io_blocks ||
         (options.es && options.version < 310) ||
         (!options.es && options.version < 150))) {
      emit_flattened_io_block(var, qual);
    } else {
      ...
      bool control_point_input_array =
          type.storage == StorageClassInput && !type.array.empty() &&
...
void CompilerGLSL::emit_specialization_constant_op(
    const SPIRConstantOp &constant) {
  auto &type = get<SPIRType>(constant.basetype);
  auto name = to_name(constant.self);
  statement("const ", variable_decl(type, name), " = ",
            constant_op_expression(constant), ";");
}
...
void CompilerGLSL::emit_constant(const SPIRConstant &constant) {
  ...
  // This specialization constant is implicitly declared by emitting layout()
  // in;
  if (constant.self == workgroup_size_id)
    return;

  // These specialization constants are implicitly declared by emitting layout()
  // in;
  // In legacy GLSL, we will still need to emit macros for these, so a layout()
  // in; declaration
  // later can use macro overrides for work group size.
  bool is_workgroup_size_constant = ConstantID(constant.self) == wg_x.id ||
                                    ConstantID(constant.self) == wg_y.id ||
                                    ConstantID(constant.self) == wg_z.id;

  if (options.vulkan_semantics && is_workgroup_size_constant) {
    // Vulkan GLSL does not need to declare workgroup spec constants explicitly,
    // it is handled in layout().
    return;
    ...
    // constant, because we need macros.
    return;
  }
  ...
  if (has_decoration(constant.self, DecorationSpecId)) {
    if (options.vulkan_semantics) {
      statement("layout(constant_id = ",
                get_decoration(constant.self, DecorationSpecId), ") const ",
...
void CompilerGLSL::replace_fragment_output(SPIRVariable &var) {
  ...
  // If our variable is arrayed, we must not emit the array part of this as the
  // SPIR-V will
  ...
  if (type.array.empty()) {
    ...
    // This gets really tricky since we'd have to inject an offset in the access
    // chain.
    // FIXME: This seems like an extremely odd-ball case, so it's probably fine
    ...
    SPIRV_CROSS_THROW("Array-of-array output variable used. This cannot be "
                      "implemented in legacy GLSL.");

  var.compat_builtin =
      true; // We don't want to declare this variable, but use the name as-is.
}
...
string CompilerGLSL::remap_swizzle(const SPIRType &out_type,
...
void CompilerGLSL::fixup_image_load_store_access() {
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t var, const SPIRVariable &) {
    ...
    if (vartype.basetype == SPIRType::Image && vartype.image.sampled == 2) {
      // Very old glslangValidator and HLSL compilers do not emit required
      // qualifiers here.
      // Solve this by making the image access as restricted as possible and
      // loosen up if we need to.
      // If any no-read/no-write flags are actually set, assume that the
      // compiler knows what it's doing.

      auto &flags = ir.meta[var].decoration.decoration_flags;
      if (!flags.get(DecorationNonWritable) &&
          !flags.get(DecorationNonReadable)) {
...
bool CompilerGLSL::should_force_emit_builtin_block(StorageClass storage) {
  // If the builtin block uses XFB, we need to force explicit redeclaration of
  // the builtin block.
    ...
    if (var.storage == storage && block && is_builtin_variable(var)) {
      ...
      for (uint32_t i = 0; i < member_count; i++) {
        if (has_member_decoration(type.self, i, DecorationBuiltIn) &&
            is_block_builtin(BuiltIn(
                get_member_decoration(type.self, i, DecorationBuiltIn))) &&
      ...
      if (is_block_builtin(
              BuiltIn(get_decoration(type.self, DecorationBuiltIn))) &&
  ...
  // If we're declaring clip/cull planes with control points we need to force
  // block declaration.
...
void CompilerGLSL::emit_declared_builtin_block(StorageClass storage,
  ...
  // These variables might have been declared, but not statically used, so we
  // haven't deduced their size yet.
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
    ...
    if (var.storage == storage && block && is_builtin_variable(var)) {
      ...
      for (auto &m : ir.meta[type.self].members) {
        if (m.builtin) {
          ...
          if (m.builtin_type == BuiltInCullDistance)
            cull_distance_size = to_array_size_literal(
                this->get<SPIRType>(type.member_types[index]));
          else if (m.builtin_type == BuiltInClipDistance)
            clip_distance_size = to_array_size_literal(
                this->get<SPIRType>(type.member_types[index]));

          if (is_block_builtin(m.builtin_type) &&
          ...
          if (is_block_builtin(m.builtin_type) &&
              m.decoration_flags.get(DecorationStream)) {
      ...
      if (storage == StorageClassOutput &&
          has_decoration(var.self, DecorationXfbBuffer) &&
      ...
      if (storage == StorageClassOutput &&
          has_decoration(var.self, DecorationStream)) {
      ...
      if (m.builtin) {
        ...
        if (is_block_builtin(m.builtin_type) &&
            m.decoration_flags.get(DecorationXfbStride) &&
            m.decoration_flags.get(DecorationXfbBuffer) &&
            m.decoration_flags.get(DecorationOffset)) {
          have_any_xfb_offset = true;
          builtin_xfb_offsets[m.builtin_type] = m.offset;
  ...
  global_builtins =
      Bitset(global_builtins.get_lower() &
             ((1ull << BuiltInPosition) | (1ull << BuiltInPointSize) |
  ...
  if (storage == StorageClassOutput) {
    ...
    if (have_xfb_buffer_stride && have_any_xfb_offset) {
      ...
      attr.push_back(
          join("xfb_buffer = ", xfb_buffer, ", xfb_stride = ", xfb_stride));
    }
  ...
  if (emitted_builtins.get(BuiltInClipDistance)) {
    ...
    if (itr != end(builtin_xfb_offsets))
      statement("layout(xfb_offset = ", itr->second, ") float gl_ClipDistance[",
                clip_distance_size, "];");
    else
  ...
  if (emitted_builtins.get(BuiltInCullDistance)) {
    ...
    if (itr != end(builtin_xfb_offsets))
      statement("layout(xfb_offset = ", itr->second, ") float gl_CullDistance[",
                cull_distance_size, "];");
    else
...
bool CompilerGLSL::variable_is_lut(const SPIRVariable &var) const {
  bool statically_assigned = var.statically_assigned &&
                             var.static_expression != ID(0) &&
...
void CompilerGLSL::emit_resources() {
  ...
  if (options.separate_shader_objects && !options.es &&
    ...
    // If we're doing passthrough, we cannot emit an output block, so the output
    // block test above will never pass.
    emit_declared_builtin_block(StorageClassInput, execution.model);
  } else {
    // Need to redeclare clip/cull distance with explicit size to use them.
    // SPIR-V mandates these builtins have a size declared.
    const char *storage =
        execution.model == ExecutionModelFragment ? "in" : "out";
  ...
  //
  {
    ...
    for (auto &id_ : ir.ids_for_constant_or_type) {
      ...
      if (id.get_type() == TypeConstant) {
        ...
        // Special case, ray payload and hit attribute blocks are not really
        // blocks, just regular structs.
        if (type->basetype == SPIRType::Struct && type->pointer &&
            has_decoration(type->self, DecorationBlock) &&
            (type->storage == StorageClassRayPayloadKHR ||
             type->storage == StorageClassIncomingRayPayloadKHR ||
  ...
  // If the work group size depends on a specialization constant, we need to
  // declare the layout() block
  ...
  if (execution.model == ExecutionModelGLCompute && !options.vulkan_semantics &&
    ...
    if ((wg_x.id != ConstantID(0)) || (wg_y.id != ConstantID(0)) ||
        (wg_z.id != ConstantID(0))) {
      SmallVector<string> inputs;
      build_workgroup_size(inputs, wg_x, wg_y, wg_z);
      statement("layout(", merge(inputs), ") in;");
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
    auto &type = this->get<SPIRType>(var.basetype);
    if (var.storage != StorageClassFunction && type.pointer &&
        type.storage == StorageClassPushConstant && !is_hidden_variable(var)) {
      emit_push_constant_block(var);
    }
  });

  bool skip_separate_image_sampler =
      !combined_image_samplers.empty() || !options.vulkan_semantics;

  // Output Uniform Constants (values, samplers, images, etc).
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
    auto &type = this->get<SPIRType>(var.basetype);

    // If we're remapping separate samplers and images, only emit the combined
    // samplers.
    if (skip_separate_image_sampler) {
      // Sampler buffers are always used without a sampler, and they will also
      // work in regular GL.
      bool sampler_buffer =
          type.basetype == SPIRType::Image && type.image.dim == DimBuffer;
      bool separate_image =
    ...
    if (var.storage != StorageClassFunction && type.pointer &&
        (type.storage == StorageClassUniformConstant ||
         ...
         type.storage == StorageClassRayPayloadKHR ||
         type.storage == StorageClassIncomingRayPayloadKHR ||
         type.storage == StorageClassCallableDataKHR ||
         type.storage == StorageClassIncomingCallableDataKHR ||
         type.storage == StorageClassHitAttributeKHR) &&
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
    ...
    // Unused output I/O variables might still be required to implement
    // framebuffer fetch.
    if (var.storage == StorageClassOutput && !is_legacy() &&
        location_is_framebuffer_fetch(
            get_decoration(var.self, DecorationLocation)) != 0) {
      is_hidden = false;
    }
    ...
    if (var.storage != StorageClassFunction && type.pointer &&
      ...
      if (!options.vulkan_semantics) {
        if (!emitted_base_instance &&
            ((options.vertex.support_nonzero_base_instance &&
              builtin == BuiltInInstanceIndex) ||
          ...
          statement("#else");
          // A crude, but simple workaround which should be good enough for
          // non-indirect draws.
...
void CompilerGLSL::emit_output_variable_initializer(const SPIRVariable &var) {
  // If a StorageClassOutput variable has an initializer, we need to initialize
  // it in main().
  ...
  bool is_control_point =
      get_execution_model() == ExecutionModelTessellationControl && !is_patch;
  ...
  if (is_block) {
    ...
    // If the initializer is a block, we must initialize each block member one
    // at a time.
    for (uint32_t i = 0; i < member_count; i++) {
      // These outputs might not have been properly declared, so don't
      // initialize them in that case.
      if (has_member_decoration(type.self, i, DecorationBuiltIn)) {
        if (get_member_decoration(type.self, i, DecorationBuiltIn) ==
                BuiltInCullDistance &&
            !cull_distance_count)
          continue;
      ...
      // We need to build a per-member array first, essentially transposing from
      // AoS to SoA.
      ...
      if (type_is_array) {
        ...
        for (uint32_t j = 0; j < array_size; j++)
          exprs.push_back(to_expression(
              get<SPIRConstant>(c.subconstants[j]).subconstants[i]));
        statement("const ", type_to_glsl(array_type), " ", lut_name,
                  type_to_array_glsl(array_type), " = ",
      ...
      for (uint32_t j = 0; j < iteration_count; j++) {
        entry_func.fixup_hooks_in.push_back([=, &var]() {
          ...
          if (is_control_point) {
            ...
            set<SPIRExpression>(ids + 1, builtin_to_glsl(BuiltInInvocationId,
                                                         StorageClassInput),
                                ids, true);
          ...
          if (type_is_array && !is_control_point) {
            ...
            auto chain = access_chain_internal(
                var.self, indices, 2, ACCESS_CHAIN_INDEX_IS_LITERAL_BIT, &meta);
            ...
            auto chain = access_chain_internal(
                var.self, &i, 1, ACCESS_CHAIN_INDEX_IS_LITERAL_BIT, &meta);
    ...
    entry_func.fixup_hooks_in.push_back([&, lut_name]() {
      statement(to_expression(var.self), "[gl_InvocationID] = ", lut_name,
                "[gl_InvocationID];");
    });
  } else if (has_decoration(var.self, DecorationBuiltIn) &&
             BuiltIn(get_decoration(var.self, DecorationBuiltIn)) ==
                 BuiltInSampleMask) {
    // We cannot copy the array since gl_SampleMask is unsized in GLSL. Unroll
    // time! <_<
    entry_func.fixup_hooks_in.push_back([&] {
      auto &c = this->get<SPIRConstant>(var.initializer);
      uint32_t num_constants = uint32_t(c.subconstants.size());
      for (uint32_t i = 0; i < num_constants; i++) {
        // Don't use to_expression on constant since it might be uint, just fish
        // out the raw int.
        statement(to_expression(var.self), "[", i, "] = ",
                  convert_to_string(
                      this->get<SPIRConstant>(c.subconstants[i]).scalar_i32()),
                  ";");
      }
    });
  } else {
...
void CompilerGLSL::emit_extension_workarounds(spv::ExecutionModel model) {
  ...
  if (!options.vulkan_semantics) {
    ...
    if (shader_subgroup_supporter.is_feature_requested(Supp::SubgroupMask)) {
      ...
      for (auto &e : exts) {
        ...
        case Supp::ARB_shader_ballot:
          statement("#define gl_SubgroupEqMask "
                    "uvec4(unpackUint2x32(gl_SubGroupEqMaskARB), 0u, 0u)");
          statement("#define gl_SubgroupGeMask "
                    "uvec4(unpackUint2x32(gl_SubGroupGeMaskARB), 0u, 0u)");
          statement("#define gl_SubgroupGtMask "
                    "uvec4(unpackUint2x32(gl_SubGroupGtMaskARB), 0u, 0u)");
          statement("#define gl_SubgroupLeMask "
                    "uvec4(unpackUint2x32(gl_SubGroupLeMaskARB), 0u, 0u)");
          statement("#define gl_SubgroupLtMask "
                    "uvec4(unpackUint2x32(gl_SubGroupLtMaskARB), 0u, 0u)");
          break;
        default:
          break;
        }
    ...
    if (shader_subgroup_supporter.is_feature_requested(
            Supp::SubgroupInvocationID)) {
      auto exts =
          Supp::get_candidates_for_feature(Supp::SubgroupInvocationID, result);
    ...
    if (shader_subgroup_supporter.is_feature_requested(
            Supp::SubgroupBroadcast_First)) {
      auto exts = Supp::get_candidates_for_feature(
          Supp::SubgroupBroadcast_First, result);
        ...
        case Supp::NV_shader_thread_shuffle:
          for (const char *t : workaround_types) {
            statement(t, " subgroupBroadcastFirst(", t,
                      " value) { return shuffleNV(value, "
                      "findLSB(ballotThreadNV(true)), gl_WarpSizeNV); }");
          }
          for (const char *t : workaround_types) {
            statement(t, " subgroupBroadcast(", t, " value, uint id) { return "
                                                   "shuffleNV(value, id, "
                                                   "gl_WarpSizeNV); }");
          }
          break;
    ...
    if (shader_subgroup_supporter.is_feature_requested(
            Supp::SubgroupBallotFindLSB_MSB)) {
      auto exts = Supp::get_candidates_for_feature(
          Supp::SubgroupBallotFindLSB_MSB, result);
      ...
      for (auto &e : exts) {
        ...
        switch (e) {
        case Supp::NV_shader_thread_group:
          statement("uint subgroupBallotFindLSB(uvec4 value) { return "
                    "findLSB(value.x); }");
          statement("uint subgroupBallotFindMSB(uvec4 value) { return "
                    "findMSB(value.x); }");
          break;
        default:
          break;
      ...
      statement("return uint(firstLive != -1 ? firstLive : (findLSB(value.y) + "
                "32));");
      end_scope();
      statement("uint subgroupBallotFindMSB(uvec4 value)");
      begin_scope();
      statement("int firstLive = findMSB(value.y);");
      statement("return uint(firstLive != -1 ? (firstLive + 32) : "
                "findMSB(value.x));");
      end_scope();
      statement("#endif");
      statement("");
    ...
    if (shader_subgroup_supporter.is_feature_requested(
            Supp::SubgroupAll_Any_AllEqualBool)) {
      auto exts = Supp::get_candidates_for_feature(
          Supp::SubgroupAll_Any_AllEqualBool, result);
      ...
      for (auto &e : exts) {
        ...
        case Supp::NV_gpu_shader_5:
          ...
          statement("bool subgroupAllEqual(bool value) { return "
                    "allThreadsEqualNV(value); }");
          break;
        case Supp::ARB_shader_group_vote:
          ...
          statement("bool subgroupAllEqual(bool v) { return "
                    "allInvocationsEqualARB(v); }");
          break;
        case Supp::AMD_gcn_shader:
          statement("bool subgroupAll(bool value) { return ballotAMD(value) == "
                    "ballotAMD(true); }");
          statement("bool subgroupAny(bool value) { return ballotAMD(value) != "
                    "0ull; }");
          statement("bool subgroupAllEqual(bool value) { uint64_t b = "
                    "ballotAMD(value); return b == 0ull || "
                    "b == ballotAMD(true); }");
          break;
        default:
          break;
    ...
    if (shader_subgroup_supporter.is_feature_requested(
            Supp::SubgroupAllEqualT)) {
      statement("#ifndef GL_KHR_shader_subgroup_vote");
      statement("#define _SPIRV_CROSS_SUBGROUP_ALL_EQUAL_WORKAROUND(type) bool "
                "subgroupAllEqual(type value) { return "
    ...
    if (shader_subgroup_supporter.is_feature_requested(Supp::SubgroupBallot)) {
      auto exts =
          Supp::get_candidates_for_feature(Supp::SubgroupBallot, result);
      ...
      for (auto &e : exts) {
        ...
        switch (e) {
        case Supp::NV_shader_thread_group:
          statement("uvec4 subgroupBallot(bool v) { return "
                    "uvec4(ballotThreadNV(v), 0u, 0u, 0u); }");
          break;
        case Supp::ARB_shader_ballot:
          statement("uvec4 subgroupBallot(bool v) { return "
                    "uvec4(unpackUint2x32(ballotARB(v)), 0u, 0u); }");
          break;
        default:
          break;
        }
    ...
    if (shader_subgroup_supporter.is_feature_requested(Supp::SubgroupBarrier)) {
      ...
      // However the GL 4.6 spec also states that `barrier` implies a shared
      // memory barrier,
      // and a specific test of optimizing scans by leveraging lock-step
      // invocation execution,
      // has shown that a `memoryBarrierShared` is needed in place of a
      // `subgroupBarrier`.
    ...
    if (shader_subgroup_supporter.is_feature_requested(
            Supp::SubgroupInverseBallot_InclBitCount_ExclBitCout)) {
  ...
  if (!workaround_ubo_load_overload_types.empty()) {
    for (auto &type_id : workaround_ubo_load_overload_types) {
      auto &type = get<SPIRType>(type_id);
      statement(type_to_glsl(type), " spvWorkaroundRowMajor(",
                type_to_glsl(type), " wrap) { return wrap; }");
    }
    statement("");
  }
  ...
  if (requires_transpose_3x3) {
    ...
    begin_scope();
    statement("return mat3(m[0][0], m[1][0], m[2][0], m[0][1], m[1][1], "
  ...
  if (requires_transpose_4x4) {
    ...
    begin_scope();
    statement(
        "return mat4(m[0][0], m[1][0], m[2][0], m[3][0], m[0][1], m[1][1], "
        "m[2][1], m[3][1], m[0][2], "
...
string CompilerGLSL::to_func_call_arg(const SPIRFunction::Parameter &,
                                      uint32_t id) {
  // Make sure that we use the name of the original variable, and not the
  // parameter alias.
  uint32_t name_id = id;
...
void CompilerGLSL::handle_invalid_expression(uint32_t id) {
  ...
  // This means we need another pass at compilation, but next time, force
  // temporary variables so that they cannot be invalidated.
...
// Sometimes we proactively enclosed an expression where it turns out we might
// have not needed it after all.
void CompilerGLSL::strip_enclosed_expression(string &expr) {
  if (expr.size() < 2 || expr.front() != '(' || expr.back() != ')')
    return;

  // Have to make sure that our first and last parens actually enclose
  ...
  for (auto &c : expr) {
    if (c == '(')
      ...
      // If we hit 0 and this is not the final char, our first and final parens
      // actually don't
...
string CompilerGLSL::enclose_expression(const string &expr) {
  ...
  // If the expression starts with a unary we need to enclose to deal with cases
  ...
  // If this expression contains any spaces which are not enclosed by
  // parentheses,
...
string CompilerGLSL::dereference_expression(const SPIRType &expr_type,
  ...
  if (expr.front() == '&')
    ...
    return join('*', expr);
  else if (expr_type.storage == StorageClassPhysicalStorageBufferEXT &&
...
string CompilerGLSL::address_of_expression(const std::string &expr) {
  if (expr.size() > 3 && expr[0] == '(' && expr[1] == '*' &&
      expr.back() == ')') {
    // If we have an expression which looks like (*foo), taking the address of
    // it is the same as stripping
    // the first two and last characters. We might have to enclose the
    // expression.
    // This doesn't work for cases like (*foo + 10),
    // but this is an r-value expression which we cannot take the address of
    // anyways.
...
// Just like to_expression except that we enclose the expression inside
...
// Used explicitly when we want to read a row-major expression, but without any
// transpose shenanigans.
// need_transpose must be forced to false.
string CompilerGLSL::to_unpacked_row_major_matrix_expression(uint32_t id) {
  return unpack_expression_type(
      to_expression(id), expression_type(id),
...
string CompilerGLSL::to_unpacked_expression(uint32_t id,
  ...
  bool is_remapped =
      has_extended_decoration(id, SPIRVCrossDecorationPhysicalTypeID);
  bool is_packed =
      has_extended_decoration(id, SPIRVCrossDecorationPhysicalTypePacked);

  if (!need_transpose && (is_remapped || is_packed)) {
    return unpack_expression_type(
        to_expression(id, register_expression_read),
...
string
CompilerGLSL::to_enclosed_unpacked_expression(uint32_t id,
                                              bool register_expression_read) {
  return enclose_expression(
      to_unpacked_expression(id, register_expression_read));
}

string CompilerGLSL::to_dereferenced_expression(uint32_t id,
                                                bool register_expression_read) {
  auto &type = expression_type(id);
  if (type.pointer && should_dereference(id))
    return dereference_expression(
        type, to_enclosed_expression(id, register_expression_read));
  else
    return to_expression(id, register_expression_read);
}
...
string CompilerGLSL::to_pointer_expression(uint32_t id,
  ...
  if (type.pointer && expression_is_lvalue(id) && !should_dereference(id))
    return address_of_expression(
        to_enclosed_expression(id, register_expression_read));
  else
    return to_unpacked_expression(id, register_expression_read);
}

string
CompilerGLSL::to_enclosed_pointer_expression(uint32_t id,
                                             bool register_expression_read) {
  auto &type = expression_type(id);
  if (type.pointer && expression_is_lvalue(id) && !should_dereference(id))
    return address_of_expression(
...
string CompilerGLSL::to_extract_component_expression(uint32_t id,
                                                     uint32_t index) {
...
string CompilerGLSL::to_extract_constant_composite_expression(
    uint32_t result_type, const SPIRConstant &c, const uint32_t *chain,
    uint32_t length) {
  // It is kinda silly if application actually enter this path since they know
  ...
  assert(composite_type.basetype != SPIRType::Struct &&
...
string CompilerGLSL::to_rerolled_array_expression(const string &base_expr,
                                                  const SPIRType &type) {
...
string
CompilerGLSL::to_composite_constructor_expression(uint32_t id,
  ...
  if (reroll_array) {
    // For this case, we need to "re-roll" an array initializer from a
    // temporary.
    // We cannot simply pass the array directly, since it decays to a pointer
    // and it cannot
    // participate in a struct initializer. E.g.
    // float arr[2] = { 1.0, 2.0 };
    // Foo foo = { arr }; must be transformed to
    // Foo foo = { { arr[0], arr[1] } };
    // The array sizes cannot be deduced from specialization constants since we
    // cannot use any loops.

    // We're only triggering one read of the array expression, but this is fine
    // since arrays have to be declared
...
string CompilerGLSL::to_expression(uint32_t id, bool register_expression_read) {
  ...
  if (ir.ids[id].get_type() == TypeExpression) {
    ...
    // %3 = OpDoSomethingAgain %2 %2. Here %3 will lose the link to %1 since we
    // don't propagate the dependencies like that.
    // OpStore %1 %foo // Here we can invalidate %1, and hence all expressions
    // which depend on %1. Only %2 will know since it's part of
    // invalid_expressions.
    // %4 = OpDoSomethingAnotherTime %3 %3 // If we forward all expressions we
    // will see %1 expression after store, not before.
    //
    // However, we can propagate up a list of depended expressions when we used
    // %2, so we can check if %2 is invalid when reading %3 after the store,
  ...
  case TypeExpression: {
    ...
    if (e.base_expression)
      ...
      // This should not be reached for access chains, since we always deal
      // explicitly with transpose state
      ...
      return convert_row_major_matrix(e.expression,
                                      get<SPIRType>(e.expression_type),
                                      physical_type_id, is_packed);
    } else if (flattened_structs.count(id)) {
      return load_flattened_struct(e.expression,
                                   get<SPIRType>(e.expression_type));
    } else {
      if (is_forcing_recompilation()) {
        // During first compilation phase, certain expression patterns can
        // trigger exponential growth of memory.
        // Avoid this by returning dummy expressions during this phase.
        // Do not use empty expressions here, because those are sentinels for
        // other cases.
        return "_";
      } else
        return e.expression;
  ...
  case TypeConstant: {
    ...
    if (dec.builtin)
      ...
      return to_name(id);
             ...
             !backend.can_declare_struct_inline)
      return to_name(id);
  ...
  case TypeVariable: {
    ...
    // If we try to use a loop variable before the loop header, we have to
    // redirect it to the static expression,
    // the variable has not been declared yet.
    if (var.statically_assigned ||
        (var.loop_variable && !var.loop_variable_enable))
  ...
  case TypeCombinedImageSampler:
    ...
    // The intention is that texture sampling functions will extract the image
    // and samplers
    // separately and take their expressions as needed.
    // GLSL does not use this type because OpSampledImage immediately creates a
  ...
  case TypeAccessChain:
    // We cannot express this type. They only have meaning in other
    // OpAccessChains, OpStore or OpLoad.
    SPIRV_CROSS_THROW(
...
#define GLSL_UOP(opname, x)                                                    \
  ...
  case OpSelect: {
    ...
    // If we cannot, fail.
    if (to_trivial_mix_op(type, op, cop.arguments[2], cop.arguments[1],
                          cop.arguments[0])) {
      // Implement as a simple cast down below.
    } else {
      // Implement a ternary and pray the compiler understands it :)
      return to_ternary_expression(type, cop.arguments[0], cop.arguments[1],
                                   cop.arguments[2]);
    }
    break;
  }
  ...
  case OpCompositeExtract: {
    auto expr = access_chain_internal(
        cop.arguments[0], &cop.arguments[1], uint32_t(cop.arguments.size() - 1),
  ...
  default:
    // Some opcodes are unimplemented here, these are currently not possible to
    // test from glslang.
...
#undef GLSL_UOP
  if (binary) {
    ...
    string cast_op0;
    string cast_op1;
    auto expected_type = binary_op_bitcast_helper(
        cast_op0, cast_op1, input_type, cop.arguments[0], cop.arguments[1],
    ...
    // Works around various casting scenarios in glslang as there is no
    // OpBitcast for specialization constants.
...
string CompilerGLSL::constant_expression(const SPIRConstant &c) {
  ...
  if (type.pointer) {
    ...
    if (backend.use_initializer_list && backend.use_typed_initializer_list &&
        type.basetype == SPIRType::Struct && type.array.empty()) {
      res = type_to_glsl_constructor(type) + "{ ";
    } else if (backend.use_initializer_list &&
               backend.use_typed_initializer_list &&
               backend.array_is_value_type && !type.array.empty()) {
      res = type_to_glsl_constructor(type) + "({ ";
    ...
    return res;
             ...
             type.member_types.size() == 0) {
    // Metal tessellation likes empty structs which are then constant
    // expressions.
...
string CompilerGLSL::convert_half_to_string(const SPIRConstant &c, uint32_t col,
                                            uint32_t row) {
  string res;
  ...
  if (std::isnan(float_value) || std::isinf(float_value)) {
    ...
    type.vecsize = 1;
    type.columns = 1;
    res = join(type_to_glsl(type), "(",
               convert_to_string(float_value, current_locale_radix_character),
               ")");
  }
...
std::string CompilerGLSL::convert_double_to_string(const SPIRConstant &c,
  ...
  if (std::isnan(double_value) || std::isinf(double_value)) {
    ...
    if (!is_legacy()) {
      ...
      sprintf(print_buffer, "0x%llx%s",
              static_cast<unsigned long long>(u64_value),
...
string CompilerGLSL::constant_expression_vector(const SPIRConstant &c,
  ...
  if (splat || swizzle_splat) {
    // Cannot use constant splatting if we have specialization constants
  ...
  case SPIRType::Int64: {
    ...
    if (splat) {
      res += convert_to_string(c.scalar_i64(vector, 0), int64_type,
                               backend.long_long_literal_suffix);
    } else {
      for (uint32_t i = 0; i < c.vector_size(); i++) {
        if (c.vector_size() > 1 && c.specialization_constant_id(vector, i) != 0)
          res += to_name(c.specialization_constant_id(vector, i));
        else
          res += convert_to_string(c.scalar_i64(vector, i), int64_type,
                                   backend.long_long_literal_suffix);
  ...
  case SPIRType::UInt:
    if (splat) {
      ...
      if (is_legacy()) {
        ...
        // Things like array sizes, etc, tend to be unsigned even though they
        // could just as easily be signed.
        if (c.scalar_i32(vector, 0) < 0)
          SPIRV_CROSS_THROW("Tried to convert uint literal into int, but this "
                            "made the literal negative.");
      ...
      for (uint32_t i = 0; i < c.vector_size(); i++) {
        if (c.vector_size() > 1 && c.specialization_constant_id(vector, i) != 0)
          ...
          if (is_legacy()) {
            // Fake unsigned constant literals with signed ones if possible.
            // Things like array sizes, etc, tend to be unsigned even though
            // they could just as easily be signed.
...
SPIRExpression &
CompilerGLSL::emit_uninitialized_temporary_expression(uint32_t type,
                                                      uint32_t id) {
  forced_temporaries.insert(id);
...
void CompilerGLSL::emit_uninitialized_temporary(uint32_t result_type,
                                                uint32_t result_id) {
  // If we're declaring temporaries inside continue blocks,
  // we must declare the temporary in the loop header so that the continue block
  // can avoid declaring new variables.
  if (current_continue_block && !hoisted_temporaries.count(result_id)) {
    ...
    // The result_id has not been made into an expression yet, so use flags
    // interface.
    ...
    statement(flags_to_qualifiers_glsl(type, flags),
              variable_decl(type, to_name(result_id)), initializer, ";");
  }
}

string CompilerGLSL::declare_temporary(uint32_t result_type,
                                       uint32_t result_id) {
  auto &type = get<SPIRType>(result_type);
  auto &flags = ir.meta[result_id].decoration.decoration_flags;
  ...
  if (current_continue_block && !hoisted_temporaries.count(result_id)) {
    ...
    // The temporary has already been declared earlier, so just "declare" the
    // temporary by writing to it.
    return join(to_name(result_id), " = ");
  } else {
    // The result_id has not been made into an expression yet, so use flags
    // interface.
    add_local_variable_name(result_id);
    return join(flags_to_qualifiers_glsl(type, flags),
                variable_decl(type, to_name(result_id)), " = ");
  }
}
...
bool CompilerGLSL::expression_read_implies_multiple_reads(uint32_t id) const {
  ...
  // If we're emitting code at a deeper loop level than when we emitted the
  // expression,
  // we're probably reading the same expression over and over.
  return current_loop_level > expr->emitted_loop_level;
}

SPIRExpression &CompilerGLSL::emit_op(uint32_t result_type, uint32_t result_id,
                                      const string &rhs, bool forwarding,
                                      bool suppress_usage_tracking) {
  if (forwarding &&
      (forced_temporaries.find(result_id) == end(forced_temporaries))) {
    // Just forward it without temporary.
    // If the forward is trivial, we do not force flushing to temporary for this
    // expression.
    ...
    // If expression isn't immutable, bind it to a temporary and make the new
    // temporary immutable (they always are).
    statement(declare_temporary(result_type, result_id), rhs, ";");
    return set<SPIRExpression>(result_id, to_name(result_id), result_type,
                               true);
  }
}

void CompilerGLSL::emit_unary_op(uint32_t result_type, uint32_t result_id,
                                 uint32_t op0, const char *op) {
  bool forward = should_forward(op0);
  emit_op(result_type, result_id,
          join(op, to_enclosed_unpacked_expression(op0)), forward);
  inherit_expression_dependencies(result_id, op0);
}

void CompilerGLSL::emit_binary_op(uint32_t result_type, uint32_t result_id,
                                  uint32_t op0, uint32_t op1, const char *op) {
  ...
  bool forward =
      should_forward(op0) && should_forward(op1) && !force_temporary_precise;

  emit_op(result_type, result_id,
          join(to_enclosed_unpacked_expression(op0), " ", op, " ",
...
void CompilerGLSL::emit_unrolled_unary_op(uint32_t result_type,
...
void CompilerGLSL::emit_unrolled_binary_op(uint32_t result_type,
                                           uint32_t result_id, uint32_t op0,
  ...
  for (uint32_t i = 0; i < type.vecsize; i++) {
    ...
    if (expected_type != SPIRType::Unknown && type0.basetype != expected_type)
      expr += bitcast_expression(target_type0, type0.basetype,
                                 to_extract_component_expression(op0, i));
    else
    ...
    if (expected_type != SPIRType::Unknown && type1.basetype != expected_type)
      expr += bitcast_expression(target_type1, type1.basetype,
                                 to_extract_component_expression(op1, i));
    else
...
SPIRType CompilerGLSL::binary_op_bitcast_helper(string &cast_op0,
                                                string &cast_op1,
                                                SPIRType::BaseType &input_type,
  ...
  // We have to bitcast if our inputs are of different type, or if our types are
  // not equal to expected inputs.
  // For some functions like OpIEqual and INotEqual, we don't care if inputs are
  // of different types than expected
  // since equality test is exactly the same.
  bool cast = (type0.basetype != type1.basetype) ||
              (!skip_cast_if_equal_type && type0.basetype != input_type);
  ...
  if (cast) {
    ...
    // If we don't cast, our actual input type is that of the first (or second)
    // argument.
...
bool CompilerGLSL::emit_complex_bitcast(uint32_t result_type, uint32_t id,
                                        uint32_t op0) {
  // Some bitcasts may require complex casting sequences, and are implemented
  // here.
  ...
  if (output_type.basetype == SPIRType::Half &&
      input_type.basetype == SPIRType::Float && input_type.vecsize == 1)
    expr = join("unpackFloat2x16(floatBitsToUint(", to_unpacked_expression(op0),
                "))");
  else if (output_type.basetype == SPIRType::Float &&
           input_type.basetype == SPIRType::Half && input_type.vecsize == 2)
...
void CompilerGLSL::emit_binary_op_cast(uint32_t result_type, uint32_t result_id,
                                       uint32_t op0, uint32_t op1,
                                       const char *op,
                                       SPIRType::BaseType input_type,
                                       bool skip_cast_if_equal_type) {
  ...
  auto expected_type = binary_op_bitcast_helper(
      cast_op0, cast_op1, input_type, op0, op1, skip_cast_if_equal_type);
  ...
  // Special case boolean outputs since relational opcodes output booleans
  // instead of int/uint.
  string expr;
...
void CompilerGLSL::emit_unary_func_op(uint32_t result_type, uint32_t result_id,
                                      uint32_t op0, const char *op) {
  bool forward = should_forward(op0);
  emit_op(result_type, result_id,
          join(op, "(", to_unpacked_expression(op0), ")"), forward);
  inherit_expression_dependencies(result_id, op0);
}

void CompilerGLSL::emit_binary_func_op(uint32_t result_type, uint32_t result_id,
                                       uint32_t op0, uint32_t op1,
                                       const char *op) {
  bool forward = should_forward(op0) && should_forward(op1);
  emit_op(result_type, result_id, join(op, "(", to_unpacked_expression(op0),
...
void CompilerGLSL::emit_atomic_func_op(uint32_t result_type, uint32_t result_id,
  ...
  emit_op(result_type, result_id,
          join(op, "(", to_non_uniform_aware_expression(op0), ", ",
               to_unpacked_expression(op1), ", ", to_unpacked_expression(op2),
               ")"),
          false);
  flush_all_atomic_capable_variables();
}

void CompilerGLSL::emit_unary_func_op_cast(
    uint32_t result_type, uint32_t result_id, uint32_t op0, const char *op,
    SPIRType::BaseType input_type, SPIRType::BaseType expected_result_type) {
  auto &out_type = get<SPIRType>(result_type);
  auto &expr_type = expression_type(op0);
  auto expected_type = out_type;

  // Bit-widths might be different in unary cases because we use it for
  // SConvert/UConvert and friends.
  expected_type.basetype = input_type;
  ...
  if (expr_type.basetype != input_type) {
    if (expr_type.basetype == SPIRType::Boolean)
      cast_op = join(type_to_glsl(expected_type), "(",
                     to_unpacked_expression(op0), ")");
    else
...
// Very special case. Handling bitfieldExtract requires us to deal with
// different bitcasts of different signs
// and different vector sizes all at once. Need a special purpose method here.
void CompilerGLSL::emit_trinary_func_op_bitextract(
    uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
  ...
  string cast_op0 = expression_type(op0).basetype != input_type0
                        ? bitcast_glsl(expected_type, op0)
                        : to_unpacked_expression(op0);

  auto op1_expr = to_unpacked_expression(op1);
  auto op2_expr = to_unpacked_expression(op2);

  // Use value casts here instead. Input must be exactly int or uint, but SPIR-V
  // might be 16-bit.
  expected_type.basetype = input_type1;
  expected_type.vecsize = 1;
  string cast_op1 =
      expression_type(op1).basetype != input_type1
  ...
  expected_type.vecsize = 1;
  string cast_op2 =
      expression_type(op2).basetype != input_type2
  ...
  emit_op(result_type, result_id, expr,
          should_forward(op0) && should_forward(op1) && should_forward(op2));
  inherit_expression_dependencies(result_id, op0);
  inherit_expression_dependencies(result_id, op1);
  inherit_expression_dependencies(result_id, op2);
}

void CompilerGLSL::emit_trinary_func_op_cast(uint32_t result_type,
                                             uint32_t result_id, uint32_t op0,
  ...
  string cast_op0 = expression_type(op0).basetype != input_type
                        ? bitcast_glsl(expected_type, op0)
                        : to_unpacked_expression(op0);
  string cast_op1 = expression_type(op1).basetype != input_type
                        ? bitcast_glsl(expected_type, op1)
                        : to_unpacked_expression(op1);
  string cast_op2 = expression_type(op2).basetype != input_type
                        ? bitcast_glsl(expected_type, op2)
...
void CompilerGLSL::emit_binary_func_op_cast_clustered(
    uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
    const char *op, SPIRType::BaseType input_type) {
  // Special purpose method for implementing clustered subgroup opcodes.
  // Main difference is that op1 does not participate in any casting, it needs
  // to be a literal.
  auto &out_type = get<SPIRType>(result_type);
  auto expected_type = out_type;
  expected_type.basetype = input_type;
...
void CompilerGLSL::emit_binary_func_op_cast(uint32_t result_type,
                                            uint32_t result_id, uint32_t op0,
                                            uint32_t op1, const char *op,
                                            SPIRType::BaseType input_type,
                                            bool skip_cast_if_equal_type) {
...
void CompilerGLSL::emit_trinary_func_op(uint32_t result_type,
                                        uint32_t result_id, uint32_t op0,
                                        uint32_t op1, uint32_t op2,
                                        const char *op) {
  bool forward =
...
void CompilerGLSL::emit_quaternary_func_op(uint32_t result_type,
                                           uint32_t result_id, uint32_t op0,
                                           uint32_t op1, uint32_t op2,
  ...
  emit_op(result_type, result_id,
          join(op, "(", to_unpacked_expression(op0), ", ",
               to_unpacked_expression(op1), ", ", to_unpacked_expression(op2),
               ", ", to_unpacked_expression(op3), ")"),
          forward);
...
void CompilerGLSL::emit_bitfield_insert_op(
    uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1,
    uint32_t op2, uint32_t op3, const char *op,
    SPIRType::BaseType offset_count_type) {
  // Only need to cast offset/count arguments. Types of base/insert must be same
  // as result type,
  ...
  emit_op(result_type, result_id, join(op, "(", op0_expr, ", ", op1_expr, ", ",
                                       op2_expr, ", ", op3_expr, ")"),
          forward);
...
string CompilerGLSL::legacy_tex_op(const std::string &op,
                                   const SPIRType &imgtype, uint32_t tex) {
  const char *type;
  ...
  if (op == "textureGrad" || op == "textureProjGrad" ||
      ((op == "textureLod" || op == "textureProjLod") &&
  ...
  if (is_comparison && is_legacy_es()) {
    if (op == "texture" || op == "textureProj")
      ...
      SPIRV_CROSS_THROW(
          join(op, " not allowed on depth samplers in legacy ES"));
  }
  ...
  if (op == "texture")
    return is_es_and_depth ? join(type_prefix, type, "EXT")
                           : join(type_prefix, type);
    ...
    return join(type_prefix, type,
                is_legacy_es() ? "GradEXT" : is_legacy_desktop() ? "GradARB"
                                                                 : "Grad");
    ...
    return join(type_prefix, type,
                is_legacy_es() ? "ProjGradEXT" : is_legacy_desktop()
                                                     ? "ProjGradARB"
                                                     : "ProjGrad");
...
bool CompilerGLSL::to_trivial_mix_op(const SPIRType &type, string &op,
  ...
  if (!backend.use_constructor_splatting &&
      value_type.vecsize != lerptype.vecsize)
    return false;
  ...
  // If our bool selects between 0 and 1, we can cast from bool instead, making
  // our trivial constructor.
  bool ret = true;
  for (uint32_t row = 0; ret && row < value_type.vecsize; row++) {
    ...
    case SPIRType::Half:
      ret = cleft->scalar_f16(0, row) == 0.0f &&
            cright->scalar_f16(0, row) == 1.0f;
      break;

    case SPIRType::Float:
      ret = cleft->scalar_f32(0, row) == 0.0f &&
            cright->scalar_f32(0, row) == 1.0f;
      break;

    case SPIRType::Double:
      ret =
          cleft->scalar_f64(0, row) == 0.0 && cright->scalar_f64(0, row) == 1.0;
      break;
...
string CompilerGLSL::to_ternary_expression(const SPIRType &restype,
                                           uint32_t select, uint32_t true_value,
                                           uint32_t false_value) {
  string expr;
  auto &lerptype = expression_type(select);

  if (lerptype.vecsize == 1)
    expr = join(to_enclosed_expression(select), " ? ",
                to_enclosed_pointer_expression(true_value), " : ",
...
void CompilerGLSL::emit_mix_op(uint32_t result_type, uint32_t id, uint32_t left,
                               uint32_t right, uint32_t lerp) {
  auto &lerptype = expression_type(lerp);
  ...
  if (trivial_mix) {
    ...
    auto expr =
        to_ternary_expression(get<SPIRType>(result_type), lerp, right, left);
    emit_op(result_type, id, expr, should_forward(left) &&
    ...
    emit_trinary_func_op(result_type, id, left, right, lerp,
                         backend.boolean_mix_function);
  else
    emit_trinary_func_op(result_type, id, left, right, lerp, "mix");
}

string CompilerGLSL::to_combined_image_sampler(VariableID image_id,
                                               VariableID samp_id) {
  // Keep track of the array indices we have used to load the image.
  // We'll need to use the same array index into the combined image sampler
  // array.
  ...
  // For GLSL and ESSL targets, we must enumerate all possible combinations for
  // sampler2D(texture2D, sampler) and redirect
  ...
  auto image_itr = find_if(begin(args), end(args),
                           [image_id](const SPIRFunction::Parameter &param) {
                             return image_id == param.id;
                           });

  auto sampler_itr = find_if(begin(args), end(args),
                             [samp_id](const SPIRFunction::Parameter &param) {
                               return samp_id == param.id;
                             });

  if (image_itr != end(args) || sampler_itr != end(args)) {
    // If any parameter originates from a parameter, we will find it in our
    // argument list.
    bool global_image = image_itr == end(args);
    bool global_sampler = sampler_itr == end(args);
    VariableID iid =
        global_image ? image_id : VariableID(uint32_t(image_itr - begin(args)));
    VariableID sid = global_sampler
                         ? samp_id
                         : VariableID(uint32_t(sampler_itr - begin(args)));

    auto &combined = current_function->combined_parameters;
    auto itr =
        find_if(begin(combined), end(combined),
                [=](const SPIRFunction::CombinedImageSamplerParameter &p) {
                  return p.global_image == global_image &&
                         p.global_sampler == global_sampler &&
    ...
    if (itr != end(combined))
      ...
      SPIRV_CROSS_THROW(
          "Cannot find mapping for combined sampler parameter, was "
    ...
    auto itr =
        find_if(begin(mapping), end(mapping),
                [image_id, samp_id](const CombinedImageSampler &combined) {
                  return combined.image_id == image_id &&
                         combined.sampler_id == samp_id;
                });

    if (itr != end(combined_image_samplers))
      return to_expression(itr->combined_id) + array_expr;
    else {
      SPIRV_CROSS_THROW("Cannot find mapping for combined sampler, was "
                        "build_combined_image_samplers() used "
                        "before compile() was called?");
    }
  }
...
void CompilerGLSL::emit_sampled_image_op(uint32_t result_type,
                                         uint32_t result_id, uint32_t image_id,
                                         uint32_t samp_id) {
  if (options.vulkan_semantics && combined_image_samplers.empty()) {
    emit_binary_func_op(
        result_type, result_id, image_id, samp_id,
        type_to_glsl(get<SPIRType>(result_type), result_id).c_str());
  } else {
    // Make sure to suppress usage tracking. It is illegal to create temporaries
    // of opaque types.
    emit_op(result_type, result_id,
            to_combined_image_sampler(image_id, samp_id), true, true);
  }
...
void CompilerGLSL::emit_sparse_feedback_temporaries(uint32_t result_type_id,
                                                    uint32_t id,
  ...
  if (return_type.basetype != SPIRType::Struct ||
      return_type.member_types.size() != 2)
...
void CompilerGLSL::emit_texture_op(const Instruction &i, bool sparse) {
  ...
  if (sparse)
    emit_sparse_feedback_temporaries(result_type_id, id, sparse_code_id,
                                     sparse_texel_id);
  ...
  if (sparse) {
    ...
    expr = join(type_to_glsl(return_type), "(", to_expression(sparse_code_id),
  ...
  // Do not register sparse ops as control dependent as they are always lowered
  // to a temporary.
  switch (op) {
...
std::string
CompilerGLSL::to_texture_op(const Instruction &i, bool sparse, bool *forward,
                            SmallVector<uint32_t> &inherited_expressions) {
  auto *ops = stream(i);
  auto op = static_cast<Op>(i.op);
  ...
  if (has_decoration(img, DecorationNonUniform) &&
      !maybe_get_backing_variable(img))
  ...
  // texture(samplerXShadow) returns float. shadowX() returns vec4. Swizzle
  // here.
  if (is_legacy() && is_depth_image(imgtype, img))
    expr += ".r";

  // Sampling from a texture which was deduced to be a depth image, might
  // actually return 1 component here.
  // Remap back to 4 components as sampling opcodes expect.
  if (backend.comparison_image_samples_scalar &&
      image_opcode_is_sample_no_dref(op)) {
    ...
    // We might have loaded an OpImage, and used that handle for two different
    // purposes.
    // Once with comparison, once without.
    auto *image_variable = maybe_get_backing_variable(image_id);
    if (image_variable &&
        is_depth_image(get<SPIRType>(image_variable->basetype),
                       image_variable->self))
      image_is_depth = true;
  ...
  if (!sparse && !backend.support_small_type_sampling_result &&
      result_type.width < 32) {
    // Just value cast (narrowing) to expected type since we cannot rely on
    // narrowing to work automatically.
    // Hopefully compiler picks this up and converts the texturing instruction
    // to the appropriate precision.
    expr = join(type_to_glsl_constructor(result_type), "(", expr, ")");
  }

  // Deals with reads from MSL. We might need to downconvert to fewer
  // components.
  if (op == OpImageRead)
...
// Returns the function name for a texture sampling function for the specified
// image and sampling characteristics.
// For some subclasses, the function is a method on the specified image.
string
CompilerGLSL::to_function_name(const TextureFunctionNameArguments &args) {
  if (args.has_min_lod) {
    if (options.es)
  ...
  // textureLod on sampler2DArrayShadow and samplerCubeShadow does not exist in
  // GLSL for some reason.
  // To emulate this, we will have to use textureGrad with a constant gradient
  // of 0.
  // The workaround will assert that the LOD is in fact constant 0, or we cannot
  // emit correct code.
  // This happens for HLSL SampleCmpLevelZero on Texture2DArray and TextureCube.
  bool workaround_lod_array_shadow_as_grad = false;
  if (((imgtype.image.arrayed && imgtype.image.dim == Dim2D) ||
       imgtype.image.dim == DimCube) &&
      is_depth_image(imgtype, tex) && args.lod) {
    if (!expression_is_constant_null(args.lod)) {
      SPIRV_CROSS_THROW("textureLod on sampler2DArrayShadow is not constant "
                        "0.0. This cannot be "
                        "expressed in GLSL.");
    }
  ...
  return (is_legacy() && !args.base.is_gather)
             ? legacy_tex_op(fname, imgtype, tex)
             : fname;
}
...
std::string CompilerGLSL::convert_separate_image_to_expression(uint32_t id) {
  ...
  // If we are fetching from a plain OpTypeImage, we must combine with a dummy
  // sampler in GLSL.
  // In Vulkan GLSL, we can make use of the newer
  ...
  if (var) {
    ...
    if (type.basetype == SPIRType::Image && type.image.sampled == 1 &&
      ...
      if (options.vulkan_semantics) {
        if (dummy_sampler_id) {
          // Don't need to consider Shadow state since the dummy sampler is
          // always non-shadow.
          auto sampled_type = type;
          sampled_type.basetype = SPIRType::SampledImage;
          return join(type_to_glsl(sampled_type), "(",
                      to_non_uniform_aware_expression(id), ", ",
                      to_expression(dummy_sampler_id), ")");
        } else {
          // Newer glslang supports this extension to deal with texture2D as
          // argument to texture functions.
          require_extension_internal("GL_EXT_samplerless_texture_functions");
        }
      } else {
        if (!dummy_sampler_id)
          SPIRV_CROSS_THROW(
              "Cannot find dummy sampler ID. Was "
...
// image and sampling characteristics.
string CompilerGLSL::to_function_args(const TextureFunctionArguments &args,
                                      bool *p_forward) {
  ...
  if (args.nonuniform_expression &&
      farg_str.find_first_of('[') != string::npos) {
    // Only emit nonuniformEXT() wrapper if the underlying expression is arrayed
    // in some way.
  ...
  auto swizzle =
      [swizz_func](uint32_t comps, uint32_t in_comps) -> const char *{
        if (comps == in_comps)
          return "";
  ...
  // The IR can give us more components than we need, so chop them off as
  // needed.
  auto swizzle_expr =
      swizzle(args.coord_components, expression_type(args.coord).vecsize);
  // Only enclose the UV expression if needed.
  auto coord_expr = (*swizzle_expr == '\0')
                        ? to_expression(args.coord)
                        : (to_enclosed_expression(args.coord) + swizzle_expr);
  ...
  if (coord_type.basetype == SPIRType::UInt) {
    ...
    coord_expr =
        bitcast_expression(expected_type, coord_type.basetype, coord_expr);
  }
  ...
  if (args.dref) {
            ...
            4) // GLSL also splits the arguments in two. Same for textureGather.
    {
      ...
      // Other shading languages splits up the arguments for coord and compare
      // value like SPIR-V.
      // The coordinate type for textureProj shadow is always vec4 even for
      // sampler1DShadow.
      farg_str += ", vec4(";

      if (imgtype.image.dim == Dim1D) {
        // Could reuse coord_expr, but we will mess up the temporary usage
        // checking.
        ...
        // checking.
        farg_str +=
            to_enclosed_expression(args.coord) + (swizz_func ? ".xy()" : ".xy");
        farg_str += ", ";
        farg_str += to_expression(args.dref);
        farg_str += ", ";
  ...
  if (args.lod) {
    if (workaround_lod_array_shadow_as_grad) {
      // Implement textureGrad() instead. LOD == 0.0 is implemented as gradient
      // of 0.0.
      // Implementing this as plain texture() is not safe on some
      // implementations.
      ...
      if (args.base.is_fetch && imgtype.image.dim != DimBuffer &&
          !imgtype.image.ms && lod_expr_type.basetype != SPIRType::Int) {
        farg_str += join("int(", to_expression(args.lod), ")");
      } else {
        farg_str += to_expression(args.lod);
      }
    }
  } else if (args.base.is_fetch && imgtype.image.dim != DimBuffer &&
             !imgtype.image.ms) {
  ...
  if (args.sparse_texel) {
    // Sparse texel output parameter comes after everything else, except it's
    // before the optional, component/bias arguments.
    farg_str += ", ";
...
void CompilerGLSL::emit_glsl_op(uint32_t result_type, uint32_t id, uint32_t eop,
  ...
  uint32_t integer_width =
      get_integer_width_for_glsl_instruction(op, args, length);
  ...
  case GLSLstd450Round:
    if (!is_legacy())
      ...
      auto expr = join("floor(", op0, " + ", type_to_glsl_constructor(op0_type),
                       "(0.5))");
  ...
  case GLSLstd450Trunc:
    ...
    break;
  case GLSLstd450SAbs:
    emit_unary_func_op_cast(result_type, id, args[0], "abs", int_type,
                            int_type);
    break;
  case GLSLstd450FAbs:
  ...
  case GLSLstd450SSign:
    emit_unary_func_op_cast(result_type, id, args[0], "sign", int_type,
                            int_type);
    break;
  ...
  case GLSLstd450Fma:
    if ((!options.es && options.version < 400) ||
        (options.es && options.version < 320)) {
      auto expr = join(to_enclosed_expression(args[0]), " * ",
                       to_enclosed_expression(args[1]), " + ",
                       to_enclosed_expression(args[2]));

      emit_op(result_type, id, expr, should_forward(args[0]) &&
                                         should_forward(args[1]) &&
                                         should_forward(args[2]));
  ...
  case GLSLstd450ModfStruct: {
    ...
    statement(to_expression(id), ".", to_member_name(type, 0), " = ", "modf(",
              to_expression(args[0]), ", ", to_expression(id), ".",
  ...
  // Minmax
  case GLSLstd450UMin:
    emit_binary_func_op_cast(result_type, id, args[0], args[1], "min",
                             uint_type, false);
    break;

  case GLSLstd450SMin:
    emit_binary_func_op_cast(result_type, id, args[0], args[1], "min", int_type,
                             false);
    break;
  ...
  case GLSLstd450UMax:
    emit_binary_func_op_cast(result_type, id, args[0], args[1], "max",
                             uint_type, false);
    break;

  case GLSLstd450SMax:
    emit_binary_func_op_cast(result_type, id, args[0], args[1], "max", int_type,
                             false);
    break;
  ...
  case GLSLstd450UClamp:
    emit_trinary_func_op_cast(result_type, id, args[0], args[1], args[2],
                              "clamp", uint_type);
    break;
  ...
  case GLSLstd450FrexpStruct: {
    auto &type = get<SPIRType>(result_type);
    emit_uninitialized_temporary_expression(result_type, id);
    statement(to_expression(id), ".", to_member_name(type, 0), " = ", "frexp(",
              to_expression(args[0]), ", ", to_expression(id), ".",
              to_member_name(type, 1), ");");
    break;
  }
  ...
  case GLSLstd450FindILsb:
    ...
    emit_unary_func_op_cast(result_type, id, args[0], "findLSB",
                            expression_type(args[0]).basetype, int_type);
    break;

  case GLSLstd450FindSMsb:
    emit_unary_func_op_cast(result_type, id, args[0], "findMSB", int_type,
                            int_type);
    break;
  ...
  case GLSLstd450InterpolateAtSample:
    emit_binary_func_op(result_type, id, args[0], args[1],
                        "interpolateAtSample");
    break;
  ...
  case GLSLstd450NClamp: {
    // Make sure we have a unique ID here to avoid aliasing the extra
    // sub-expressions between clamp and NMin sub-op.
    // IDs cannot exceed 24 bits, so we can make use of the higher bits for some
    // unique flags.
    uint32_t &max_id =
        extra_sub_expressions[id | EXTRA_SUB_EXPRESSION_TYPE_AUX];
    if (!max_id)
...
void CompilerGLSL::emit_nminmax_op(uint32_t result_type, uint32_t id,
                                   uint32_t op0, uint32_t op1, GLSLstd450 op) {
  ...
  emit_binary_func_op(result_type, tmp_id, op0, op1,
                      op == GLSLstd450NMin ? "min" : "max");
  emit_mix_op(result_type, mixed_first_id, tmp_id, op1, left_nan_id);
  emit_mix_op(result_type, id, mixed_first_id, op0, right_nan_id);
}

void CompilerGLSL::emit_spv_amd_shader_ballot_op(uint32_t result_type,
                                                 uint32_t id, uint32_t eop,
                                                 const uint32_t *args,
                                                 uint32_t) {
...
void CompilerGLSL::emit_spv_amd_shader_explicit_vertex_parameter_op(
    uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t *args,
    uint32_t) {
  require_extension_internal("GL_AMD_shader_explicit_vertex_parameter");
  ...
  default:
    statement("// unimplemented SPV AMD shader explicit vertex parameter op ",
              eop);
    break;
  }
}

void CompilerGLSL::emit_spv_amd_shader_trinary_minmax_op(uint32_t result_type,
                                                         uint32_t id,
                                                         uint32_t eop,
                                                         const uint32_t *args,
                                                         uint32_t) {
...
void CompilerGLSL::emit_spv_amd_gcn_shader_op(uint32_t result_type, uint32_t id,
                                              uint32_t eop,
...
void CompilerGLSL::emit_subgroup_op(const Instruction &i) {
  ...
  case OpGroupNonUniformBallotBitCount: {
    ...
    if (operation == GroupOperationReduce)
      request_subgroup_feature(
          ShaderSubgroupSupportHelper::SubgroupBallotBitCount);
  ...
  case OpGroupNonUniformBallotBitExtract:
    request_subgroup_feature(
        ShaderSubgroupSupportHelper::SubgroupBallotBitExtract);
    break;
  ...
  case OpGroupNonUniformBallotFindMSB:
    request_subgroup_feature(
        ShaderSubgroupSupportHelper::SubgroupBallotFindLSB_MSB);
    break;

  case OpGroupNonUniformBroadcast:
  case OpGroupNonUniformBroadcastFirst:
    request_subgroup_feature(
        ShaderSubgroupSupportHelper::SubgroupBroadcast_First);
    break;
  ...
  case OpGroupNonUniformAllEqual: {
    ...
    if (type.basetype == SPIRType::BaseType::Boolean && type.vecsize == 1u)
      request_subgroup_feature(
          ShaderSubgroupSupportHelper::SubgroupAll_Any_AllEqualBool);
    else
  ...
  case OpGroupNonUniformBallotBitCount: {
    ...
    if (operation == GroupOperationReduce)
      ...
      emit_unary_func_op(result_type, id, ops[4],
...
string CompilerGLSL::bitcast_glsl_op(const SPIRType &out_type,
  ...
  if (out_type.width == 8 && in_type.width >= 16 && integral_cast &&
      in_type.vecsize == 1)
    return "unpack8";
  else if (in_type.width == 8 && out_type.width == 16 && integral_cast &&
           out_type.vecsize == 1)
    return "pack16";
  else if (in_type.width == 8 && out_type.width == 32 && integral_cast &&
           out_type.vecsize == 1)
    return "pack32";

  // Floating <-> Integer special casts. Just have to enumerate all cases. :(
  // 16-bit, 32-bit and 64-bit floats.
  if (out_type.basetype == SPIRType::UInt &&
      in_type.basetype == SPIRType::Float) {
    if (is_legacy_es())
  ...
  } else if (out_type.basetype == SPIRType::Int &&
             in_type.basetype == SPIRType::Float) {
    if (is_legacy_es())
  ...
  } else if (out_type.basetype == SPIRType::Float &&
             in_type.basetype == SPIRType::UInt) {
    if (is_legacy_es())
  ...
  else if (out_type.basetype == SPIRType::Int64 &&
           in_type.basetype == SPIRType::Double)
    return "doubleBitsToInt64";
  else if (out_type.basetype == SPIRType::UInt64 &&
           in_type.basetype == SPIRType::Double)
    return "doubleBitsToUint64";
  else if (out_type.basetype == SPIRType::Double &&
           in_type.basetype == SPIRType::Int64)
    return "int64BitsToDouble";
  else if (out_type.basetype == SPIRType::Double &&
           in_type.basetype == SPIRType::UInt64)
    return "uint64BitsToDouble";
  else if (out_type.basetype == SPIRType::Short &&
  ...
  else if (out_type.basetype == SPIRType::Half &&
           in_type.basetype == SPIRType::Short)
  ...
  if (out_type.basetype == SPIRType::UInt64 &&
      in_type.basetype == SPIRType::UInt && in_type.vecsize == 2)
    return "packUint2x32";
  else if (out_type.basetype == SPIRType::UInt &&
           in_type.basetype == SPIRType::UInt64 && out_type.vecsize == 2)
    return "unpackUint2x32";
  else if (out_type.basetype == SPIRType::Half &&
           in_type.basetype == SPIRType::UInt && in_type.vecsize == 1)
    return "unpackFloat2x16";
  else if (out_type.basetype == SPIRType::UInt &&
           in_type.basetype == SPIRType::Half && in_type.vecsize == 2)
    return "packFloat2x16";
  else if (out_type.basetype == SPIRType::Int &&
           in_type.basetype == SPIRType::Short && in_type.vecsize == 2)
    return "packInt2x16";
  else if (out_type.basetype == SPIRType::Short &&
           in_type.basetype == SPIRType::Int && in_type.vecsize == 1)
    return "unpackInt2x16";
  else if (out_type.basetype == SPIRType::UInt &&
           in_type.basetype == SPIRType::UShort && in_type.vecsize == 2)
    return "packUint2x16";
  else if (out_type.basetype == SPIRType::UShort &&
           in_type.basetype == SPIRType::UInt && in_type.vecsize == 1)
    return "unpackUint2x16";
  else if (out_type.basetype == SPIRType::Int64 &&
           in_type.basetype == SPIRType::Short && in_type.vecsize == 4)
    return "packInt4x16";
  else if (out_type.basetype == SPIRType::Short &&
           in_type.basetype == SPIRType::Int64 && in_type.vecsize == 1)
    return "unpackInt4x16";
  else if (out_type.basetype == SPIRType::UInt64 &&
           in_type.basetype == SPIRType::UShort && in_type.vecsize == 4)
    return "packUint4x16";
  else if (out_type.basetype == SPIRType::UShort &&
           in_type.basetype == SPIRType::UInt64 && in_type.vecsize == 1)
    return "unpackUint4x16";

  return "";
}

string CompilerGLSL::bitcast_glsl(const SPIRType &result_type,
                                  uint32_t argument) {
...
std::string CompilerGLSL::bitcast_expression(SPIRType::BaseType target_type,
                                             uint32_t arg) {
...
std::string CompilerGLSL::bitcast_expression(const SPIRType &target_type,
...
string CompilerGLSL::builtin_to_glsl(BuiltIn builtin, StorageClass storage) {
  ...
  case BuiltInVertexId:
    if (options.vulkan_semantics)
      SPIRV_CROSS_THROW("Cannot implement gl_VertexID in Vulkan GLSL. This "
                        "shader was created "
                        "with GL semantics.");
    return "gl_VertexID";
  case BuiltInInstanceId:
    if (options.vulkan_semantics) {
      ...
      default:
        SPIRV_CROSS_THROW(
            "Cannot implement gl_InstanceID in Vulkan GLSL. This shader was "
  ...
  case BuiltInInstanceIndex:
    ...
    if (options.vertex.support_nonzero_base_instance) {
      if (!options.vulkan_semantics) {
        // This is a soft-enable. We will opt-in to using gl_BaseInstanceARB if
        // supported.
        require_extension_internal("GL_ARB_shader_draw_parameters");
      }
      return "(gl_InstanceID + SPIRV_Cross_BaseInstance)"; // ... but not
                                                           // gl_InstanceID.
    } else
      return "gl_InstanceID";
  ...
  case BuiltInWorldRayDirectionKHR:
    return ray_tracing_is_khr ? "gl_WorldRayDirectionEXT"
                              : "gl_WorldRayDirectionNV";
  case BuiltInObjectRayOriginKHR:
    return ray_tracing_is_khr ? "gl_ObjectRayOriginEXT"
                              : "gl_ObjectRayOriginNV";
  case BuiltInObjectRayDirectionKHR:
    return ray_tracing_is_khr ? "gl_ObjectRayDirectionEXT"
  ...
  case BuiltInInstanceCustomIndexKHR:
    return ray_tracing_is_khr ? "gl_InstanceCustomIndexEXT"
  ...
  case BuiltInIncomingRayFlagsKHR:
    return ray_tracing_is_khr ? "gl_IncomingRayFlagsEXT"
...
const char *CompilerGLSL::index_to_swizzle(uint32_t index) {
  ...
  case 3:
    return "w";
  default:
    return "x"; // Don't crash, but engage the "undefined behavior" described
                // for out-of-bounds logical addressing in spec.
  }
}

void CompilerGLSL::access_chain_internal_append_index(
    std::string &expr, uint32_t /*base*/, const SPIRType * /*type*/,
    AccessChainFlags flags, bool & /*access_chain_is_arrayed*/,
    uint32_t index) {
  bool index_is_literal = (flags & ACCESS_CHAIN_INDEX_IS_LITERAL_BIT) != 0;
  bool register_expression_read =
      (flags & ACCESS_CHAIN_SKIP_REGISTER_EXPRESSION_READ_BIT) == 0;
...
string CompilerGLSL::access_chain_internal(uint32_t base,
  ...
  bool flatten_member_reference =
      (flags & ACCESS_CHAIN_FLATTEN_ALL_MEMBERS_BIT) != 0;
  ...
  if (!backend.native_pointers) {
    if (ptr_chain)
      SPIRV_CROSS_THROW("Backend does not support native pointers and does not "
                        "support OpPtrAccessChain.");

    // Wrapped buffer reference pointer types will need to poke into the
    // internal "value" member before
  ...
  bool is_packed =
      has_extended_decoration(base, SPIRVCrossDecorationPhysicalTypePacked);
  uint32_t physical_type =
      get_extended_decoration(base, SPIRVCrossDecorationPhysicalTypeID);
  ...
  const auto append_index = [&](uint32_t index, bool is_literal) {
    ...
    access_chain_internal_append_index(expr, base, type, mod_flags,
                                       access_chain_is_arrayed, index);
  };
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    // Pointer chains
    if (ptr_chain && i == 0) {
      // If we are flattening multidimensional arrays, only create opening
      // bracket on first
      // array index.
      ...
      if (options.flatten_multidimensional_arrays && dimension_flatten) {
        // If we are flattening multidimensional arrays, do manual stride
        // computation.
        if (is_literal)
          expr += convert_to_string(index);
        else
    ...
    // Arrays
      ...
      if (backend.force_gl_in_out_block && i == 0 && var &&
          is_builtin_variable(*var) &&
          !has_decoration(type->self, DecorationBlock)) {
        // This deals with scenarios for tesc/geom where arrays of gl_Position[]
        // are declared.
        ...
        switch (builtin) {
        // case BuiltInCullDistance: // These are already arrays, need to figure
        // out rules for these in tess/geom.
        // case BuiltInClipDistance:
        case BuiltInPosition:
        case BuiltInPointSize:
          if (var->storage == StorageClassInput)
            expr =
                join("gl_in[", to_expression(index, register_expression_read),
                     "].", expr);
          else if (var->storage == StorageClassOutput)
            expr =
                join("gl_out[", to_expression(index, register_expression_read),
                     "].", expr);
          else
            append_index(index, is_literal);
          break;
      ...
      // Some builtins are arrays in SPIR-V but not in other languages, e.g.
      // gl_SampleMask[] is an array in SPIR-V but not in Metal.
      // By throwing away the index, we imply the index was 0, which it must be
      // for gl_SampleMask.
      else if (!builtin_translates_to_nonarray(
                   BuiltIn(get_decoration(base, DecorationBuiltIn)))) {
        append_index(index, is_literal);
      }
    ...
    // For structs, the index refers to a constant, which indexes into the
    // members, possibly through a redirection mapping.
    // We also check if this member is a builtin, since we then replace the
    // entire expression with the builtin one.
    else if (type->basetype == SPIRType::Struct) {
      if (!is_literal)
      ...
      if (is_member_builtin(*type, index, &builtin) &&
          access_chain_needs_stage_io_builtin_translation(base)) {
      ...
      if (member_is_remapped_physical_type(*type, index))
        physical_type = get_extended_member_decoration(
            type->self, index, SPIRVCrossDecorationPhysicalTypeID);
      else
        physical_type = 0;

      row_major_matrix_needs_conversion =
          member_is_non_native_row_major_matrix(*type, index);
      type = &get<SPIRType>(type->member_types[index]);
    }
    // Matrix -> Vector
    else if (type->columns > 1) {
      // If we have a row-major matrix here, we need to defer any transpose in
      // case this access chain
      // is used to store a column. We can resolve it right here and now if we
      // access a scalar directly,
    ...
    // Vector -> Scalar
      ...
      if ((flags & ACCESS_CHAIN_FORCE_COMPOSITE_BIT) == 0) {
        ...
        // Control points can only be written by InvocationID, so there is no
        // need
        // to consider scalar access chains here.
        // Cleans up some cases where it's very painful to determine the
        // accurate storage class
      ...
      if (!row_major_matrix_needs_conversion &&
          !ignore_potential_sliced_writes) {
        // On some backends, we might not be able to safely access individual
        // scalars in a vector.
        // To work around this, we might have to cast the access chain reference
        // to something which can,
        // like a pointer to scalar, which we can then index into.
        prepare_access_chain_for_scalar_access(expr,
                                               get<SPIRType>(type->parent_type),
                                               effective_storage, is_packed);
      }
      ...
      if (is_literal && !is_packed && !row_major_matrix_needs_conversion) {
        ...
        if (c.specialization) {
          // If the index is a spec constant, we cannot turn extract into a
          // swizzle.
        ...
        // For packed vectors, we can only access them as an array, not by
        // swizzle.
        expr += join("[", index, "]");
      } else {
        expr += "[";
  ...
  if (pending_array_enclose) {
    SPIRV_CROSS_THROW("Flattening of multidimensional arrays were enabled, "
                      "but the access chain was terminated in the middle of a "
...
void CompilerGLSL::prepare_access_chain_for_scalar_access(std::string &,
                                                          const SPIRType &,
                                                          spv::StorageClass,
                                                          bool &) {}

string CompilerGLSL::to_flattened_struct_member(const string &basename,
                                                const SPIRType &type,
                                                uint32_t index) {
...
string CompilerGLSL::access_chain(uint32_t base, const uint32_t *indices,
                                  uint32_t count, const SPIRType &target_type,
                                  AccessChainMeta *meta, bool ptr_chain) {
  if (flattened_buffer_blocks.count(base)) {
    uint32_t matrix_stride = 0;
    uint32_t array_stride = 0;
    bool need_transpose = false;
    flattened_access_chain_offset(expression_type(base), indices, count, 0, 16,
                                  &need_transpose, &matrix_stride,
    ...
    return flattened_access_chain(base, indices, count, target_type, 0,
                                  matrix_stride, array_stride, need_transpose);
  } else if (flattened_structs.count(base) && count > 0) {
    AccessChainFlags flags = ACCESS_CHAIN_CHAIN_ONLY_BIT |
    ...
    auto chain =
        access_chain_internal(base, indices, count, flags, nullptr).substr(1);
    if (meta) {
...
string CompilerGLSL::load_flattened_struct(const string &basename,
                                           const SPIRType &type) {
  auto expr = type_to_glsl_constructor(type);
  expr += '(';
  ...
  for (uint32_t i = 0; i < uint32_t(type.member_types.size()); i++) {
    ...
    auto &member_type = get<SPIRType>(type.member_types[i]);
    if (member_type.basetype == SPIRType::Struct)
      expr += load_flattened_struct(
          to_flattened_struct_member(basename, type, i), member_type);
    else
...
void CompilerGLSL::store_flattened_struct(
    const string &basename, uint32_t rhs_id, const SPIRType &type,
    const SmallVector<uint32_t> &indices) {
  ...
  for (uint32_t i = 0; i < uint32_t(member_type->member_types.size()); i++) {
    ...
    if (get<SPIRType>(member_type->member_types[i]).basetype ==
      ...
      auto rhs =
          to_expression(rhs_id) + to_multi_member_reference(type, sub_indices);
      statement(lhs, " = ", rhs, ";");
    }
  }
}
...
std::string CompilerGLSL::flattened_access_chain(
  ...
  if (!target_type.array.empty())
    ...
    return flattened_access_chain_struct(base, indices, count, target_type,
                                         offset);
  else if (target_type.columns > 1)
    return flattened_access_chain_matrix(base, indices, count, target_type,
                                         offset, matrix_stride, need_transpose);
  else
    return flattened_access_chain_vector(base, indices, count, target_type,
                                         offset, matrix_stride, need_transpose);
}

std::string CompilerGLSL::flattened_access_chain_struct(
    uint32_t base, const uint32_t *indices, uint32_t count,
  ...
  for (uint32_t i = 0; i < uint32_t(target_type.member_types.size()); ++i) {
    ...
    // The access chain terminates at the struct, so we need to find matrix
    // strides and row-major information
    // ahead of time.
    bool need_transpose = false;
    uint32_t matrix_stride = 0;
    if (member_type.columns > 1) {
      need_transpose = combined_decoration_for_member(target_type, i)
                           .get(DecorationRowMajor);
      matrix_stride = type_struct_member_matrix_stride(target_type, i);
    }

    auto tmp = flattened_access_chain(base, indices, count, member_type,
                                      offset + member_offset, matrix_stride,
...
std::string CompilerGLSL::flattened_access_chain_matrix(
    uint32_t base, const uint32_t *indices, uint32_t count,
    const SPIRType &target_type, uint32_t offset, uint32_t matrix_stride,
    bool need_transpose) {
  ...
  for (uint32_t i = 0; i < tmp_type.columns; i++) {
    ...
    expr +=
        flattened_access_chain_vector(base, indices, count, tmp_type,
                                      offset + i * matrix_stride, matrix_stride,
...
std::string CompilerGLSL::flattened_access_chain_vector(
    uint32_t base, const uint32_t *indices, uint32_t count,
    const SPIRType &target_type, uint32_t offset, uint32_t matrix_stride,
    bool need_transpose) {
  auto result = flattened_access_chain_offset(expression_type(base), indices,
                                              count, offset, 16);
  ...
  if (need_transpose) {
    ...
    for (uint32_t i = 0; i < target_type.vecsize; ++i) {
      ...
      expr += buffer_name;
      expr += "[";
      expr += result.first; // this is a series of N1 * k1 + N2 * k2 + ... that
                            // is either empty or ends with a +
    ...
    expr += buffer_name;
    expr += "[";
    expr += result.first; // this is a series of N1 * k1 + N2 * k2 + ... that is
                          // either empty or ends with a +
    expr += convert_to_string(index / 4);
    expr += "]";
...
std::pair<std::string, uint32_t> CompilerGLSL::flattened_access_chain_offset(
    const SPIRType &basetype, const uint32_t *indices, uint32_t count,
    uint32_t offset, uint32_t word_stride, bool *need_transpose,
  ...
  // Inherit matrix information in case we are access chaining a vector which
  // might have come from a row major layout.
  bool row_major_matrix_needs_conversion =
      need_transpose ? *need_transpose : false;
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    if (ptr_chain && i == 0) {
      ...
      if (constant) {
        ...
        if (array_stride % word_stride) {
          SPIRV_CROSS_THROW(
              "Array stride for dynamic indexing must be divisible by the size "
    ...
    // entire expression with the builtin one.
      ...
      if (type->columns > 1) {
        ...
        row_major_matrix_needs_conversion =
            combined_decoration_for_member(struct_type, index)
                .get(DecorationRowMajor);
      } else
        row_major_matrix_needs_conversion = false;
    ...
    // Matrix -> Vector
      ...
      if (constant) {
        index = evaluate_constant_u32(index);
        offset += index * (row_major_matrix_needs_conversion ? (type->width / 8)
                                                             : matrix_stride);
      } else {
        uint32_t indexing_stride = row_major_matrix_needs_conversion
                                       ? (type->width / 8)
                                       : matrix_stride;
        // Dynamic array access.
        if (indexing_stride % word_stride) {
          SPIRV_CROSS_THROW(
              "Matrix stride for dynamic indexing must be divisible by the "
              "size of a "
              "4-component vector. "
              "Likely culprit here is a row-major matrix being accessed "
              "dynamically. "
              "This cannot be flattened. Try using std140 layout instead.");
        }
    ...
    // Vector -> Scalar
      ...
      if (constant) {
        index = evaluate_constant_u32(index);
        offset +=
            index * (row_major_matrix_needs_conversion ? matrix_stride
                                                       : (type->width / 8));
      } else {
        uint32_t indexing_stride = row_major_matrix_needs_conversion
                                       ? matrix_stride
        ...
        if (indexing_stride % word_stride) {
          SPIRV_CROSS_THROW(
              "Stride for dynamic vector indexing must be divisible by the "
...
bool CompilerGLSL::should_forward(uint32_t id) const {
  // If id is a variable we will try to forward it regardless of force_temporary
  // check below
  // This is important because otherwise we'll get local sampler copies (highp
  // sampler2D foo = bar) that are invalid in OpenGL GLSL
  auto *var = maybe_get<SPIRVariable>(id);
...
bool CompilerGLSL::should_suppress_usage_tracking(uint32_t id) const {
  // Used only by opcodes which don't do any real "work", they just swizzle data
  // in some fashion.
  return !expression_is_forwarded(id) ||
         expression_suppresses_usage_tracking(id);
}
...
void CompilerGLSL::track_expression_read(uint32_t id) {
  ...
  // If we try to read a forwarded temporary more than once we will stamp out
  // possibly complex code twice.
  // In this case, it's better to just bind the complex expression to the
  // temporary and read that temporary twice.
  if (expression_is_forwarded(id) &&
    ...
    // If the expression in question is expensive, we should hoist it out to
    // avoid relying on loop-invariant code motion
    ...
    if (v >= 2) {
      // if (v == 2)
      //    fprintf(stderr, "ID %u was forced to temporary due to more than 1
      //    expression use!\n", id);
...
bool CompilerGLSL::args_will_forward(uint32_t id, const uint32_t *args,
                                     uint32_t num_args, bool pure) {
...
void CompilerGLSL::register_impure_function_call() {
  // Impure functions can modify globals and aliased variables, so invalidate
  // them as well.
  for (auto global : global_variables)
...
void CompilerGLSL::emit_variable_temporary_copies(const SPIRVariable &var) {
  ...
  if (var.allocate_temporary_copy && !flushed_phi_variables.count(var.self)) {
    ...
    auto &flags = get_decoration_bitset(var.self);
    statement(flags_to_qualifiers_glsl(type, flags),
              variable_decl(type, join("_", var.self, "_copy")), ";");
...
void CompilerGLSL::flush_variable_declaration(uint32_t id) {
  ...
  if (var && var->deferred_declaration) {
    ...
    if (options.force_zero_initialized_variables &&
      ...
      initializer =
          join(" = ",
               to_zero_initialized_expression(get_variable_data_type_id(*var)));
    }
...
bool CompilerGLSL::remove_duplicate_swizzle(string &op) {
  ...
  for (auto i = prevpos; i < pos; i++) {
    if (op[i] < 'w' || op[i] > 'z') {
      // If swizzles are foo.xyz() like in C++ backend for example, check for
      // that.
      if (backend.swizzle_is_function && i + 2 == pos && op[i] == '(' &&
          op[i + 1] == ')')
        break;
      return false;
    }
...
string CompilerGLSL::build_composite_combiner(uint32_t return_type,
  ...
  bool can_apply_swizzle_opt = type.basetype != SPIRType::Struct &&
                               type.array.empty() && type.columns == 1;
  ...
  for (uint32_t i = 0; i < length; i++) {
    ...
    // object, just merge the swizzles to avoid triggering more than 1
    // expression read as much as possible!
    if (can_apply_swizzle_opt && e && e->base_expression &&
        e->base_expression == base) {
      ...
      if (swizzle_optimization) {
        ...
        // Don't attempt to remove unity swizzling if we managed to remove
        // duplicate swizzles.
        // The base "foo" might be vec4, while foo.xyz is vec3 (OpVectorShuffle)
        // and looks like a vec3 due to the .xyz tacked on.
        // We only want to remove the swizzles if we're certain that the
        // resulting base will be the same vecsize.
        // Essentially, we can only remove one set of swizzles, since that's
        // what we have control over ...
        // Case 1:
        //  foo.yxz.xyz: Duplicate swizzle kicks in, giving foo.yxz, we are
        //  done.
        //               foo.yxz was the result of OpVectorShuffle and we don't
        //               know the type of foo.
        // Case 2:
        ...
        // Strips away redundant parens if we created them during component
        // extraction.
...
bool CompilerGLSL::optimize_read_modify_write(const SPIRType &type,
                                              const string &lhs,
                                              const string &rhs) {
  // Do this with strings because we have a very clear pattern we can check for
  // and it avoids
  ...
  // (in which order does operation happen?), and it does not work on MSL
  // anyways.
  if (type.vecsize > 1 && type.columns > 1)
    return false;
  ...
  // Try to find increments and decrements. Makes it look neater as += 1, -= 1
  // is fairly rare to see in real code.
  // Find some common patterns which are equivalent.
  if ((bop == '+' || bop == '-') &&
      (expr == "1" || expr == "uint(1)" || expr == "1u" || expr == "int(1u)"))
...
void CompilerGLSL::disallow_forwarding_in_expression_chain(
    const SPIRExpression &expr) {
  ...
  // Our only concern is to make sure arithmetic operations are done in similar
  // ways.
  if (expression_is_forwarded(expr.self) &&
      !expression_suppresses_usage_tracking(expr.self) &&
...
void CompilerGLSL::handle_store_to_invariant_variable(uint32_t store_id,
                                                      uint32_t value_id) {
  // Variables or access chains marked invariant are complicated. We will need
  // to make sure the code-gen leading up to
  // this variable is consistent. The failure case for SPIRV-Cross is when an
  // expression is forced to a temporary
  // in one translation unit, but not another, e.g. due to multiple use of an
  // expression.
  // This causes variance despite the output variable being marked invariant, so
  // the solution here is to force all dependent
  // expressions to be temporaries.
  // It is uncertain if this is enough to support invariant in all possible
...
void CompilerGLSL::emit_store_statement(uint32_t lhs_expression,
                                        uint32_t rhs_expression) {
  auto rhs = to_pointer_expression(rhs_expression);

  // Statements to OpStore may be empty if it is a struct with zero members.
  // Just forward the store to /dev/null.
  if (!rhs.empty()) {
    ...
    if (!unroll_array_to_complex_store(lhs_expression, rhs_expression)) {
      ...
      cast_to_variable_store(lhs_expression, rhs,
                             expression_type(rhs_expression));

      // Tries to optimize assignments like "<lhs> = <lhs> op expr".
      // While this is purely cosmetic, this is important for legacy ESSL where
      // loop
      // variable increments must be in either i++ or i += const-expr.
      // Without this, we end up with i = i + 1, which is correct GLSL, but not
      // correct GLES 2.0.
...
uint32_t CompilerGLSL::get_integer_width_for_instruction(
...
uint32_t CompilerGLSL::get_integer_width_for_glsl_instruction(
    GLSLstd450 op, const uint32_t *ops, uint32_t length) const {
  if (length < 1)
    return 32;
...
#define GLSL_UFOP(op) emit_unary_func_op(ops[0], ops[1], ops[2], #op)
  ...
  case OpLoad: {
    ...
    // If an expression is mutable and forwardable, we speculate that it is
    // immutable.
    bool forward = should_forward(ptr) &&
                   forced_temporaries.find(id) == end(forced_temporaries);

    // If loading a non-native row-major matrix, mark the expression as
    // need_transpose.
    bool need_transpose = false;
    ...
    if (forward) {
      // If we're forwarding the load, we're also going to forward transpose
      // state, so don't transpose while
    ...
    // don't register the read to access chain here, defer that to when we
    ...
    if (forward || (!is_packed && !is_remapped)) {
      ...
      // If we are not forwarding the expression, we need to unpack and resolve
      // any physical type remapping here before
    ...
    if (!type_is_opaque_value(type) &&
        has_decoration(ptr, DecorationNonUniform)) {
      // If we're loading something non-opaque, we need to handle non-uniform
      // descriptor access.
    ...
    bool flattened =
        ptr_expression &&
        flattened_buffer_blocks.count(ptr_expression->loaded_from) != 0;

    if (backend.needs_row_major_load_workaround &&
        !is_non_native_row_major_matrix(ptr) && !flattened)
      rewrite_load_for_wrapped_row_major(expr, result_type, ptr);

    // By default, suppress usage tracking since using same expression multiple
    // times does not imply any extra work.
    // However, if we try to load a complex, composite object from a flattened
    // buffer,
    // we should avoid emitting the same code over and over and lower the result
    // to a temporary.
    bool usage_tracking =
        flattened && (type.basetype == SPIRType::Struct || (type.columns > 1));

    SPIRExpression *e = nullptr;
    if (!forward && expression_is_non_value_type_array(ptr)) {
      // Complicated load case where we need to make a copy of ptr, but we
      // cannot, because
      ...
      emit_array_copy(to_expression(id), id, ptr, StorageClassFunction,
                      get_expression_effective_storage_class(ptr));
    } else
    ...
    if (forward) {
      // Pass through whether the result is of a packed type and the physical
      // type ID.
      ...
      if (has_extended_decoration(ptr, SPIRVCrossDecorationPhysicalTypeID)) {
        set_extended_decoration(
            id, SPIRVCrossDecorationPhysicalTypeID,
            get_extended_decoration(ptr, SPIRVCrossDecorationPhysicalTypeID));
      }
    } else {
      // This might have been set on an earlier compilation iteration, force it
      // to be unset.
  ...
  case OpPtrAccessChain: {
    ...
    auto e = access_chain(ops[2], &ops[3], length - 3, get<SPIRType>(ops[0]),
                          &meta, ptr_chain);
    ...
    // Mark the result as being packed. Some platforms handled packed vectors
  ...
  case OpStore: {
    ...
    // Storing a pointer results in a variable pointer, so we must
  ...
  case OpArrayLength: {
    ...
    set<SPIRExpression>(id, join(type_to_glsl(get<SPIRType>(result_type)), "(",
                                 e, ".length())"),
                        result_type, true);
    break;
  }
  ...
  case OpFunctionCall: {
    ...
    // Invalidate out variables passed to functions since they can be OpStore'd
    // to.
    for (uint32_t i = 0; i < length; i++) {
    ...
    if (emit_return_value_as_argument) {
      statement(type_to_glsl(return_type), " ", to_name(id),
                type_to_array_glsl(return_type), ";");
    ...
    for (auto &combined : callee.combined_parameters) {
      auto image_id = combined.global_image
                          ? combined.image_id
                          : VariableID(arg[combined.image_id]);
      auto sampler_id = combined.global_sampler
                            ? combined.sampler_id
                            : VariableID(arg[combined.sampler_id]);
  ...
  case OpCompositeConstruct: {
    ...
    bool composite =
        !out_type.array.empty() || out_type.basetype == SPIRType::Struct;

    bool splat = false;
    bool swizzle_splat = false;

    if (in_type) {
      splat = in_type->vecsize == 1 && in_type->columns == 1 && !composite &&
              backend.use_constructor_splatting;
      swizzle_splat = in_type->vecsize == 1 && in_type->columns == 1 &&
                      backend.can_swizzle_scalar;

      if (ir.ids[elems[0]].get_type() == TypeConstant &&
    ...
    if (backend.use_initializer_list && composite) {
      ...
      if (backend.use_typed_initializer_list &&
          out_type.basetype == SPIRType::Struct && out_type.array.empty())
        constructor_op += type_to_glsl_constructor(get<SPIRType>(result_type));
      else if (backend.use_typed_initializer_list &&
               backend.array_is_value_type && !out_type.array.empty()) {
        // MSL path. Array constructor is baked into type here, do not use
        // _constructor variant.
        constructor_op +=
            type_to_glsl_constructor(get<SPIRType>(result_type)) + "(";
        needs_trailing_tracket = true;
      }
      constructor_op += "{ ";
      ...
      constructor_op = remap_swizzle(get<SPIRType>(result_type), 1,
                                     to_unpacked_expression(elems[0]));
    } else {
      constructor_op =
          type_to_glsl_constructor(get<SPIRType>(result_type)) + "(";
      if (type_is_empty(out_type) && !backend.supports_empty_struct)
  ...
  case OpCompositeExtract: {
    ...
    // We can only split the expression here if our expression is forwarded as a
    // temporary.
    bool allow_base_expression =
        forced_temporaries.find(id) == end(forced_temporaries);

    // Do not allow base expression for struct members. We risk doing "swizzle"
    // optimizations in this case.
    auto &composite_type = expression_type(ops[2]);
    bool composite_type_is_complex =
        composite_type.basetype == SPIRType::Struct ||
        !composite_type.array.empty();
    ...
    // Cannot use base expression for row-major matrix row-extraction since we
    // need to interleave access pattern
    ...
    if (c && !c->specialization && !composite_type_is_complex) {
      auto expr = to_extract_constant_composite_expression(result_type, *c,
                                                           ops + 3, length);
      e = &emit_op(result_type, id, expr, true, true);
    } else if (allow_base_expression && should_forward(ops[2]) &&
               type.vecsize == 1 && type.columns == 1 && length == 1) {
      ...
      auto expr = access_chain_internal(ops[2], &ops[3], length,
                                        ...
                                        &meta);
      e = &emit_op(result_type, id, expr, true,
                   should_suppress_usage_tracking(ops[2]));
      inherit_expression_dependencies(id, ops[2]);
      ...
      auto expr = access_chain_internal(ops[2], &ops[3], length,
                                        ACCESS_CHAIN_INDEX_IS_LITERAL_BIT |
                                            ACCESS_CHAIN_FORCE_COMPOSITE_BIT,
                                        &meta);
      e = &emit_op(result_type, id, expr, should_forward(ops[2]),
                   should_suppress_usage_tracking(ops[2]));
      inherit_expression_dependencies(id, ops[2]);
    }

    // Pass through some meta information to the loaded expression.
    // We can still end up loading a buffer type to a variable, then
    // CompositeExtract from it
  ...
  case OpCompositeInsert: {
    ...
    set<SPIRExpression>(id, to_name(id), result_type, true);
    auto chain = access_chain_internal(
        id, elems, length, ACCESS_CHAIN_INDEX_IS_LITERAL_BIT, nullptr);
  ...
  case OpCopyMemory: {
    ...
    if (lhs != rhs) {
      uint32_t &tmp_id =
          extra_sub_expressions[instruction.offset |
                                EXTRA_SUB_EXPRESSION_TYPE_STREAM_OFFSET];
      if (!tmp_id)
      ...
      // Load and Store do a *lot* of workarounds, and we'd like to reuse them
      // as much as possible.
  ...
  case OpCopyObject: {
    ...
    if (chain) {
      ...
      // Cannot lower to a SPIRExpression, just copy the object.
      // GLSL does not currently use this type and will never get here, but MSL
      // does.
      // Handled here instead of CompilerMSL for better integration and general
      // handling,
  ...
  case OpVectorShuffle: {
    ...
    // If we have the undefined swizzle index -1, we need to swizzle in
    // undefined data,
    // or in our case, T(0).
    bool shuffle = false;
    for (uint32_t i = 0; i < length; i++)
    ...
    if (!shuffle &&
        has_extended_decoration(vec0, SPIRVCrossDecorationPhysicalTypePacked))
      shuffle = true;
    ...
    if (shuffle) {
      ...
      trivial_forward = should_suppress_usage_tracking(vec0) &&
                        should_suppress_usage_tracking(vec1);
      ...
      for (uint32_t i = 0; i < length; i++) {
        if (elems[i] == 0xffffffffu) {
          ...
          // We could use the first component or similar, but then we risk
          // propagating
          ...
          args.push_back(
              to_extract_component_expression(vec1, elems[i] - type0.vecsize));
        else
          args.push_back(to_extract_component_expression(vec0, elems[i]));
      }
      expr += join(type_to_glsl_constructor(get<SPIRType>(result_type)), "(",
                   merge(args), ")");
    } else {
      ...
      // If the vector is packed, unpack it before applying a swizzle (needed
      // for MSL)
    ...
    // We inherit the forwardedness from our arguments to avoid flushing out to
    // temporaries when it's not really needed.
  ...
  case OpIAdd: {
    // For simple arith ops, prefer the output type if there's a mismatch to
    // avoid extra bitcasts.
  ...
  case OpSRem: {
    ...
    bool forward = should_forward(op0) && should_forward(op1);
    auto expr =
        join(to_enclosed_expression(op0), " - ", to_enclosed_expression(op1),
             " * ", "(", to_enclosed_expression(op0), " / ",
             to_enclosed_expression(op1), ")");
  ...
  case OpISubBorrow: {
    ...
    statement(to_expression(result_id), ".", to_member_name(type, 0), " = ", op,
              "(", to_expression(op0), ", ", to_expression(op1), ", ",
  ...
  case OpSMulExtended: {
    ...
    statement(op, "(", to_expression(op0), ", ", to_expression(op1), ", ",
              to_expression(result_id), ".", to_member_name(type, 1), ", ",
  ...
  case OpFRem: {
    if (is_legacy())
      SPIRV_CROSS_THROW("OpFRem requires trunc() and is only supported on "
                        "non-legacy targets. A workaround is "
                        "needed for legacy.");
    ...
    auto expr =
        join(to_enclosed_expression(op0), " - ", to_enclosed_expression(op1),
             " * ", "trunc(", to_enclosed_expression(op0), " / ",
             to_enclosed_expression(op1), ")");
  ...
  case OpConvertUToF: {
    auto input_type =
        opcode == OpSConvert || opcode == OpConvertSToF ? int_type : uint_type;
    uint32_t result_type = ops[0];
    uint32_t id = ops[1];
    ...
    if (arg_type.width < type.width || type_is_floating_point(type))
      emit_unary_func_op_cast(result_type, id, ops[2], func.c_str(), input_type,
                              type.basetype);
    else
  ...
  case OpConvertFToU:
  case OpConvertFToS: {
    // Cast to expected arithmetic type, then potentially bitcast away to
    // desired signedness.
    uint32_t result_type = ops[0];
    uint32_t id = ops[1];
    auto &type = get<SPIRType>(result_type);
    ...
    expected_type.basetype = opcode == OpConvertFToS
                                 ? to_signed_basetype(type.width)
                                 : to_unsigned_basetype(type.width);

    auto func = type_to_glsl_constructor(expected_type);
    emit_unary_func_op_cast(result_type, id, ops[2], func.c_str(),
                            float_type.basetype, expected_type.basetype);
    break;
  }
  ...
  case OpBitcast: {
    ...
    if (!emit_complex_bitcast(result_type, id, arg)) {
      auto op =
          bitcast_glsl_op(get<SPIRType>(result_type), expression_type(arg));
  ...
  case OpQuantizeToF16: {
    ...
    switch (type.vecsize) {
    case 1:
      op = join("unpackHalf2x16(packHalf2x16(vec2(", to_expression(arg),
                "))).x");
      break;
    case 2:
    ...
    case 3: {
      auto op0 =
          join("unpackHalf2x16(packHalf2x16(", to_expression(arg), ".xy))");
      auto op1 =
          join("unpackHalf2x16(packHalf2x16(", to_expression(arg), ".zz)).x");
    ...
    case 4: {
      auto op0 =
          join("unpackHalf2x16(packHalf2x16(", to_expression(arg), ".xy))");
      auto op1 =
          join("unpackHalf2x16(packHalf2x16(", to_expression(arg), ".zw))");
  ...
  case OpBitFieldSExtract: {
    emit_trinary_func_op_bitextract(ops[0], ops[1], ops[2], ops[3], ops[4],
                                    "bitfieldExtract", int_type, int_type,
  ...
  case OpBitFieldUExtract: {
    emit_trinary_func_op_bitextract(ops[0], ops[1], ops[2], ops[3], ops[4],
                                    "bitfieldExtract", uint_type, uint_type,
                                    SPIRType::Int, SPIRType::Int);
    break;
  }
  ...
  case OpBitCount: {
    ...
    emit_unary_func_op_cast(ops[0], ops[1], ops[2], "bitCount", basetype,
                            int_type);
    break;
  }
  ...
  case OpAtomicExchange: {
    ...
    uint32_t val = ops[5];
    const char *op =
        check_atomic_image(ptr) ? "imageAtomicExchange" : "atomicExchange";
  ...
  case OpAtomicCompareExchange: {
    ...
    uint32_t comp = ops[7];
    const char *op =
        check_atomic_image(ptr) ? "imageAtomicCompSwap" : "atomicCompSwap";
  ...
  case OpAtomicLoad: {
    // In plain GLSL, we have no atomic loads, so emulate this by fetch adding
    // by 0 and hope compiler figures it out.
    // Alternatively, we could rely on KHR_memory_model, but that's not very
    // helpful for GL.
    ...
    bool unsigned_type =
        ...
        (atomic_image &&
         get<SPIRType>(type.image.type).basetype == SPIRType::UInt);
    const char *op = atomic_image ? "imageAtomicAdd" : "atomicAdd";
    const char *increment = unsigned_type ? "0u" : "0";
    emit_op(ops[0], ops[1],
            join(op, "(", to_non_uniform_aware_expression(ops[2]), ", ",
                 increment, ")"),
            false);
    flush_all_atomic_capable_variables();
    break;
  ...
  case OpAtomicStore: {
    // In plain GLSL, we have no atomic stores, so emulate this with an atomic
    // exchange where we don't consume the result.
    // Alternatively, we could rely on KHR_memory_model, but that's not very
    // helpful for GL.
    uint32_t ptr = ops[0];
    // Ignore semantics for now, probably only relevant to CL.
    ...
    const char *op =
        check_atomic_image(ptr) ? "imageAtomicExchange" : "atomicExchange";
    statement(op, "(", to_non_uniform_aware_expression(ptr), ", ",
              to_expression(val), ");");
    flush_all_atomic_capable_variables();
    break;
  }
  ...
  case OpAtomicIAdd: {
    const char *op =
        check_atomic_image(ops[2]) ? "imageAtomicAdd" : "atomicAdd";
  ...
  case OpAtomicISub: {
    const char *op =
        check_atomic_image(ops[2]) ? "imageAtomicAdd" : "atomicAdd";
    forced_temporaries.insert(ops[1]);
    auto expr = join(op, "(", to_non_uniform_aware_expression(ops[2]), ", -",
                     to_enclosed_expression(ops[5]), ")");
    emit_op(ops[0], ops[1], expr,
            should_forward(ops[2]) && should_forward(ops[5]));
    flush_all_atomic_capable_variables();
    break;
  }
  ...
  case OpAtomicUMin: {
    const char *op =
        check_atomic_image(ops[2]) ? "imageAtomicMin" : "atomicMin";
    emit_atomic_func_op(ops[0], ops[1], ops[2], ops[5], op);
    break;
  }
  ...
  case OpAtomicUMax: {
    const char *op =
        check_atomic_image(ops[2]) ? "imageAtomicMax" : "atomicMax";
    emit_atomic_func_op(ops[0], ops[1], ops[2], ops[5], op);
    break;
  }
  ...
  case OpAtomicAnd: {
    const char *op =
        check_atomic_image(ops[2]) ? "imageAtomicAnd" : "atomicAnd";
    emit_atomic_func_op(ops[0], ops[1], ops[2], ops[5], op);
    break;
  }
  ...
  case OpAtomicXor: {
    const char *op =
        check_atomic_image(ops[2]) ? "imageAtomicXor" : "atomicXor";
    emit_atomic_func_op(ops[0], ops[1], ops[2], ops[5], op);
    break;
  }
  ...
  case OpImageSparseTexelsResident:
    ...
    require_extension_internal("GL_ARB_sparse_texture2");
    emit_unary_func_op_cast(ops[0], ops[1], ops[2], "sparseTexelsResidentARB",
                            int_type, SPIRType::Boolean);
    break;
  ...
  case OpImage: {
    ...
    // When using the image, we need to know which variable it is actually
    // loaded from.
  ...
  case OpImageQueryLod: {
    ...
    if (has_decoration(ops[2], DecorationNonUniform)) {
      if (maybe_get_backing_variable(ops[2]))
        ...
        sampler_expr =
            join(backend.nonuniform_qualifier, "(", sampler_expr, ")");
    }

    bool forward = should_forward(ops[3]);
    emit_op(ops[0], ops[1], join(op, "(", sampler_expr, ", ",
                                 to_unpacked_expression(ops[3]), ")"),
            forward);
    inherit_expression_dependencies(ops[1], ops[2]);
    inherit_expression_dependencies(ops[1], ops[3]);
    register_control_dependent_expression(ops[1]);
  ...
  case OpImageQueryLevels: {
    ...
    auto expr = join("textureQueryLevels(",
                     convert_separate_image_to_expression(ops[2]), ")");
  ...
  case OpImageQuerySamples: {
    ...
    if (type.image.sampled == 2)
      expr =
          join("imageSamples(", to_non_uniform_aware_expression(ops[2]), ")");
    else
      expr = join("textureSamples(",
                  convert_separate_image_to_expression(ops[2]), ")");
  ...
  case OpImageRead:
  case OpImageSparseRead: {
    // We added Nonreadable speculatively to the OpImage variable due to
    // glslangValidator
    // not adding the proper qualifiers.
    // If it turns out we need to read the image after all, remove the qualifier
    // and recompile.
    auto *var = maybe_get_backing_variable(ops[2]);
    if (var) {
      auto &flags = ir.meta[var->self].decoration.decoration_flags;
    ...
    if (var && var->remapped_variable) // Remapped input, just read as-is
                                       // without any op-code
    {
      if (type.image.ms)
        SPIRV_CROSS_THROW("Trying to remap multisampled image to variable, "
                          "this is not possible.");

      auto itr =
          find_if(begin(pls_inputs), end(pls_inputs),
                  [var](const PlsRemap &pls) { return pls.id == var->self; });

      if (itr == end(pls_inputs)) {
        // For non-PLS inputs, we rely on subpass type remapping information to
        // get it right
        // since ImageRead always returns 4-component vectors and the backing
        // type is opaque.
        if (!var->remapped_components)
          SPIRV_CROSS_THROW("subpassInput was remapped, but remap_components "
                            "is not set correctly.");
        imgexpr =
            remap_swizzle(get<SPIRType>(result_type), var->remapped_components,
                          to_expression(ops[2]));
      } else {
        // PLS input could have different number of components than what the
        // SPIR expects, swizzle to
        // the appropriate vector size.
        uint32_t components = pls_format_to_components(itr->format);
        imgexpr = remap_swizzle(get<SPIRType>(result_type), components,
                                to_expression(ops[2]));
      }
      pure = true;
      ...
      if (var && subpass_input_is_framebuffer_fetch(var->self)) {
        ...
        if (type.image.ms) {
          ...
          if (operands != ImageOperandsSampleMask || length != 6)
            SPIRV_CROSS_THROW(
                "Multisampled image used in OpImageRead, but unexpected "
                "operand mask was used.");

          uint32_t samples = ops[5];
          imgexpr =
              join("subpassLoad(", to_non_uniform_aware_expression(ops[2]),
                   ", ", to_expression(samples), ")");
        } else
          imgexpr = join("subpassLoad(",
                         to_non_uniform_aware_expression(ops[2]), ")");
      } else {
        if (type.image.ms) {
          uint32_t operands = ops[4];
          if (operands != ImageOperandsSampleMask || length != 6)
          ...
          uint32_t samples = ops[5];
          imgexpr =
              join("texelFetch(", to_non_uniform_aware_expression(ops[2]),
                   ", ivec2(gl_FragCoord.xy), ", to_expression(samples), ")");
        } else {
          // Implement subpass loads via texture barrier style sampling.
          imgexpr = join("texelFetch(", to_non_uniform_aware_expression(ops[2]),
      ...
      uint32_t sparse_texel_id = 0;
      if (sparse)
        emit_sparse_feedback_temporaries(ops[0], ops[1], sparse_code_id,
                                         sparse_texel_id);
      ...
      coord_expr = bitcast_expression(
          target_coord_type, expression_type(ops[3]).basetype, coord_expr);
      ...
      if (sparse) {
        ...
        if (type.image.ms) {
          ...
          uint32_t samples = ops[5];
          imgexpr = join("imageLoad(", to_non_uniform_aware_expression(ops[2]),
                         ", ", coord_expr, ", ", to_expression(samples), ")");
        } else
          imgexpr = join("imageLoad(", to_non_uniform_aware_expression(ops[2]),
                         ", ", coord_expr, ")");
      }
    ...
    if (var && var->forwardable) {
      ...
      // We only need to track dependencies if we're reading from image
      // load/store.
      if (!pure) {
  ...
  case OpImageWrite: {
    // We added Nonwritable speculatively to the OpImage variable due to
    // glslangValidator
    // not adding the proper qualifiers.
    // If it turns out we need to write to the image after all, remove the
    // qualifier and recompile.
    ...
    target_coord_type.basetype = SPIRType::Int;
    coord_expr = bitcast_expression(
        target_coord_type, expression_type(ops[1]).basetype, coord_expr);

    if (type.image.ms) {
      uint32_t operands = ops[3];
      if (operands != ImageOperandsSampleMask || length != 5)
        SPIRV_CROSS_THROW("Multisampled image used in OpImageWrite, but "
                          "unexpected operand mask was used.");
      uint32_t samples = ops[4];
      statement(
          "imageStore(", to_non_uniform_aware_expression(ops[0]), ", ",
          coord_expr, ", ", to_expression(samples), ", ",
          remap_swizzle(store_type, value_type.vecsize, to_expression(ops[2])),
          ");");
    } else
      statement("imageStore(", to_non_uniform_aware_expression(ops[0]), ", ",
                coord_expr, ", ", remap_swizzle(store_type, value_type.vecsize,
                                                to_expression(ops[2])),
                ");");
  ...
  case OpImageQuerySize: {
    ...
    if (type.basetype == SPIRType::Image) {
      ...
      if (type.image.sampled == 2) {
        ...
        // This path is hit for samplerBuffers and multisampled images which do
        // not have LOD.
        std::string fname = "textureSize";
        if (is_legacy()) {
          auto &imgtype = get<SPIRType>(type.self);
        ...
        expr =
            join(fname, "(", convert_separate_image_to_expression(ops[2]), ")");
      }
  ...
  case OpMemoryBarrier: {
    ...
    if (execution_scope == ScopeSubgroup || memory == ScopeSubgroup) {
      ...
      if (opcode != OpControlBarrier) {
        request_subgroup_feature(
            ShaderSubgroupSupportHelper::SubgroupMemBarrier);
      } else {
    ...
    // We only care about these flags, acquire/release and friends are not
    // relevant to GLSL.
    semantics = mask_relevant_memory_semantics(semantics);

    if (opcode == OpMemoryBarrier) {
      // If we are a memory barrier, and the next instruction is a control
      // barrier, check if that memory barrier
      ...
      if (next && next->op == OpControlBarrier) {
        ...
        if (next_memory == memory)
          ...
          // If we only care about workgroup memory, either Device or Workgroup
          // scope is fine,
          ...
          // The control barrier has device scope, but the memory barrier just
          // has workgroup scope.
          memory_scope_covered = true;
        }

        // If we have the same memory scope, and all memory types are covered,
        // we're good.
    ...
    if (memory == ScopeWorkgroup) // Only need to consider memory within a group
    {
      if (semantics == MemorySemanticsWorkgroupMemoryMask) {
        ...
        bool implies_shared_barrier =
            opcode == OpControlBarrier && execution_scope == ScopeWorkgroup;
      ...
      if (semantics & (MemorySemanticsCrossWorkgroupMemoryMask |
  ...
  case OpExtInst: {
    ...
    } else if (get<SPIRExtension>(extension_set).ext ==
               SPIRExtension::SPV_AMD_shader_ballot) {
      emit_spv_amd_shader_ballot_op(ops[0], ops[1], ops[3], &ops[4],
                                    length - 4);
    } else if (get<SPIRExtension>(extension_set).ext ==
               SPIRExtension::SPV_AMD_shader_explicit_vertex_parameter) {
      emit_spv_amd_shader_explicit_vertex_parameter_op(ops[0], ops[1], ops[3],
                                                       &ops[4], length - 4);
    } else if (get<SPIRExtension>(extension_set).ext ==
               SPIRExtension::SPV_AMD_shader_trinary_minmax) {
      emit_spv_amd_shader_trinary_minmax_op(ops[0], ops[1], ops[3], &ops[4],
                                            length - 4);
    } else if (get<SPIRExtension>(extension_set).ext ==
  ...
  case OpSubgroupBallotKHR: {
    ...
    uint32_t id = ops[1];
    string expr;
    expr = join("uvec4(unpackUint2x32(ballotARB(" + to_expression(ops[2]) +
                ")), 0u, 0u)");
    emit_op(result_type, id, expr, should_forward(ops[2]));
  ...
  case OpFragmentMaskFetchAMD: {
    ...
    if (type.image.dim == spv::DimSubpassData) {
      ...
      emit_binary_func_op(result_type, id, ops[2], ops[3],
  ...
  case OpFragmentFetchAMD: {
    ...
    if (type.image.dim == spv::DimSubpassData) {
      ...
      emit_trinary_func_op(result_type, id, ops[2], ops[3], ops[4],
                           "fragmentFetchAMD");
    }
  ...
  case OpFUnordGreaterThanEqual: {
    // GLSL doesn't specify if floating point comparisons are ordered or
    // unordered,
    // but glslang always emits ordered floating point compares for GLSL.
    // To get unordered compares, we can test the opposite thing and invert the
    // result.
    ...
    if (expression_type(op0).vecsize > 1) {
      ...
      expr = join("not(", comp_op, "(", to_unpacked_expression(op0), ", ",
                  to_unpacked_expression(op1), "))");
    } else {
      const char *comp_op = nullptr;
      switch (opcode) {
      ...
      expr = join("!(", to_enclosed_unpacked_expression(op0), comp_op,
                  to_enclosed_unpacked_expression(op1), ")");
    }
  ...
  case OpTerminateRayNV:
    ...
    break;
  case OpTraceNV:
    statement("traceNV(", to_non_uniform_aware_expression(ops[0]), ", ",
              to_expression(ops[1]), ", ", to_expression(ops[2]), ", ",
              to_expression(ops[3]), ", ", to_expression(ops[4]), ", ",
              to_expression(ops[5]), ", ", to_expression(ops[6]), ", ",
              to_expression(ops[7]), ", ", to_expression(ops[8]), ", ",
  ...
  case OpTraceRayKHR:
    ...
    statement("traceRayEXT(", to_non_uniform_aware_expression(ops[0]), ", ",
              to_expression(ops[1]), ", ", to_expression(ops[2]), ", ",
              to_expression(ops[3]), ", ", to_expression(ops[4]), ", ",
              to_expression(ops[5]), ", ", to_expression(ops[6]), ", ",
              to_expression(ops[7]), ", ", to_expression(ops[8]), ", ",
              to_expression(ops[9]), ", ",
              get_decoration(ops[10], DecorationLocation), ");");
    flush_control_dependent_expressions(current_emitting_block->self);
    break;
  case OpExecuteCallableNV:
    statement("executeCallableNV(", to_expression(ops[0]), ", ",
              to_expression(ops[1]), ");");
    flush_control_dependent_expressions(current_emitting_block->self);
    break;
  case OpExecuteCallableKHR:
    ...
    statement("executeCallableEXT(", to_expression(ops[0]), ", ",
              get_decoration(ops[1], DecorationLocation), ");");
    flush_control_dependent_expressions(current_emitting_block->self);
    break;

  // Don't bother forwarding temporaries. Avoids having to test expression
  // invalidation with ray query objects.
  case OpRayQueryInitializeKHR:
    flush_variable_declaration(ops[0]);
    statement("rayQueryInitializeEXT(", to_expression(ops[0]), ", ",
              to_expression(ops[1]), ", ", to_expression(ops[2]), ", ",
              to_expression(ops[3]), ", ", to_expression(ops[4]), ", ",
              to_expression(ops[5]), ", ", to_expression(ops[6]), ", ",
              to_expression(ops[7]), ");");
  ...
  case OpRayQueryProceedKHR:
    flush_variable_declaration(ops[0]);
    emit_op(ops[0], ops[1],
            join("rayQueryProceedEXT(", to_expression(ops[2]), ")"), false);
    break;
  ...
  case OpRayQueryGenerateIntersectionKHR:
    flush_variable_declaration(ops[0]);
    statement("rayQueryGenerateIntersectionEXT(", to_expression(ops[0]), ", ",
              to_expression(ops[1]), ");");
    break;
...
#define GLSL_RAY_QUERY_GET_OP(op)                                              \
  case OpRayQueryGet##op##KHR:                                                 \
    ...
    emit_op(ops[0], ops[1],                                                    \
            join("rayQueryGet" #op "EXT(", to_expression(ops[2]), ")"),        \
            false);                                                            \
    break
...
#undef GLSL_RAY_QUERY_GET_OP2
  ...
  case OpConvertUToPtr: {
    auto &type = get<SPIRType>(ops[0]);
    if (type.storage != StorageClassPhysicalStorageBufferEXT)
      SPIRV_CROSS_THROW("Only StorageClassPhysicalStorageBufferEXT is "
                        "supported by OpConvertUToPtr.");
  ...
  case OpDemoteToHelperInvocationEXT:
    if (!options.vulkan_semantics)
      SPIRV_CROSS_THROW("GL_EXT_demote_to_helper_invocation is only supported "
                        "in Vulkan GLSL.");
...
// Appends function arguments, mapped from global variables, beyond the
// specified arg index.
// This is used when a function call uses fewer arguments than the function
// defines.
// This situation may occur if the function signature has been dynamically
// modified to
// extract global variables referenced from within the function, and convert
// them to
// function arguments. This is necessary for shader languages that do not
// support global
// access to shader input content from within a function (eg. Metal). Each
// additional
// function args uses the name of the global variable. Function nesting will
// modify the
// functions and function calls all the way up the nesting chain.
void CompilerGLSL::append_global_func_args(const SPIRFunction &func,
                                           uint32_t index,
                                           SmallVector<string> &arglist) {
...
string CompilerGLSL::to_member_reference(uint32_t, const SPIRType &type,
                                         uint32_t index, bool) {
  return join(".", to_member_name(type, index));
}

string
CompilerGLSL::to_multi_member_reference(const SPIRType &type,
                                        const SmallVector<uint32_t> &indices) {
  string ret;
  auto *member_type = &type;
  for (auto &index : indices) {
...
// before use
bool CompilerGLSL::member_is_non_native_row_major_matrix(const SPIRType &type,
                                                         uint32_t index) {
  // Natively supported row-major matrices do not need to be converted.
  if (backend.native_row_major_matrix && !is_legacy())
    return false;
  ...
  // Converting non-square matrices will require defining custom GLSL function
  // that
  // swaps matrix elements while retaining the original dimensional form of the
  // matrix.
...
// Checks if we need to remap physical type IDs when declaring the type in a
// buffer.
bool CompilerGLSL::member_is_remapped_physical_type(const SPIRType &type,
                                                    uint32_t index) const {
  return has_extended_member_decoration(type.self, index,
                                        SPIRVCrossDecorationPhysicalTypeID);
}

// Checks whether the member is in packed data type, that might need to be
// unpacked.
bool CompilerGLSL::member_is_packed_physical_type(const SPIRType &type,
                                                  uint32_t index) const {
  return has_extended_member_decoration(type.self, index,
                                        SPIRVCrossDecorationPhysicalTypePacked);
}
...
string CompilerGLSL::convert_row_major_matrix(string exp_str,
  ...
  if (!is_matrix(exp_type)) {
    ...
    if (exp_type.vecsize == 2 && exp_type.columns == 2) {
      ...
      SPIRV_CROSS_THROW("Non-square matrices are not supported in legacy GLSL, "
                        "cannot transpose.");
...
bool CompilerGLSL::variable_decl_is_remapped_storage(
    const SPIRVariable &var, StorageClass storage) const {
...
void CompilerGLSL::emit_struct_member(const SPIRType &type,
  ...
  statement(layout_for_member(type, index), qualifiers, qualifier,
            flags_to_qualifiers_glsl(membertype, memberflags),
            variable_decl(membertype, to_member_name(type, index)), ";");
}

void CompilerGLSL::emit_struct_padding_target(const SPIRType &) {}

string CompilerGLSL::flags_to_qualifiers_glsl(const SPIRType &type,
                                              const Bitset &flags) {
  ...
  if (type_is_floating_point(type) && flags.get(DecorationNoContraction) &&
      backend.support_precise_qualifier)
    qual = "precise ";

  // Structs do not have precision qualifiers, neither do doubles (desktop only
  // anyways, so no mediump/highp).
  bool type_supports_precision =
      type.basetype == SPIRType::Float || type.basetype == SPIRType::Int ||
  ...
  if (options.es) {
    ...
    // Vulkan GLSL supports precision qualifiers, even in desktop profiles,
    // which is convenient.
    // The default is highp however, so only emit mediump in the rare case that
    // a shader has these.
    if (flags.get(DecorationRelaxedPrecision))
      qual += "mediump ";
  }
...
string CompilerGLSL::to_precision_qualifiers_glsl(uint32_t id) {
  ...
  if (use_precision_qualifiers && (type.basetype == SPIRType::Image ||
                                   type.basetype == SPIRType::SampledImage)) {
    // Force mediump for the sampler type. We cannot declare 16-bit or smaller
    // image types.
    auto &result_type = get<SPIRType>(type.image.type);
    if (result_type.width < 32)
      return "mediump ";
  }
  return flags_to_qualifiers_glsl(type,
                                  ir.meta[id].decoration.decoration_flags);
}
...
void CompilerGLSL::fixup_io_block_patch_qualifiers(const SPIRVariable &var) {
  ...
  // a patch out block is translated to just block members getting the
  // decoration.
  // To make glslang not complain when we compile again, we have to transform
  // this back to a case where
...
string CompilerGLSL::argument_decl(const SPIRFunction::Parameter &arg) {
  // glslangValidator seems to make all arguments pointer no matter what which
  // is rather bizarre ...
  ...
  return join(direction, to_qualifiers_glsl(arg.id),
              variable_decl(type, to_name(arg.id), arg.id));
}
...
#ifndef NDEBUG
  auto &type = get<SPIRType>(type_id);
  assert(type.storage == StorageClassPrivate ||
...
string CompilerGLSL::variable_decl(const SPIRVariable &variable) {
  ...
  if (variable.loop_variable && variable.static_expression) {
    ...
    else if (options.force_zero_initialized_variables &&
             type_can_zero_initialize(type))
      res += join(" = ", to_zero_initialized_expression(
                             get_variable_data_type_id(variable)));
  } else if (variable.initializer &&
             !variable_decl_is_remapped_storage(variable,
...
string CompilerGLSL::pls_decl(const PlsRemap &var) {
  ...
  return join(to_pls_layout(var.format), to_pls_qualifiers_glsl(variable),
              type_to_glsl(type), " ", to_name(variable.self));
}
...
uint32_t CompilerGLSL::to_array_size_literal(const SPIRType &type,
                                             uint32_t index) const {
...
string CompilerGLSL::type_to_array_glsl(const SPIRType &type) {
  if (type.pointer && type.storage == StorageClassPhysicalStorageBufferEXT &&
      type.basetype != SPIRType::Struct) {
    // We are using a wrapped pointer type, and we should not emit any array
    // declarations here.
    return "";
  }
...
string CompilerGLSL::image_type_glsl(const SPIRType &type, uint32_t id) {
  ...
  // For half image types, we will force mediump for the sampler, and cast to
  // f16 after any sampling operation.
  // We cannot express a true half texture type in GLSL. Neither for short
  // integer formats for that matter.

  if (type.basetype == SPIRType::Image && type.image.dim == DimSubpassData &&
      options.vulkan_semantics)
    return res + "subpassInput" + (type.image.ms ? "MS" : "");
  else if (type.basetype == SPIRType::Image &&
           type.image.dim == DimSubpassData &&
  ...
  if (type.basetype == SPIRType::Image && type.image.dim != DimSubpassData) {
    // Sampler buffers are always declared as samplerBuffer even though they
    // might be separate images in the SPIR-V.
  ...
  case DimSubpassData:
    ...
    break;
  default:
    SPIRV_CROSS_THROW("Only 1D, 2D, 2DRect, 3D, Buffer, InputTarget and Cube "
                      "textures supported.");
  }
  ...
  // "Shadow" state in GLSL only exists for samplers and combined image
  // samplers.
...
string CompilerGLSL::type_to_glsl_constructor(const SPIRType &type) {
  if (backend.use_array_constructor && type.array.size() > 1) {
    if (options.flatten_multidimensional_arrays)
      SPIRV_CROSS_THROW(
          "Cannot flatten constructors of multidimensional array constructors, "
          "e.g. float[][]().");
    else if (!options.es && options.version < 430)
      require_extension_internal("GL_ARB_arrays_of_arrays");
    else if (options.es && options.version < 310)
...
string CompilerGLSL::type_to_glsl(const SPIRType &type, uint32_t id) {
  if (type.pointer && type.storage == StorageClassPhysicalStorageBufferEXT &&
      type.basetype != SPIRType::Struct) {
    // Need to create a magic type name which compacts the entire type
    // information.
  ...
  case SPIRType::Sampler:
    // The depth field is set by calling code based on the variable ID of the
    // sampler, effectively reintroducing
    // this distinction into the type system.
    return comparison_ids.count(id) ? "samplerShadow" : "sampler";

  case SPIRType::AccelerationStructure:
    return ray_tracing_is_khr ? "accelerationStructureEXT"
...
void CompilerGLSL::add_variable(
    unordered_set<string> &variables_primary,
...
bool CompilerGLSL::builtin_translates_to_nonarray(
    spv::BuiltIn /*builtin*/) const {
  return false; // GLSL itself does not need to translate array builtin types to
                // non-array builtin types
}
...
void CompilerGLSL::add_function_overload(const SPIRFunction &func) {
  ...
  for (auto &arg : func.arguments) {
    ...
    if (!combined_image_samplers.empty()) {
      // If we have combined image samplers, we cannot really trust the image
      // and sampler arguments
      // we pass down to callees, because they may be shuffled around.
      // Ignore these arguments, to make sure that functions need to differ in
      // some other way
...
void CompilerGLSL::emit_function_prototype(SPIRFunction &func,
  ...
  for (auto &arg : func.arguments) {
    ...
    // Since we want to make the GLSL debuggable and somewhat sane, use fallback
    // names for variables which are duplicates.
...
void CompilerGLSL::emit_function(SPIRFunction &func,
                                 const Bitset &return_flags) {
  ...
  // If we depend on a function, emit that function before we emit our own
  // function.
  for (auto block : func.blocks) {
    ...
    for (auto &i : b.ops) {
      ...
      if (op == OpFunctionCall) {
        ...
        uint32_t id = ops[2];
        emit_function(get<SPIRFunction>(id),
                      ir.meta[ops[1]].decoration.decoration_flags);
      }
    }
  }
  ...
  sort(begin(func.constant_arrays_needed_on_stack),
       end(func.constant_arrays_needed_on_stack));
  for (auto &array : func.constant_arrays_needed_on_stack) {
    auto &c = get<SPIRConstant>(array);
    auto &type = get<SPIRType>(c.constant_type);
    statement(variable_decl(type, join("_", array, "_array_copy")), " = ",
  ...
  for (auto &v : func.local_variables) {
    ...
    if (variable_decl_is_remapped_storage(var, StorageClassWorkgroup)) {
      ...
      // Comes from MSL which can push global variables as local variables in
      // main function.
      add_local_variable_name(var.self);
      statement(variable_decl(var), ";");
      var.deferred_declaration = false;
    } else if (var.storage == StorageClassPrivate) {
      // These variables will not have had their CFG usage analyzed, so move it
      // to the entry block.
      // Comes from MSL which can push global variables as local variables in
      // main function.
      // We could just declare them right now, but we would miss out on an
      // important initialization case which is
      // LUT declaration in MSL.
      // If we don't declare the variable when it is assigned we're forced to go
    ...
    } else if (var.storage == StorageClassFunction && var.remapped_variable &&
               var.static_expression) {
      ...
      // Loop variables should never be declared early, they are explicitly
      // emitted in a loop.
      if (var.initializer && !var.loop_variable)
        statement(variable_decl_function_local(var), ";");
      else {
        // Don't declare variable until first use to declutter the GLSL output
        // quite a lot.
        // If we don't touch the variable before first branch,
        // declare it then since we need variable declaration to be in top
        // scope.
        var.deferred_declaration = true;
      }
    } else {
      // HACK: SPIR-V in older glslang output likes to use samplers and images
      // as local variables, but GLSL does not allow this.
      // For these types (non-lvalue), we enforce forwarding through a shadowed
      // variable.
      // This means that when we OpStore to these variables, we just write in
      // the expression ID directly.
      // This breaks any kind of branching, since the variable must be
      // statically assigned.
      // Branching on samplers and images would be pretty much impossible to
      // fake in GLSL.
    ...
    // Loop variables are never declared outside their for-loop, so block any
  ...
  // Make sure deferred declaration state for local variables is cleared when we
  // are done with function.
  // We risk declaring Private/Workgroup variables in places we are not supposed
  // to otherwise.
  for (auto &v : func.local_variables) {
    auto &var = get<SPIRVariable>(v);
    var.deferred_declaration = false;
...
void CompilerGLSL::emit_fixup() {
  if (is_vertex_like_shader()) {
    if (options.vertex.fixup_clipspace) {
      ...
      statement("gl_Position.z = 2.0", suffix,
...
void CompilerGLSL::flush_phi(BlockID from, BlockID to) {
  ...
  for (auto itr = begin(child.phi_variables); itr != end(child.phi_variables);
       ++itr) {
    auto &phi = *itr;
    ...
    if (phi.parent == from) {
      ...
      if (var.loop_variable && !var.loop_variable_enable)
        ...
        // Check if we are going to write to a Phi variable that another
        // statement will read from
        // as part of another Phi node in our target block.
        // For this case, we will need to copy phi.function_variable to a
        // temporary, and use that for future reads.
        // This is judged to be extremely rare, so deal with it here using a
        // simple, but suboptimal algorithm.
        bool need_saved_temporary =
            find_if(itr + 1, end(child.phi_variables),
                    [&](const SPIRBlock::Phi &future_phi) -> bool {
                      return future_phi.local_variable ==
                                 ID(phi.function_variable) &&
                             future_phi.parent == from;
                    }) != end(child.phi_variables);

        if (need_saved_temporary) {
          // Need to make sure we declare the phi variable with a copy at the
          // right scope.
          // We cannot safely declare a temporary here since we might be inside
          // a continue block.
          if (!var.allocate_temporary_copy) {
            var.allocate_temporary_copy = true;
            force_recompile();
          }
          statement("_", phi.function_variable, "_copy", " = ",
...
void CompilerGLSL::branch_to_continue(BlockID from, BlockID to) {
  ...
  if (to_block.complex_continue) {
    ...
    if (loop_dominator != 0) {
      ...
      // For non-complex continue blocks, we implicitly branch to the continue
      // block
      // by having the continue block be part of the loop header in for (; ;
      // continue-block).
      outside_control_flow =
          cfg.node_terminates_control_flow_in_sub_graph(loop_dominator, from);
    }

    // Some simplification for for-loops. We always end up with a useless
    // continue;
    // statement since we branch to a loop block.
    // Walk the CFG, if we unconditionally execute the block calling continue
    // assuming we're in the loop block,
    // we can avoid writing out an explicit continue statement.
    // Similar optimization to return statements if we know we're outside flow
    // control.
...
void CompilerGLSL::branch(BlockID from, BlockID to) {
  ...
  if ((ir.block_meta[to] & ParsedIR::BLOCK_META_LOOP_HEADER_BIT) != 0 &&
      get<SPIRBlock>(from).loop_dominator == to) {
    // This can happen if we had a complex continue block which was emitted.
    // Once the continue block tries to branch to the loop header, just emit
    // continue;
    ...
    // Switch constructs are able to break, but they cannot break out of a loop
    // at the same time.
    // Only sensible solution is to make a ladder variable, which we declare at
    // the top of the switch block,
    // write to the ladder here, and defer the break.
    // The loop we're breaking out of must dominate the switch block, or there
    // is no ladder breaking case.
    if (current_emitting_switch && is_loop_break(to) &&
        current_emitting_switch->loop_dominator !=
            BlockID(SPIRBlock::NoDominator) &&
        get<SPIRBlock>(current_emitting_switch->loop_dominator).merge_block ==
            to) {
    ...
    // For from == to case can happen for a do-while loop which branches into
    // itself.
    // We don't mark these cases as continue blocks, but the only possible way
    // to branch into
    // ourselves is through means of continue blocks.

    // If we are merging to a continue block, there is no need to emit the block
    // chain for continue here.
    // We can branch to the continue block after we merge execution.

    // Here we make use of structured control flow rules from spec:
    // 2.11: - the merge block declared by a header block cannot be a merge
    // block declared by any other header block
    //       - each header block must strictly dominate its merge block, unless
    //       the merge block is unreachable in the CFG
    // If we are branching to a merge block, we must be inside a construct which
    // dominates the merge block.
    auto &block_meta = ir.block_meta[to];
    bool branching_to_merge =
        (block_meta & (ParsedIR::BLOCK_META_SELECTION_MERGE_BIT |
...
void CompilerGLSL::branch(BlockID from, uint32_t cond, BlockID true_block,
                          BlockID false_block) {
  auto &from_block = get<SPIRBlock>(from);
  BlockID merge_block = from_block.merge == SPIRBlock::MergeSelection
                            ? from_block.next_block
                            : BlockID(0);

  // If we branch directly to our selection merge target, we don't need a code
  // path.
  bool true_block_needs_code =
      true_block != merge_block || flush_phi_required(from, true_block);
  bool false_block_needs_code =
      false_block != merge_block || flush_phi_required(from, false_block);

  if (!true_block_needs_code && !false_block_needs_code)
    return;

  // We might have a loop merge here. Only consider selection flattening
  // constructs.
  // Loop hints are handled explicitly elsewhere.
  if (from_block.hint == SPIRBlock::HintFlatten ||
      from_block.hint == SPIRBlock::HintDontFlatten)
...
string CompilerGLSL::emit_continue_block(uint32_t continue_block,
  ...
  while ((ir.block_meta[block->self] & ParsedIR::BLOCK_META_LOOP_HEADER_BIT) ==
         0) {
...
string CompilerGLSL::emit_for_loop_initializers(const SPIRBlock &block) {
  ...
  // We can only declare for loop initializers if all variables are of same
  // type.
  // If we cannot do this, declare individual variables before the loop header.

  // We might have a loop variable candidate which was not assigned to for some
  // reason.
  ...
  for (auto &variable : block.loop_variables) {
    ...
    // Sometimes loop variables are initialized with OpUndef, but we can just
    // declare
  ...
  if (block.loop_variables.size() == 1 && missing_initializers == 0) {
    return variable_decl(get<SPIRVariable>(block.loop_variables.front()));
             ...
             missing_initializers == uint32_t(block.loop_variables.size())) {
    for (auto &loop_var : block.loop_variables)
      statement(variable_decl(get<SPIRVariable>(loop_var)), ";");
    return "";
  } else {
    // We have a mix of loop variables, either ones with a clear initializer, or
    // ones without.
    ...
    for (auto &loop_var : block.loop_variables) {
      ...
      if (static_expr == 0 || ir.ids[static_expr].get_type() == TypeUndef) {
        ...
        if (expr.empty()) {
          // For loop initializers are of the form <type id = value, id = value,
          // id = value, etc ...
        ...
        expr += join(to_name(loop_var), " = ",
                     to_pointer_expression(var.static_expression));
      }
    }
    return expr;
  }
...
bool CompilerGLSL::for_loop_initializers_are_same_type(const SPIRBlock &block) {
  ...
  for (auto &var : block.loop_variables) {
    // Don't care about uninitialized variables as they will not be part of the
    // initializers.
...
bool CompilerGLSL::attempt_emit_loop_header(SPIRBlock &block,
                                            SPIRBlock::Method method) {
  SPIRBlock::ContinueBlockType continue_type =
      continue_block_type(get<SPIRBlock>(block.continue_block));

  if (method == SPIRBlock::MergeToSelectForLoop ||
      method == SPIRBlock::MergeToSelectContinueForLoop) {
    uint32_t current_count = statement_count;
    // If we're trying to create a true for loop,
    // we need to make sure that all opcodes before branch statement do not
    // actually emit any code.
    // We can then take the condition expression and create a for (; cond ; ) {
    // body; } structure instead.
    emit_block_instructions(block);

    bool condition_is_temporary =
        forced_temporaries.find(block.condition) == end(forced_temporaries);

    // This can work! We only did trivial things which could be forwarded in
    // block body!
    if (current_count == statement_count && condition_is_temporary) {
      switch (continue_type) {
      case SPIRBlock::ForLoop: {
        // This block may be a dominating block, so make sure we flush
        // undeclared variables before building the for loop header.
        ...
        if (execution_is_noop(get<SPIRBlock>(block.true_block),
                              get<SPIRBlock>(block.merge_block)))
      ...
      case SPIRBlock::WhileLoop: {
        // This block may be a dominating block, so make sure we flush
        // undeclared variables before building the while loop header.
        flush_undeclared_variables(block);
    ...
    // variables before building the for loop header.
    ...
    bool condition_is_temporary =
        forced_temporaries.find(child.condition) == end(forced_temporaries);
    ...
    if (current_count == statement_count && condition_is_temporary) {
      ...
      case SPIRBlock::ForLoop: {
        ...
        // Condition might have to be inverted.
        if (execution_is_noop(get<SPIRBlock>(child.true_block),
                              get<SPIRBlock>(block.merge_block))) {
          condition = join("!", enclose_expression(condition));
...
void CompilerGLSL::emit_hoisted_temporaries(
    SmallVector<pair<TypeID, ID>> &temporaries) {
  // If we need to force temporaries for certain IDs due to continue blocks, do
  // it before starting loop header.
  // Need to sort these to ensure that reference output is stable.
  sort(begin(temporaries), end(temporaries),
       [](const pair<TypeID, ID> &a, const pair<TypeID, ID> &b) {
  ...
  for (auto &tmp : temporaries) {
    ...
    statement(flags_to_qualifiers_glsl(type, flags),
              variable_decl(type, to_name(tmp.second)), initializer, ";");

    hoisted_temporaries.insert(tmp.second);
    forced_temporaries.insert(tmp.second);

    // The temporary might be read from before it's assigned, set up the
    // expression now.
...
void CompilerGLSL::emit_block_chain(SPIRBlock &block) {
  ...
  if (block.continue_block) {
    ...
    // If we know we cannot emit a loop, mark the block early as a complex loop
    // so we don't force unnecessary recompiles.
  ...
  SmallVector<bool, 64> rearm_dominated_variables(
      block.dominated_variables.size());
  ...
  // However, don't attempt this on ESSL 1.0, because if a loop variable is used
  // in a continue block,
  // it *MUST* be used in the continue block. This loop method will not work.
  if (!is_legacy_es() &&
      block_is_loop_candidate(block, SPIRBlock::MergeToSelectContinueForLoop)) {
    flush_undeclared_variables(block);
    if (attempt_emit_loop_header(block,
  ...
  // This is the older loop behavior in glslang which branches to loop body
  // directly from the loop header.
    ...
    if (attempt_emit_loop_header(block, SPIRBlock::MergeToSelectForLoop)) {
      // The body of while, is actually just the true (or false) block, so
      // always branch there unconditionally.
      if (execution_is_noop(get<SPIRBlock>(block.true_block),
                            get<SPIRBlock>(block.merge_block)))
        select_branch_to_false_block = true;
      else
  ...
  // This is the newer loop behavior in glslang which branches from Loop header
  // directly to
  // a new block, which in turn has a OpBranchSelection without a selection
  // merge.
    ...
    // We have a generic loop without any distinguishable pattern like for,
    // while or do while.
  ...
  // If we didn't successfully emit a loop header and we had loop variable
  // candidates, we have a problem
  // as writes to said loop variables might have been masked out, we need a
  // recompile.
  ...
  case SPIRBlock::Direct:
    ...
    // True if MergeToDirectForLoop succeeded.
      ...
      emit_next_block = false;
    else if (is_continue(block.next_block) || is_break(block.next_block) ||
  ...
  case SPIRBlock::Select:
    ...
    if (select_branch_to_true_block) {
      if (force_complex_continue_block) {
        ...
        // We're going to emit a continue block directly here, so make sure it's
        // marked as complex.
        auto &complex_continue =
            get<SPIRBlock>(block.continue_block).complex_continue;
  ...
  case SPIRBlock::MultiSelect: {
    ...
    bool unsigned_case = type.basetype == SPIRType::UInt ||
                         type.basetype == SPIRType::UShort ||
                         type.basetype == SPIRType::UByte;
    ...
    if (type.basetype == SPIRType::UInt64 || type.basetype == SPIRType::Int64) {
      // SPIR-V spec suggests this is allowed, but we cannot support it in
    ...
    // If a switch case branches to the default block for some reason, we can
    // just remove that literal from consideration
    // and let the default: block handle it.
    // 2.11 in SPIR-V spec states that for fall-through cases, there is a very
    // strict declaration order which we can take advantage of here.
    // We only need to consider possible fallthrough if order[i] branches to
    // order[i + 1].
    ...
    for (auto &c : cases) {
      ...
      } else if (c.block == block.next_block &&
                 block.default_block != block.next_block) {
    ...
    if (block.default_block != block.next_block) {
      ...
      // if there are fall-through scenarios since the default is declared
      // separately in OpSwitch.
      ...
      for (size_t i = 0; i < num_blocks; i++) {
        ...
        if (execution_is_direct_branch(case_block, default_block)) {
          // Fallthrough to default block, we must inject the default block
          // here.
          block_declaration_order.insert(begin(block_declaration_order) + i + 1,
                                         block.default_block);
          injected_block = true;
          break;
        } else if (execution_is_direct_branch(default_block, case_block)) {
          // Default case is falling through to another case label, we must
          // inject the default block here.
          block_declaration_order.insert(begin(block_declaration_order) + i,
                                         block.default_block);
          injected_block = true;
          break;
        }
      ...
      if (!injected_block)
        ...
        SPIRV_CROSS_THROW("Default case label fallthrough to other case label "
                          "is not supported in ESSL 1.0.");
    ...
    const auto to_case_label =
        [](uint32_t literal, bool is_unsigned_case) -> string {
          return is_unsigned_case ? convert_to_string(literal)
                                  : convert_to_string(int32_t(literal));
        };

    const auto to_legacy_case_label =
        [&](uint32_t condition, const SmallVector<uint32_t> &labels,
          ...
          for (size_t i = 0; i < count; i++) {
            if (i)
              ret += " || ";
            ret += join(count > 1 ? "(" : "", to_enclosed_expression(condition),
                        " == ", labels[i], suffix, count > 1 ? ")" : "");
          }
          return ret;
        };

    // We need to deal with a complex scenario for OpPhi. If we have
    // case-fallthrough and Phi in the picture,
    // we need to flush phi nodes outside the switch block in a branch,
    // and skip any Phi handling inside the case label to make fall-through work
    // as expected.
    // This kind of code-gen is super awkward and it's a last resort. Normally
    // we would want to handle this
    // inside the case label if at all possible.
    for (size_t i = 1; backend.support_case_fallthrough && i < num_blocks;
         i++) {
      if (flush_phi_required(block.self, block_declaration_order[i]) &&
          flush_phi_required(block_declaration_order[i - 1],
                             block_declaration_order[i])) {
        uint32_t target_block = block_declaration_order[i];

        // Make sure we flush Phi, it might have been marked to be ignored
        // earlier.
        ...
        if (literals.empty()) {
          ...
          // Find all possible literals that would *not* make us enter the
          // default block.
          // If none of those literals match, we flush Phi ...
          SmallVector<string> conditions;
          for (size_t j = 0; j < num_blocks; j++) {
            auto &negative_literals =
                case_constructs[block_declaration_order[j]];
            for (auto &case_label : negative_literals)
              conditions.push_back(
                  join(to_enclosed_expression(block.condition), " != ",
    ...
    // If there is only one default block, and no cases, this is a case where
    // SPIRV-opt decided to emulate
    // non-structured exits with the help of a switch block.
    // This is buggy on FXC, so just emit the logical equivalent of a do { }
    // while(false), which is more idiomatic.
    bool degenerate_switch =
        block.default_block != block.merge_block && block.cases_32bit.empty();
    ...
    for (size_t i = 0; i < num_blocks; i++) {
      ...
      if (literals.empty()) {
        ...
        if (is_legacy_es()) {
          ...
          for (auto &case_literal : literals) {
            // The case label value must be sign-extended properly in SPIR-V, so
            // we can assume 32-bit values here.
            statement("case ", to_case_label(case_literal, unsigned_case),
                      label_suffix, ":");
          }
        }
      }
      ...
      if (backend.support_case_fallthrough && i + 1 < num_blocks &&
          execution_is_direct_branch(
              case_block, get<SPIRBlock>(block_declaration_order[i + 1]))) {
        // We will fall through here, so just terminate the block chain early.
        // We still need to deal with Phi potentially.
        // No need for a stack-like thing here since we only do fall-through
        // when there is a
    ...
    // Might still have to flush phi variables if we branch from loop header
    // directly to merge target.
    // This is supposed to emit all cases where we branch from header to merge
    // block directly.
    // There are two main scenarios where cannot rely on default fallthrough.
    // - There is an explicit default: label already.
    //   In this case, literals_to_merge need to form their own "default" case,
    //   so that we avoid executing that block.
    // - Header -> Merge requires flushing PHI. In this case, we need to collect
    // all cases and flush PHI there.
    bool header_merge_requires_phi =
        flush_phi_required(block.self, block.next_block);
    bool need_fallthrough_block =
        block.default_block == block.next_block || !literals_to_merge.empty();
    if ((header_merge_requires_phi && need_fallthrough_block) ||
        !literals_to_merge.empty()) {
  ...
  case SPIRBlock::Return: {
    ...
    if (block.return_value) {
      ...
      if (!type.array.empty() && !backend.can_return_array) {
        // If we cannot return arrays, we will have a special out argument we
        // can write to instead.
        // The backend is responsible for setting this up, and redirection the
        // return values as appropriate.
        if (ir.ids[block.return_value].get_type() != TypeUndef) {
          emit_array_copy(
              "spvReturnValue", 0, block.return_value, StorageClassFunction,
              get_expression_effective_storage_class(block.return_value));
        }

        if (!cfg.node_terminates_control_flow_in_sub_graph(
                current_function->entry_block, block.self) ||
    ...
    } else if (!cfg.node_terminates_control_flow_in_sub_graph(
                   current_function->entry_block, block.self) ||
               block.loop_dominator != BlockID(SPIRBlock::NoDominator)) {
      // If this block is the very final block and not called from control flow,
      // we do not need an explicit return which looks out of place. Just end
      // the function here.
      // In the very weird case of for(;;) { return; } executing return is
      // unconditional,
  ...
  // If the Kill is terminating a block with a (probably synthetic) return
  // value, emit a return value statement.
  case SPIRBlock::Kill:
  ...
  if (block.next_block && emit_next_block) {
    // If we hit this case, we're dealing with an unconditional branch, which
    // means we will output
    // that block after this. If we had selection merge, we already flushed phi
    // variables.
    if (block.merge != SPIRBlock::MergeSelection) {
      flush_phi(block.self, block.next_block);
      // For a direct branch, need to remember to invalidate expressions in the
      // next linear block instead.
      get<SPIRBlock>(block.next_block).invalidate_expressions =
          block.invalidate_expressions;
    }

    // For switch fallthrough cases, we terminate the chain here, but we still
    // need to handle Phi.
    if (!current_emitting_switch_fallthrough) {
      ...
      if (is_loop_break(block.next_block)) {
        // Cannot check for just break, because switch statements will also use
        // break.
  ...
  if (block.merge == SPIRBlock::MergeLoop) {
    if (continue_type == SPIRBlock::DoWhileLoop) {
      // Make sure that we run the continue block to get the expressions set,
      // but this
      ...
      uint32_t current_count = statement_count;
      auto statements = emit_continue_block(block.continue_block, positive_test,
                                            !positive_test);
      if (statement_count != current_count) {
        // The DoWhile block has side effects, force ComplexLoop pattern next
        // pass.
        get<SPIRBlock>(block.continue_block).complex_continue = true;
        force_recompile();
      }
    ...
    // Using block.self as the "from" block isn't quite right, but it has the
    // same scope
  ...
  // After we return, we must be out of scope, so if we somehow have to re-emit
  // this function,
  ...
  // Just like for deferred declaration, we need to forget about loop variable
  // enable
...
void CompilerGLSL::check_function_call_constraints(const uint32_t *args,
                                                   uint32_t length) {
  ...
  // Fixing this is non-trivial without stamping out variants of the same
  // function,
  ...
  for (uint32_t i = 0; i < length; i++) {
    ...
    if (type.basetype == SPIRType::Image && type.image.dim == DimSubpassData) {
      SPIRV_CROSS_THROW(
          "Tried passing a remapped subpassInput variable to a function. "
          "This will not work correctly because type-remapping information is "
          "lost. "
          "To workaround, please consider not passing the subpass input as a "
          "function parameter, "
          "or use in/out variables instead which do not need type remapping "
          "information.");
    }
  }
}
...
const Instruction *
CompilerGLSL::get_next_instruction_in_block(const Instruction &instr) {
...
uint32_t CompilerGLSL::mask_relevant_memory_semantics(uint32_t semantics) {
  return semantics &
         (MemorySemanticsAtomicCounterMemoryMask |
          MemorySemanticsImageMemoryMask | MemorySemanticsWorkgroupMemoryMask |
          MemorySemanticsUniformMemoryMask |
          MemorySemanticsCrossWorkgroupMemoryMask |
          MemorySemanticsSubgroupMemoryMask);
}

void CompilerGLSL::emit_array_copy(const string &lhs, uint32_t, uint32_t rhs_id,
                                   StorageClass, StorageClass) {
  statement(lhs, " = ", to_expression(rhs_id), ";");
}

bool CompilerGLSL::unroll_array_to_complex_store(uint32_t target_id,
                                                 uint32_t source_id) {
  ...
  statement(to_expression(target_id), "[i] = ",
            bitcast_expression(target_type, type.basetype,
                               join(to_expression(source_id), "[i]")),
            ";");
  end_scope();
...
void CompilerGLSL::unroll_array_from_complex_load(uint32_t target_id,
                                                  uint32_t source_id,
                                                  std::string &expr) {
  if (!backend.force_gl_in_out_block)
    return;
  ...
  auto builtin = BuiltIn(get_decoration(var->self, DecorationBuiltIn));
  bool is_builtin = is_builtin_variable(*var) && (builtin == BuiltInPointSize ||
  ...
  // Tessellation input arrays are special in that they are unsized, so we
  // cannot directly copy from it.
  ...
  if (!is_patch && (is_builtin || is_tess)) {
    ...
    // The array size might be a specialization constant, so use a for-loop
    // instead.
    statement("for (int i = 0; i < int(", array_expr, "); i++)");
    begin_scope();
    if (is_builtin && !is_sample_mask)
      ...
      statement(
          new_expr, "[i] = ",
          bitcast_expression(target_type, type.basetype, join(expr, "[i]")),
          ";");
    } else
...
void CompilerGLSL::cast_from_variable_load(uint32_t source_id,
                                           std::string &expr,
...
void CompilerGLSL::cast_to_variable_store(uint32_t target_id, std::string &expr,
                                          const SPIRType &expr_type) {
...
void CompilerGLSL::convert_non_uniform_expression(string &expr,
                                                  uint32_t ptr_id) {
  ...
  // Doesn't really make sense to declare a non-arrayed image with
  // nonuniformEXT, but there's
  // nothing we can do here to express that.
  if (start_array_index == string::npos || end_array_index == string::npos ||
      end_array_index < start_array_index)
    return;

  start_array_index++;

  expr =
      join(expr.substr(0, start_array_index), backend.nonuniform_qualifier, "(",
...
void CompilerGLSL::fixup_type_alias() {
  // Due to how some backends work, the "master" type of type_alias must be a
  // block-like type if it exists.
  ir.for_each_typed_id<SPIRType>([&](uint32_t self, SPIRType &type) {
    if (!type.type_alias)
      return;
    ...
    if (has_decoration(type.self, DecorationBlock) ||
      ...
      // A block-like type is any type which contains Offset decoration, but not
      // top-level blocks,
...
void CompilerGLSL::reorder_type_alias() {
  // Reorder declaration of types so that the master of the type alias is always
  // emitted first.
  // We need this in case a type B depends on type A (A must come before in the
  // vector), but A is an alias of a type Abuffer, which
  // means declaration of A doesn't happen (yet), and order would be B, ABuffer
  // and not ABuffer, B. Fix this up here.
  auto loop_lock = ir.create_loop_hard_lock();
  ...
  for (auto alias_itr = begin(type_ids); alias_itr != end(type_ids);
    ...
    if (type.type_alias != TypeID(0) &&
        !has_extended_decoration(type.type_alias,
                                 SPIRVCrossDecorationBufferBlockRepacked)) {
      // We will skip declaring this type, so make sure the type_alias type
      // comes before.
      auto master_itr =
          find(begin(type_ids), end(type_ids), ID(type.type_alias));
      ...
      if (alias_itr < master_itr) {
        ...
        auto alt_alias_itr =
            find(begin(joined_types), end(joined_types), *alias_itr);
        auto alt_master_itr =
            find(begin(joined_types), end(joined_types), *master_itr);
...
void CompilerGLSL::emit_line_directive(uint32_t file_id,
  ...
  if (options.emit_line_directives) {
    ...
    statement_no_indent("#line ", line_literal, " \"",
                        get<SPIRString>(file_id).str, "\"");
  }
}
...
void CompilerGLSL::emit_copy_logical_type(uint32_t lhs_id, uint32_t lhs_type_id,
  ...
  if (!lhs_type.array.empty()) {
    // Could use a loop here to support specialization constants, but it gets
    // rather complicated with nested array types,
    // and this is a rather obscure opcode anyways, keep it simple unless we are
    // forced to.
    ...
    for (uint32_t i = 0; i < array_size; i++) {
      chain.back() = i;
      emit_copy_logical_type(lhs_id, lhs_type.parent_type, rhs_id,
                             rhs_type.parent_type, chain);
    }
    ...
    for (uint32_t i = 0; i < member_count; i++) {
      chain.back() = i;
      emit_copy_logical_type(lhs_id, lhs_type.member_types[i], rhs_id,
                             rhs_type.member_types[i], chain);
    }
  } else {
    // Need to handle unpack/packing fixups since this can differ wildly between
    // the logical types,
    // particularly in MSL.
    // To deal with this, we emit access chains and go through
    // emit_store_statement
    // to deal with all the special cases we can encounter.

    AccessChainMeta lhs_meta, rhs_meta;
    auto lhs =
        access_chain_internal(lhs_id, chain.data(), uint32_t(chain.size()),
                              ACCESS_CHAIN_INDEX_IS_LITERAL_BIT, &lhs_meta);
    auto rhs =
        access_chain_internal(rhs_id, chain.data(), uint32_t(chain.size()),
    ...
    {
      auto &lhs_expr =
          set<SPIRExpression>(lhs_id, move(lhs), lhs_type_id, true);
      ...
      if (lhs_meta.storage_physical_type != 0)
        set_extended_decoration(lhs_id, SPIRVCrossDecorationPhysicalTypeID,
    ...
    {
      auto &rhs_expr =
          set<SPIRExpression>(rhs_id, move(rhs), rhs_type_id, true);
      ...
      if (rhs_meta.storage_physical_type != 0)
        set_extended_decoration(rhs_id, SPIRVCrossDecorationPhysicalTypeID,
...
bool CompilerGLSL::subpass_input_is_framebuffer_fetch(uint32_t id) const {
  ...
  uint32_t input_attachment_index =
      get_decoration(id, DecorationInputAttachmentIndex);
...
const SPIRVariable *
CompilerGLSL::find_subpass_input_by_attachment_index(uint32_t index) const {
...
const SPIRVariable *
CompilerGLSL::find_color_output_by_location(uint32_t location) const {
  const SPIRVariable *ret = nullptr;
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable &var) {
    if (var.storage == StorageClassOutput &&
        get_decoration(var.self, DecorationLocation) == location)
      ret = &var;
  });
  return ret;
}
...
void CompilerGLSL::emit_inout_fragment_outputs_copy_to_subpass_inputs() {
  for (auto &remap : subpass_to_framebuffer_fetch_attachment) {
    ...
    if (!subpass_var)
      continue;
    if (!output_var)
      SPIRV_CROSS_THROW("Need to declare the corresponding fragment output "
                        "variable to be able "
                        "to read from it.");
    if (is_array(get<SPIRType>(output_var->basetype)))
      SPIRV_CROSS_THROW("Cannot use GL_EXT_shader_framebuffer_fetch with "
                        "arrays of color outputs.");
    ...
    func.fixup_hooks_in.push_back([=]() {
      if (is_legacy()) {
        ...
        uint32_t num_rt_components =
            this->get<SPIRType>(output_var->basetype).vecsize;
        statement(to_expression(subpass_var->self),
                  vector_swizzle(num_rt_components, 0), " = ",
...
const char *
CompilerGLSL::ShaderSubgroupSupportHelper::get_extension_name(Candidate c) {
  static const char *const retval[CandidateCount] = {
      "GL_KHR_shader_subgroup_ballot", "GL_KHR_shader_subgroup_basic",
      "GL_KHR_shader_subgroup_vote", "GL_NV_gpu_shader_5",
      "GL_NV_shader_thread_group", "GL_NV_shader_thread_shuffle",
      "GL_ARB_shader_ballot", "GL_ARB_shader_group_vote", "GL_AMD_gcn_shader"};
  return retval[c];
}

SmallVector<std::string>
CompilerGLSL::ShaderSubgroupSupportHelper::get_extra_required_extension_names(
    Candidate c) {
  switch (c) {
...
const char *CompilerGLSL::ShaderSubgroupSupportHelper::
...
CompilerGLSL::ShaderSubgroupSupportHelper::get_feature_dependencies(
    Feature feature) {
  switch (feature) {
...
CompilerGLSL::ShaderSubgroupSupportHelper::get_feature_dependency_mask(
    Feature feature) {
  return build_mask(get_feature_dependencies(feature));
}

bool CompilerGLSL::ShaderSubgroupSupportHelper::
...
CompilerGLSL::ShaderSubgroupSupportHelper::get_KHR_extension_for_feature(
    Feature feature) {
  static const Candidate extensions[FeatureCount] = {
      KHR_shader_subgroup_ballot, KHR_shader_subgroup_basic,
      KHR_shader_subgroup_basic,  KHR_shader_subgroup_basic,
      KHR_shader_subgroup_basic,  KHR_shader_subgroup_ballot,
      KHR_shader_subgroup_ballot, KHR_shader_subgroup_vote,
      KHR_shader_subgroup_vote,   KHR_shader_subgroup_basic,
      KHR_shader_subgroup_basic,  KHR_shader_subgroup_basic,
      KHR_shader_subgroup_ballot, KHR_shader_subgroup_ballot,
      KHR_shader_subgroup_ballot, KHR_shader_subgroup_ballot};

  return extensions[feature];
}

void CompilerGLSL::ShaderSubgroupSupportHelper::request_feature(
    Feature feature) {
  feature_mask |=
      (FeatureMask(1) << feature) | get_feature_dependency_mask(feature);
}

bool CompilerGLSL::ShaderSubgroupSupportHelper::is_feature_requested(
...
CompilerGLSL::ShaderSubgroupSupportHelper::get_candidates_for_feature(
    Feature ft, const Result &r) {
...
CompilerGLSL::ShaderSubgroupSupportHelper::get_candidates_for_feature(
  ...
  case SubgroupMask:
    return {KHR_shader_subgroup_ballot, NV_shader_thread_group,
            ARB_shader_ballot};
  case SubgroupSize:
    return {KHR_shader_subgroup_basic, NV_shader_thread_group, AMD_gcn_shader,
            ARB_shader_ballot};
  ...
  case SubgroupBroadcast_First:
    return {KHR_shader_subgroup_ballot, NV_shader_thread_shuffle,
            ARB_shader_ballot};
  case SubgroupBallotFindLSB_MSB:
    return {KHR_shader_subgroup_ballot, NV_shader_thread_group};
  case SubgroupAll_Any_AllEqualBool:
    return {KHR_shader_subgroup_vote, NV_gpu_shader_5, ARB_shader_group_vote,
            AMD_gcn_shader};
  case SubgroupAllEqualT:
  ...
  case SubgroupBarrier:
    return {KHR_shader_subgroup_basic, NV_shader_thread_group,
            ARB_shader_ballot, AMD_gcn_shader};
...
CompilerGLSL::ShaderSubgroupSupportHelper::FeatureMask
CompilerGLSL::ShaderSubgroupSupportHelper::build_mask(
...
void CompilerGLSL::request_workaround_wrapper_overload(TypeID id) {
  ...
  if (find(begin(workaround_ubo_load_overload_types),
           end(workaround_ubo_load_overload_types),
           id) == end(workaround_ubo_load_overload_types)) {
    force_recompile();
    workaround_ubo_load_overload_types.push_back(id);
  }
}

void CompilerGLSL::rewrite_load_for_wrapped_row_major(std::string &expr,
                                                      TypeID loaded_type,
                                                      ID ptr) {
  // Loading row-major matrices from UBOs on older AMD Windows OpenGL drivers is
  // problematic.
  // To load these types correctly, we must first wrap them in a dummy function
  ...
  if (is_matrix(*type)) {
    // To avoid adding a lot of unnecessary meta tracking to forward the
    // row_major state,
    // we will simply look at the base struct itself. It is exceptionally rare
    // to mix and match row-major/col-major state.
    // If there is any row-major action going on, we apply the workaround.
    // It is harmless to apply the workaround to column-major matrices, so this
    // is still a valid solution.
    // If an access chain occurred, the workaround is not required, so loading
    // vectors or scalars don't need workaround.
    type = &backing_type;
  }

  if (type->basetype == SPIRType::Struct) {
    // If we're loading a struct where any member is a row-major matrix, apply
    // the workaround.
    for (uint32_t i = 0; i < uint32_t(type->member_types.size()); i++) {
...
void CompilerGLSL::mask_stage_output_by_location(uint32_t location,
                                                 uint32_t component) {
...
bool CompilerGLSL::is_stage_output_variable_masked(
    const SPIRVariable &var) const {
  auto &type = get<SPIRType>(var.basetype);
  bool is_block = has_decoration(type.self, DecorationBlock);
  ...
  if (is_builtin) {
    return is_stage_output_builtin_masked(
        BuiltIn(get_decoration(var.self, DecorationBuiltIn)));
  } else {
...
bool CompilerGLSL::is_stage_output_block_member_masked(const SPIRVariable &var,
                                                       uint32_t index,
...
bool CompilerGLSL::is_stage_output_location_masked(uint32_t location,
...
uint32_t CompilerGLSL::get_declared_member_location(const SPIRVariable &var,
                                                    uint32_t mbr_idx,
                                                    bool strip_array) const {
...
uint32_t CompilerGLSL::get_accumulated_member_location(const SPIRVariable &var,
                                                       uint32_t mbr_idx,
                                                       bool strip_array) const {
  auto &type = strip_array ? get_variable_element_type(var)
...
StorageClass
CompilerGLSL::get_expression_effective_storage_class(uint32_t ptr) {
  auto *var = maybe_get_backing_variable(ptr);

  // If the expression has been lowered to a temporary, we need to use the
  ...
  // will generally have the storage class of the underlying variable, but if
  // the load was not forwarded
  ...
  bool forced_temporary = ir.ids[ptr].get_type() == TypeExpression &&
                          ...
                          (forced_temporaries.count(ptr) != 0 ||
                           forwarded_temporaries.count(ptr) == 0);
  ...
  if (var && !forced_temporary) {
    ...
    if (var->storage == StorageClassUniform &&
        has_decoration(get<SPIRType>(var->basetype).self,
                       DecorationBufferBlock))
      return StorageClassStorageBuffer;
    else
      return var->storage;
