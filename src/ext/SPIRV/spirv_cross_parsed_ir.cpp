...
ParsedIR::ParsedIR() {
  // If we move ParsedIR, we need to make sure the pointer stays fixed since the
  // child Variant objects consume a pointer to this group,
  ...
  pool_group->pools[TypeFunctionPrototype].reset(
      new ObjectPool<SPIRFunctionPrototype>);
  ...
  pool_group->pools[TypeCombinedImageSampler].reset(
      new ObjectPool<SPIRCombinedImageSampler>);
...
ParsedIR::ParsedIR(const ParsedIR &other) : ParsedIR() { *this = other; }
...
ParsedIR &ParsedIR::operator=(const ParsedIR &other) {
  if (this != &other) {
    ...
    // Very deliberate copying of IDs. There is no default copy constructor, nor
    // a simple default constructor.
    // Construct object first so we have the correct allocator set-up, then we
    // can copy object into our new pool group.
    ids.clear();
...
// Roll our own versions of these functions to avoid potential locale
// shenanigans.
...
static bool is_reserved_prefix(const string &name) {
  ...
  return name.compare(0, 3, "gl_", 3) == 0 ||
         // Ignore this case for now, might rewrite internal code to always use
         // spv prefix.
         // name.compare(0, 11, "SPIRV_Cross", 11) == 0 ||
         name.compare(0, 3, "spv", 3) == 0;
}

static bool is_reserved_identifier(const string &name, bool member,
...
bool ParsedIR::is_globally_reserved_identifier(std::string &str,
                                               bool allow_reserved_prefixes) {
  return is_reserved_identifier(str, false, allow_reserved_prefixes);
}

uint32_t ParsedIR::get_spirv_version() const { return spirv[1]; }
...
static string ensure_valid_identifier(const string &name) {
  ...
  // Normally, we would never see '(' in any legal identifiers, so just strip
  // them out.
...
void ParsedIR::sanitize_identifier(std::string &name, bool member,
                                   bool allow_reserved_prefixes) {
...
void ParsedIR::set_decoration_string(ID id, Decoration decoration,
...
void ParsedIR::set_member_decoration(TypeID id, uint32_t index,
...
// Recursively marks any constants referenced by the specified constant
// instruction as being used
// as an array length. The id must be a constant instruction (SPIRConstant or
// SPIRConstantOp).
...
const Bitset &ParsedIR::get_member_decoration_bitset(TypeID id,
...
const string &ParsedIR::get_decoration_string(ID id,
...
bool ParsedIR::has_member_decoration(TypeID id, uint32_t index,
                                     Decoration decoration) const {
  return get_member_decoration_bitset(id, index).get(decoration);
}

uint32_t ParsedIR::get_member_decoration(TypeID id, uint32_t index,
                                         Decoration decoration) const {
  auto *m = find_meta(id);
  if (!m)
    return 0;
...
void ParsedIR::set_member_decoration_string(TypeID id, uint32_t index,
...
const string &
ParsedIR::get_member_decoration_string(TypeID id, uint32_t index,
                                       Decoration decoration) const {
  auto *m = find_meta(id);
  if (m) {
...
void ParsedIR::unset_member_decoration(TypeID id, uint32_t index,
                                       Decoration decoration) {
  auto &m = meta[id];
  if (index >= m.members.size())
    return;
...
ParsedIR::LoopLock &ParsedIR::LoopLock::
operator=(LoopLock &&other) SPIRV_CROSS_NOEXCEPT {
  if (lock)
    (*lock)--;
  lock = other.lock;
  other.lock = nullptr;
...
void ParsedIR::make_constant_null(uint32_t id, uint32_t type,
  ...
  if (constant_type.pointer) {
    ...
    if (add_to_typed_id_set)
      add_typed_id(TypeConstant, id);
    variant_set<SPIRConstant>(ids[id], type, elements.data(),
                              uint32_t(elements.size()), false)
        .self = id;
  } else if (!constant_type.member_types.empty()) {
    uint32_t member_ids =
        increase_bound_by(uint32_t(constant_type.member_types.size()));
