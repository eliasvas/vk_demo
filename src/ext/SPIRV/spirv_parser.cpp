...
Parser::Parser(vector<uint32_t> spirv) { ir.spirv = move(spirv); }
...
void Parser::parse() {
  ...
  if (s[0] == swap_endian(MagicNumber))
    transform(begin(spirv), end(spirv), begin(spirv),
              [](uint32_t c) { return swap_endian(c); });
...
void Parser::parse(const Instruction &instruction) {
  ...
  case OpExtInstImport: {
    ...
    if (ext == "GLSL.std.450")
      ...
      set<SPIRExtension>(
          id, SPIRExtension::SPV_AMD_shader_explicit_vertex_parameter);
  ...
  case OpEntryPoint: {
    auto itr = ir.entry_points.insert(make_pair(
        ops[1],
        SPIREntryPoint(ops[1], static_cast<ExecutionModel>(ops[0]),
  ...
  case OpDecorationGroup: {
    ...
    // The meta array is already a flat array of decorations which will contain
    // the relevant decorations.
    break;
  }
    ...
    for (uint32_t i = 1; i < length; i++) {
      ...
      flags.for_each_bit([&](uint32_t bit) {
        ...
        if (decoration_is_string(decoration)) {
          ...
          ir.set_decoration(target, decoration,
                            ir.get_decoration(group_id, decoration));
        }
      });
    }
    break;
  ...
  case OpGroupMemberDecorate: {
    ...
    for (uint32_t i = 1; i + 1 < length; i += 2) {
      ...
      flags.for_each_bit([&](uint32_t bit) {
        ...
        if (decoration_is_string(decoration))
          ir.set_member_decoration_string(
              target, index, decoration,
              ir.get_decoration_string(group_id, decoration));
        else
          ir.set_member_decoration(target, index, decoration,
                                   ir.get_decoration(group_id, decoration));
      });
    }
    break;
  ...
  case OpDecorateStringGOOGLE: {
    uint32_t id = ops[0];
    auto decoration = static_cast<Decoration>(ops[1]);
    ir.set_decoration_string(id, decoration,
                             extract_string(ir.spirv, instruction.offset + 2));
    break;
  }
  ...
  case OpMemberDecorateStringGOOGLE: {
    ...
    ir.set_member_decoration_string(
        ...
        extract_string(ir.spirv, instruction.offset + 3));
    break;
  }
  ...
  case OpTypeInt: {
    ...
    auto &type = set<SPIRType>(id);
    type.basetype =
        signedness ? to_signed_basetype(width) : to_unsigned_basetype(width);
    type.width = width;
    break;
  }

  // Build composite types by "inheriting".
  // NOTE: The self member is also copied! For pointers and array modifiers this
  // is a good thing
  // since we can refer to decorations on pointee classes which is needed for
  // UBO/SSBO, I/O blocks in geometry/tess etc.
  case OpTypeVector: {
    uint32_t id = ops[0];
  ...
  case OpTypeArray: {
    ...
    // We're copying type information into Array types, so we'll need a fixup
    // for any physical pointer
    // references.
  ...
  case OpTypeImage: {
    ...
    type.image.access = (length >= 9) ? static_cast<AccessQualifier>(ops[8])
                                      : AccessQualifierMax;
    break;
  }
  ...
  case OpTypeStruct: {
    ...
    // We should not normally have to consider type aliases like this to begin
    // with
    // however ... glslang issues #304, #307 cover this.

    // For stripped names, never consider struct type aliasing.
    // We risk declaring the same struct multiple times, but type-punning is not
    // allowed
    // so this is safe.
  ...
  // In high-level languages we can "de-SSA" by creating a function local, and
  // flush out temporaries to this function-local
  ...
  case OpPhi: {
    ...
    // Instead of a temporary, create a new function-wide temporary with this ID
    // instead.
  ...
  case OpConstant: {
    ...
    if (type.width > 32)
      set<SPIRConstant>(id, ops[0], ops[2] | (uint64_t(ops[3]) << 32),
                        op == OpSpecConstant);
    else
  ...
  case OpConstantComposite: {
    ...
    // In this case, our SPIRConstant will be a list of other SPIRConstant ids
    // which we
    // can refer to.
    if (ctype.basetype == SPIRType::Struct || !ctype.array.empty()) {
      set<SPIRConstant>(id, type, ops + 2, length - 2,
      ...
      for (uint32_t i = 0; i < elements; i++) {
        ...
        // We do not know their value, so any attempt to query SPIRConstant
        // later
        // will fail. We can only propagate the ID of the expression and use
        // to_expression on it.
        auto *constant_op = maybe_get<SPIRConstantOp>(ops[2 + i]);
        auto *undef_op = maybe_get<SPIRUndef>(ops[2 + i]);
        if (constant_op) {
          if (op == OpConstantComposite)
            SPIRV_CROSS_THROW("Specialization constant operation used in "
                              "OpConstantComposite.");

          remapped_constant_ops[i].make_null(
              get<SPIRType>(constant_op->basetype));
  ...
  case OpFunctionEnd: {
    if (current_block) {
      ...
      SPIRV_CROSS_THROW(
          ...
          "Likely cause: If this SPIR-V was created from glslang HLSL, make "
          "sure the entry point is valid.");
    }
  ...
  case OpBranchConditional: {
    ...
    if (current_block->true_block == current_block->false_block) {
      ...
      // There are some super jank cases where the merge block is different from
      // the true/false,
      // and later branches can "break" out of the selection construct this way.
      // This is complete nonsense, but CTS hits this case.
      // In this scenario, we should see the selection construct as more of a
      // Switch with one default case.
      // The problem here is that this breaks any attempt to break out of outer
      // switch statements,
      // but it's theoretically solvable if this ever comes up using the ladder
      // breaking system ...
      ...
      if (current_block->true_block != current_block->next_block &&
        ...
        ir.block_meta[current_block->next_block] &=
  ...
  case OpLoopMerge: {
    ...
    ir.block_meta[current_block->merge_block] |=
        ParsedIR::BLOCK_META_LOOP_MERGE_BIT;

    ir.continue_block_to_loop_header[current_block->continue_block] =
        BlockID(current_block->self);

    // Don't add loop headers to continue blocks,
    // which would make it impossible branch into the loop header since
    // they are treated as continues.
    if (current_block->continue_block != BlockID(current_block->self))
      ir.block_meta[current_block->continue_block] |=
  ...
  case OpLine: {
    ...
    // they will not be declared in any
...
bool Parser::types_are_logically_equivalent(const SPIRType &a,
...
bool Parser::variable_storage_is_aliased(const SPIRVariable &v) const {
  ...
  bool ssbo =
      ...
      (type_meta &&
       type_meta->decoration.decoration_flags.get(DecorationBufferBlock));
