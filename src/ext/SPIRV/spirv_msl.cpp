 ...
 *  1. The Apache License, Version 2.0, found at
...
static const char *force_inline =
...
void CompilerMSL::add_msl_resource_binding(const MSLResourceBinding &binding) {
  ...
  if (msl_options.pad_argument_buffer_resources) {
    StageSetBinding arg_idx_tuple = {binding.stage, binding.desc_set,
                                     k_unknown_component};
...
#define ADD_ARG_IDX_TO_BINDING_NUM_LOOKUP(rez)                                 \
  ...
  resource_arg_buff_idx_to_binding_number[arg_idx_tuple] = binding.binding
    ...
    case SPIRType::SampledImage:
      ...
      break;
    default:
      SPIRV_CROSS_THROW("Unexpected argument buffer resource base type. When "
                        "padding argument buffer elements, "
                        "all descriptor set resources must be supplied with a "
                        "base type by the app.");
    }
#undef ADD_ARG_IDX_TO_BINDING_NUM_LOOKUP
  }
}

void CompilerMSL::add_dynamic_buffer(uint32_t desc_set, uint32_t binding,
                                     uint32_t index) {
  SetBindingPair pair = {desc_set, binding};
  buffers_requiring_dynamic_offset[pair] = {index, 0};
}

void CompilerMSL::add_inline_uniform_block(uint32_t desc_set,
                                           uint32_t binding) {
  SetBindingPair pair = {desc_set, binding};
...
void CompilerMSL::set_argument_buffer_device_address_space(
    uint32_t desc_set, bool device_storage) {
  if (desc_set < kMaxArgumentBuffers) {
    if (device_storage)
...
uint32_t
CompilerMSL::get_automatic_builtin_input_location(spv::BuiltIn builtin) const {
...
bool CompilerMSL::is_msl_resource_binding_used(ExecutionModel model,
                                               uint32_t desc_set,
...
// Returns the size of the array of resources used by the variable with the
// specified id.
// The returned value is retrieved from the resource binding added using
...
uint32_t
CompilerMSL::get_automatic_msl_resource_binding_secondary(uint32_t id) const {
  return get_extended_decoration(id,
                                 SPIRVCrossDecorationResourceIndexSecondary);
}

uint32_t
CompilerMSL::get_automatic_msl_resource_binding_tertiary(uint32_t id) const {
  return get_extended_decoration(id, SPIRVCrossDecorationResourceIndexTertiary);
}

uint32_t
CompilerMSL::get_automatic_msl_resource_binding_quaternary(uint32_t id) const {
  return get_extended_decoration(id,
                                 SPIRVCrossDecorationResourceIndexQuaternary);
}

void CompilerMSL::set_fragment_output_components(uint32_t location,
                                                 uint32_t components) {
...
void CompilerMSL::build_implicit_builtins() {
  ...
  bool need_vertex_params = capture_output_to_buffer &&
                            get_execution_model() == ExecutionModelVertex &&
                            !msl_options.vertex_for_tessellation;
  bool need_tesc_params =
      get_execution_model() == ExecutionModelTessellationControl;
  bool need_subgroup_mask = active_input_builtins.get(BuiltInSubgroupEqMask) ||
                            active_input_builtins.get(BuiltInSubgroupGeMask) ||
                            active_input_builtins.get(BuiltInSubgroupGtMask) ||
                            active_input_builtins.get(BuiltInSubgroupLeMask) ||
  ...
  bool need_multiview =
      get_execution_model() == ExecutionModelVertex &&
      !msl_options.view_index_from_device_index &&
  ...
  bool need_dispatch_base =
      msl_options.dispatch_base &&
      get_execution_model() == ExecutionModelGLCompute &&
      (active_input_builtins.get(BuiltInWorkgroupId) ||
       active_input_builtins.get(BuiltInGlobalInvocationId));
  bool need_grid_params = get_execution_model() == ExecutionModelVertex &&
                          msl_options.vertex_for_tessellation;
  bool need_vertex_base_params =
      need_grid_params && (active_input_builtins.get(BuiltInVertexId) ||
                           active_input_builtins.get(BuiltInVertexIndex) ||
                           active_input_builtins.get(BuiltInBaseVertex) ||
                           active_input_builtins.get(BuiltInInstanceId) ||
                           active_input_builtins.get(BuiltInInstanceIndex) ||
                           active_input_builtins.get(BuiltInBaseInstance));
  bool need_local_invocation_index =
      msl_options.emulate_subgroups &&
      active_input_builtins.get(BuiltInSubgroupId);
  bool need_workgroup_size = msl_options.emulate_subgroups &&
                             active_input_builtins.get(BuiltInNumSubgroups);

  if (need_subpass_input || need_sample_pos || need_subgroup_mask ||
      need_vertex_params || need_tesc_params || need_multiview ||
      need_dispatch_base || need_vertex_base_params || need_grid_params ||
      needs_sample_id || needs_subgroup_invocation_id || needs_subgroup_size ||
      has_additional_fixed_sample_mask() || need_local_invocation_index ||
      need_workgroup_size) {
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
      ...
      if (need_vertex_params) {
        ...
        case BuiltInInstanceIndex:
          ...
          mark_implicit_builtin(StorageClassInput, BuiltInInstanceIndex,
                                var.self);
          has_instance_idx = true;
          break;
        case BuiltInBaseInstance:
          builtin_base_instance_id = var.self;
          mark_implicit_builtin(StorageClassInput, BuiltInBaseInstance,
                                var.self);
      ...
      if (need_tesc_params) {
        ...
        case BuiltInInvocationId:
          ...
          mark_implicit_builtin(StorageClassInput, BuiltInInvocationId,
                                var.self);
          has_invocation_id = true;
          break;
        case BuiltInPrimitiveId:
          builtin_primitive_id_id = var.self;
          mark_implicit_builtin(StorageClassInput, BuiltInPrimitiveId,
                                var.self);
      ...
      if ((need_subgroup_mask || needs_subgroup_invocation_id) &&
          builtin == BuiltInSubgroupLocalInvocationId) {
        builtin_subgroup_invocation_id_id = var.self;
        mark_implicit_builtin(StorageClassInput,
                              BuiltInSubgroupLocalInvocationId, var.self);
        has_subgroup_invocation_id = true;
      }

      if ((need_subgroup_ge_mask || needs_subgroup_size) &&
          builtin == BuiltInSubgroupSize) {
      ...
      if (need_multiview) {
        ...
        case BuiltInInstanceIndex:
          ...
          break;
        case BuiltInBaseInstance:
          // If a non-zero base instance is used, we need to adjust for it when
          // calculating the view index.
          builtin_base_instance_id = var.self;
          mark_implicit_builtin(StorageClassInput, BuiltInBaseInstance,
                                var.self);
          has_base_instance = true;
      ...
      if (need_local_invocation_index &&
          builtin == BuiltInLocalInvocationIndex) {
        builtin_local_invocation_index_id = var.self;
        mark_implicit_builtin(StorageClassInput, BuiltInLocalInvocationIndex,
                              var.self);
      ...
      if (need_workgroup_size && builtin == BuiltInLocalInvocationId) {
        ...
        mark_implicit_builtin(StorageClassInput, BuiltInWorkgroupSize,
                              var.self);
      ...
      if (need_dispatch_base && workgroup_id_type == 0 &&
          (builtin == BuiltInWorkgroupId ||
           builtin == BuiltInGlobalInvocationId))
        workgroup_id_type = var.basetype;
    });

    // Use Metal's native frame-buffer fetch API for subpass inputs.
    if ((!has_frag_coord || (msl_options.multiview && !has_view_idx) ||
         (msl_options.arrayed_subpass_input && !msl_options.multiview &&
          !has_layer)) &&
      ...
      if (!has_layer && msl_options.arrayed_subpass_input &&
          !msl_options.multiview) {
    ...
    if ((need_vertex_params && (!has_vertex_idx || !has_base_vertex ||
                                !has_instance_idx || !has_base_instance)) ||
        (need_multiview &&
         (!has_instance_idx || !has_base_instance || !has_view_idx))) {
      ...
      if (need_multiview) {
        // Multiview shaders are not allowed to write to gl_Layer, ostensibly
        // because
        // it is implicitly written from gl_ViewIndex, but we have to do that
        // explicitly.
        // Note that we can't just abuse gl_ViewIndex for this purpose: it's an
        // input, but
    ...
    if ((need_tesc_params && (msl_options.multi_patch_workgroup ||
                              !has_invocation_id || !has_primitive_id)) ||
        need_grid_params) {
      uint32_t type_ptr_id = ir.increase_bound_by(1);
      ...
      if (msl_options.multi_patch_workgroup || need_grid_params) {
        ...
        mark_implicit_builtin(StorageClassInput, BuiltInGlobalInvocationId,
                              var_id);
      ...
      if (need_grid_params) {
        ...
        set<SPIRVariable>(var_id,
                          build_extended_vector_type(get_uint_type_id(), 3),
                          StorageClassInput);
        set_extended_decoration(var_id,
    ...
    if (!has_subgroup_invocation_id &&
      ...
      set<SPIRVariable>(var_id, type_ptr_id, StorageClassInput);
      set_decoration(var_id, DecorationBuiltIn,
                     BuiltInSubgroupLocalInvocationId);
      builtin_subgroup_invocation_id_id = var_id;
      mark_implicit_builtin(StorageClassInput, BuiltInSubgroupLocalInvocationId,
                            var_id);
    }
    ...
    if (need_dispatch_base || need_vertex_base_params) {
      ...
      if (msl_options.supports_msl_version(1, 2)) {
        ...
        set_decoration(var_id, DecorationBinding,
  ...
  if (needs_swizzle_buffer_def) {
    ...
    set_extended_decoration(var_id, SPIRVCrossDecorationResourceIndexPrimary,
                            msl_options.swizzle_buffer_index);
  ...
  if (!buffers_requiring_array_length.empty()) {
    ...
    set_decoration(var_id, DecorationBinding,
                   msl_options.buffer_size_buffer_index);
    set_extended_decoration(var_id, SPIRVCrossDecorationResourceIndexPrimary,
                            msl_options.buffer_size_buffer_index);
  ...
  if (needs_view_mask_buffer()) {
    ...
    set_decoration(var_id, DecorationBinding,
                   msl_options.view_mask_buffer_index);
    set_extended_decoration(var_id, SPIRVCrossDecorationResourceIndexPrimary,
                            msl_options.view_mask_buffer_index);
  ...
  // If we're returning a struct from a vertex-like entry point, we must return
  ...
  if (need_position) {
    // If we can get away with returning void from entry point, we don't need to
    // care.
    // If there is at least one other stage output, we need to return
    // [[position]],
    // so we need to create one if it doesn't appear in the SPIR-V. Before
    // adding the
    // implicit variable, check if it actually exists already, but just has not
    // been used
    // or initialized, and if so, mark it as active, and do not create the
    // implicit variable.
...
// Checks if the specified builtin variable (e.g. gl_InstanceIndex) is marked as
// active.
// If not, it marks it as active and forces a recompilation.
// This might be used when the optimization of inactive builtins was too
// optimistic (e.g. when "spvOut" is emitted).
void CompilerMSL::ensure_builtin(spv::StorageClass storage,
                                 spv::BuiltIn builtin) {
  ...
  // At this point, the specified builtin variable must have already been
  // declared in the entry point.
...
void CompilerMSL::mark_implicit_builtin(StorageClass storage, BuiltIn builtin,
                                        uint32_t id) {
  Bitset *active_builtins = nullptr;
  switch (storage) {
  case StorageClassInput:
...
void CompilerMSL::emit_entry_point_declarations() {
  ...
  // Constant arrays of non-primitive types (i.e. matrices) won't link properly
  // into Metal libraries
  ...
  for (auto &samp : constexpr_samplers_by_id) {
    ...
    if (s.s_address == MSL_SAMPLER_ADDRESS_CLAMP_TO_BORDER ||
        s.t_address == MSL_SAMPLER_ADDRESS_CLAMP_TO_BORDER ||
    ...
    if (s.lod_clamp_enable) {
      args.push_back(join(
          "lod_clamp(",
          convert_to_string(s.lod_clamp_min, current_locale_radix_character),
          ", ",
          convert_to_string(s.lod_clamp_max, current_locale_radix_character),
          ")"));
    }

    // If we would emit no arguments, then omit the parentheses entirely.
    // Otherwise,
    // we'll wind up with a "most vexing parse" situation.
    if (args.empty())
      statement("constexpr sampler ", type.basetype == SPIRType::SampledImage
                                          ? to_sampler_expression(samp.first)
                                          : to_name(samp.first),
                ";");
    else
      statement("constexpr sampler ", type.basetype == SPIRType::SampledImage
  ...
  for (auto &dynamic_buffer : buffers_requiring_dynamic_offset) {
    ...
    if (!type.array.empty()) {
      ...
      // And it's even worse if the outermost dimension is a runtime array,
      // because now
      ...
      if (!type.array[type.array.size() - 1])
        ...
        statement(get_argument_address_space(var), " ", type_to_glsl(type),
                  "* ", to_restrict(var_id), name, type_to_array_glsl(type),
                  " =");

        uint32_t dim = uint32_t(type.array.size());
        uint32_t j = 0;
        for (SmallVector<uint32_t> indices(type.array.size());
             indices[type.array.size() - 1] < to_array_size_literal(type);
             j++) {
          while (dim > 0) {
            begin_scope();
            --dim;
          ...
          statement("(", get_argument_address_space(var), " ",
                    type_to_glsl(type), "* ", to_restrict(var_id, false), ")((",
                    get_argument_address_space(var), " char* ",
                    to_restrict(var_id, false), ")", to_name(arg_id), ".",
                    ensure_valid_name(name, "m"), arrays, " + ",
                    to_name(dynamic_offsets_buffer_id), "[", base_index + j,
                    "]),");

          while (++indices[dim] >= to_array_size_literal(type, dim) &&
                 dim < type.array.size() - 1) {
            end_scope(",");
            indices[dim++] = 0;
          }
        }
      ...
      statement(get_argument_address_space(var), " auto& ", to_restrict(var_id),
                name, " = *(", get_argument_address_space(var), " ",
                type_to_glsl(type), "* ", to_restrict(var_id, false), ")((",
                get_argument_address_space(var), " char* ",
                to_restrict(var_id, false), ")", to_name(arg_id), ".",
                ensure_valid_name(name, "m"), " + ",
                to_name(dynamic_offsets_buffer_id), "[", base_index, "]);");
    }
  }
...
string CompilerMSL::compile() {
  ...
  // Arrays which are part of buffer objects are never considered to be native
  // arrays.
  ...
  is_rasterization_disabled =
      msl_options.disable_rasterization || capture_output_to_buffer;
  ...
  if (get_execution_model() == ExecutionModelTessellationControl)
    stage_out_ptr_var_id =
        add_interface_block_pointer(stage_out_var_id, StorageClassOutput);
  if (is_tessellation_shader())
    stage_in_ptr_var_id =
        add_interface_block_pointer(stage_in_var_id, StorageClassInput);

  // Metal vertex functions that define no output must disable rasterization and
  // return void.
  if (!stage_out_var_id)
  ...
  // Add fixup hooks required by shader inputs and outputs. This needs to happen
  // before
  // the loop, so the hooks aren't added multiple times.
  fix_up_shader_inputs_outputs();

  // If we are using argument buffers, we create argument buffer structures for
  // them here.
  // These buffers will be used in the entry point, not the individual
  // resources.
...
void CompilerMSL::preprocess_op_codes() {
  ...
  traverse_all_reachable_opcodes(get<SPIRFunction>(ir.default_entry_point),
                                 preproc);
  ...
  // build_implicit_builtins() hasn't run yet, and in fact, this needs to
  // execute
  ...
  if (preproc.needs_sample_id || msl_options.force_sample_rate_shading ||
      (is_sample_rate() &&
       (active_input_builtins.get(BuiltInFragCoord) ||
...
void CompilerMSL::extract_global_variables_from_functions() {
  ...
  // Local vars that are declared in the main function and accessed directly by
  // a function
  auto &entry_func = get<SPIRFunction>(ir.default_entry_point);
  ...
  extract_global_variables_from_function(ir.default_entry_point, added_arg_ids,
                                         global_var_ids, processed_func_ids);
}

// MSL does not support the use of global variables for shader input content.
// For any global variable accessed directly by the specified function, extract
// that variable,
// add it as an argument to that function, and the arg to the added_arg_ids
// collection.
void CompilerMSL::extract_global_variables_from_function(
    uint32_t func_id, std::set<uint32_t> &added_arg_ids,
    unordered_set<uint32_t> &global_var_ids,
    unordered_set<uint32_t> &processed_func_ids) {
  ...
  for (auto block : func.blocks) {
    ...
    for (auto &i : b.ops) {
      ...
      case OpFunctionCall: {
        ...
        // Then recurse into the function itself to extract globals used
        // internally in the function
        uint32_t inner_func_id = ops[2];
        std::set<uint32_t> inner_func_args;
        extract_global_variables_from_function(
            inner_func_id, inner_func_args, global_var_ids, processed_func_ids);
      ...
      case OpImageTexelPointer: {
        // When using the pointer, we need to know which variable it is actually
        // loaded from.
        uint32_t base_id = ops[2];
      ...
      case OpExtInst: {
        ...
        if (get<SPIRExtension>(extension_set).ext == SPIRExtension::GLSL) {
          ...
          case GLSLstd450InterpolateAtOffset: {
            // For these, we really need the stage-in block. It is theoretically
            // possible to pass the
            // interpolant object, but a) doing so would require us to create an
            // entirely new variable
            // with Interpolant type, and b) if we have a struct or array,
            // handling all the members and
  ...
  if (func_id != ir.default_entry_point) {
    ...
    for (uint32_t arg_id : added_arg_ids) {
      ...
      bool is_patch_block_storage =
          is_patch && is_block && var.storage == StorageClassOutput;
      bool is_builtin = is_builtin_variable(var);
      bool variable_is_stage_io = !is_builtin || bi_type == BuiltInPosition ||
                                  bi_type == BuiltInPointSize ||
                                  bi_type == BuiltInClipDistance ||
      ...
      // If output is masked it is not considered part of the global stage IO
      // interface.
      if (is_redirected_to_global_stage_io && var.storage == StorageClassOutput)
        is_redirected_to_global_stage_io =
            !is_stage_output_variable_masked(var);

      if (is_redirected_to_global_stage_io) {
        // Tessellation control shaders see inputs and per-vertex outputs as
        // arrays.
        // Similarly, tessellation evaluation shaders see per-vertex inputs as
        // arrays.
        // We collected them into a structure; we must pass the array of this
        // structure to the function.
        std::string name;
        if (is_patch)
          name = var.storage == StorageClassInput ? patch_stage_in_var_name
        ...
        if (var.storage == StorageClassOutput &&
          ...
          // If we're redirecting a block, we might still need to access the
          // original block
          // variable if we're masking some members.
          for (uint32_t mbr_idx = 0;
               mbr_idx < uint32_t(p_type->member_types.size()); mbr_idx++) {
        ...
        if (var.storage == StorageClassInput) {
          ...
          auto &added_out =
              is_patch ? patch_added_out : control_point_added_out;
          if (added_out)
            continue;
        ...
        for (auto &mbr_type_id : p_type->member_types) {
          ...
          if (is_builtin && has_active_builtin(builtin, var.storage)) {
            // Add a arg variable with the same type and decorations as the
            // member
            ...
            // so that we will get the appropriate address space when declaring
            // these builtins.
        ...
        // Ensure the existing variable has a valid name and the new variable
        // has all the same meta info
...
// For all variables that are some form of non-input-output interface block,
// mark that all the structs
// that are recursively contained within the type referenced by that variable
...
// are marked as packable with the SPIRVCrossDecorationBufferBlockRepacked
// decoration,
...
// If a shader input exists at the location, it is marked as being used by this
// shader
...
uint32_t CompilerMSL::get_target_components_for_fragment_location(
...
uint32_t CompilerMSL::build_extended_vector_type(uint32_t type_id,
                                                 uint32_t components,
...
uint32_t CompilerMSL::build_msl_interpolant_type(uint32_t type_id,
                                                 bool is_noperspective) {
  uint32_t new_type_id = ir.increase_bound_by(1);
  SPIRType &type = set<SPIRType>(new_type_id, get<SPIRType>(type_id));
  type.basetype = SPIRType::Interpolant;
  type.parent_type = type_id;
  // In Metal, the pull-model interpolant type encodes
  // perspective-vs-no-perspective in the type itself.
...
bool CompilerMSL::add_component_variable_to_interface_block(
    spv::StorageClass storage, const std::string &ib_var_ref, SPIRVariable &var,
    const SPIRType &type, InterfaceBlockMeta &meta) {
  ...
  // Check if we need to pad fragment output to match a certain number of
  // components.
  if (location_meta) {
    ...
    if (pad_fragment_output) {
      ...
      num_components = std::max(
          num_components, get_target_components_for_fragment_location(locn));
    }

    // We have already declared an IO block member as m_location_N.
    // Just emit an early-declared variable and fixup as needed.
    // Arrays need to be unrolled here since each location might need a
    ...
    if (var.storage == StorageClassInput) {
      entry_func.fixup_hooks_in.push_back([=, &type, &var]() {
        if (!type.array.empty()) {
          ...
          statement(to_name(var.self), " = ", ib_var_ref, ".m_location_",
                    location, vector_swizzle(type_components, start_component),
                    ";");
        }
      });
    } else {
...
void CompilerMSL::add_plain_variable_to_interface_block(
    StorageClass storage, const string &ib_var_ref, SPIRType &ib_type,
  ...
  if (add_component_variable_to_interface_block(storage, ib_var_ref, var, type,
                                                meta))
    return;
  ...
  if (storage == StorageClassInput && pull_model_inputs.count(var.self))
    ib_type.member_types.push_back(
        build_msl_interpolant_type(type_id, is_noperspective));
  else
  ...
  // If using pull-model interpolation, need to add a call to the correct
  // interpolation method.
  if (storage == StorageClassInput && pull_model_inputs.count(var.self)) {
    if (is_centroid)
      qual_var_name += ".interpolate_at_centroid()";
    else if (is_sample)
      qual_var_name += join(".interpolate_at_sample(",
  ...
  if (padded_output || padded_input) {
    ...
    if (padded_output) {
      ...
      entry_func.fixup_hooks_in.push_back([=, &var]() {
        statement(to_name(var.self), " = ", qual_var_name,
                  vector_swizzle(type_components, start_component), ";");
      });
    }
  ...
  if (var.storage == StorageClassOutput && var.initializer != ID(0)) {
    if (padded_output || padded_input) {
      ...
      if (meta.strip_array) {
        entry_func.fixup_hooks_in.push_back([=, &var]() {
          uint32_t index = get_extended_decoration(
              var.self, SPIRVCrossDecorationInterfaceMemberIndex);
          auto invocation = to_tesc_invocation_id();
          statement(to_expression(stage_out_ptr_var_id), "[", invocation, "].",
                    to_member_name(ib_type, index), " = ",
                    to_expression(var.initializer), "[", invocation, "];");
        });
      } else {
        entry_func.fixup_hooks_in.push_back([=, &var]() {
  ...
  if (get_decoration_bitset(var.self).get(DecorationLocation)) {
    ...
    if (storage == StorageClassInput) {
      type_id = ensure_correct_input_type(var.basetype, locn, comp, 0,
                                          meta.strip_array);
      var.basetype = type_id;
      ...
      if (pull_model_inputs.count(var.self))
        ib_type.member_types[ib_mbr_idx] =
            build_msl_interpolant_type(type_id, is_noperspective);
      else
    ...
    if (comp)
      set_member_decoration(ib_type.self, ib_mbr_idx, DecorationComponent,
                            comp);
    mark_location_as_used_by_shader(locn, get<SPIRType>(type_id), storage);
  } else if (is_builtin && is_tessellation_shader() &&
             storage == StorageClassInput && inputs_by_builtin.count(builtin)) {
  ...
  set_extended_member_decoration(ib_type.self, ib_mbr_idx,
                                 SPIRVCrossDecorationInterfaceOrigID, var.self);
}

void CompilerMSL::add_composite_variable_to_interface_block(
    StorageClass storage, const string &ib_var_ref, SPIRType &ib_type,
    SPIRVariable &var, InterfaceBlockMeta &meta) {
  auto &entry_func = get<SPIRFunction>(ir.default_entry_point);
  auto &var_type = meta.strip_array ? get_variable_element_type(var)
                                    : get_variable_data_type(var);
  uint32_t elem_cnt = 0;

  if (add_component_variable_to_interface_block(storage, ib_var_ref, var,
                                                var_type, meta))
    return;
  ...
  if (storage == StorageClassOutput && is_builtin &&
    ...
    set_member_decoration(ib_type.self, clip_array_mbr_idx, DecorationBuiltIn,
                          BuiltInClipDistance);

    flatten_from_ib_mbr_name =
        builtin_to_glsl(BuiltInClipDistance, StorageClassOutput);
    ...
    // Only flatten/unflatten IO composites for non-tessellation cases where
    // arrays are not stripped.
    entry_func.add_local_variable(var.self);
  ...
  for (uint32_t i = 0; i < elem_cnt; i++) {
    ...
    if (get_decoration_bitset(var.self).get(DecorationLocation) &&
      ...
      if (usable_type->vecsize < target_components) {
        // Make a new type here.
        type_id =
            build_extended_vector_type(usable_type->self, target_components);
        padded_output = true;
      }
    }

    if (storage == StorageClassInput && pull_model_inputs.count(var.self))
      ib_type.member_types.push_back(build_msl_interpolant_type(
          get_pointee_type_id(type_id), is_noperspective));
    else
      ib_type.member_types.push_back(get_pointee_type_id(type_id));

    // Give the member a name
    string mbr_name =
        ensure_valid_name(join(to_expression(var.self), "_", i), "m");
    set_member_name(ib_type.self, ib_mbr_idx, mbr_name);

    // There is no qualified alias since we need to flatten the internal array
    // on return.
    if (get_decoration_bitset(var.self).get(DecorationLocation)) {
      uint32_t locn = get_decoration(var.self, DecorationLocation) + i;
      uint32_t comp = get_decoration(var.self, DecorationComponent);
      if (storage == StorageClassInput) {
        var.basetype = ensure_correct_input_type(var.basetype, locn, comp, 0,
                                                 meta.strip_array);
        uint32_t mbr_type_id = ensure_correct_input_type(
            usable_type->self, locn, comp, 0, meta.strip_array);
        if (storage == StorageClassInput && pull_model_inputs.count(var.self))
          ib_type.member_types[ib_mbr_idx] =
              build_msl_interpolant_type(mbr_type_id, is_noperspective);
        else
      ...
      set_member_decoration(ib_type.self, ib_mbr_idx, DecorationBuiltIn,
                            builtin);
    ...
    if (storage != StorageClassInput || !pull_model_inputs.count(var.self)) {
      ...
      if (is_flat)
        set_member_decoration(ib_type.self, ib_mbr_idx, DecorationFlat);
      if (is_noperspective)
        set_member_decoration(ib_type.self, ib_mbr_idx,
      ...
      case StorageClassInput:
        entry_func.fixup_hooks_in.push_back([=, &var]() {
          if (pull_model_inputs.count(var.self)) {
            ...
            statement(to_name(var.self), "[", i, "] = ", ib_var_ref, ".",
                      mbr_name, lerp_call, ";");
          } else {
            statement(to_name(var.self), "[", i, "] = ", ib_var_ref, ".",
                      mbr_name, ";");
...
void CompilerMSL::add_composite_member_variable_to_interface_block(
    StorageClass storage, const string &ib_var_ref, SPIRType &ib_type,
    SPIRVariable &var, uint32_t mbr_idx, InterfaceBlockMeta &meta) {
  auto &entry_func = get<SPIRFunction>(ir.default_entry_point);
  auto &var_type = meta.strip_array ? get_variable_element_type(var)
                                    : get_variable_data_type(var);
  ...
  for (uint32_t i = 0; i < elem_cnt; i++) {
    ...
    if (storage == StorageClassInput && pull_model_inputs.count(var.self))
      ib_type.member_types.push_back(
          build_msl_interpolant_type(usable_type->self, is_noperspective));
    else
      ib_type.member_types.push_back(usable_type->self);

    // Give the member a name
    string mbr_name = ensure_valid_name(
        join(to_qualified_member_name(var_type, mbr_idx), "_", i), "m");
    set_member_name(ib_type.self, ib_mbr_idx, mbr_name);

    if (has_member_decoration(var_type.self, mbr_idx, DecorationLocation)) {
      uint32_t locn =
          get_member_decoration(var_type.self, mbr_idx, DecorationLocation) + i;
      set_member_decoration(ib_type.self, ib_mbr_idx, DecorationLocation, locn);
      mark_location_as_used_by_shader(locn, *usable_type, storage);
    } else if (has_decoration(var.self, DecorationLocation)) {
      uint32_t locn =
          get_accumulated_member_location(var, mbr_idx, meta.strip_array) + i;
      set_member_decoration(ib_type.self, ib_mbr_idx, DecorationLocation, locn);
      mark_location_as_used_by_shader(locn, *usable_type, storage);
    } else if (is_builtin && is_tessellation_shader() &&
               storage == StorageClassInput &&
        ...
        entry_func.fixup_hooks_in.push_back([=, &var, &var_type]() {
          if (pull_model_inputs.count(var.self)) {
            ...
            if (is_centroid)
              ...
              lerp_call = ".interpolate_at_center()";
            statement(to_name(var.self), ".", to_member_name(var_type, mbr_idx),
                      "[", i, "] = ", ib_var_ref, ".", mbr_name, lerp_call,
                      ";");
          } else {
            statement(to_name(var.self), ".", to_member_name(var_type, mbr_idx),
                      "[", i, "] = ", ib_var_ref, ".", mbr_name, ";");
          }
        });
        break;
...
void CompilerMSL::add_plain_member_variable_to_interface_block(
  ...
  // Give the member a name
  string mbr_name =
      ensure_valid_name(to_qualified_member_name(var_type, mbr_idx), "m");
  set_member_name(ib_type.self, ib_mbr_idx, mbr_name);
  ...
  if (is_builtin && !meta.strip_array) {
    ...
    case StorageClassOutput:
      ...
      entry_func.fixup_hooks_out.push_back([=, &var, &var_type]() {
        statement(qual_var_name, " = ", to_name(var.self), ".",
                  to_member_name(var_type, mbr_idx), ";");
      });
      break;
  ...
  if (has_member_decoration(var_type.self, mbr_idx, DecorationLocation)) {
    uint32_t locn =
        get_member_decoration(var_type.self, mbr_idx, DecorationLocation);
    uint32_t comp =
        get_member_decoration(var_type.self, mbr_idx, DecorationComponent);
    if (storage == StorageClassInput) {
      mbr_type_id = ensure_correct_input_type(mbr_type_id, locn, comp, 0,
                                              meta.strip_array);
      var_type.member_types[mbr_idx] = mbr_type_id;
      if (storage == StorageClassInput && pull_model_inputs.count(var.self))
        ib_type.member_types[ib_mbr_idx] =
    ...
    // The block itself might have a location and in this case, all members of
    // the block
    // receive incrementing locations.
    uint32_t locn =
        get_accumulated_member_location(var, mbr_idx, meta.strip_array);
    if (storage == StorageClassInput) {
      mbr_type_id =
          ensure_correct_input_type(mbr_type_id, locn, 0, 0, meta.strip_array);
      var_type.member_types[mbr_idx] = mbr_type_id;
      if (storage == StorageClassInput && pull_model_inputs.count(var.self))
        ib_type.member_types[ib_mbr_idx] =
            build_msl_interpolant_type(mbr_type_id, is_noperspective);
  ...
  if (!flatten_stage_out && var.storage == StorageClassOutput &&
      var.initializer != ID(0) &&
      (c = maybe_get<SPIRConstant>(var.initializer))) {
    if (meta.strip_array) {
      entry_func.fixup_hooks_in.push_back([=, &var]() {
        auto &type = this->get<SPIRType>(var.basetype);
        uint32_t index = get_extended_member_decoration(
            var.self, mbr_idx, SPIRVCrossDecorationInterfaceMemberIndex);

        auto invocation = to_tesc_invocation_id();
        auto constant_chain =
            join(to_expression(var.initializer), "[", invocation, "]");
        statement(to_expression(stage_out_ptr_var_id), "[", invocation, "].",
                  to_member_name(ib_type, index), " = ", constant_chain, ".",
...
// In Metal, the tessellation levels are stored as tightly packed half-precision
// floating point values.
// But, stage-in attribute offsets and strides must be multiples of four, so we
// can't pass the levels
// individually. Therefore, we must pass them as vectors. Triangles get a single
// float4, with the outer
// levels in 'xyz' and the inner level in 'w'. Quads get a float4 containing the
// outer levels and a
// float2 containing the inner levels.
void CompilerMSL::add_tess_level_input_to_interface_block(
    const std::string &ib_var_ref, SPIRType &ib_type, SPIRVariable &var) {
  auto &entry_func = get<SPIRFunction>(ir.default_entry_point);
  auto &var_type = get_variable_element_type(var);
  ...
  if (triangles) {
    ...
    if (!added_builtin_tess_level) {
      ...
      // it's marked as builtin so we can get automatic attribute assignment if
      // needed.
      set_member_decoration(ib_type.self, ib_mbr_idx, DecorationBuiltIn,
                            builtin);
    ...
    uint32_t type_id = build_extended_vector_type(
        var_type.self, builtin == BuiltInTessLevelOuter ? 4 : 2);
...
bool CompilerMSL::variable_storage_requires_stage_io(
...
string CompilerMSL::to_tesc_invocation_id() {
  if (msl_options.multi_patch_workgroup) {
    ...
    return join(to_expression(builtin_invocation_id_id), ".x % ",
                get_entry_point().output_vertices);
  } else
    return builtin_to_glsl(BuiltInInvocationId, StorageClassInput);
}

void CompilerMSL::emit_local_masked_variable(const SPIRVariable &masked_var,
                                             bool strip_array) {
  auto &entry_func = get<SPIRFunction>(ir.default_entry_point);
  bool threadgroup_storage =
      variable_decl_is_remapped_storage(masked_var, StorageClassWorkgroup);
  ...
  if (threadgroup_storage && msl_options.multi_patch_workgroup) {
    ...
    entry_func.fixup_hooks_in.push_back([this, &masked_var]() {
      ...
      uint32_t max_num_instances = (max_control_points_per_patch +
                                    get_entry_point().output_vertices - 1u) /
                                   get_entry_point().output_vertices;
      statement("threadgroup ", type_to_glsl(type), " ", "spvStorage",
                to_name(masked_var.self), "[", max_num_instances, "]",
      ...
      // FIXME: We will likely need an option to support passing down target
      // workgroup size,
      // so we can emit appropriate size here.
      statement("threadgroup ", type_to_glsl(type), " ", "(&",
                to_name(masked_var.self), ")", type_to_array_glsl(type),
                " = spvStorage", to_name(masked_var.self), "[", "(",
                to_expression(builtin_invocation_id_id), ".x / ",
                get_entry_point().output_vertices, ") % ", max_num_instances,
                "];");
  ...
  if (!threadgroup_storage) {
    ...
    if (strip_array) {
      entry_func.fixup_hooks_in.push_back([this, &masked_var, initializer]() {
        auto invocation = to_tesc_invocation_id();
        statement(to_expression(masked_var.self), "[", invocation, "] = ",
                  to_expression(initializer), "[", invocation, "];");
      });
    } else {
      entry_func.fixup_hooks_in.push_back([this, &masked_var, initializer]() {
...
void CompilerMSL::add_variable_to_interface_block(StorageClass storage,
                                                  const string &ib_var_ref,
                                                  SPIRType &ib_type,
                                                  SPIRVariable &var,
                                                  InterfaceBlockMeta &meta) {
  auto &entry_func = get<SPIRFunction>(ir.default_entry_point);
  // Tessellation control I/O variables and tessellation evaluation per-point
  // inputs are
  // usually declared as arrays. In these cases, we want to add the element type
  // to the
  // interface block, since in Metal it's the interface block itself which is
  // arrayed.
  auto &var_type = meta.strip_array ? get_variable_element_type(var)
                                    : get_variable_data_type(var);
  bool is_builtin = is_builtin_variable(var);
  ...
  if (storage == StorageClassOutput && is_stage_output_variable_masked(var)) {
    // If we ignore an output, we must still emit it, since it might be used by
    // app.
  ...
  if (var_type.basetype == SPIRType::Struct) {
    bool block_requires_flattening =
        variable_storage_requires_stage_io(storage) || is_block;
    bool needs_local_declaration = !is_builtin && block_requires_flattening &&
                                   meta.allow_local_declaration;
    ...
    if (!block_requires_flattening) {
      ...
      // This makes things
      // very complicated, since stage-in structures in MSL don't support nested
      // structures.
      // Luckily, for stage-out when capturing output, we can avoid this and
      // just add
      // composite members directly, because the stage-out structure is stored
      // to a buffer,
      // not returned.
      ...
      for (uint32_t mbr_idx = 0;
           mbr_idx < uint32_t(var_type.member_types.size()); mbr_idx++) {
        builtin = BuiltInMax;
        is_builtin = is_member_builtin(var_type, mbr_idx, &builtin);
        auto &mbr_type = get<SPIRType>(var_type.member_types[mbr_idx]);

        if (storage == StorageClassOutput &&
            is_stage_output_block_member_masked(var, mbr_idx,
                                                meta.strip_array)) {
          if (is_block)
            masked_block = true;

          // Non-builtin block output variables are just ignored, since they
          // will still access
          ...
          if (is_builtin && !meta.strip_array) {
            ...
            set<SPIRVariable>(var_id, ptr_type_id, StorageClassOutput,
                              initializer);
          ...
          bool storage_is_stage_io =
              variable_storage_requires_stage_io(storage);
          ...
          if ((!is_builtin || attribute_load_store) && storage_is_stage_io &&
              is_composite_type) {
            add_composite_member_variable_to_interface_block(
                storage, ib_var_ref, ib_type, var, mbr_idx, meta);
          } else {
      ...
      if (masked_block && !needs_local_declaration &&
        ...
        if (is_builtin_variable(var)) {
          // Ensure correct names for the block members if we're actually going
          // to
          // declare gl_PerVertex.
          for (uint32_t mbr_idx = 0;
               mbr_idx < uint32_t(var_type.member_types.size()); mbr_idx++) {
            set_member_name(
                var_type.self, mbr_idx,
                builtin_to_glsl(BuiltIn(get_member_decoration(
                                    var_type.self, mbr_idx, DecorationBuiltIn)),
                                StorageClassOutput));
          }
        ...
        emit_local_masked_variable(var, meta.strip_array);
             ...
             storage == StorageClassInput && !meta.strip_array && is_builtin &&
             (builtin == BuiltInTessLevelOuter ||
              builtin == BuiltInTessLevelInner)) {
    add_tess_level_input_to_interface_block(ib_var_ref, ib_type, var);
  } else if (var_type.basetype == SPIRType::Boolean ||
             var_type.basetype == SPIRType::Char ||
    ...
    if (!is_builtin || has_active_builtin(builtin, storage)) {
      ...
      // MSL does not allow matrices or arrays in input or output variables, so
      // need to handle it specially.
      if ((!is_builtin || attribute_load_store) && storage_is_stage_io &&
          is_composite_type) {
        add_composite_variable_to_interface_block(storage, ib_var_ref, ib_type,
                                                  var, meta);
      } else {
        add_plain_variable_to_interface_block(storage, ib_var_ref, ib_type, var,
                                              meta);
...
// Fix up the mapping of variables to interface member indices, which is used to
// compile access chains
// for per-vertex variables in a tessellation control shader.
void CompilerMSL::fix_up_interface_member_indices(StorageClass storage,
                                                  uint32_t ib_type_id) {
  ...
  if (get_execution_model() != ExecutionModelTessellationControl &&
      !(get_execution_model() == ExecutionModelTessellationEvaluation &&
        storage == StorageClassInput) &&
      !(get_execution_model() == ExecutionModelFragment &&
        storage == StorageClassInput && !pull_model_inputs.empty()))
    return;

  auto mbr_cnt = uint32_t(ir.meta[ib_type_id].members.size());
  for (uint32_t i = 0; i < mbr_cnt; i++) {
    uint32_t var_id = get_extended_member_decoration(
        ib_type_id, i, SPIRVCrossDecorationInterfaceOrigID);
    if (!var_id)
      continue;
    auto &var = get<SPIRVariable>(var_id);
    ...
    if (type.basetype == SPIRType::Struct && (flatten_composites || is_block))
      mbr_idx = get_extended_member_decoration(
          ib_type_id, i, SPIRVCrossDecorationInterfaceMemberIndex);

    if (mbr_idx != uint32_t(-1)) {
      // Only set the lowest InterfaceMemberIndex for each variable member.
      // IB struct members will be emitted in-order w.r.t. interface member
      // index.
      if (!has_extended_member_decoration(
              var_id, mbr_idx, SPIRVCrossDecorationInterfaceMemberIndex))
        set_extended_member_decoration(
            var_id, mbr_idx, SPIRVCrossDecorationInterfaceMemberIndex, i);
    } else {
      // Only set the lowest InterfaceMemberIndex for each variable.
      // IB struct members will be emitted in-order w.r.t. interface member
      // index.
      if (!has_extended_decoration(var_id,
                                   SPIRVCrossDecorationInterfaceMemberIndex))
        set_extended_decoration(var_id,
...
// Add an interface structure for the type of storage, which is either
// StorageClassInput or StorageClassOutput.
...
uint32_t CompilerMSL::add_interface_block(StorageClass storage, bool patch) {
  ...
  bool incl_builtins =
      storage == StorageClassOutput || is_tessellation_shader();
  bool has_seen_barycentric = false;

  InterfaceBlockMeta meta;

  // Varying interfaces between stages which use "user()" attribute can be dealt
  // with
  // without explicit packing and unpacking of components. For any variables
  // which link against the runtime
  // in some way (vertex attributes, fragment output, etc), we'll need to deal
  // with it somehow.
  bool pack_components = (storage == StorageClassInput &&
                          get_execution_model() == ExecutionModelVertex) ||
                         (storage == StorageClassOutput &&
                          get_execution_model() == ExecutionModelFragment) ||
                         (storage == StorageClassOutput &&
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t var_id, SPIRVariable &var) {
    ...
    bool builtin_is_stage_in_out =
        builtin_is_gl_in_out || bi_type == BuiltInLayer ||
        bi_type == BuiltInViewportIndex || bi_type == BuiltInBaryCoordNV ||
        bi_type == BuiltInBaryCoordNoPerspNV || bi_type == BuiltInFragDepth ||
    ...
    bool is_interface_block_builtin =
        ...
        (get_execution_model() == ExecutionModelTessellationEvaluation &&
         (bi_type == BuiltInTessLevelOuter ||
          bi_type == BuiltInTessLevelInner));

    bool is_active = interface_variable_exists_in_entry_point(var.self);
    if (is_builtin && is_active) {
      // Only emit the builtin if it's active in this entry point. Interface
      // variable list might lie.
      if (is_block) {
        // If any builtin is active, the block is active.
        uint32_t mbr_cnt = uint32_t(type.member_types.size());
        for (uint32_t i = 0; !is_active && i < mbr_cnt; i++)
          is_active = has_active_builtin(
              BuiltIn(get_member_decoration(type.self, i, DecorationBuiltIn)),
              storage);
      } else {
    ...
    bool filter_patch_decoration = (has_decoration(var_id, DecorationPatch) ||
                                    is_patch_block(type)) == patch;

    bool hidden = is_hidden_variable(var, incl_builtins);

    // ClipDistance is never hidden, we need to emulate it when used as an
    // input.
    if (bi_type == BuiltInClipDistance || bi_type == BuiltInCullDistance)
      hidden = false;

    // It's not enough to simply avoid marking fragment outputs if the pipeline
    // won't
    // accept them. We can't put them in the struct at all, or otherwise the
    // compiler
    // complains that the outputs weren't explicitly marked.
    // Frag depth and stencil outputs are incompatible with explicit early
    // fragment tests.
    // In GLSL, depth and stencil outputs are just ignored when explicit early
    // fragment tests are required.
    // In Metal, it's a compilation error, so we need to exclude them from the
    // output struct.
    if (get_execution_model() == ExecutionModelFragment &&
        storage == StorageClassOutput && !patch &&
        ((is_builtin && ((bi_type == BuiltInFragDepth &&
                          (!msl_options.enable_frag_depth_builtin ||
                           uses_explicit_early_fragment_test())) ||
                         (bi_type == BuiltInFragStencilRefEXT &&
                          (!msl_options.enable_frag_stencil_ref_builtin ||
                           uses_explicit_early_fragment_test())))) ||
         (!is_builtin &&
          !(msl_options.enable_frag_output_mask & (1 << location))))) {
      hidden = true;
      disabled_frag_outputs.push_back(var_id);
      // If a builtin, force it to have the proper name, and mark it as not part
    ...
    // Barycentric inputs must be emitted in stage-in, because they can have
    // interpolation arguments.
    if (is_active && (bi_type == BuiltInBaryCoordNV ||
                      bi_type == BuiltInBaryCoordNoPerspNV)) {
      if (has_seen_barycentric)
        SPIRV_CROSS_THROW("Cannot declare both BaryCoordNV and "
                          "BaryCoordNoPerspNV in same shader in MSL.");
    ...
    if (is_active && !hidden && type.pointer && filter_patch_decoration &&
      ...
      if (!is_builtin) {
        ...
        // Multiple variables can alias the same Location, and try to make sure
        // each location is declared only once.
        // We will swizzle data in and out to make this work.
        // This is only relevant for vertex inputs and fragment outputs.
        // Technically tessellation as well, but it is too complicated to
        // support.
        uint32_t component = get_decoration(var_id, DecorationComponent);
        if (component != 0) {
          if (is_tessellation_shader())
            SPIRV_CROSS_THROW("Component decoration is not supported in "
            ...
            for (uint32_t location_offset = 0; location_offset < array_size;
                 location_offset++) {
              auto &location_meta =
                  meta.location_meta[location + location_offset];
              location_meta.num_components = std::max(
                  location_meta.num_components, component + type.vecsize);

              // For variables sharing location, decorations and base type must
              // match.
              location_meta.base_type_id = type.self;
              location_meta.flat = has_decoration(var.self, DecorationFlat);
              location_meta.noperspective =
                  has_decoration(var.self, DecorationNoPerspective);
              location_meta.centroid =
                  has_decoration(var.self, DecorationCentroid);
  ...
  // For patch input in a tessellation evaluation shader, the per-vertex stage
  // inputs
  // are included in a special patch control point array.
  if (vars.empty() &&
      !(storage == StorageClassInput && patch && stage_in_var_id))
    return 0;

  // Add a new typed variable for this interface structure.
  // The initializer expression is allocated here, but populated when the
  // function
  ...
  case StorageClassInput:
    ...
    if (get_execution_model() == ExecutionModelTessellationControl) {
      // Add a hook to populate the shared workgroup memory containing the gl_in
      // array.
      entry_func.fixup_hooks_in.push_back([=]() {
        // Can't use PatchVertices, PrimitiveId, or InvocationId yet; the hooks
        // for those may not have run yet.
        if (msl_options.multi_patch_workgroup) {
          // n.b. builtin_invocation_id_id here is the dispatch global
          // invocation ID,
          // not the TC invocation ID.
          statement("device ", to_name(ir.default_entry_point), "_", ib_var_ref,
                    "* gl_in = &", input_buffer_var_name, "[min(",
                    to_expression(builtin_invocation_id_id), ".x / ",
                    get_entry_point().output_vertices,
                    ", spvIndirectParams[1] - 1) * spvIndirectParams[0]];");
        } else {
          // It's safe to use InvocationId here because it's directly mapped to
          // a
          // Metal builtin, and therefore doesn't need a hook.
          statement("if (", to_expression(builtin_invocation_id_id),
                    " < spvIndirectParams[0])");
          statement("    ", input_wg_var_name, "[",
                    to_expression(builtin_invocation_id_id), "] = ", ib_var_ref,
                    ";");
  ...
  case StorageClassOutput: {
    ...
    // Add the output interface struct as a local variable to the entry
    // function.
    // If the entry point should return the output struct, set the entry
    // function
    // to return the output interface struct, otherwise to return nothing.
    // Watch out for the rare case where the terminator of the last entry point
    // block is a
    // Kill, instead of a Return. Based on SPIR-V's block-domination rules, we
    // assume that
    // any block that has a Kill will also have a terminating Return, except the
    // last block.
    ...
    if (!capture_output_to_buffer) {
      ...
      for (auto &blk_id : entry_func.blocks) {
        ...
        if (blk.terminator == SPIRBlock::Return ||
            (blk.terminator == SPIRBlock::Kill &&
             blk_id == entry_func.blocks.back()))
      ...
      case ExecutionModelTessellationEvaluation:
        ...
        entry_func.fixup_hooks_in.push_back([=]() {
          if (stage_out_var_id) {
            // The first member of the indirect buffer is always the number of
            // vertices
            // to draw.
            // We zero-base the InstanceID & VertexID variables for HLSL
            // emulation elsewhere, so don't do it twice
            if (get_execution_model() == ExecutionModelVertex &&
                msl_options.vertex_for_tessellation) {
              statement("device ", to_name(ir.default_entry_point), "_",
                        ib_var_ref, "& ", ib_var_ref, " = ",
      ...
      case ExecutionModelTessellationControl:
        if (msl_options.multi_patch_workgroup) {
          // We cannot use PrimitiveId here, because the hook may not have run
          // yet.
          if (patch) {
            entry_func.fixup_hooks_in.push_back([=]() {
              statement("device ", to_name(ir.default_entry_point), "_",
            ...
            entry_func.fixup_hooks_in.push_back([=]() {
              statement("device ", to_name(ir.default_entry_point), "_",
                        ib_var_ref, "* gl_out = &", output_buffer_var_name, "[",
                        to_expression(builtin_invocation_id_id), ".x - ",
                        to_expression(builtin_invocation_id_id), ".x % ",
                        get_entry_point().output_vertices, "];");
            });
  ...
  for (auto *p_var : vars) {
    ...
    // Fixing up flattened stores in TESC is impossible since the memory is
    // group shared either via
    // device (not masked) or threadgroup (masked) storage classes and it's race
    // condition city.
  ...
  if (get_execution_model() == ExecutionModelTessellationControl &&
      msl_options.multi_patch_workgroup && storage == StorageClassInput) {
    // For tessellation control inputs, add all outputs from the vertex shader
    // to ensure
    ...
    for (auto &input : inputs_by_location) {
      ...
      add_variable_to_interface_block(storage, ib_var_ref, ib_type, fake_var,
                                      meta);
    }
  }

  // When multiple variables need to access same location,
  // unroll locations one by one and we will flatten output or input as
  // necessary.
  for (auto &loc : meta.location_meta) {
    ...
    uint32_t ib_mbr_idx = uint32_t(ib_type.member_types.size());
    uint32_t type_id = build_extended_vector_type(location_meta.base_type_id,
                                                  location_meta.num_components);
    ib_type.member_types.push_back(type_id);

    set_member_name(ib_type.self, ib_mbr_idx, join("m_location_", location));
    set_member_decoration(ib_type.self, ib_mbr_idx, DecorationLocation,
                          location);
  ...
  MemberSorter member_sorter(ib_type, ir.meta[ib_type_id],
                             MemberSorter::LocationThenBuiltInType);
  member_sorter.sort();

  // The member indices were saved to the original variables, but after the
  // members
  // were sorted, those indices are now likely incorrect. Fix those up now.
  fix_up_interface_member_indices(storage, ib_type_id);

  // For patch inputs, add one more member, holding the array of control point
  // data.
  if (get_execution_model() == ExecutionModelTessellationEvaluation &&
      storage == StorageClassInput && patch && stage_in_var_id) {
    ...
    pcp_type.parent_type = pcp_type.type_alias =
        get_stage_in_struct_type().self;
...
uint32_t CompilerMSL::add_interface_block_pointer(uint32_t ib_var_id,
                                                  StorageClass storage) {
  if (!ib_var_id)
    return 0;
  ...
  if (get_execution_model() == ExecutionModelTessellationControl) {
    ...
    ib_ptr_type.storage =
        storage == StorageClassInput
            ? (msl_options.multi_patch_workgroup ? StorageClassStorageBuffer
    ...
    set<SPIRVariable>(ib_ptr_var_id, ib_ptr_ptr_type_id, StorageClassFunction,
                      0);
    ...
    // But, in Metal, this array uses a very special type,
    // 'patch_control_point<T>',
    // which is a container that can be used to access the control point data.
    // To represent this, a special 'ControlPointArray' type has been added to
    // the
    // SPIRV-Cross type system. It should only be generated by and seen in the
    // MSL
    ...
    ir.meta[ib_ptr_var_id].decoration.qualified_alias =
        join(patch_stage_in_var_name, ".gl_in");
  }
...
uint32_t CompilerMSL::ensure_correct_builtin_type(uint32_t type_id,
  ...
  if ((builtin == BuiltInSampleMask && is_array(type)) ||
      ((builtin == BuiltInLayer || builtin == BuiltInViewportIndex ||
...
uint32_t CompilerMSL::ensure_correct_input_type(uint32_t type_id,
                                                uint32_t location,
                                                uint32_t component,
  ...
  if (type.basetype == SPIRType::Struct ||
      type.array.size() > max_array_dimensions)
    return type_id;
  ...
  case MSL_SHADER_INPUT_FORMAT_UINT8: {
    ...
    case SPIRType::Short:
      return build_extended_vector_type(type_id, num_components > type.vecsize
                                                     ? num_components
                                                     : type.vecsize,
                                        SPIRType::UShort);
    case SPIRType::Int:
...
void CompilerMSL::mark_struct_members_packed(const SPIRType &type) {
  ...
  for (uint32_t i = 0; i < mbr_cnt; i++) {
    ...
    if (mbr_type.basetype == SPIRType::Struct) {
      ...
      set_extended_member_decoration(type.self, i,
...
void CompilerMSL::mark_scalar_layout_structs(const SPIRType &type) {
  ...
  for (uint32_t i = 0; i < mbr_cnt; i++) {
    ...
    if (mbr_type.basetype == SPIRType::Struct) {
      ...
      if (has_extended_decoration(struct_type->self,
                                  SPIRVCrossDecorationPhysicalTypePacked))
        continue;

      uint32_t msl_alignment =
          get_declared_struct_member_alignment_msl(type, i);
      ...
      // Both are complicated cases. In scalar layout, a struct of float3 might
      // just consume 12 bytes,
      ...
      // Verify that if a struct is used as an array that ArrayStride matches
      // the effective size of the struct.
      if (!mbr_type.array.empty()) {
        ...
        // If struct size is larger than array stride, we might be able to fit,
        // if we tightly pack.
      ...
      if (struct_needs_explicit_padding) {
        ...
        if (array_stride < msl_size) {
          SPIRV_CROSS_THROW("Cannot express an array stride smaller than size "
                            "of struct type.");
        } else {
          if (has_extended_decoration(struct_type->self,
                                      SPIRVCrossDecorationPaddingTarget)) {
            if (array_stride !=
                get_extended_decoration(struct_type->self,
                                        SPIRVCrossDecorationPaddingTarget))
              SPIRV_CROSS_THROW("A struct is used with different array "
                                "strides. Cannot express this in MSL.");
          } else
            set_extended_decoration(struct_type->self,
                                    SPIRVCrossDecorationPaddingTarget,
                                    array_stride);
        }
      }
...
// Sort the members of the struct type by offset, and pack and then pad members
// where needed
// to align MSL members with SPIR-V offsets. The struct members are iterated
// twice. Packing
// occurs first, followed by padding, because packing a member reduces both its
// size and its
// natural alignment, possibly requiring a padding member to be added ahead of
// it.
void CompilerMSL::align_struct(SPIRType &ib_type,
  ...
  // Test the alignment of each member, and if a member should be closer to the
  // previous
  // member than the default spacing expects, it is likely that the previous
  // member is in
  // a packed format. If so, and the previous member is packable, pack it.
  // For example ... this applies to any 3-element vector that is followed by a
  // scalar.
  uint32_t msl_offset = 0;
  for (uint32_t mbr_idx = 0; mbr_idx < mbr_cnt; mbr_idx++) {
    // This checks the member in isolation, if the member needs some kind of
    // type remapping to conform to SPIR-V
    // offsets, array strides and matrix strides.
    ensure_member_packing_rules_msl(ib_type, mbr_idx);

    // Align current offset to the current member's default alignment. If the
    // member was packed, it will observe
    // the updated alignment here.
    uint32_t msl_align_mask =
        get_declared_struct_member_alignment_msl(ib_type, mbr_idx) - 1;
    uint32_t aligned_msl_offset =
        (msl_offset + msl_align_mask) & ~msl_align_mask;

    // Fetch the member offset as declared in the SPIRV.
    uint32_t spirv_mbr_offset =
        get_member_decoration(ib_type_id, mbr_idx, DecorationOffset);
    if (spirv_mbr_offset > aligned_msl_offset) {
      // Since MSL and SPIR-V have slightly different struct member alignment
      // and
      // size rules, we'll pad to standard C-packing rules with a char[] array.
      // If the member is farther
      // away than C-packing, expects, add an inert padding member before the
      // the member.
      uint32_t padding_bytes = spirv_mbr_offset - aligned_msl_offset;
      set_extended_member_decoration(ib_type_id, mbr_idx,
                                     SPIRVCrossDecorationPaddingTarget,
                                     padding_bytes);
      ...
      // It *might* happen if a sub-struct has a larger alignment requirement in
      // MSL than SPIR-V.
    ...
    // Increment the current offset to be positioned immediately after the
    // current member.
    // Don't do this for the last member since it can be unsized, and it is not
    // relevant for padding purposes here.
    if (mbr_idx + 1 < mbr_cnt)
      msl_offset = aligned_msl_offset +
                   get_declared_struct_member_size_msl(ib_type, mbr_idx);
  }
}

bool CompilerMSL::validate_member_packing_rules_msl(const SPIRType &type,
                                                    uint32_t index) const {
  auto &mbr_type = get<SPIRType>(type.member_types[index]);
  uint32_t spirv_offset =
      get_member_decoration(type.self, index, DecorationOffset);

  if (index + 1 < type.member_types.size()) {
    // First, we will check offsets. If SPIR-V offset + MSL size > SPIR-V offset
    // of next member,
    ...
    uint32_t spirv_offset_next =
        get_member_decoration(type.self, index + 1, DecorationOffset);
  ...
  if (!mbr_type.array.empty()) {
    ...
    // If app tries to be cheeky and access the member out of bounds, this will
    // not work, but this is the best we can do.
    // In OpAccessChain with logical memory models, access chains must be
    // in-bounds in SPIR-V specification.
    bool relax_array_stride =
        mbr_type.array.back() == 1 && mbr_type.array_size_literal.back();

    if (!relax_array_stride) {
      uint32_t spirv_array_stride =
          type_struct_member_array_stride(type, index);
      uint32_t msl_array_stride =
          get_declared_struct_member_array_stride_msl(type, index);
  ...
  if (is_matrix(mbr_type)) {
    ...
    uint32_t spirv_matrix_stride =
        type_struct_member_matrix_stride(type, index);
    uint32_t msl_matrix_stride =
        get_declared_struct_member_matrix_stride_msl(type, index);
  ...
  uint32_t msl_alignment =
      get_declared_struct_member_alignment_msl(type, index);
...
// Here we need to verify that the member type we declare conforms to Offset,
// ArrayStride or MatrixStride restrictions.
// If there is a mismatch, we need to emit remapped types, either normal types,
// or "packed_X" types.
// In odd cases we need to emit packed and remapped types, for e.g. weird
// matrices or arrays with weird array strides.
void CompilerMSL::ensure_member_packing_rules_msl(SPIRType &ib_type,
                                                  uint32_t index) {
  if (validate_member_packing_rules_msl(ib_type, index))
    return;

  // We failed validation.
  // This case will be nightmare-ish to deal with. This could possibly happen if
  // struct alignment does not quite
  // match up with what we want. Scalar block layout comes to mind here where we
  // might have to work around the rule
  // that struct alignment == max alignment of all members and struct size
  // depends on this alignment.
  auto &mbr_type = get<SPIRType>(ib_type.member_types[index]);
  if (mbr_type.basetype == SPIRType::Struct)
    SPIRV_CROSS_THROW("Cannot perform any repacking for structs when it is "
  ...
  if (!is_scalar(ib_type))
    set_extended_member_decoration(ib_type.self, index,
                                   SPIRVCrossDecorationPhysicalTypePacked);
  ...
  // We're in deep trouble, and we need to create a new PhysicalType which
  // matches up with what we expect.
  // A lot of work goes here ...
  // We will need remapping on Load and Store to translate the types between
  // Logical and Physical.

  // First, we check if we have small vector std140 array.
  // We detect this if we have an array of vectors, and array stride is greater
  ...
  if (!mbr_type.array.empty() && !is_matrix(mbr_type)) {
    ...
    // Hack off array-of-arrays until we find the array stride per element we
    // must have to make it work.
    uint32_t dimensions = uint32_t(mbr_type.array.size() - 1);
    for (uint32_t dim = 0; dim < dimensions; dim++)
    ...
    bool row_major =
        has_member_decoration(ib_type.self, index, DecorationRowMajor);
  ...
  // We might have a particular odd scalar layout case where the last element of
  // an array
  // does not take up as much space as the ArrayStride or MatrixStride. This can
  // happen with DX cbuffers.
  // The "proper" workaround for this is extremely painful and essentially
  // impossible in the edge case of float3[],
  // so we hack around it by declaring the offending array or matrix with one
  // less array size/col/row,
  // and rely on padding to get the correct value. We will technically access
  // arrays out of bounds into the padding region,
  // but it should spill over gracefully without too much trouble. We rely on
  // behavior like this for unsized arrays anyways.

  // E.g. we might observe a physical layout of:
  // { float2 a[2]; float b; } in cbuffer layout where ArrayStride of a is 16,
  // but offset of b is 24, packed right after a[1] ...
  uint32_t type_id = get_extended_member_decoration(
      ib_type.self, index, SPIRVCrossDecorationPhysicalTypeID);
  auto &type = get<SPIRType>(type_id);

  // Modify the physical type in-place. This is safe since each physical type
  // workaround is a copy.
  if (is_array(type)) {
    if (type.array.back() > 1) {
      if (!type.array_size_literal.back())
        SPIRV_CROSS_THROW("Cannot apply scalar layout workaround with spec "
                          "constant array size.");
      type.array.back() -= 1;
    } else {
      // We have an array of size 1, so we cannot decrement that. Our only
      // option now is to
      // force a packed layout instead, and drop the physical type remap since
      // ArrayStride is meaningless now.
      unset_extended_member_decoration(ib_type.self, index,
    ...
    bool row_major =
        has_member_decoration(ib_type.self, index, DecorationRowMajor);
    if (!row_major) {
      // Slice off one column. If we only have 2 columns, this might turn the
      // matrix into a vector with one array element instead.
      if (type.columns > 2) {
        type.columns--;
      ...
      // Slice off one row. If we only have 2 rows, this might turn the matrix
      // into a vector with one array element instead.
      if (type.vecsize > 2) {
        type.vecsize--;
...
void CompilerMSL::emit_store_statement(uint32_t lhs_expression,
                                       uint32_t rhs_expression) {
  auto &type = expression_type(rhs_expression);

  bool lhs_remapped_type = has_extended_decoration(
      lhs_expression, SPIRVCrossDecorationPhysicalTypeID);
  bool lhs_packed_type = has_extended_decoration(
      lhs_expression, SPIRVCrossDecorationPhysicalTypePacked);
  ...
  // No physical type remapping, and no packed type, so can just emit a store
  // directly.
  if (!lhs_remapped_type && !lhs_packed_type) {
    ...
    // In this case, we just flip transpose states, and emit the store, a
    // transpose must be in the RHS expression, if any.
    if (is_matrix(type) && lhs_e && lhs_e->need_transpose) {
      ...
      if (rhs_e && rhs_e->need_transpose) {
        ...
        // Skip the transpose, as we will transpose when writing to LHS and
        // transpose(transpose(T)) == T.
        rhs_e->need_transpose = false;
        statement(to_expression(lhs_expression), " = ",
                  to_unpacked_row_major_matrix_expression(rhs_expression), ";");
        rhs_e->need_transpose = true;
      } else
        statement(to_expression(lhs_expression), " = transpose(",
    ...
    // Even if the target type is packed, we can directly store to it. We cannot
    // store to packed matrices directly,
    // since they are declared as array of vectors instead, and we need the
    // fallback path below.
    CompilerGLSL::emit_store_statement(lhs_expression, rhs_expression);
  } else {
    ...
    TypeID physical_type_id =
        lhs_remapped_type
            ? ID(get_extended_decoration(lhs_expression,
                                         SPIRVCrossDecorationPhysicalTypeID))
            : type.self;
    ...
    if (is_matrix(type)) {
      ...
      if (transpose) {
        ...
        if (physical_type.columns != type.columns)
          cast_expr =
              join("(device ", packed_pfx, type_to_glsl(write_type), "&)");
        ...
        if (rhs_transpose) {
          ...
          // Transpose on the fly. Emitting a lot of full transpose() ops and
          // extracting lanes seems very bad,
          ...
          for (uint32_t i = 0; i < type.vecsize; i++) {
            ...
            for (uint32_t j = 0; j < vector_type.vecsize; j++) {
              rhs_row += join(to_enclosed_unpacked_expression(rhs_expression),
                              "[", j, "][", i, "]");
        ...
        if (rhs_transpose) {
          ...
          for (uint32_t i = 0; i < type.columns; i++) {
            string rhs_row = type_to_glsl_constructor(vector_type) + "(";
            for (uint32_t j = 0; j < vector_type.vecsize; j++) {
              // Need to explicitly unpack expression since we've mucked with
              // transpose state.
              auto unpacked_expr =
                  to_unpacked_row_major_matrix_expression(rhs_expression);
      ...
      // It's still an l-value, so it's fine. Most other unpacking of
      // expressions turn them into r-values instead.
      lhs = join("(device ", type_to_glsl(type), "&)", enclose_expression(lhs));
      if (!optimize_read_modify_write(expression_type(rhs_expression), lhs,
                                      rhs))
        statement(lhs, " = ", rhs, ";");
    } else if (!is_matrix(type)) {
      string lhs = to_dereferenced_expression(lhs_expression);
...
static bool expression_ends_with(const string &expr_str,
                                 const std::string &ending) {
  if (expr_str.length() >= ending.length())
    return (expr_str.compare(expr_str.length() - ending.length(),
...
// Also, handle special physical ID remapping scenarios, similar to
// emit_store_statement().
string CompilerMSL::unpack_expression_type(string expr_str,
                                           const SPIRType &type,
  ...
  if (physical_type && is_vector(*physical_type) && is_array(*physical_type) &&
    ...
    // we can't just pass the array straight to the matrix constructor. We have
    // to
    // pass each vector individually, so that they can be unpacked to normal
    // vectors.
    if (!physical_type)
      physical_type = &type;
    ...
    uint32_t physical_vecsize =
        row_major ? physical_type->columns : physical_type->vecsize;
    ...
    for (uint32_t i = 0; i < columns; i++) {
      ...
      if (packed)
        unpack_expr += join(base_type, physical_vecsize, "(", expr_str, "[", i,
                            "]", ")", load_swiz);
      else
...
void CompilerMSL::emit_header() {
  // This particular line can be overridden during compilation, so make it a
  // flag and not a pragma line.
  if (suppress_missing_prototypes)
    statement("#pragma clang diagnostic ignored \"-Wmissing-prototypes\"");

  // Disable warning about missing braces for array<T> template to make arrays a
  // value type
...
// Template struct like spvUnsafeArray<> need to be declared *before* any
...
void CompilerMSL::emit_custom_templates() {
  for (const auto &spv_func : spv_function_implementations) {
    ...
    case SPVFuncImplUnsafeArray:
      ...
      end_scope();
      statement("");
      statement("constexpr const constant T& operator [] (size_t pos) const "
                "constant");
      begin_scope();
      statement("return elements[pos];");
      end_scope();
...
// Metal helper functions must be static force-inline, i.e. static inline
...
void CompilerMSL::emit_custom_functions() {
  for (uint32_t i = kArrayCopyMultidimMax; i >= 2; i--)
    if (spv_function_implementations.count(
            static_cast<SPVFuncImpl>(SPVFuncImplArrayCopyMultidimBase + i)))
      spv_function_implementations.insert(
          static_cast<SPVFuncImpl>(SPVFuncImplArrayCopyMultidimBase + i - 1));
  ...
  if (spv_function_implementations.count(SPVFuncImplDynamicImageSampler)) {
    ...
    if (!msl_options.supports_msl_version(2))
      SPIRV_CROSS_THROW("spvDynamicImageSampler requires default-constructible "
                        "texture objects, which require MSL 2.0.");
    ...
    for (uint32_t i = SPVFuncImplChromaReconstructNearest2Plane;
         i <= SPVFuncImplChromaReconstructLinear420XMidpointYMidpoint3Plane;
         i++)
  ...
  for (const auto &spv_func : spv_function_implementations) {
    switch (spv_func) {
    case SPVFuncImplMod:
      statement("// Implementation of the GLSL mod() function, which is "
                "slightly different than Metal fmod()");
    ...
    case SPVFuncImplSSign:
      ...
      statement("template<typename T, typename E = typename "
                "enable_if<is_integral<T>::value>::type>");
      statement("inline T sign(T x)");
      begin_scope();
      statement("return select(select(select(x, T(0), x == T(0)), T(1), x > "
                "T(0)), T(-1), x < T(0));");
      end_scope();
      statement("");
      break;
    ...
    case SPVFuncImplArrayOfArrayCopy6Dim: {
      // Unfortunately we cannot template on the address space, so combinatorial
      // explosion it is.
      static const char *function_name_tags[] = {
          "FromConstantToStack", "FromConstantToThreadGroup",
          "FromStackToStack", "FromStackToThreadGroup",
          "FromThreadGroupToStack", "FromThreadGroupToThreadGroup",
          "FromDeviceToDevice", "FromConstantToDevice", "FromStackToDevice",
          "FromThreadGroupToDevice", "FromDeviceToStack",
          "FromDeviceToThreadGroup",
      };

      static const char *src_address_space[] = {
          "constant",          "constant",          "thread const",
          "thread const",      "threadgroup const", "threadgroup const",
          "device const",      "constant",          "thread const",
          "threadgroup const", "device const",      "device const",
      };

      static const char *dst_address_space[] = {
          "thread", "threadgroup", "thread", "threadgroup",
          "thread", "threadgroup", "device", "device",
          "device", "device",      "thread", "threadgroup",
      };
      ...
      for (uint32_t variant = 0; variant < 12; variant++) {
        ...
        statement("inline void spvArrayCopy", function_name_tags[variant],
                  dimensions, "(", dst_address_space[variant], " T (&dst)",
                  array_arg, ", ", src_address_space[variant], " T (&src)",
                  array_arg, ")");
        ...
        if (dimensions == 1)
          ...
          statement("spvArrayCopy", function_name_tags[variant], dimensions - 1,
                    "(dst[i], src[i]);");
        end_scope();
        end_scope();
        statement("");
    ...
    case SPVFuncImplTexelBufferCoords: {
      if (msl_options.texel_buffer_texture_width > 0) {
        string tex_width_str =
            convert_to_string(msl_options.texel_buffer_texture_width);
        statement("// Returns 2D texture coords corresponding to 1D texel "
                  "buffer coords");
        statement(force_inline);
        statement("uint2 spvTexelBufferCoord(uint tc)");
        begin_scope();
        statement(join("return uint2(tc % ", tex_width_str, ", tc / ",
                       tex_width_str, ");"));
        end_scope();
        statement("");
      } else {
        statement("// Returns 2D texture coords corresponding to 1D texel "
                  "buffer coords");
        statement("#define spvTexelBufferCoord(tc, tex) uint2((tc) % "
                  "(tex).get_width(), (tc) / (tex).get_width())");
        statement("");
      }
      break;
    }
    ...
    case SPVFuncImplImage2DAtomicCoords: {
      if (msl_options.supports_msl_version(1, 2)) {
        ...
        statement("constant uint spvLinearTextureAlignmentOverride "
                  "[[function_constant(",
        ...
        statement("constant uint spvLinearTextureAlignment = ",
                  ...
                  "spvLinearTextureAlignmentOverride : ",
                  msl_options.r32ui_linear_texture_alignment, ";");
      } else {
        statement(
            "// The required alignment of a linear texture of R32Uint format.");
        statement("constant uint spvLinearTextureAlignment = ",
      ...
      statement("// Returns buffer coords corresponding to 2D texture coords "
                "for emulating 2D texture atomics");
      statement(
    ...
    case SPVFuncImplFMul:
      ...
      statement("[[clang::optnone]] vec<T, Cols> spvFMulVectorMatrix(vec<T, "
                "Rows> v, matrix<T, Cols, Rows> m)");
      begin_scope();
      ...
      statement("template<typename T, int Cols, int Rows>");
      statement("[[clang::optnone]] vec<T, Rows> spvFMulMatrixVector(matrix<T, "
                "Cols, Rows> m, vec<T, Cols> v)");
      begin_scope();
      ...
      statement("[[clang::optnone]] matrix<T, RCols, LRows> "
                "spvFMulMatrixMatrix(matrix<T, LCols, LRows> l, matrix<T, "
                "RCols, RRows> r)");
      begin_scope();
    ...
    case SPVFuncImplQuantizeToF16:
      ...
      // SpvHalfTypeSelector is used to match the half* template type to the
      // float* template type.
      // Depending on GPU, MSL does not always flush converted subnormal halfs
      // to zero,
      // as required by OpQuantizeToF16, so check for subnormals and flush them
      // to zero.
      statement("template <typename F> struct SpvHalfTypeSelector;");
      statement("template <> struct SpvHalfTypeSelector<float> { public: using "
                "H = half; };");
      statement("template<uint N> struct SpvHalfTypeSelector<vec<float, N>> { "
                "using H = vec<half, N>; };");
      statement("template<typename F, typename H = typename "
                "SpvHalfTypeSelector<F>::H>");
      statement("[[clang::optnone]] F spvQuantizeToF16(F fval)");
      begin_scope();
      statement("H hval = H(fval);");
      statement("hval = select(copysign(H(0), hval), hval, isnormal(hval) || "
                "isinf(hval) || isnan(hval));");
      statement("return F(hval);");
      end_scope();
      statement("");
      break;

    // Emulate texturecube_array with texture2d_array for iOS where this type is
    // not available
    ...
    case SPVFuncImplInverse4x4:
      ...
      statement(force_inline);
      statement("float spvDet3x3(float a1, float a2, float a3, float b1, float "
                "b2, float b3, float c1, "
                "float c2, float c3)");
      begin_scope();
      statement("return a1 * spvDet2x2(b2, b3, c2, c3) - b1 * spvDet2x2(a2, "
                "a3, c2, c3) + c1 * spvDet2x2(a2, a3, "
                "b2, b3);");
      end_scope();
      statement("");
      statement("// Returns the inverse of a matrix, by using the algorithm of "
                "calculating the classical");
      statement("// adjoint and dividing by the determinant. The contents of "
                "the matrix are changed.");
      statement(force_inline);
      statement("float4x4 spvInverse4x4(float4x4 m)");
      begin_scope();
      statement("float4x4 adj;	// The adjoint matrix (inverse after dividing "
                "by determinant)");
      statement_no_indent("");
      statement("// Create the transpose of the cofactors, as the classical "
                "adjoint of the matrix.");
      statement("adj[0][0] =  spvDet3x3(m[1][1], m[1][2], m[1][3], m[2][1], "
                "m[2][2], m[2][3], m[3][1], m[3][2], "
                "m[3][3]);");
      statement("adj[0][1] = -spvDet3x3(m[0][1], m[0][2], m[0][3], m[2][1], "
                "m[2][2], m[2][3], m[3][1], m[3][2], "
                "m[3][3]);");
      statement("adj[0][2] =  spvDet3x3(m[0][1], m[0][2], m[0][3], m[1][1], "
                "m[1][2], m[1][3], m[3][1], m[3][2], "
                "m[3][3]);");
      statement("adj[0][3] = -spvDet3x3(m[0][1], m[0][2], m[0][3], m[1][1], "
                "m[1][2], m[1][3], m[2][1], m[2][2], "
                "m[2][3]);");
      statement_no_indent("");
      statement("adj[1][0] = -spvDet3x3(m[1][0], m[1][2], m[1][3], m[2][0], "
                "m[2][2], m[2][3], m[3][0], m[3][2], "
                "m[3][3]);");
      statement("adj[1][1] =  spvDet3x3(m[0][0], m[0][2], m[0][3], m[2][0], "
                "m[2][2], m[2][3], m[3][0], m[3][2], "
                "m[3][3]);");
      statement("adj[1][2] = -spvDet3x3(m[0][0], m[0][2], m[0][3], m[1][0], "
                "m[1][2], m[1][3], m[3][0], m[3][2], "
                "m[3][3]);");
      statement("adj[1][3] =  spvDet3x3(m[0][0], m[0][2], m[0][3], m[1][0], "
                "m[1][2], m[1][3], m[2][0], m[2][2], "
                "m[2][3]);");
      statement_no_indent("");
      statement("adj[2][0] =  spvDet3x3(m[1][0], m[1][1], m[1][3], m[2][0], "
                "m[2][1], m[2][3], m[3][0], m[3][1], "
                "m[3][3]);");
      statement("adj[2][1] = -spvDet3x3(m[0][0], m[0][1], m[0][3], m[2][0], "
                "m[2][1], m[2][3], m[3][0], m[3][1], "
                "m[3][3]);");
      statement("adj[2][2] =  spvDet3x3(m[0][0], m[0][1], m[0][3], m[1][0], "
                "m[1][1], m[1][3], m[3][0], m[3][1], "
                "m[3][3]);");
      statement("adj[2][3] = -spvDet3x3(m[0][0], m[0][1], m[0][3], m[1][0], "
                "m[1][1], m[1][3], m[2][0], m[2][1], "
                "m[2][3]);");
      statement_no_indent("");
      statement("adj[3][0] = -spvDet3x3(m[1][0], m[1][1], m[1][2], m[2][0], "
                "m[2][1], m[2][2], m[3][0], m[3][1], "
                "m[3][2]);");
      statement("adj[3][1] =  spvDet3x3(m[0][0], m[0][1], m[0][2], m[2][0], "
                "m[2][1], m[2][2], m[3][0], m[3][1], "
                "m[3][2]);");
      statement("adj[3][2] = -spvDet3x3(m[0][0], m[0][1], m[0][2], m[1][0], "
                "m[1][1], m[1][2], m[3][0], m[3][1], "
                "m[3][2]);");
      statement("adj[3][3] =  spvDet3x3(m[0][0], m[0][1], m[0][2], m[1][0], "
                "m[1][1], m[1][2], m[2][0], m[2][1], "
                "m[2][2]);");
      statement_no_indent("");
      statement("// Calculate the determinant as a combination of the "
                "cofactors of the first row.");
      statement("float det = (adj[0][0] * m[0][0]) + (adj[0][1] * m[1][0]) + "
                "(adj[0][2] * m[2][0]) + (adj[0][3] "
                "* m[3][0]);");
      statement_no_indent("");
      statement("// Divide the classical adjoint matrix by the determinant.");
      statement("// If determinant is zero, matrix is not invertable, so leave "
                "it unchanged.");
    ...
    case SPVFuncImplInverse3x3:
      ...
      begin_scope();
      statement("float3x3 adj;	// The adjoint matrix (inverse after dividing "
                "by determinant)");
      statement_no_indent("");
      statement("// Create the transpose of the cofactors, as the classical "
                "adjoint of the matrix.");
    ...
    case SPVFuncImplInverse2x2:
      ...
      begin_scope();
      statement("float2x2 adj;	// The adjoint matrix (inverse after dividing "
                "by determinant)");
      statement_no_indent("");
      statement("// Create the transpose of the cofactors, as the classical "
                "adjoint of the matrix.");
    ...
    case SPVFuncImplForwardArgs:
      statement("template<typename T> struct spvRemoveReference { typedef T "
                "type; };");
      statement("template<typename T> struct spvRemoveReference<thread T&> { "
                "typedef T type; };");
      statement("template<typename T> struct spvRemoveReference<thread T&&> { "
                "typedef T type; };");
      statement("template<typename T> inline constexpr thread T&& "
                "spvForward(thread typename "
    ...
    case SPVFuncImplTextureSwizzle:
      ...
      statement("if (!s)");
      statement("    return x;");
      statement("return vec<T, 4>(spvGetSwizzle(x, x.r, spvSwizzle((s >> 0) & "
                "0xFF)), "
                "spvGetSwizzle(x, x.g, spvSwizzle((s >> 8) & 0xFF)), "
                "spvGetSwizzle(x, x.b, spvSwizzle((s >> 16) "
                "& 0xFF)), "
    ...
    case SPVFuncImplGatherSwizzle:
      ...
      statement("template<typename T, template<typename, access = "
                "access::sample, typename = void> class Tex, "
                "typename... Ts>");
      statement("inline vec<T, 4> spvGatherSwizzle(const thread Tex<T>& t, "
                "sampler s, "
      ...
      // expression, so we need this silly workaround just to compile the
      // shader.
      statement("switch (c)");
      begin_scope();
    ...
    case SPVFuncImplGatherCompareSwizzle:
      ...
      statement("template<typename T, template<typename, access = "
                "access::sample, typename = void> class Tex, "
                "typename... Ts>");
      statement("inline vec<T, 4> spvGatherCompareSwizzle(const thread Tex<T>& "
                "t, sampler "
                "s, uint sw, Ts... params) ");
      begin_scope();
      statement("if (sw)");
    ...
    case SPVFuncImplSubgroupBroadcast:
      // Metal doesn't allow broadcasting boolean values directly, but we can
      // work around that by broadcasting
      // them as integers.
      statement("template<typename T>");
      ...
      statement("inline vec<bool, N> spvSubgroupBroadcast(vec<bool, N> value, "
                "ushort lane)");
      begin_scope();
      if (msl_options.is_ios() && !msl_options.ios_use_simdgroup_functions)
        statement("return (vec<bool, N>)quad_broadcast((vec<ushort, N>)value, "
                  "lane);");
      else
        statement("return (vec<bool, N>)simd_broadcast((vec<ushort, N>)value, "
                  "lane);");
      end_scope();
      statement("");
      break;
    ...
    case SPVFuncImplSubgroupBroadcastFirst:
      ...
      begin_scope();
      if (msl_options.is_ios() && !msl_options.ios_use_simdgroup_functions)
        statement("return (vec<bool, N>)quad_broadcast_first((vec<ushort, "
                  "N>)value);");
      else
        statement("return (vec<bool, N>)simd_broadcast_first((vec<ushort, "
                  "N>)value);");
      end_scope();
      statement("");
      break;
    ...
    case SPVFuncImplSubgroupBallotFindLSB:
      statement("inline uint spvSubgroupBallotFindLSB(uint4 ballot, uint "
                "gl_SubgroupSize)");
      begin_scope();
      if (msl_options.is_ios()) {
        statement("uint4 mask = uint4(extract_bits(0xFFFFFFFF, 0, "
                  "gl_SubgroupSize), uint3(0));");
      } else {
        statement("uint4 mask = uint4(extract_bits(0xFFFFFFFF, 0, "
                  "min(gl_SubgroupSize, 32u)), "
                  "extract_bits(0xFFFFFFFF, 0, (uint)max((int)gl_SubgroupSize "
                  "- 32, 0)), uint2(0));");
      }
      statement("ballot &= mask;");
      statement("return select(ctz(ballot.x), select(32 + ctz(ballot.y), "
                "select(64 + ctz(ballot.z), select(96 + "
                "ctz(ballot.w), uint(-1), ballot.w == 0), ballot.z == 0), "
                "ballot.y == 0), ballot.x == 0);");
      end_scope();
      statement("");
      break;

    case SPVFuncImplSubgroupBallotFindMSB:
      statement("inline uint spvSubgroupBallotFindMSB(uint4 ballot, uint "
                "gl_SubgroupSize)");
      begin_scope();
      if (msl_options.is_ios()) {
        statement("uint4 mask = uint4(extract_bits(0xFFFFFFFF, 0, "
      ...
      statement("ballot &= mask;");
      statement("return select(128 - (clz(ballot.w) + 1), select(96 - "
                "(clz(ballot.z) + 1), select(64 - "
                "(clz(ballot.y) + 1), select(32 - (clz(ballot.x) + 1), "
                "uint(-1), ballot.x == 0), ballot.y == 0), "
    ...
    case SPVFuncImplSubgroupBallotBitCount:
      ...
      begin_scope();
      statement("return popcount(ballot.x) + popcount(ballot.y) + "
                "popcount(ballot.z) + popcount(ballot.w);");
      end_scope();
      statement("");
      statement("inline uint spvSubgroupBallotBitCount(uint4 ballot, uint "
                "gl_SubgroupSize)");
      begin_scope();
      if (msl_options.is_ios()) {
        statement("uint4 mask = uint4(extract_bits(0xFFFFFFFF, 0, "
      ...
      end_scope();
      statement("");
      statement("inline uint spvSubgroupBallotInclusiveBitCount(uint4 ballot, "
                "uint gl_SubgroupInvocationID)");
      begin_scope();
      if (msl_options.is_ios()) {
        statement("uint4 mask = uint4(extract_bits(0xFFFFFFFF, 0, "
                  "gl_SubgroupInvocationID + 1), uint3(0));");
      } else {
        statement("uint4 mask = uint4(extract_bits(0xFFFFFFFF, 0, "
                  "min(gl_SubgroupInvocationID + 1, 32u)), "
                  "extract_bits(0xFFFFFFFF, 0, "
                  "(uint)max((int)gl_SubgroupInvocationID + 1 - 32, 0)), "
                  "uint2(0));");
      }
      statement("return spvPopCount4(ballot & mask);");
      end_scope();
      statement("");
      statement("inline uint spvSubgroupBallotExclusiveBitCount(uint4 ballot, "
                "uint gl_SubgroupInvocationID)");
      begin_scope();
      if (msl_options.is_ios()) {
        statement("uint4 mask = uint4(extract_bits(0xFFFFFFFF, 0, "
                  "gl_SubgroupInvocationID), uint2(0));");
      } else {
        statement("uint4 mask = uint4(extract_bits(0xFFFFFFFF, 0, "
                  "min(gl_SubgroupInvocationID, 32u)), "
                  "extract_bits(0xFFFFFFFF, 0, "
                  "(uint)max((int)gl_SubgroupInvocationID - 32, 0)), "
                  "uint2(0));");
      }
      statement("return spvPopCount4(ballot & mask);");
      end_scope();
    ...
    case SPVFuncImplSubgroupAllEqual:
      ...
      // (in this case, the value of the first active thread). Then, by the
      // transitive
      // property of equality, if all comparisons return true, then they are all
      // equal.
      statement("template<typename T>");
      ...
      begin_scope();
      if (msl_options.is_ios() && !msl_options.ios_use_simdgroup_functions)
        statement("return quad_all(all(value == (vec<bool, "
                  "N>)quad_broadcast_first((vec<ushort, N>)value)));");
      else
        statement("return simd_all(all(value == (vec<bool, "
                  "N>)simd_broadcast_first((vec<ushort, N>)value)));");
      end_scope();
      statement("");
      break;
    ...
    case SPVFuncImplSubgroupShuffle:
      ...
      statement("");
      statement("template<uint N>");
      statement("inline vec<bool, N> spvSubgroupShuffle(vec<bool, N> value, "
                "ushort lane)");
      begin_scope();
      if (msl_options.is_ios() && !msl_options.ios_use_simdgroup_functions)
        statement(
    ...
    case SPVFuncImplSubgroupShuffleXor:
      ...
      statement("");
      statement("template<uint N>");
      statement("inline vec<bool, N> spvSubgroupShuffleXor(vec<bool, N> value, "
                "ushort mask)");
      begin_scope();
      if (msl_options.is_ios() && !msl_options.ios_use_simdgroup_functions)
        statement("return (vec<bool, N>)quad_shuffle_xor((vec<ushort, "
                  "N>)value, mask);");
      else
        statement("return (vec<bool, N>)simd_shuffle_xor((vec<ushort, "
                  "N>)value, mask);");
      end_scope();
      statement("");
      break;
    ...
    case SPVFuncImplSubgroupShuffleUp:
      ...
      statement("");
      statement("template<uint N>");
      statement("inline vec<bool, N> spvSubgroupShuffleUp(vec<bool, N> value, "
                "ushort delta)");
      begin_scope();
      if (msl_options.is_ios() && !msl_options.ios_use_simdgroup_functions)
        statement("return (vec<bool, N>)quad_shuffle_up((vec<ushort, N>)value, "
                  "delta);");
      else
        statement("return (vec<bool, N>)simd_shuffle_up((vec<ushort, N>)value, "
                  "delta);");
      end_scope();
      statement("");
      break;
    ...
    case SPVFuncImplSubgroupShuffleDown:
      ...
      statement("");
      statement("template<uint N>");
      statement("inline vec<bool, N> spvSubgroupShuffleDown(vec<bool, N> "
                "value, ushort delta)");
      begin_scope();
      if (msl_options.is_ios() && !msl_options.ios_use_simdgroup_functions)
        statement("return (vec<bool, N>)quad_shuffle_down((vec<ushort, "
                  "N>)value, delta);");
      else
        statement("return (vec<bool, N>)simd_shuffle_down((vec<ushort, "
                  "N>)value, delta);");
      end_scope();
      statement("");
      break;
    ...
    case SPVFuncImplQuadBroadcast:
      ...
      statement("");
      statement("template<uint N>");
      statement("inline vec<bool, N> spvQuadBroadcast(vec<bool, N> value, uint "
                "lane)");
      begin_scope();
      statement(
    ...
    case SPVFuncImplQuadSwap:
      ...
      begin_scope();
      statement("return (vec<bool, N>)quad_shuffle_xor((vec<ushort, N>)value, "
                "dir + 1);");
      end_scope();
      statement("");
      break;
    ...
    case SPVFuncImplChromaReconstructNearest2Plane:
      ...
      statement("inline vec<T, 4> spvChromaReconstructNearest(texture2d<T> "
                "plane0, texture2d<T> plane1, sampler "
                "samp, float2 coord, LodOptions... options)");
      begin_scope();
      statement("vec<T, 4> ycbcr = vec<T, 4>(0, 0, 0, 1);");
      statement("ycbcr.g = plane0.sample(samp, coord, "
                "spvForward<LodOptions>(options)...).r;");
      statement("ycbcr.br = plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)...).rg;");
    ...
    case SPVFuncImplChromaReconstructNearest3Plane:
      statement("template<typename T, typename... LodOptions>");
      statement("inline vec<T, 4> spvChromaReconstructNearest(texture2d<T> "
                "plane0, texture2d<T> plane1, "
                "texture2d<T> plane2, sampler samp, float2 coord, "
                "LodOptions... options)");
      begin_scope();
      statement("vec<T, 4> ycbcr = vec<T, 4>(0, 0, 0, 1);");
      statement("ycbcr.g = plane0.sample(samp, coord, "
                "spvForward<LodOptions>(options)...).r;");
      statement("ycbcr.b = plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)...).r;");
      statement("ycbcr.r = plane2.sample(samp, coord, "
                "spvForward<LodOptions>(options)...).r;");
      statement("return ycbcr;");
      end_scope();
      statement("");
    ...
    case SPVFuncImplChromaReconstructLinear422CositedEven2Plane:
      statement("template<typename T, typename... LodOptions>");
      statement("inline vec<T, 4> "
                "spvChromaReconstructLinear422CositedEven(texture2d<T> plane0, "
                "texture2d<T> "
      ...
      begin_scope();
      statement("ycbcr.br = vec<T, 2>(mix(plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)...), "
                "plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., int2(1, 0)), 0.5).rg);");
      end_scope();
      statement("else");
      begin_scope();
      statement("ycbcr.br = plane1.sample(samp, coord, "
    ...
    case SPVFuncImplChromaReconstructLinear422CositedEven3Plane:
      ...
      statement("inline vec<T, 4> "
                "spvChromaReconstructLinear422CositedEven(texture2d<T> plane0, "
                "texture2d<T> "
                "plane1, texture2d<T> plane2, sampler samp, float2 coord, "
                "LodOptions... options)");
      begin_scope();
      statement("vec<T, 4> ycbcr = vec<T, 4>(0, 0, 0, 1);");
      statement("ycbcr.g = plane0.sample(samp, coord, "
      ...
      statement("if (fract(coord.x * plane1.get_width()) != 0.0)");
      begin_scope();
      statement("ycbcr.b = T(mix(plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)...), "
                "plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., int2(1, 0)), 0.5).r);");
      statement("ycbcr.r = T(mix(plane2.sample(samp, coord, "
                "spvForward<LodOptions>(options)...), "
                "plane2.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., int2(1, 0)), 0.5).r);");
      end_scope();
      statement("else");
      begin_scope();
    ...
    case SPVFuncImplChromaReconstructLinear422Midpoint2Plane:
      statement("template<typename T, typename... LodOptions>");
      statement("inline vec<T, 4> "
                "spvChromaReconstructLinear422Midpoint(texture2d<T> plane0, "
                "texture2d<T> "
                "plane1, sampler samp, float2 coord, LodOptions... options)");
      begin_scope();
      statement("vec<T, 4> ycbcr = vec<T, 4>(0, 0, 0, 1);");
      statement("ycbcr.g = plane0.sample(samp, coord, "
                "spvForward<LodOptions>(options)...).r;");
      statement("int2 offs = int2(fract(coord.x * plane1.get_width()) != 0.0 ? "
                "1 : -1, 0);");
      statement("ycbcr.br = vec<T, 2>(mix(plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)...), "
                "plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., offs), 0.25).rg);");
      statement("return ycbcr;");
      end_scope();
      statement("");
      break;
    ...
    case SPVFuncImplChromaReconstructLinear422Midpoint3Plane:
      ...
      statement("ycbcr.b = T(mix(plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)...), "
                "plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., offs), 0.25).r);");
      statement("ycbcr.r = T(mix(plane2.sample(samp, coord, "
                "spvForward<LodOptions>(options)...), "
                "plane2.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., offs), 0.25).r);");
    ...
    case SPVFuncImplChromaReconstructLinear420XCositedEvenYCositedEven2Plane:
      ...
      statement("inline vec<T, 4> "
                ...
                "texture2d<T> plane0, "
                "texture2d<T> plane1, sampler samp, float2 coord, "
                "LodOptions... options)");
      begin_scope();
      statement("vec<T, 4> ycbcr = vec<T, 4>(0, 0, 0, 1);");
      statement("ycbcr.g = plane0.sample(samp, coord, "
                "spvForward<LodOptions>(options)...).r;");
      statement("float2 ab = fract(round(coord * float2(plane0.get_width(), "
                "plane0.get_height())) * 0.5);");
      statement("ycbcr.br = vec<T, 2>(mix(mix(plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)...), "
                "plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., int2(1, 0)), ab.x), "
                "mix(plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., int2(0, 1)), "
                "plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., int2(1, 1)), ab.x), "
                "ab.y).rg);");
      statement("return ycbcr;");
      end_scope();
      statement("");
    ...
    case SPVFuncImplChromaReconstructLinear420XCositedEvenYCositedEven3Plane:
      ...
      statement("inline vec<T, 4> "
                "spvChromaReconstructLinear420XCositedEvenYCositedEven("
                "texture2d<T> plane0, "
                "texture2d<T> plane1, texture2d<T> plane2, sampler samp, "
                "float2 coord, LodOptions... options)");
      begin_scope();
      statement("vec<T, 4> ycbcr = vec<T, 4>(0, 0, 0, 1);");
      statement("ycbcr.g = plane0.sample(samp, coord, "
                "spvForward<LodOptions>(options)...).r;");
      statement("float2 ab = fract(round(coord * float2(plane0.get_width(), "
                "plane0.get_height())) * 0.5);");
      statement("ycbcr.b = T(mix(mix(plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)...), "
                "plane1.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., int2(1, 0)), ab.x), "
                "mix(plane1.sample(samp, coord, "
                ...
                "spvForward<LodOptions>(options)..., int2(1, 1)), ab.x), "
                "ab.y).r);");
      statement("ycbcr.r = T(mix(mix(plane2.sample(samp, coord, "
                "spvForward<LodOptions>(options)...), "
                "plane2.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., int2(1, 0)), ab.x), "
                "mix(plane2.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., int2(0, 1)), "
                "plane2.sample(samp, coord, "
                "spvForward<LodOptions>(options)..., int2(1, 1)), ab.x), "
                "ab.y).r);");
    ...
    case SPVFuncImplChromaReconstructLinear420XMidpointYCositedEven2Plane:
      statement("template<typename T, typename... LodOptions>");
      statement("inline vec<T, 4> "
                "spvChromaReconstructLinear420XMidpointYCositedEven(texture2d<"
                "T> plane0, "
                "texture2d<T> plane1, sampler samp, float2 coord, "
                "LodOptions... options)");
      begin_scope();
    ...
    case SPVFuncImplChromaReconstructLinear420XCositedEvenYMidpoint2Plane:
      statement("template<typename T, typename... LodOptions>");
      statement("inline vec<T, 4> "
                "spvChromaReconstructLinear420XCositedEvenYMidpoint(texture2d<"
                "T> plane0, "
                "texture2d<T> plane1, sampler samp, float2 coord, "
                "LodOptions... options)");
      begin_scope();
    ...
    case SPVFuncImplChromaReconstructLinear420XMidpointYMidpoint2Plane:
      statement("template<typename T, typename... LodOptions>");
      statement("inline vec<T, 4> "
                "spvChromaReconstructLinear420XMidpointYMidpoint(texture2d<T> "
                "plane0, "
                "texture2d<T> plane1, sampler samp, float2 coord, "
                "LodOptions... options)");
      begin_scope();
    ...
    case SPVFuncImplExpandITUNarrowRange:
      ...
      begin_scope();
      statement("ycbcr.g = (ycbcr.g * (exp2(T(n)) - 1) - ldexp(T(16), n - "
                "8))/ldexp(T(219), n - 8);");
      statement("ycbcr.br = (ycbcr.br * (exp2(T(n)) - 1) - ldexp(T(128), n - "
                "8))/ldexp(T(224), n - 8);");
      statement("return ycbcr;");
      end_scope();
      statement("");
    ...
    case SPVFuncImplConvertYCbCrBT709:
      ...
      statement("constant float3x3 spvBT709Factors = {{1, 1, 1}, {0, "
                "-0.13397432/0.7152, 1.8556}, {1.5748, "
                "-0.33480248/0.7152, 0}};");
      statement("");
      statement("template<typename T>");
    ...
    case SPVFuncImplConvertYCbCrBT601:
      ...
      statement("constant float3x3 spvBT601Factors = {{1, 1, 1}, {0, "
                "-0.202008/0.587, 1.772}, {1.402, "
                "-0.419198/0.587, 0}};");
      statement("");
      statement("template<typename T>");
    ...
    case SPVFuncImplConvertYCbCrBT2020:
      ...
      statement("constant float3x3 spvBT2020Factors = {{1, 1, 1}, {0, "
                "-0.11156702/0.6780, 1.8814}, {1.4746, "
                "-0.38737742/0.6780, 0}};");
      statement("");
      statement("template<typename T>");
    ...
    case SPVFuncImplDynamicImageSampler:
      ...
      statement("constexpr spvYCbCrSampler(const thread spvYCbCrSampler& s) "
                "thread = default;");
      statement("");
      statement("spvFormatResolution get_resolution() const thread");
      begin_scope();
      statement("return spvFormatResolution((val & resolution_mask) >> "
                "resolution_base);");
      end_scope();
      statement("spvChromaFilter get_chroma_filter() const thread");
      begin_scope();
      statement("return spvChromaFilter((val & chroma_filter_mask) >> "
                "chroma_filter_base);");
      end_scope();
      statement("spvXChromaLocation get_x_chroma_offset() const thread");
      begin_scope();
      statement("return spvXChromaLocation((val & x_chroma_off_mask) >> "
                "x_chroma_off_base);");
      end_scope();
      statement("spvYChromaLocation get_y_chroma_offset() const thread");
      begin_scope();
      statement("return spvYChromaLocation((val & y_chroma_off_mask) >> "
                "y_chroma_off_base);");
      end_scope();
      statement("spvYCbCrModelConversion get_ycbcr_model() const thread");
      begin_scope();
      statement("return spvYCbCrModelConversion((val & ycbcr_model_mask) >> "
                "ycbcr_model_base);");
      end_scope();
      statement("spvYCbCrRange get_ycbcr_range() const thread");
      begin_scope();
      statement("return spvYCbCrRange((val & ycbcr_range_mask) >> "
                "ycbcr_range_base);");
      end_scope();
      statement("int get_bpc() const thread { return (val & bpc_mask) >> "
                "bpc_base; }");
      statement("");
      statement("private:");
      ...
      statement("");
      statement("constexpr static constant ushort resolution_mask = ((1 << "
                "resolution_bits) - 1) << resolution_base;");
      statement("constexpr static constant ushort chroma_filter_mask = ((1 << "
                "chroma_filter_bits) - 1) << "
                "chroma_filter_base;");
      statement("constexpr static constant ushort x_chroma_off_mask = ((1 << "
                "x_chroma_off_bit) - 1) << "
                "x_chroma_off_base;");
      statement("constexpr static constant ushort y_chroma_off_mask = ((1 << "
                "y_chroma_off_bit) - 1) << "
                "y_chroma_off_base;");
      statement("constexpr static constant ushort ycbcr_model_mask = ((1 << "
                "ycbcr_model_bits) - 1) << "
                "ycbcr_model_base;");
      statement("constexpr static constant ushort ycbcr_range_mask = ((1 << "
                "ycbcr_range_bit) - 1) << "
                "ycbcr_range_base;");
      statement("constexpr static constant ushort bpc_mask = ((1 << bpc_bits) "
                "- 1) << bpc_base;");
      statement("");
      ...
      begin_scope();
      statement("return (ushort(res) << resolution_base) | (build(t...) & "
                "~resolution_mask);");
      end_scope();
      statement("");
      statement("template<typename... Ts>");
      ...
      begin_scope();
      statement("return (ushort(filt) << chroma_filter_base) | (build(t...) & "
                "~chroma_filter_mask);");
      end_scope();
      statement("");
      statement("template<typename... Ts>");
      ...
      begin_scope();
      statement("return (ushort(loc) << x_chroma_off_base) | (build(t...) & "
                "~x_chroma_off_mask);");
      end_scope();
      statement("");
      statement("template<typename... Ts>");
      ...
      begin_scope();
      statement("return (ushort(loc) << y_chroma_off_base) | (build(t...) & "
                "~y_chroma_off_mask);");
      end_scope();
      statement("");
      statement("template<typename... Ts>");
      statement("static constexpr ushort build(spvYCbCrModelConversion model, "
                "Ts... t)");
      begin_scope();
      statement("return (ushort(model) << ycbcr_model_base) | (build(t...) & "
                "~ycbcr_model_mask);");
      end_scope();
      statement("");
      statement("template<typename... Ts>");
      ...
      begin_scope();
      statement("return (ushort(range) << ycbcr_range_base) | (build(t...) & "
                "~ycbcr_range_mask);");
      end_scope();
      statement("");
      statement("template<typename... Ts>");
      ...
      begin_scope();
      statement("return (ushort(bpc.value) << bpc_base) | (build(t...) & "
                "~bpc_mask);");
      end_scope();
      end_scope_decl();
      statement("");
      statement("// A class which can hold up to three textures and a sampler, "
                "including");
      statement(
      ...
      statement("");
      if (msl_options.swizzle_texture_samples) {
        statement("constexpr spvDynamicImageSampler(texture2d<T> tex, sampler "
                  "samp, uint sw) thread :");
      ...
      statement("constexpr spvDynamicImageSampler(texture2d<T> tex, sampler "
                "samp, spvYCbCrSampler ycbcr_samp, "
                "uint sw) thread :");
      statement("    plane0(tex), samp(samp), ycbcr_samp(ycbcr_samp), "
                "swizzle(sw) {}");
      statement("constexpr spvDynamicImageSampler(texture2d<T> plane0, "
                "texture2d<T> plane1,");
      statement("                                 sampler samp, "
                "spvYCbCrSampler ycbcr_samp, uint sw) thread :");
      statement("    plane0(plane0), plane1(plane1), samp(samp), "
                "ycbcr_samp(ycbcr_samp), swizzle(sw) {}");
      statement("constexpr spvDynamicImageSampler(texture2d<T> plane0, "
                "texture2d<T> plane1, texture2d<T> plane2,");
      statement("                                 sampler samp, "
                "spvYCbCrSampler ycbcr_samp, uint sw) thread :");
      statement("    plane0(plane0), plane1(plane1), plane2(plane2), "
                "samp(samp), ycbcr_samp(ycbcr_samp), "
                "swizzle(sw) {}");
      statement("");
      // XXX This is really hard to follow... I've left comments to make it a
      // bit easier.
      statement("template<typename... LodOptions>");
      statement("vec<T, 4> do_sample(float2 coord, LodOptions... options) "
                "const thread");
      begin_scope();
      ...
      statement("    return spvChromaReconstructNearest(plane0, plane1, "
                "plane2, samp, coord,");
      statement("                                       "
                "spvForward<LodOptions>(options)...);");
      statement("return spvChromaReconstructNearest(plane0, plane1, samp, "
      ...
      statement("return plane0.sample(samp, coord, "
                "spvForward<LodOptions>(options)...);");
      end_scope(); // do_sample()
      statement("template <typename... LodOptions>");
      statement(
      ...
      begin_scope();
      statement("vec<T, 4> s = spvTextureSwizzle(do_sample(coord, "
                "spvForward<LodOptions>(options)...), swizzle);");
      statement("if (ycbcr_samp.get_ycbcr_model() == "
                "spvYCbCrModelConversion::rgb_identity)");
      statement("    return s;");
      statement("");
      ...
      begin_scope();
      statement("case spvYCbCrModelConversion::rgb_identity:"); // Silence Clang
                                                                // warning
      ...
      begin_scope();
      if (msl_options.swizzle_texture_samples)
        statement("return spvTextureSwizzle(plane0.sample(samp, coord, "
                  "offset), swizzle);");
      else
        statement("return plane0.sample(samp, coord, offset);");
      end_scope();
      statement("template<typename lod_options>");
      statement("vec<T, 4> sample(float2 coord, lod_options options, int2 "
                "offset) const thread");
      begin_scope();
      if (msl_options.swizzle_texture_samples)
        statement("return spvTextureSwizzle(plane0.sample(samp, coord, "
                  "options, offset), swizzle);");
      else
        statement("return plane0.sample(samp, coord, options, offset);");
      end_scope();
      statement("#if __HAVE_MIN_LOD_CLAMP__");
      statement("vec<T, 4> sample(float2 coord, bias b, min_lod_clamp min_lod, "
                "int2 offset) const thread");
      begin_scope();
      statement("return plane0.sample(samp, coord, b, min_lod, offset);");
      end_scope();
      statement("vec<T, 4> sample(float2 coord, gradient2d grad, min_lod_clamp "
                "min_lod, int2 offset) const thread");
      begin_scope();
      ...
      end_scope();
      statement("");
      statement("vec<T, 4> gather(float2 coord, int2 offset = int2(0), "
                "component c = component::x) const thread");
      begin_scope();
      if (msl_options.swizzle_texture_samples)
        statement("return spvGatherSwizzle(plane0, samp, swizzle, c, coord, "
                  "offset);");
      else
...
static string inject_top_level_storage_qualifier(const string &expr,
                                                 const string &qualifier) {
  // Easier to do this through text munging since the qualifier does not exist
  // in the type system at all,
  ...
  if (last_significant == string::npos)
    ...
    return join(expr.substr(0, last_significant + 1), " ", qualifier,
                expr.substr(last_significant + 1, string::npos));
  }
}

// Undefined global memory is not allowed in MSL.
// Declare constant and init to zeros. Use {}, as global constructors can break
// Metal.
void CompilerMSL::declare_undefined_values() {
  ...
  ir.for_each_typed_id<SPIRUndef>([&](uint32_t, SPIRUndef &undef) {
    ...
    statement(
        inject_top_level_storage_qualifier(
            variable_decl(type, to_name(undef.self), undef.self), "constant"),
        " = {};");
    emitted = true;
  });
...
void CompilerMSL::declare_constant_arrays() {
  ...
  // MSL cannot declare arrays inline (except when declaring a variable), so we
  // must move them out to
  // global constants directly, so we are able to use constants as variable
  // expressions.
  bool emitted = false;
  ...
  ir.for_each_typed_id<SPIRConstant>([&](uint32_t, SPIRConstant &c) {
    ...
    // Constant arrays of non-primitive types (i.e. matrices) won't link
    // properly into Metal libraries.
    // FIXME: However, hoisting constants to main() means we need to pass down
    // constant arrays to leaf functions if they are used there.
    // If there are multiple functions in the module, drop this case to avoid
    // breaking use cases which do not need to
    // link into Metal libraries. This is hacky.
    if (!type.array.empty() &&
        (!fully_inlined || is_scalar(type) || is_vector(type))) {
      auto name = to_name(c.self);
      statement(inject_top_level_storage_qualifier(variable_decl(type, name),
                                                   "constant"),
...
void CompilerMSL::declare_complex_constant_arrays() {
  ...
  // declaring constant arrays of complex types. See
  // CompilerMSL::declare_constant_arrays().
  bool fully_inlined = ir.ids_for_type[TypeFunction].size() == 1;
  if (!fully_inlined)
    return;
...
void CompilerMSL::emit_resources() {
  ...
  // Emit the special [[stage_in]] and [[stage_out]] interface blocks which we
  // created.
...
void CompilerMSL::emit_specialization_constants_and_structs() {
  ...
  // It is possible that a struct may have to be placed at an alignment which
  // does not match the innate alignment of the struct itself.
  // In that case, if such a case exists for a struct, we must force that all
  // elements of the struct become packed_ types.
  // This makes the struct alignment as small as physically possible.
  // When we actually align the struct later, we can insert padding as necessary
  // to make the packed members behave like normally aligned types.
  ir.for_each_typed_id<SPIRType>([&](uint32_t type_id, const SPIRType &type) {
    if (type.basetype == SPIRType::Struct &&
        has_extended_decoration(type_id,
                                SPIRVCrossDecorationBufferBlockRepacked))
      mark_scalar_layout_structs(type);
  });

  bool builtin_block_type_is_required = false;
  // Very special case. If gl_PerVertex is initialized as an array
  // (tessellation)
  // we have to potentially emit the gl_PerVertex struct type so that we can
  // emit a constant LUT.
  ir.for_each_typed_id<SPIRConstant>([&](uint32_t, SPIRConstant &c) {
    auto &type = this->get<SPIRType>(c.constant_type);
    if (is_array(type) && has_decoration(type.self, DecorationBlock) &&
        is_builtin_type(type))
      builtin_block_type_is_required = true;
  });

  // Very particular use of the soft loop lock.
  // align_struct may need to create custom types on the fly, but we don't care
  // about
  // these types for purpose of iterating over them in ir.ids_for_type and
  // friends.
  ...
  for (auto &id_ : ir.ids_for_constant_or_type) {
    ...
    if (id.get_type() == TypeConstant) {
      ...
      if (c.self == workgroup_size_id) {
        // TODO: This can be expressed as a [[threads_per_threadgroup]] input
        // semantic, but we need to know
        // the work group size at compile time in SPIR-V, and
        // [[threads_per_threadgroup]] would need to be passed around as a
        // global.
        // The work group size may be a specialization constant.
        statement("constant uint3 ",
                  builtin_to_glsl(BuiltInWorkgroupSize, StorageClassWorkgroup),
                  " [[maybe_unused]] = ",
        ...
        // This "default" value can be overridden to the true specialization
        // constant by the API user.
        // Specialization constants which are used as array length expressions
        // cannot be function constants in MSL,
        // so just fall back to macros.
        if (msl_options.supports_msl_version(1, 2) &&
            has_decoration(c.self, DecorationSpecId) &&
          ...
          statement("constant ", sc_type_name, " ", sc_tmp_name,
                    " [[function_constant(", constant_id, ")]];");
          statement("constant ", sc_type_name, " ", sc_name,
                    " = is_function_constant_defined(", sc_tmp_name, ") ? ",
          ...
          statement("#define ", c.specialization_constant_macro_name, " ",
                    constant_expression(c));
          statement("#endif");
          statement("constant ", sc_type_name, " ", sc_name, " = ",
                    c.specialization_constant_macro_name, ";");
        } else {
          // Composite specialization constants must be built from other
      ...
      auto name = to_name(c.self);
      statement("constant ", variable_decl(type, name), " = ",
                constant_op_expression(c), ";");
      emitted = true;
    } else if (id.get_type() == TypeType) {
      // Output non-builtin interface structs. These include local function
      // structs
      // and structs nested within uniform and read-write buffers.
      auto &type = id.get<SPIRType>();
      TypeID type_id = type.self;

      bool is_struct = (type.basetype == SPIRType::Struct) &&
                       type.array.empty() && !type.pointer;
      bool is_block = has_decoration(type.self, DecorationBlock) ||
                      has_decoration(type.self, DecorationBufferBlock);

      bool is_builtin_block = is_block && is_builtin_type(type);
      bool is_declarable_struct =
          is_struct && (!is_builtin_block || builtin_block_type_is_required);
      ...
      if (patch_stage_out_var_id &&
          get_patch_stage_out_struct_type().self == type_id)
        is_declarable_struct = false;
      if (stage_in_var_id && get_stage_in_struct_type().self == type_id)
        is_declarable_struct = false;
      if (patch_stage_in_var_id &&
          get_patch_stage_in_struct_type().self == type_id)
        is_declarable_struct = false;

      // Special case. Declare builtin struct anyways if we need to emit a
      // threadgroup version of it.
      if (stage_out_masked_builtin_type_id == type_id)
        is_declarable_struct = true;

      // Align and emit declarable structs...but avoid declaring each more than
      // once.
      if (is_declarable_struct && declared_structs.count(type_id) == 0) {
        ...
        if (has_extended_decoration(type_id,
                                    SPIRVCrossDecorationBufferBlockRepacked))
          align_struct(type, aligned_structs);

        // Make sure we declare the underlying struct type, and not the
        // "decorated" type with pointers, etc.
...
void CompilerMSL::emit_binary_unord_op(uint32_t result_type, uint32_t result_id,
                                       uint32_t op0, uint32_t op1,
                                       const char *op) {
  bool forward = should_forward(op0) && should_forward(op1);
  emit_op(result_type, result_id,
          join("(isunordered(", to_enclosed_unpacked_expression(op0), ", ",
               to_enclosed_unpacked_expression(op1), ") || ",
               to_enclosed_unpacked_expression(op0), " ", op, " ",
               to_enclosed_unpacked_expression(op1), ")"),
          forward);
...
bool CompilerMSL::emit_tessellation_io_load(uint32_t result_type_id,
                                            uint32_t id, uint32_t ptr) {
  auto &ptr_type = expression_type(ptr);
  auto &result_type = get<SPIRType>(result_type_id);
  if (ptr_type.storage != StorageClassInput &&
      ptr_type.storage != StorageClassOutput)
    return false;
  if (ptr_type.storage == StorageClassOutput &&
      get_execution_model() == ExecutionModelTessellationEvaluation)
    return false;
  ...
  bool flat_data_type =
      flattened_io && (is_matrix(result_type) || is_array(result_type) ||
                       result_type.basetype == SPIRType::Struct);
  ...
  // Now, we must unflatten a composite type and take care of interleaving array
  // access with gl_in/gl_out.
  // Lots of painful code duplication since we *really* should not unroll these
  // kinds of loads in entry point fixup
  // unless we're forced to do this when the code is emitting inoptimal OpLoads.
  string expr;

  uint32_t interface_index =
      get_extended_decoration(ptr, SPIRVCrossDecorationInterfaceMemberIndex);
  ...
  if (!flattened_io) {
    ...
    uint32_t num_control_points = to_array_size_literal(
        result_type, uint32_t(result_type.array.size()) - 1);
    ...
    if (!ptr_is_io_variable)
      SPIRV_CROSS_THROW("Loading an array-of-array must be loaded directly "
                        "from an IO variable.");
    if (interface_index == uint32_t(-1))
    ...
    if (is_array_of_struct && !ptr_is_io_variable)
      SPIRV_CROSS_THROW("Loading array of struct from IO variable must come "
                        "directly from IO variable.");
    ...
    auto &struct_type = is_array_of_struct
                            ? get<SPIRType>(result_type.parent_type)
                            : result_type;
    ...
    for (uint32_t i = 0; i < num_control_points; i++) {
      ...
      for (uint32_t j = 0; j < uint32_t(struct_type.member_types.size()); j++) {
        ...
        if (var) {
          interface_index = get_extended_member_decoration(
              var->self, j, SPIRVCrossDecorationInterfaceMemberIndex);
        }
        ...
        if (is_matrix(mbr_type) && ptr_type.storage == StorageClassInput) {
          ...
          for (uint32_t k = 0; k < mbr_type.columns; k++, interface_index++) {
            if (is_array_of_struct) {
              ...
              expr += to_expression(ptr) + "." +
                      to_member_name(iface_type, interface_index);
    ...
    if (is_array_of_matrix && !ptr_is_io_variable)
      SPIRV_CROSS_THROW("Loading array of matrix from IO variable must come "
                        "directly from IO variable.");
    if (interface_index == uint32_t(-1))
      SPIRV_CROSS_THROW("Interface index is unknown. Cannot continue.");
...
bool CompilerMSL::emit_tessellation_access_chain(const uint32_t *ops,
                                                 uint32_t length) {
  // If this is a per-vertex output, remap it to the I/O array buffer.

  // Any object which did not go through IO flattening shenanigans will go there
  // instead.
  // We will unflatten on-demand instead as needed, but not all possible cases
  // can be supported, especially with arrays.
  ...
  if (var) {
    ...
    // Patch inputs are treated as normal block IO variables, so they don't deal
    // with this path at all.
    if (patch && (!is_block || var->storage == StorageClassInput))
      flat_data = false;

    // We might have a chained access chain, where
    // we first take the access chain to the control point, and then we chain
    // into a member or something similar.
  ...
  if (var && flat_data) {
    ...
    variable_is_flat =
        !builtin_variable || is_block || bi_type == BuiltInPosition ||
        bi_type == BuiltInPointSize || bi_type == BuiltInClipDistance ||
        bi_type == BuiltInCullDistance;
  }

  if (variable_is_flat) {
    // If output is masked, it is emitted as a "normal" variable, just go
    // through normal code paths.
    // Only check this for the first level of access chain.
    // Dealing with this for partial access chains should be possible, but
    // awkward.
    if (var->storage == StorageClassOutput && !ptr_is_chain) {
      bool masked = false;
      if (is_block) {
        uint32_t relevant_member_index = patch ? 3 : 4;
        // FIXME: This won't work properly if the application first access
        // chains into gl_out element,
        // then access chains into the member. Super weird, but theoretically
        // possible ...
        if (length > relevant_member_index) {
          uint32_t mbr_idx =
              get<SPIRConstant>(ops[relevant_member_index]).scalar();
    ...
    if (patch)
      stage_var_id = var->storage == StorageClassInput ? patch_stage_in_var_id
                                                       : patch_stage_out_var_id;
    else
      stage_var_id = var->storage == StorageClassInput ? stage_in_ptr_var_id
                                                       : stage_out_ptr_var_id;
    ...
    uint32_t index = get_extended_decoration(
        ops[2], SPIRVCrossDecorationInterfaceMemberIndex);

    // If we have a pointer chain expression, and we are no longer pointing to a
    // composite
    // object, we are in the clear. There is no longer a need to flatten
    // anything.
    bool further_access_chain_is_trivial = false;
    if (ptr_is_chain && flatten_composites) {
      auto &ptr_type = expression_type(ptr);
      if (!is_array(ptr_type) && !is_matrix(ptr_type) &&
          ptr_type.basetype != SPIRType::Struct)
    ...
    if (!further_access_chain_is_trivial && (flatten_composites || is_block)) {
      ...
      if (index == uint32_t(-1) && length >= (first_non_array_index + 1)) {
        ...
        index = get_extended_member_decoration(
            var->self, mbr_idx, SPIRVCrossDecorationInterfaceMemberIndex);
      ...
      // In this case, we're poking into flattened structures and arrays, so now
      // we have to
      ...
      for (; flatten_composites && i < length; ++i) {
        ...
        if (!c || c->specialization)
          SPIRV_CROSS_THROW("Trying to dynamically index into an array "
                            "interface variable in tessellation. "
                            "This is currently unsupported.");

        // We're in flattened space, so just increment the member index into IO
        // block.
        // We can only do this once in the current implementation, so either:
        // Struct, Matrix or 1-dimensional array for a control point.
        if (type->basetype == SPIRType::Struct &&
            var->storage == StorageClassOutput) {
          // Need to consider holes, since individual block members might be
          // masked away.
      ...
      // We're not going to emit the actual member name, we let any further
      // OpLoad take care of that.
      // Tag the access chain with the member index we're referencing.
      bool defer_access_chain =
          flatten_composites &&
          (is_matrix(result_ptr_type) || is_array(result_ptr_type) ||
      ...
      if (index != uint32_t(-1))
        set_extended_decoration(
            ops[1], SPIRVCrossDecorationInterfaceMemberIndex, index);
      else
        unset_extended_decoration(ops[1],
                                  SPIRVCrossDecorationInterfaceMemberIndex);
    } else {
      if (index != uint32_t(-1)) {
        set<SPIRConstant>(const_mbr_id, get_uint_type_id(), index, false);
    ...
    if (!ptr_is_chain) {
      // This is the start of an access chain, use ptr_chain to index into
      // control point array.
      e = access_chain(ptr, indices.data(), uint32_t(indices.size()),
                       result_ptr_type, &meta, !patch);
    } else {
      // If we're accessing a struct, we need to use member indices which are
      // based on the IO block,
      // not actual struct type, so we have to use a split access chain here
      // where
      // first path resolves the control point index, i.e. gl_in[index], and
      ...
      // For this case, we fall back to a natural access chain since we have
      // already dealt with remapping struct members.
      // One way to check this here is if we have 2 implied read expressions.
      // First one is the gl_in/gl_out struct itself, then an index into that
      // array.
      ...
      if (split_access_chain_formulation) {
        e = join(to_expression(ptr),
                 access_chain_internal(stage_var_id, indices.data(),
                                       uint32_t(indices.size()),
                                       ACCESS_CHAIN_CHAIN_ONLY_BIT, &meta));
      } else {
        e = access_chain_internal(ptr, indices.data(), uint32_t(indices.size()),
                                  0, &meta);
      }
    }

    // Get the actual type of the object that was accessed. If it's a vector
    // type and we changed it,
    // then we'll need to add a swizzle.
    // For this, we can't necessarily rely on the type of the base expression,
    // because it might be
    // another access chain, and it will therefore already have the "correct"
    // type.
    auto *expr_type = &get_variable_data_type(*var);
    if (has_extended_decoration(ops[2],
                                SPIRVCrossDecorationTessIOOriginalInputTypeID))
      expr_type = &get<SPIRType>(get_extended_decoration(
          ops[2], SPIRVCrossDecorationTessIOOriginalInputTypeID));
    for (uint32_t i = 3; i < length; i++) {
      if (!is_array(*expr_type) && expr_type->basetype == SPIRType::Struct)
        expr_type =
            &get<SPIRType>(
                expr_type->member_types[get<SPIRConstant>(ops[i]).scalar()]);
      else
        expr_type = &get<SPIRType>(expr_type->parent_type);
    }
    if (!is_array(*expr_type) && !is_matrix(*expr_type) &&
        expr_type->basetype != SPIRType::Struct &&
    ...
    if (meta.storage_physical_type != 0)
      set_extended_decoration(ops[1], SPIRVCrossDecorationPhysicalTypeID,
                              meta.storage_physical_type);
    if (meta.storage_is_invariant)
      set_decoration(ops[1], DecorationInvariant);
    // Save the type we found in case the result is used in another access
    // chain.
    set_extended_decoration(
        ops[1], SPIRVCrossDecorationTessIOOriginalInputTypeID, expr_type->self);

    // If we have some expression dependencies in our access chain, this access
    // chain is technically a forwarded
    // temporary which could be subject to invalidation.
    // Need to assume we're forwarded while calling
    // inherit_expression_depdendencies.
    forwarded_temporaries.insert(ops[1]);
    // The access chain itself is never forced to a temporary, but its
    // dependencies might.
    ...
    // If we have no dependencies after all, i.e., all indices in the access
    // chain are immutable temporaries,
...
bool CompilerMSL::is_out_of_bounds_tessellation_level(uint32_t id_lhs) {
  ...
  // In SPIR-V, TessLevelInner always has two elements and TessLevelOuter always
  // has
  // four. This is true even if we are tessellating triangles. This allows
  // clients
  // to use a single tessellation control shader with multiple tessellation
  // evaluation
  // shaders.
  // In Metal, however, only the first element of TessLevelInner and the first
  // three
  // of TessLevelOuter are accessible. This stems from how in Metal, the
  // tessellation
  // levels must be stored to a dedicated buffer in a particular format that
  // depends
...
void CompilerMSL::prepare_access_chain_for_scalar_access(
    std::string &expr, const SPIRType &type, spv::StorageClass storage,
    bool &is_packed) {
  ...
  // we must cast the access chain to a plain pointer to ensure we only access
  // the exact scalars we expect.
  // The MSL compiler refuses to allow component-level access for any non-packed
  // vector types.
  if (!is_packed && (storage == StorageClassStorageBuffer ||
                     storage == StorageClassWorkgroup)) {
    const char *addr_space =
        storage == StorageClassWorkgroup ? "threadgroup" : "device";
    expr = join("((", addr_space, " ", type_to_glsl(type), "*)&",
                enclose_expression(expr), ")");

    // Further indexing should happen with packed rules (array index, not
    // swizzle).
    is_packed = true;
  }
}

bool CompilerMSL::access_chain_needs_stage_io_builtin_translation(
    uint32_t base) {
  auto *var = maybe_get_backing_variable(base);
  if (!var || !is_tessellation_shader())
    return true;

  // We only need to rewrite builtin access chains when accessing flattened
  // builtins like gl_ClipDistance_N.
  // Avoid overriding it back to just gl_ClipDistance.
  // This can only happen in scenarios where we cannot flatten/unflatten access
  // chains, so, the only case
...
// Sets the interface member index for an access chain to a pull-model
// interpolant.
void CompilerMSL::fix_up_interpolant_access_chain(const uint32_t *ops,
                                                  uint32_t length) {
  auto *var = maybe_get_backing_variable(ops[2]);
  ...
  if (has_extended_decoration(ops[2],
                              SPIRVCrossDecorationInterfaceMemberIndex)) {
    interface_index = get_extended_decoration(
        ops[2], SPIRVCrossDecorationInterfaceMemberIndex);
  } else {
    ...
    interface_index = get_extended_member_decoration(
        var->self, c.scalar(), SPIRVCrossDecorationInterfaceMemberIndex);
  }
  // Accumulate indices. We'll have to skip over the one for the struct, if
  // present, because we already accounted
  ...
  for (uint32_t i = 3; i < length; ++i) {
    if (is_vector(*type) && !is_array(*type) && is_scalar(result_type)) {
      ...
      set_extended_decoration(
          ops[1], SPIRVCrossDecorationInterpolantComponentExpr, ops[i]);
      break;
    }

    auto *c = maybe_get<SPIRConstant>(ops[i]);
    if (!c || c->specialization)
      SPIRV_CROSS_THROW("Trying to dynamically index into an array interface "
                        "variable using pull-model "
  ...
  // Save this to the access chain itself so we can recover it later when
  // calling an interpolation function.
  set_extended_decoration(ops[1], SPIRVCrossDecorationInterfaceMemberIndex,
                          interface_index);
}
...
#define MSL_QFOP(op)                                                           \
  emit_quaternary_func_op(ops[0], ops[1], ops[2], ops[3], ops[4], ops[5], #op)
#define MSL_TFOP(op)                                                           \
  emit_trinary_func_op(ops[0], ops[1], ops[2], ops[3], ops[4], #op)
#define MSL_BFOP(op) emit_binary_func_op(ops[0], ops[1], ops[2], ops[3], #op)
#define MSL_BFOP_CAST(op, type)                                                \
  emit_binary_func_op_cast(ops[0], ops[1], ops[2], ops[3], #op, type,          \
                           opcode_is_sign_invariant(opcode))
#define MSL_UFOP(op) emit_unary_func_op(ops[0], ops[1], ops[2], #op)
#define MSL_UNORD_BOP(op)                                                      \
  emit_binary_unord_op(ops[0], ops[1], ops[2], ops[3], #op)

  auto ops = stream(instruction);
  auto opcode = static_cast<Op>(instruction.op);

  // If we need to do implicit bitcasts, make sure we do it with the correct
  // type.
  ...
  case OpBitFieldSExtract: {
    emit_trinary_func_op_bitextract(ops[0], ops[1], ops[2], ops[3], ops[4],
                                    "extract_bits", int_type, int_type,
  ...
  case OpBitFieldUExtract: {
    emit_trinary_func_op_bitextract(ops[0], ops[1], ops[2], ops[3], ops[4],
                                    "extract_bits", uint_type, uint_type,
                                    SPIRType::UInt, SPIRType::UInt);
    break;
  }

  case OpBitReverse:
    // BitReverse does not have issues with sign since result type must match
    // input type.
  ...
  case OpBitCount: {
    ...
    emit_unary_func_op_cast(ops[0], ops[1], ops[2], "popcount", basetype,
                            basetype);
    break;
  }
  ...
  case OpFMul:
    if (msl_options.invariant_float_math ||
        has_decoration(ops[1], DecorationNoContraction))
      MSL_BFOP(spvFMul);
    else
      MSL_BOP(*);
    break;
  ...
  case OpAtomicExchange: {
    ...
    uint32_t val = ops[5];
    emit_atomic_func_op(result_type, id, "atomic_exchange_explicit", mem_sem,
                        mem_sem, false, ptr, val);
    break;
  }
  ...
  case OpAtomicCompareExchange: {
    ...
    uint32_t val = ops[6];
    uint32_t comp = ops[7];
    emit_atomic_func_op(result_type, id,
                        "atomic_compare_exchange_weak_explicit", mem_sem_pass,
                        mem_sem_fail, true, ptr, comp, true, false, val);
    break;
  }
  ...
  case OpAtomicLoad: {
    ...
    uint32_t ptr = ops[2];
    uint32_t mem_sem = ops[4];
    emit_atomic_func_op(result_type, id, "atomic_load_explicit", mem_sem,
                        mem_sem, false, ptr, 0);
    break;
  }
  ...
  case OpAtomicStore: {
    ...
    uint32_t val = ops[3];
    emit_atomic_func_op(result_type, id, "atomic_store_explicit", mem_sem,
                        mem_sem, false, ptr, val);
    break;
  }
...
#define MSL_AFMO_IMPL(op, valsrc, valconst)                                    \
  do {                                                                         \
    ...
    uint32_t mem_sem = ops[4];                                                 \
    uint32_t val = valsrc;                                                     \
    emit_atomic_func_op(result_type, id, "atomic_fetch_" #op "_explicit",      \
                        mem_sem, mem_sem, false, ptr, val, false, valconst);   \
  } while (false)
...
#define MSL_AFMIO(op) MSL_AFMO_IMPL(op, 1, true)
  ...
  case OpImageTexelPointer: {
    ...
    if (var && atomic_image_vars.count(var->self)) {
      ...
      if (type.image.dim == Dim2D) {
        coord = join("spvImage2DAtomicCoord(", coord, ", ",
                     to_expression(ops[2]), ")");
      }

      auto &e = set<SPIRExpression>(
          id, join(to_expression(ops[2]), "_atomic[", coord, "]"), result_type,
          true);
      ...
      uint32_t id = ops[1];
      auto &e = set<SPIRExpression>(
          id, join(to_expression(ops[2]), ", ", to_expression(ops[3])),
          result_type, true);
  ...
  case OpImageQueryLod: {
    ...
    auto image_expr =
        combined ? to_expression(combined->image) : to_expression(image_id);
    ...
    statement(to_expression(id), ".x = ", image_expr, ".calculate_clamped_lod(",
              sampler_expr, ", ", to_expression(coord_id), ");");
    statement(to_expression(id), ".y = ", image_expr,
              ".calculate_unclamped_lod(", sampler_expr, ", ",
...
#define MSL_ImgQry(qrytype)                                                    \
  do {                                                                         \
    ...
    string img_exp = to_expression(img_id);                                    \
    string expr =                                                              \
        type_to_glsl(rslt_type) + "(" + img_exp + ".get_num_" #qrytype "())";  \
    emit_op(rslt_type_id, id, expr, should_forward(img_id));                   \
  } while (false)
  ...
  case OpImage: {
    ...
    if (combined) {
      ...
      auto *var = maybe_get_backing_variable(ops[2]);
      SPIRExpression *e;
      if (var && has_extended_decoration(
                     var->self, SPIRVCrossDecorationDynamicImageSampler))
        e = &emit_op(result_type, id, join(to_expression(ops[2]), ".plane0"),
                     true, true);
      else
  ...
  case OpMatrixTimesMatrix: {
    ...
    // If both matrices need transpose, we can multiply in flipped order and tag
    // the expression as transposed.
    // a^T * b^T = (b * a)^T.
  ...
  case OpISubBorrow: {
    ...
    if (opcode == OpIAddCarry) {
      statement(to_expression(result_id), ".", to_member_name(type, 0), " = ",
                to_enclosed_unpacked_expression(op0), " + ",
                to_enclosed_unpacked_expression(op1), ";");
      statement(to_expression(result_id), ".", to_member_name(type, 1),
                " = select(", type_to_glsl(res_type), "(1), ",
                type_to_glsl(res_type), "(0), ",
                to_unpacked_expression(result_id), ".", to_member_name(type, 0),
                " >= max(", to_unpacked_expression(op0), ", ",
  ...
  case OpSMulExtended: {
    ...
    statement(to_expression(result_id), ".", to_member_name(type, 0), " = ",
              ...
              to_enclosed_unpacked_expression(op1), ";");
    statement(to_expression(result_id), ".", to_member_name(type, 1),
              " = mulhi(", to_unpacked_expression(op0), ", ",
              to_unpacked_expression(op1), ");");
    break;
  }
  ...
  case OpArrayLength: {
    ...
    auto expr = join("(", to_buffer_size_expression(ops[2]), " - ", offset,
                     ") / ", stride);
  ...
  case OpIMul32x16INTEL: {
    ...
    emit_op(result_type, id,
            join("int(short(", to_unpacked_expression(a), ")) * int(short(",
  ...
  case OpUMul32x16INTEL: {
    ...
    bool forward = should_forward(a) && should_forward(b);
    emit_op(result_type, id,
            join("uint(ushort(", to_unpacked_expression(a), ")) * uint(ushort(",
                 to_unpacked_expression(b), "))"),
            forward);
    inherit_expression_dependencies(id, a);
    inherit_expression_dependencies(id, b);
  ...
  case OpDemoteToHelperInvocationEXT:
    if (!msl_options.supports_msl_version(2, 3))
      SPIRV_CROSS_THROW("discard_fragment() does not formally have demote "
                        "semantics until MSL 2.3.");
    CompilerGLSL::emit_instruction(instruction);
    break;
  ...
  case OpRayQueryInitializeKHR: {
    ...
    statement(to_expression(ops[0]), ".reset(", "ray(", to_expression(ops[4]),
              ", ", to_expression(ops[6]), ", ", to_expression(ops[5]), ", ",
              to_expression(ops[7]), "), ", to_expression(ops[1]),
              ", intersection_params());");
    break;
  }
...
#define MSL_RAY_QUERY_GET_OP(op, msl_op)                                       \
  case OpRayQueryGet##op##KHR:                                                 \
    ...
    emit_op(ops[0], ops[1], join(to_expression(ops[2]), ".get_" #msl_op "()"), \
            false);                                                            \
    break
...
#define MSL_RAY_QUERY_OP_INNER2(op, msl_prefix, msl_op)                        \
  case OpRayQueryGet##op##KHR:                                                 \
    ...
    if (MSL_RAY_QUERY_IS_CANDIDATE)                                            \
      emit_op(ops[0], ops[1], join(to_expression(ops[2]),                      \
                                   #msl_prefix "_candidate_" #msl_op "()"),    \
              false);                                                          \
    else                                                                       \
      emit_op(ops[0], ops[1], join(to_expression(ops[2]),                      \
                                   #msl_prefix "_committed_" #msl_op "()"),    \
              false);                                                          \
    break

#define MSL_RAY_QUERY_GET_OP2(op, msl_op)                                      \
  MSL_RAY_QUERY_OP_INNER2(op, .get, msl_op)
#define MSL_RAY_QUERY_IS_OP2(op, msl_op)                                       \
  MSL_RAY_QUERY_OP_INNER2(op, .is, msl_op)
  ...
  case OpRayQueryGetIntersectionTypeKHR:
    flush_variable_declaration(ops[2]);
    if (MSL_RAY_QUERY_IS_CANDIDATE)
      emit_op(ops[0], ops[1], join("uint(", to_expression(ops[2]),
  ...
  case OpRayQueryGetIntersectionTKHR:
    ...
    if (MSL_RAY_QUERY_IS_CANDIDATE)
      emit_op(ops[0], ops[1],
              join(to_expression(ops[2]), ".get_candidate_triangle_distance()"),
              false);
    else
      emit_op(ops[0], ops[1],
              join(to_expression(ops[2]), ".get_committed_distance()"), false);
    break;
  ...
  case OpRayQueryGenerateIntersectionKHR:
    flush_variable_declaration(ops[0]);
    statement(to_expression(ops[0]), ".commit_bounding_box_intersection(",
              to_expression(ops[1]), ");");
    break;
...
void CompilerMSL::emit_barrier(uint32_t id_exe_scope, uint32_t id_mem_scope,
                               uint32_t id_mem_sem) {
  if (get_execution_model() != ExecutionModelGLCompute &&
      get_execution_model() != ExecutionModelTessellationControl)
    return;

  uint32_t exe_scope = id_exe_scope ? evaluate_constant_u32(id_exe_scope)
                                    : uint32_t(ScopeInvocation);
  uint32_t mem_scope = id_mem_scope ? evaluate_constant_u32(id_mem_scope)
                                    : uint32_t(ScopeInvocation);
  // Use the wider of the two scopes (smaller value)
  exe_scope = min(exe_scope, mem_scope);

  if (msl_options.emulate_subgroups && exe_scope >= ScopeSubgroup &&
      !id_mem_sem)
    // In this case, we assume a "subgroup" size of 1. The barrier, then, is a
    // noop.
    return;

  string bar_stmt;
  if ((msl_options.is_ios() && msl_options.supports_msl_version(1, 2)) ||
      msl_options.supports_msl_version(2))
    bar_stmt =
        exe_scope < ScopeSubgroup ? "threadgroup_barrier" : "simdgroup_barrier";
  else
    bar_stmt = "threadgroup_barrier";
  bar_stmt += "(";

  uint32_t mem_sem = id_mem_sem ? evaluate_constant_u32(id_mem_sem)
                                : uint32_t(MemorySemanticsMaskNone);

  // Use the | operator to combine flags if we can.
  if (msl_options.supports_msl_version(1, 2)) {
    string mem_flags = "";
    // For tesc shaders, this also affects objects in the Output storage class.
    // Since in Metal, these are placed in a device buffer, we have to sync
    // device memory here.
    if (get_execution_model() == ExecutionModelTessellationControl ||
    ...
    if (get_execution_model() == ExecutionModelTessellationControl ||
        (mem_sem & (MemorySemanticsSubgroupMemoryMask |
    ...
    if ((mem_sem & (MemorySemanticsUniformMemoryMask |
                    MemorySemanticsCrossWorkgroupMemoryMask)) &&
        (mem_sem & (MemorySemanticsSubgroupMemoryMask |
                    MemorySemanticsWorkgroupMemoryMask)))
      bar_stmt += "mem_flags::mem_device_and_threadgroup";
    else if (mem_sem & (MemorySemanticsUniformMemoryMask |
                        MemorySemanticsCrossWorkgroupMemoryMask))
      bar_stmt += "mem_flags::mem_device";
    else if (mem_sem & (MemorySemanticsSubgroupMemoryMask |
                        MemorySemanticsWorkgroupMemoryMask))
...
void CompilerMSL::emit_array_copy(const string &lhs, uint32_t lhs_id,
                                  uint32_t rhs_id, StorageClass lhs_storage,
                                  StorageClass rhs_storage) {
  // Allow Metal to use the array<T> template to make arrays a value type.
  // This, however, cannot be used for threadgroup address specifiers, so
  // consider the custom array copy as fallback.
  ...
  // If the variable is actually backed by non-user visible device storage, we
  // use array templates for those.
  //
  // Another special consideration is given to thread local variables which
  // happen to have Offset decorations
  // applied to them. Block-like types do not use array templates, so we need to
  // force POD path if we detect
  // these scenarios. This check isn't perfect since it would be technically
  // possible to mix and match these things,
  // and for a fully correct solution we might have to track array template
  // state through access chains as well,
  // but for all reasonable use cases, this should suffice.
  // This special case should also only apply to Function/Private storage
  // classes.
  // We should not check backing variable for temporaries.
  auto *lhs_var = maybe_get_backing_variable(lhs_id);
  if (lhs_var && lhs_storage == StorageClassStorageBuffer &&
      storage_class_array_is_thread(lhs_var->storage))
    lhs_is_array_template = true;
  else if (lhs_var && (lhs_storage == StorageClassFunction ||
  ...
  if (rhs_var && rhs_storage == StorageClassStorageBuffer &&
      storage_class_array_is_thread(rhs_var->storage))
    rhs_is_array_template = true;
  else if (rhs_var && (rhs_storage == StorageClassFunction ||
  ...
  // Avoid spvCopy* wrapper functions; Otherwise, spvUnsafeArray<> template
  // cannot be used with that storage qualifier.
  if (lhs_is_array_template && rhs_is_array_template &&
      !using_builtin_array()) {
    ...
    if (type.array.size() > 1) {
      ...
      auto func = static_cast<SPVFuncImpl>(SPVFuncImplArrayCopyMultidimBase +
                                           type.array.size());
    ...
    if (lhs_is_thread_storage && is_constant)
      ...
      tag = "FromThreadGroupToStack";
             ...
             rhs_storage == StorageClassWorkgroup)
      tag = "FromThreadGroupToThreadGroup";
    else if (lhs_storage == StorageClassStorageBuffer &&
             rhs_storage == StorageClassStorageBuffer)
    ...
    if (lhs_is_array_template && rhs_is_array_template &&
      ...
      statement("spvArrayCopy", tag, type.array.size(), "(", lhs, ".elements, ",
                to_expression(rhs_id), ".elements);");
    if (lhs_is_array_template && !msl_options.force_native_arrays)
      statement("spvArrayCopy", tag, type.array.size(), "(", lhs, ".elements, ",
                to_expression(rhs_id), ");");
    else if (rhs_is_array_template && !msl_options.force_native_arrays)
      statement("spvArrayCopy", tag, type.array.size(), "(", lhs, ", ",
                to_expression(rhs_id), ".elements);");
    else
      statement("spvArrayCopy", tag, type.array.size(), "(", lhs, ", ",
                to_expression(rhs_id), ");");
...
uint32_t
CompilerMSL::get_physical_tess_level_array_size(spv::BuiltIn builtin) const {
...
bool CompilerMSL::maybe_emit_array_assignment(uint32_t id_lhs,
                                              uint32_t id_rhs) {
  ...
  if (ir.ids[id_rhs].get_type() == TypeConstant && var &&
      var->deferred_declaration) {
    // Special case, if we end up declaring a variable when assigning the
    // constant array,
    // we can avoid the copy by directly assigning the constant expression.
    // This is likely necessary to be able to use a variable as a true look-up
    // table, as it is unlikely
    // the compiler will be able to optimize the spvArrayCopy() into a constant
    // LUT.
    // After a variable has been declared, we can no longer assign constant
    // arrays in MSL unfortunately.
    statement(to_expression(id_lhs), " = ",
              constant_expression(get<SPIRConstant>(id_rhs)), ";");
    return true;
  }
  ...
  if (get_execution_model() == ExecutionModelTessellationControl &&
    ...
    if (builtin == BuiltInTessLevelInner || builtin == BuiltInTessLevelOuter) {
      ...
      if (array_size == 1)
        statement(to_expression(id_lhs), " = half(", to_expression(id_rhs),
                  "[0]);");
      else {
        for (uint32_t i = 0; i < array_size; i++)
          statement(to_expression(id_lhs), "[", i, "] = half(",
                    to_expression(id_rhs), "[", i, "]);");
      }
      return true;
    }
  }
  ...
  emit_array_copy(to_expression(id_lhs), id_lhs, id_rhs, lhs_storage,
                  rhs_storage);
...
// Emits one of the atomic functions. In MSL, the atomic functions operate on
// pointers
void CompilerMSL::emit_atomic_func_op(uint32_t result_type, uint32_t result_id,
                                      const char *op, uint32_t mem_order_1,
                                      uint32_t mem_order_2,
                                      bool has_mem_order_2, uint32_t obj,
                                      uint32_t op1, bool op1_is_pointer,
                                      bool op1_is_literal, uint32_t op2) {
  ...
  if (res_type.storage == StorageClassUniformConstant &&
      res_type.basetype == SPIRType::Image) {
    exp += "device";
  } else {
  ...
  if (is_atomic_compare_exchange_strong) {
    ...
    // MSL only supports the weak atomic compare exchange, so emit a CAS loop
    // here.
    // The MSL function returns false if the atomic write fails OR the
    // comparison test fails,
    // so we must validate that it wasn't the comparison test that failed before
    // continuing
    // the CAS loop, otherwise it will loop infinitely, with the comparison test
    // always failing.
    // The function updates the comparitor value from the memory value, so the
    // additional
    ...
    end_scope_decl(join("while (!", exp, " && ", to_name(result_id), " == ",
...
// In some cases, deliberately select either the fast or precise versions of the
// MSL functions to match Vulkan math precision results.
void CompilerMSL::emit_glsl_op(uint32_t result_type, uint32_t id, uint32_t eop,
  ...
  uint32_t integer_width =
      get_integer_width_for_glsl_instruction(op, args, count);
  auto int_type = to_signed_basetype(integer_width);
  auto uint_type = to_unsigned_basetype(integer_width);
  ...
  case GLSLstd450FindILsb: {
    ...
    emit_unary_func_op_cast(result_type, id, args[0], "spvFindLSB", basetype,
                            basetype);
    break;
  }

  case GLSLstd450FindSMsb:
    emit_unary_func_op_cast(result_type, id, args[0], "spvFindSMSB", int_type,
                            int_type);
    break;

  case GLSLstd450FindUMsb:
    emit_unary_func_op_cast(result_type, id, args[0], "spvFindUMSB", uint_type,
                            uint_type);
    break;
  ...
  case GLSLstd450PackDouble2x32:
    emit_unary_func_op(
        result_type, id, args[0],
        "unsupported_GLSLstd450PackDouble2x32"); // Currently unsupported
    break;
  case GLSLstd450UnpackDouble2x32:
    emit_unary_func_op(
        result_type, id, args[0],
        "unsupported_GLSLstd450UnpackDouble2x32"); // Currently unsupported
    break;
  ...
  case GLSLstd450InterpolateAtCentroid: {
    // We can't just emit the expression normally, because the qualified name
    // contains a call to the default
    // interpolate method, or refers to a local variable. We saved the interface
    // index we need; use it to construct
    // the base for the method call.
    uint32_t interface_index = get_extended_decoration(
        args[0], SPIRVCrossDecorationInterfaceMemberIndex);
    string component;
    if (has_extended_decoration(args[0],
                                SPIRVCrossDecorationInterpolantComponentExpr)) {
      uint32_t index_expr = get_extended_decoration(
          args[0], SPIRVCrossDecorationInterpolantComponentExpr);
    ...
    emit_op(result_type, id,
            join(to_name(stage_in_var_id), ".",
                 to_member_name(get_stage_in_struct_type(), interface_index),
  ...
  case GLSLstd450InterpolateAtSample: {
    ...
    emit_op(result_type, id,
            join(to_name(stage_in_var_id), ".",
                 to_member_name(get_stage_in_struct_type(), interface_index),
                 ".interpolate_at_sample(", to_expression(args[1]), ")",
                 component),
  ...
  case GLSLstd450InterpolateAtOffset: {
    ...
    // Like Direct3D, Metal puts the (0, 0) at the upper-left corner, not the
    // center as SPIR-V and GLSL do.
    // Offset the offset by (1/2 - 1/16), or 0.4375, to compensate for this.
    // It has to be (1/2 - 1/16) and not 1/2, or several CTS tests subtly break
    // on Intel.
    emit_op(result_type, id,
            join(to_name(stage_in_var_id), ".",
                 to_member_name(get_stage_in_struct_type(), interface_index),
                 ".interpolate_at_offset(", to_expression(args[1]),
                 " + 0.4375)", component),
            should_forward(args[0]) && should_forward(args[1]));
    break;
  }
  ...
  case GLSLstd450Modf:
  case GLSLstd450Frexp: {
    // Special case. If the variable is a scalar access chain, we cannot use it
    // directly. We have to emit a temporary.
    // Another special case is if the variable is in a storage class which is
    // not thread.
    ...
    if (!is_thread_storage ||
      ...
      // We cannot directly take the reference of a vector swizzle in MSL, even
      // if it's scalar ...
      ...
      emit_binary_func_op(result_type, id, args[0], tmp_id,
                          eop == GLSLstd450Modf ? "modf" : "frexp");
...
void CompilerMSL::emit_spv_amd_shader_trinary_minmax_op(uint32_t result_type,
                                                        uint32_t id,
                                                        uint32_t eop,
                                                        const uint32_t *args,
                                                        uint32_t count) {
  ...
  case SMid3AMD:
    ...
    break;
  default:
    CompilerGLSL::emit_spv_amd_shader_trinary_minmax_op(result_type, id, eop,
                                                        args, count);
    break;
  }
}
...
// If this is the entry point function, Metal-specific return value and function
// arguments are added.
void CompilerMSL::emit_function_prototype(SPIRFunction &func, const Bitset &) {
  ...
  // Metal helper functions must be static force-inline otherwise they will
  // cause problems when linked together in a single Metallib.
  ...
  if (!type.array.empty() && msl_options.force_native_arrays) {
    // We cannot return native arrays in MSL, so "return" through an out
    // variable.
    decl += "void";
  } else {
  ...
  if (processing_entry_point) {
    ...
    for (auto var_id : vars_needing_early_declaration) {
      ...
      if (ir.ids[initializer].get_type() == TypeNone ||
          ir.ids[initializer].get_type() == TypeExpression)
  ...
  for (auto &arg : func.arguments) {
    ...
    if (var) {
      // If we need to modify the name of the variable, make sure we modify the
      // original variable.
      // Our alias is just a shadow variable.
      if (arg.alias_global_variable && var->basevariable)
        name_id = var->basevariable;

      var->parameter = &arg; // Hold a pointer to the parameter so we can
                             // invalidate the readonly field if needed.
    }
    ...
    bool is_dynamic_img_sampler = has_extended_decoration(
        arg.id, SPIRVCrossDecorationDynamicImageSampler);
...
static bool
needs_chroma_reconstruction(const MSLConstexprSampler *constexpr_sampler) {
  // For now, only multiplanar images need explicit reconstruction. GBGR and
  // BGRG images
  // use implicit reconstruction.
  return constexpr_sampler && constexpr_sampler->ycbcr_conversion_enable &&
         constexpr_sampler->planes > 1;
}

// Returns the texture sampling function string for the specified image and
...
string CompilerMSL::to_function_name(const TextureFunctionNameArguments &args) {
  ...
  if (auto *var = maybe_get_backing_variable(img)) {
    constexpr_sampler = find_constexpr_sampler(
        var->basevariable ? var->basevariable : VariableID(var->self));
    is_dynamic_img_sampler = has_extended_decoration(
        var->self, SPIRVCrossDecorationDynamicImageSampler);
  }

  // Special-case gather. We have to alter the component being looked up
  // in the swizzle case.
  if (msl_options.swizzle_texture_samples && args.base.is_gather &&
      !is_dynamic_img_sampler &&
      (!constexpr_sampler || !constexpr_sampler->ycbcr_conversion_enable)) {
    bool is_compare = comparison_ids.count(img);
    add_spv_func_and_recompile(is_compare ? SPVFuncImplGatherCompareSwizzle
...
static inline bool sampling_type_needs_f32_conversion(const SPIRType &type) {
  // Double is not supported to begin with, but doesn't hurt to check for
  // completion.
  return type.basetype == SPIRType::Half || type.basetype == SPIRType::Double;
}

// Returns the function args for a texture sampling function for the specified
// image and sampling characteristics.
string CompilerMSL::to_function_args(const TextureFunctionArguments &args,
                                     bool *p_forward) {
  VariableID img = args.base.img;
  ...
  if (!is_dynamic_img_sampler) {
    ...
    if (needs_chroma_reconstruction(constexpr_sampler)) {
      ...
      for (uint32_t i = 1; i < constexpr_sampler->planes; i++)
        farg_str += join(", ", to_expression(img), plane_name_suffix, i);
                ...
                !constexpr_sampler->ycbcr_conversion_enable) &&
    ...
    // Swizzled gather puts the component before the other args, to allow
    // template
    // deduction to work.
  ...
  case DimBuffer:
    ...
    if (msl_options.texture_buffer_native) {
      ...
      if (args.base.is_fetch) {
        if (msl_options.texel_buffer_texture_width > 0) {
          tex_coords = "spvTexelBufferCoord(" +
                       round_fp_tex_coords(tex_coords, coord_is_fp) + ")";
        } else {
          tex_coords = "spvTexelBufferCoord(" +
                       round_fp_tex_coords(tex_coords, coord_is_fp) + ", " +
                       to_expression(img) + ")";
  ...
  case Dim2D:
    ...
    if (args.base.is_fetch)
      tex_coords =
          "uint2(" + round_fp_tex_coords(tex_coords, coord_is_fp) + ")";
    else if (sampling_type_needs_f32_conversion(coord_type))
  ...
  case Dim3D:
    ...
    if (args.base.is_fetch)
      tex_coords =
          "uint3(" + round_fp_tex_coords(tex_coords, coord_is_fp) + ")";
    else if (sampling_type_needs_f32_conversion(coord_type))
  ...
  if (args.base.is_fetch && (args.offset || args.coffset)) {
    ...
    if (imgtype.image.dim == Dim1D && msl_options.texture_1D_as_2D) {
      if (type.basetype != SPIRType::UInt)
        tex_coords +=
            join(" + uint2(", bitcast_expression(SPIRType::UInt, offset_expr),
                 ", 0)");
      else
        tex_coords +=
            join(" + uint2(", to_enclosed_expression(offset_expr), ", 0)");
    } else {
      if (type.basetype != SPIRType::UInt)
  ...
  if (args.base.is_proj) {
    if (sampling_type_needs_f32_conversion(coord_type))
      tex_coords += " / " + convert_to_f32(to_extract_component_expression(
                                               args.coord, alt_coord_component),
                                           1);
    else
      tex_coords += " / " + to_extract_component_expression(
                                args.coord, alt_coord_component);
  }
  ...
  if (imgtype.image.dim == DimCube && imgtype.image.arrayed &&
      msl_options.emulate_cube_array) {
    farg_str += "spvCubemapTo2DArrayFace(" + tex_coords + ").xy";

    if (is_cube_fetch)
      farg_str +=
          ", uint(" + to_extract_component_expression(args.coord, 2) + ")";
    else
      farg_str +=
    ...
    if (is_cube_fetch) {
      // Special case for cube arrays, face and layer are packed in one
      // dimension.
      if (imgtype.image.arrayed)
        ...
        farg_str +=
            ", uint(" +
            round_fp_tex_coords(to_extract_component_expression(args.coord, 2),
                                coord_is_fp) +
            ")";
    }
    ...
    if (imgtype.image.arrayed) {
      ...
      if (imgtype.image.dim == DimCube && args.base.is_fetch) {
        ...
        farg_str += ", uint(" +
                    round_fp_tex_coords(to_extract_component_expression(
                                            args.coord, alt_coord_component),
                                        coord_is_fp) +
                    ")";
        if (imgtype.image.dim == DimSubpassData) {
  ...
  if (args.dref) {
    ...
    if (args.base.is_proj)
      dref_expr = join(
          to_enclosed_expression(args.dref), " / ",
          to_extract_component_expression(args.coord, alt_coord_component));
    else
    ...
    if (msl_options.is_macos() && (grad_x || grad_y)) {
      // For sample compare, MSL does not support gradient2d for all targets
      // (only iOS apparently according to docs).
      // However, the most common case here is to have a constant gradient of 0,
      // as that is the only way to express
      // LOD == 0 in GLSL with sampler2DArrayShadow (cascaded shadow mapping).
      // We will detect a compile-time constant 0 value for gradient and promote
      // that to level(0) on MSL.
      ...
      if (constant_zero_x && constant_zero_y) {
        ...
        SPIRV_CROSS_THROW("Using non-constant 0.0 gradient() qualifier for "
                          "sample_compare. This is not "
    ...
    if (msl_options.is_macos() && bias) {
      ...
      if (expression_is_constant_null(bias)) {
        ...
        SPIRV_CROSS_THROW("Using non-constant 0.0 bias() qualifier for "
                          "sample_compare. This is not supported "
  ...
  } else if (args.base.is_fetch && !lod &&
             (imgtype.image.dim != Dim1D || msl_options.texture_1D_as_2D) &&
             imgtype.image.dim != DimBuffer && !imgtype.image.ms &&
             imgtype.image.sampled != 2) {
    // Lod argument is optional in OpImageFetch, but we require a LOD value,
    // pick 0 as the default.
    // Check for sampled type as well, because is_fetch is also used for
    // OpImageRead in MSL.
    farg_str += ", 0";
  }
  ...
  if ((grad_x || grad_y) &&
    ...
    farg_str += ", gradient" + grad_opt + "(" + to_expression(grad_x) + ", " +
                to_expression(grad_y) + ")";
  }
  ...
  if (args.component) {
    ...
    if (!msl_options.swizzle_texture_samples || is_dynamic_img_sampler) {
      ...
      if (image_var == 0 ||
          !is_depth_image(expression_type(image_var), image_var))
        farg_str += ", " + to_component_argument(args.component);
    }
  }
...
// If the texture coordinates are floating point, invokes MSL round() function
// to round them.
...
string CompilerMSL::to_component_argument(uint32_t id) {
  ...
  default:
    SPIRV_CROSS_THROW(
        "The value (" + to_string(component_index) + ") of OpConstant ID " +
        to_string(id) +
...
void CompilerMSL::emit_sampled_image_op(uint32_t result_type,
                                        uint32_t result_id, uint32_t image_id,
                                        uint32_t samp_id) {
  set<SPIRCombinedImageSampler>(result_id, result_type, image_id, samp_id);
}

string
CompilerMSL::to_texture_op(const Instruction &i, bool sparse, bool *forward,
  ...
  if (constexpr_sampler && constexpr_sampler->ycbcr_conversion_enable &&
    ...
    if (constexpr_sampler->ycbcr_model !=
  ...
  } else if (msl_options.swizzle_texture_samples && !is_gather &&
             is_sampled_image_type(imgtype) && !is_dynamic_img_sampler) {
    add_spv_func_and_recompile(SPVFuncImplTextureSwizzle);
    expr += "spvTextureSwizzle(";
  }

  string inner_expr =
      CompilerGLSL::to_texture_op(i, sparse, forward, inherited_expressions);
      ...
      if (!constexpr_sampler->swizzle_has_one_or_zero()) {
        ...
        for (uint32_t c = 0; c < 4; c++) {
          ...
          case MSL_COMPONENT_SWIZZLE_A:
            expr += swizzle_names[constexpr_sampler->swizzle[c] -
        ...
        for (uint32_t c = 0; c < 4; c++) {
          ...
          case MSL_COMPONENT_SWIZZLE_A:
            expr += to_expression(temp_id) + "." +
                    swizzle_names[constexpr_sampler->swizzle[c] -
                                  MSL_COMPONENT_SWIZZLE_R];
            break;
          default:
            SPIRV_CROSS_THROW("Invalid component swizzle.");
...
// Manufacture automatic sampler arg for SampledImage texture.
string CompilerMSL::to_func_call_arg(const SPIRFunction::Parameter &arg,
                                     uint32_t id) {
  string arg_str;

  auto &type = expression_type(id);
  bool is_dynamic_img_sampler =
      has_extended_decoration(arg.id, SPIRVCrossDecorationDynamicImageSampler);
  // If the argument *itself* is a "dynamic" combined-image sampler, then we can
  // just pass that around.
  bool arg_is_dynamic_img_sampler =
      has_extended_decoration(id, SPIRVCrossDecorationDynamicImageSampler);
  if (is_dynamic_img_sampler && !arg_is_dynamic_img_sampler)
    arg_str = join("spvDynamicImageSampler<",
                   type_to_glsl(get<SPIRType>(type.image.type)), ">(");

  auto *c = maybe_get<SPIRConstant>(id);
  if (msl_options.force_native_arrays && c &&
      !get<SPIRType>(c->constant_type).array.empty()) {
    // If we are passing a constant array directly to a function for some
    // reason,
    ...
    // This kind of code gen should be rare enough that performance is not a
    // real concern.
  ...
  if (!arg_is_dynamic_img_sampler) {
    ...
    if (type.basetype == SPIRType::SampledImage) {
      ...
      for (uint32_t i = 1; i < planes; i++)
        arg_str += join(", ", CompilerGLSL::to_func_call_arg(arg, id),
                        plane_name_suffix, i);
      ...
      if (is_dynamic_img_sampler && constexpr_sampler &&
          constexpr_sampler->ycbcr_conversion_enable) {
        ...
        samp_args.push_back(
            join("spvComponentBits(", constexpr_sampler->bpc, ")"));
    ...
    if (is_dynamic_img_sampler && constexpr_sampler &&
        constexpr_sampler->ycbcr_conversion_enable)
      arg_str += join(
          ", (uint(", create_swizzle(constexpr_sampler->swizzle[3]),
          ") << 24) | (uint(", create_swizzle(constexpr_sampler->swizzle[2]),
          ") << 16) | (uint(", create_swizzle(constexpr_sampler->swizzle[1]),
          ") << 8) | uint(", create_swizzle(constexpr_sampler->swizzle[0]),
          ")");
    else if (msl_options.swizzle_texture_samples && has_sampled_images &&
             is_sampled_image_type(type))
...
// If the ID represents a sampled image that has been assigned a sampler
// already,
// generate an expression for the sampler, otherwise generate a fake sampler
// name
...
string CompilerMSL::to_sampler_expression(uint32_t id) {
  ...
  if (index == string::npos)
    ...
    return samp_id ? to_expression(samp_id)
...
string CompilerMSL::to_swizzle_expression(uint32_t id) {
  ...
  // If an image is part of an argument buffer translate this to a legal
  // identifier.
  string::size_type period = 0;
  while ((period = expr.find_first_of('.', period)) != string::npos &&
         period < index)
    expr[period] = '_';
...
string CompilerMSL::to_buffer_size_expression(uint32_t id) {
  ...
  // This is quite crude, but we need to translate the reference name
  // (*spvDescriptorSetN.name) to
  // the pointer expression spvDescriptorSetN.name to make a reasonable
  // expression here.
  // This only happens if we have argument buffers and we are using
  // OpArrayLength on a lone SSBO in that set.
  if (expr.size() >= 3 && expr[0] == '(' && expr[1] == '*')
    expr = address_of_expression(expr);

  // If a buffer is part of an argument buffer translate this to a legal
  // identifier.
  for (auto &c : expr)
    if (c == '.')
      c = '_';
...
// Checks whether the ID is a row_major matrix that requires conversion before
// use
...
// Checks whether the member is a row_major matrix that requires conversion
// before use
bool CompilerMSL::member_is_non_native_row_major_matrix(const SPIRType &type,
                                                        uint32_t index) {
...
string CompilerMSL::convert_row_major_matrix(string exp_str,
  ...
  if (!is_matrix(exp_type)) {
    return CompilerGLSL::convert_row_major_matrix(move(exp_str), exp_type,
                                                  physical_type_id, is_packed);
  } else {
    strip_enclosed_expression(exp_str);
    if (physical_type_id != 0 || is_packed)
      exp_str = unpack_expression_type(exp_str, exp_type, physical_type_id,
                                       is_packed, true);
...
void CompilerMSL::emit_fixup() {
  if (is_vertex_like_shader() && stage_out_var_id &&
      !qual_pos_var_name.empty() && !capture_output_to_buffer) {
    if (options.vertex.fixup_clipspace)
      statement(qual_pos_var_name, ".z = (", qual_pos_var_name, ".z + ",
                qual_pos_var_name,
                ".w) * 0.5;       // Adjust clip-space for Metal");

    if (options.vertex.flip_vert_y)
      statement(qual_pos_var_name, ".y = -(", qual_pos_var_name, ".y);",
                "    // Invert Y-axis for Metal");
  }
}

// Return a string defining a structure member, with padding and packing.
string CompilerMSL::to_struct_member(const SPIRType &type,
                                     uint32_t member_type_id, uint32_t index,
                                     const string &qualifier) {
  if (member_is_remapped_physical_type(type, index))
    member_type_id = get_extended_member_decoration(
        type.self, index, SPIRVCrossDecorationPhysicalTypeID);
  ...
  if (has_extended_member_decoration(type.self, index,
                                     SPIRVCrossDecorationInterfaceOrigID))
    orig_id = get_extended_member_decoration(
  ...
  // and generally we cannot copy full arrays in and out of buffers into
  // Function
  // address space.
  // Array of resources should also be declared as builtin arrays.
  if (has_member_decoration(type.self, index, DecorationOffset))
    is_using_builtin_array = true;
  else if (has_extended_member_decoration(
               type.self, index, SPIRVCrossDecorationResourceIndexPrimary))
    is_using_builtin_array = true;
  ...
  if (member_is_packed_physical_type(type, index)) {
    ...
    if (physical_type.basetype == SPIRType::Struct) {
      ...
      td_line += base_type + to_string(physical_type.columns) + "x" +
                 to_string(physical_type.vecsize);
  ...
  // Very specifically, image load-store in argument buffers are disallowed on
  // MSL on iOS.
  if (msl_options.is_ios() && physical_type.basetype == SPIRType::Image &&
  ...
  if (physical_type.basetype != SPIRType::Image &&
      physical_type.basetype != SPIRType::Sampler &&
      physical_type.basetype != SPIRType::SampledImage) {
    BuiltIn builtin = BuiltInMax;

    // Special handling. In [[stage_out]] or [[stage_in]] blocks,
    // we need flat arrays, but if we're somehow declaring gl_PerVertex for
    // constant array reasons, we want
  ...
  auto result = join(pack_pfx, type_to_glsl(*declared_type, orig_id), " ",
                     qualifier, to_member_name(type, index),
...
// Emit a structure member, padding and packing to maintain the correct memeber
// alignments.
void CompilerMSL::emit_struct_member(const SPIRType &type,
                                     uint32_t member_type_id, uint32_t index,
                                     const string &qualifier, uint32_t) {
  // If this member requires padding to maintain its declared offset, emit a
  // dummy padding member before it.
  if (has_extended_member_decoration(type.self, index,
                                     SPIRVCrossDecorationPaddingTarget)) {
    uint32_t pad_len = get_extended_member_decoration(
        type.self, index, SPIRVCrossDecorationPaddingTarget);
...
void CompilerMSL::emit_struct_padding_target(const SPIRType &type) {
  ...
  uint32_t target_size =
      get_extended_decoration(type.self, SPIRVCrossDecorationPaddingTarget);
...
string CompilerMSL::member_attribute_qualifier(const SPIRType &type,
                                               uint32_t index) {
  ...
  if (has_extended_member_decoration(
          type.self, index, SPIRVCrossDecorationResourceIndexPrimary)) {
    string quals = join(" [[id(", get_extended_member_decoration(
                                      type.self, index,
                                      SPIRVCrossDecorationResourceIndexPrimary),
                        ")");
    if (interlocked_resources.count(get_extended_member_decoration(
            type.self, index, SPIRVCrossDecorationInterfaceOrigID)))
  ...
  if (execution.model == ExecutionModelVertex &&
    ...
    uint32_t locn;
    if (is_builtin)
      locn = get_or_allocate_builtin_input_member_location(builtin, type.self,
                                                           index);
    else
  ...
  if (((execution.model == ExecutionModelVertex &&
    ...
    if (is_builtin) {
      ...
      case BuiltInPointSize:
        ...
        // non-point topologies, and Metal will reject this builtin when
        // compiling
        // the shader into a render pipeline that uses a non-point topology.
        return msl_options.enable_point_size_builtin
      ...
      case BuiltInLayer:
        return string(" [[") + builtin_qualifier(builtin) + "]]" +
               (mbr_type.array.empty() ? "" : " ");

      case BuiltInClipDistance:
        if (has_member_decoration(type.self, index, DecorationIndex))
          return join(" [[user(clip",
                      get_member_decoration(type.self, index, DecorationIndex),
                      ")]]");
        else
          return string(" [[") + builtin_qualifier(builtin) + "]]" +
                 (mbr_type.array.empty() ? "" : " ");
  ...
  if (execution.model == ExecutionModelTessellationEvaluation &&
    ...
    if (get_type(type.member_types[index]).basetype ==
        SPIRType::ControlPointArray)
      return "";
  ...
  if (execution.model == ExecutionModelFragment &&
    ...
    if (builtin == BuiltInBaryCoordNV || builtin == BuiltInBaryCoordNoPerspNV) {
      if (has_member_decoration(type.self, index, DecorationFlat) ||
        ...
        SPIRV_CROSS_THROW("Flat, Centroid, Sample, NoPerspective decorations "
                          "are not supported for BaryCoord inputs.");
      }
    }
    ...
    if (!type_is_integral(mbr_type) &&
        (!is_builtin || builtin != BuiltInFragCoord)) {
      if (has_member_decoration(type.self, index, DecorationFlat)) {
        if (!quals.empty())
          quals += ", ";
        quals += "flat";
      ...
      } else if (has_member_decoration(type.self, index,
                                       DecorationNoPerspective)) {
        if (!quals.empty())
          quals += ", ";
  ...
  if (execution.model == ExecutionModelFragment &&
    ...
    if (is_builtin) {
      ...
      case BuiltInFragStencilRefEXT:
        // Similar to PointSize, only mark FragStencilRef if there's a stencil
        // buffer.
        // Some shaders may include a FragStencilRef builtin even when used to
        // render
    ...
    if (locn != k_unknown_location &&
        !(msl_options.enable_frag_output_mask & (1 << locn)))
      return "";
    if (locn != k_unknown_location &&
        has_member_decoration(type.self, index, DecorationIndex))
      return join(" [[color(", locn, "), index(",
                  get_member_decoration(type.self, index, DecorationIndex),
                  ")]]");
...
// the subsequent
// stage if the two variables are declared with the same Location and Component
// decoration and
// match in type and decoration, except that interpolation decorations are not
// required to match.
...
string CompilerMSL::member_location_attribute_qualifier(const SPIRType &type,
                                                        uint32_t index) {
  string quals;
  uint32_t comp;
...
// Returns the location decoration of the member with the specified index in the
// specified type.
// If the location of the member has been explicitly set, that location is used.
// If not, this
// function assumes the members are ordered in their location order, and simply
// returns the
// index as the location.
uint32_t CompilerMSL::get_member_location(uint32_t type_id, uint32_t index,
                                          uint32_t *comp) const {
  if (comp) {
...
uint32_t CompilerMSL::get_or_allocate_builtin_input_member_location(
    spv::BuiltIn builtin, uint32_t type_id, uint32_t index, uint32_t *comp) {
  ...
  // This can happen for built-in inputs in tessellation which are mixed and
  // matched with user inputs.
  auto &mbr_type = get<SPIRType>(get<SPIRType>(type_id).member_types[index]);
  uint32_t count = type_to_location_count(mbr_type);

  loc = 0;

  const auto location_range_in_use =
      [this](uint32_t location, uint32_t location_count) -> bool {
...
string CompilerMSL::func_type_decl(SPIRType &type) {
  // The regular function return type. If not processing the entry point
  // function, that's all we need
  string return_type = type_to_glsl(type) + type_to_array_glsl(type);
  if (!processing_entry_point)
    return return_type;

  // If an outgoing interface block has been defined, and it should be returned,
  // override the entry point return type
  bool ep_should_return_output = !get_is_rasterization_disabled();
  if (stage_out_var_id && ep_should_return_output)
    return_type =
        type_to_glsl(get_stage_out_struct_type()) + type_to_array_glsl(type);
  ...
  case ExecutionModelTessellationEvaluation:
    ...
    if (msl_options.is_ios())
      entry_type = join("[[ patch(",
                        execution.flags.get(ExecutionModeTriangles) ? "triangle"
                                                                    : "quad",
                        ") ]] vertex");
    else
      entry_type = join("[[ patch(",
  ...
  case ExecutionModelFragment:
    entry_type = uses_explicit_early_fragment_test()
...
bool CompilerMSL::uses_explicit_early_fragment_test() {
  ...
  return ep_flags.get(ExecutionModeEarlyFragmentTests) ||
         ep_flags.get(ExecutionModePostDepthCoverage);
}

// In MSL, address space qualifiers are required for all pointer or reference
// variables
...
string CompilerMSL::get_type_address_space(const SPIRType &type, uint32_t id,
                                           bool argument) {
  // This can be called for variable pointer contexts as well, so be very
  // careful about which method we choose.
  Bitset flags;
  auto *var = maybe_get<SPIRVariable>(id);
  ...
  case StorageClassStorageBuffer: {
    // For arguments from variable pointers, we use the write count deduction,
    // so
  ...
  case StorageClassPushConstant:
    if (type.basetype == SPIRType::Struct) {
      ...
      if (ssbo)
        addr_space =
            flags.get(DecorationNonWritable) ? "const device" : "device";
      else
        addr_space = "constant";
    } else if (!argument) {
      addr_space = "constant";
  ...
  case StorageClassInput:
    if (get_execution_model() == ExecutionModelTessellationControl && var &&
      ...
      addr_space =
          msl_options.multi_patch_workgroup ? "constant" : "threadgroup";
    if (get_execution_model() == ExecutionModelFragment && var &&
        var->basevariable == stage_in_var_id)
      addr_space = "thread";
    break;
      ...
      if (var && type.storage == StorageClassOutput) {
        ...
        if (is_masked) {
          if (is_tessellation_shader())
            ...
            addr_space = "thread";
        } else if (variable_decl_is_remapped_storage(*var,
                                                     StorageClassWorkgroup))
          addr_space = "threadgroup";
      }
  ...
  if (!addr_space) {
    ...
    addr_space = type.pointer || (argument &&
                                  type.basetype == SPIRType::ControlPointArray)
                     ? "thread"
                     : "";
  }

  return join(flags.get(DecorationVolatile) || flags.get(DecorationCoherent)
                  ? "volatile "
                  : "",
              addr_space);
}
...
const char *CompilerMSL::to_restrict(uint32_t id, bool space) {
  ...
  if (ir.ids[id].get_type() == TypeVariable) {
    ...
    auto &type = expression_type(id);
    if (type.basetype == SPIRType::Struct &&
        (has_decoration(type_id, DecorationBlock) ||
         has_decoration(type_id, DecorationBufferBlock)))
      flags = get_buffer_block_flags(id);
    else
      flags = get_decoration_bitset(id);
  } else
...
// Returns true if this input builtin should be a direct parameter on a shader
...
bool CompilerMSL::is_direct_input_builtin(BuiltIn bi_type) {
  ...
  case BuiltInInstanceIndex:
  case BuiltInBaseInstance:
    return get_execution_model() != ExecutionModelVertex ||
           !msl_options.vertex_for_tessellation;
  ...
  case BuiltInViewIndex:
    return get_execution_model() == ExecutionModelFragment &&
           msl_options.multiview && msl_options.multiview_layered_rendering;
  // Compute function in
  case BuiltInSubgroupId:
  case BuiltInNumSubgroups:
...
// Returns true if this is a fragment shader that runs per sample, and false
// otherwise.
bool CompilerMSL::is_sample_rate() const {
  auto &caps = get_declared_capabilities();
  return get_execution_model() == ExecutionModelFragment &&
         (msl_options.force_sample_rate_shading ||
          std::find(caps.begin(), caps.end(), CapabilitySampleRateShading) !=
              caps.end() ||
          (msl_options.use_framebuffer_fetch_subpasses && need_subpass_input));
}
...
bool CompilerMSL::is_intersection_query() const {
  auto &caps = get_declared_capabilities();
  return std::find(caps.begin(), caps.end(), CapabilityRayQueryKHR) !=
         caps.end();
}
...
void CompilerMSL::entry_point_args_builtin(string &ep_args) {
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t var_id, SPIRVariable &var) {
    ...
    if (is_builtin_variable(var) &&
      ...
      // If the builtin is not part of the active input builtin set, don't emit
      // it.
      // Relevant for multiple entry-point modules which might declare unused
      // builtins.
      if (!active_input_builtins.get(bi_type) ||
          !interface_variable_exists_in_entry_point(var_id))
        return;
      ...
      if (is_direct_input_builtin(bi_type)) {
        ...
        builtin_declaration = true;
        ep_args +=
            builtin_type_decl(bi_type, var_id) + " " + to_expression(var_id);
        ep_args += " [[" + builtin_qualifier(bi_type);
        if (bi_type == BuiltInSampleMask &&
            get_entry_point().flags.get(ExecutionModePostDepthCoverage)) {
          if (!msl_options.supports_msl_version(2))
    ...
    if (has_extended_decoration(var_id,
                                SPIRVCrossDecorationBuiltInDispatchBase)) {
      // This is a special implicit builtin, not corresponding to any SPIR-V
      // builtin,
      // which holds the base that was passed to vkCmdDispatchBase() or
      // vkCmdDrawIndexed(). If it's present,
      // assume we emitted it for a good reason.
      assert(msl_options.supports_msl_version(1, 2));
      if (!ep_args.empty())
        ep_args += ", ";

      ep_args += type_to_glsl(get_variable_data_type(var)) + " " +
                 to_expression(var_id) + " [[grid_origin]]";
    }

    if (has_extended_decoration(var_id,
                                SPIRVCrossDecorationBuiltInStageInputSize)) {
      // This is another special implicit builtin, not corresponding to any
      // SPIR-V builtin,
      // which holds the number of vertices and instances to draw. If it's
      // present,
      // assume we emitted it for a good reason.
      assert(msl_options.supports_msl_version(1, 2));
      if (!ep_args.empty())
      ...
      ep_args += type_to_glsl(get_variable_data_type(var)) + " " +
                 to_expression(var_id) + " [[grid_size]]";
    }
  });

  // Correct the types of all encountered active builtins. We couldn't do this
  // before
  // because ensure_correct_builtin_type() may increase the bound, which isn't
  // allowed
  // while iterating over IDs.
  for (auto &var : active_builtins)
    var.first->basetype =
        ensure_correct_builtin_type(var.first->basetype, var.second);
  ...
  if (capture_output_to_buffer) {
    // Add parameters to hold the indirect draw parameters and the shader
    // output. This has to be handled
    // specially because it needs to be a pointer, not a reference.
    if (stage_out_var_id) {
      if (!ep_args.empty())
        ep_args += ", ";
      ep_args += join("device ", type_to_glsl(get_stage_out_struct_type()),
                      "* ", output_buffer_var_name, " [[buffer(",
    ...
    if (get_execution_model() == ExecutionModelTessellationControl) {
      if (!ep_args.empty())
        ep_args += ", ";
      ep_args += join("constant uint* spvIndirectParams [[buffer(",
                      msl_options.indirect_params_buffer_index, ")]]");
      ...
      if (!ep_args.empty())
        ep_args += ", ";
      ep_args += join("device uint* spvIndirectParams [[buffer(",
                      msl_options.indirect_params_buffer_index, ")]]");
    }
    ...
    if (get_execution_model() == ExecutionModelVertex &&
        msl_options.vertex_for_tessellation &&
        (active_input_builtins.get(BuiltInVertexIndex) ||
         active_input_builtins.get(BuiltInVertexId)) &&
      ...
      case Options::IndexType::UInt16:
        ep_args +=
            join("const device ushort* ", index_buffer_var_name, " [[buffer(",
                 msl_options.shader_index_buffer_index, ")]]");
        break;
      case Options::IndexType::UInt32:
        ep_args +=
            join("const device uint* ", index_buffer_var_name, " [[buffer(",
                 msl_options.shader_index_buffer_index, ")]]");
        break;
      }
    }
    ...
    if (get_execution_model() == ExecutionModelTessellationControl) {
      if (patch_stage_out_var_id) {
        if (!ep_args.empty())
          ep_args += ", ";
        ep_args += join(
            "device ", type_to_glsl(get_patch_stage_out_struct_type()), "* ",
      ...
      if (!ep_args.empty())
        ep_args += ", ";
      ep_args +=
          join("device ", get_tess_factor_struct_name(), "* ",
               tess_factor_buffer_var_name, " [[buffer(",
               convert_to_string(msl_options.shader_tess_factor_buffer_index),
               ")]]");

      // Initializer for tess factors must be handled specially since it's never
      // declared as a normal variable.
      uint32_t outer_factor_initializer_id = 0;
      uint32_t inner_factor_initializer_id = 0;
      ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
        if (!has_decoration(var.self, DecorationBuiltIn) ||
            var.storage != StorageClassOutput || !var.initializer)
          return;
      ...
      if (outer_factor_initializer_id &&
        ...
        entry_func.fixup_hooks_in.push_back([=]() {
          uint32_t components =
              get_execution_mode_bitset().get(ExecutionModeTriangles) ? 3 : 4;
          for (uint32_t i = 0; i < components; i++) {
            statement(
                builtin_to_glsl(BuiltInTessLevelOuter, StorageClassOutput), "[",
                i, "] = ", "half(", to_expression(c->subconstants[i]), ");");
          }
        });
      }
      ...
      if (inner_factor_initializer_id &&
        ...
        if (get_execution_mode_bitset().get(ExecutionModeTriangles)) {
          entry_func.fixup_hooks_in.push_back([=]() {
            statement(
                builtin_to_glsl(BuiltInTessLevelInner, StorageClassOutput),
                " = ", "half(", to_expression(c->subconstants[0]), ");");
          });
        } else {
          entry_func.fixup_hooks_in.push_back([=]() {
      ...
      if (stage_in_var_id) {
        ...
        if (msl_options.multi_patch_workgroup) {
          ep_args += join(
              "device ", type_to_glsl(get_stage_in_struct_type()), "* ",
              input_buffer_var_name, " [[buffer(",
              convert_to_string(msl_options.shader_input_buffer_index), ")]]");
        } else {
          ep_args +=
              join("threadgroup ", type_to_glsl(get_stage_in_struct_type()),
                   "* ", input_wg_var_name, " [[threadgroup(",
...
string CompilerMSL::entry_point_args_argument_buffer(bool append_comma) {
  ...
  for (uint32_t i = 0; i < kMaxArgumentBuffers; i++) {
    ...
    auto itr = resource_bindings.find(
        {get_entry_point().model, i, kArgumentBufferBinding});
...
CompilerMSL::find_constexpr_sampler(uint32_t id) const {
  // Try by ID.
  {
...
void CompilerMSL::entry_point_args_discrete_descriptors(string &ep_args) {
  ...
  // We need to sort to work around a bug on macOS 10.13 with NVidia drivers
  // where switching between shaders
  // with different order of buffers can result in issues with buffer
  // assignments inside the driver.
  struct Resource {
    SPIRVariable *var;
    string name;
    SPIRType::BaseType basetype;
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t var_id, SPIRVariable &var) {
    if ((var.storage == StorageClassUniform ||
      ...
      if (is_supported_argument_buffer_type(type) &&
          var.storage != StorageClassPushConstant) {
      ...
      if (type.basetype == SPIRType::SampledImage ||
        ...
        if (constexpr_sampler) {
          // Mark this ID as a constexpr sampler for later in case it came from
          // set/bindings.
      ...
      if (atomic_image_vars.count(var.self)) {
        secondary_index =
            get_metal_resource_index(var, SPIRType::AtomicCounter, 0);
      }
      ...
      if (type.basetype == SPIRType::SampledImage) {
        ...
        resources.push_back({&var, to_name(var_id), type.basetype,
                             get_metal_resource_index(var, type.basetype), 0,
                             secondary_index});
      }
    }
  });
  ...
  for (auto &r : resources) {
      ...
      if (!type.array.empty()) {
        ...
        for (uint32_t i = 0; i < array_size; ++i) {
          ...
          ep_args += get_argument_address_space(var) + " " +
                     type_to_glsl(type) + "* " + to_restrict(var_id) + r.name +
    ...
    case SPIRType::Image: {
      ...
      // Emulate texture2D atomic operations
      if (atomic_image_vars.count(var.self)) {
        ep_args += ", device atomic_" +
                   type_to_glsl(get<SPIRType>(basetype.image.type), 0);
...
// Returns a string containing a comma-delimited list of args for the entry
// point function
// This is the "classic" method of MSL 1 when we don't have argument buffer
// support.
...
void CompilerMSL::fix_up_shader_inputs_outputs() {
  ...
  // Vertex shaders shouldn't have the problems with barriers in non-uniform
  // control flow that
  // tessellation control shaders do, so early returns should be OK. We may need
  // to revisit this
  ...
  // Look for sampled images and buffer. Add hooks to set up the swizzle
  // constants or array lengths.
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
    auto &type = get_variable_data_type(var);
    uint32_t var_id = var.self;
    bool ssbo = has_decoration(type.self, DecorationBufferBlock);
    ...
    if (var.storage == StorageClassUniformConstant &&
      ...
      if (msl_options.swizzle_texture_samples && has_sampled_images &&
        ...
        entry_func.fixup_hooks_in.push_back([this, &type, &var, var_id]() {
          ...
          if (descriptor_set_is_argument_buffer(desc_set)) {
            statement(
                "constant uint", is_array_type ? "* " : "& ",
                to_swizzle_expression(var_id), is_array_type ? " = &" : " = ",
                to_name(argument_buffer_ids[desc_set]), ".spvSwizzleConstants",
                "[", convert_to_string(
                         get_metal_resource_index(var, SPIRType::Image)),
                "];");
          } else {
            // If we have an array of images, we need to be able to index into
            // it, so take a pointer instead.
            statement("constant uint", is_array_type ? "* " : "& ",
                      to_swizzle_expression(var_id),
                      is_array_type ? " = &" : " = ",
                      to_name(swizzle_buffer_id), "[",
                      convert_to_string(
                          get_metal_resource_index(var, SPIRType::Image)),
                      "];");
          }
                ...
                (var.storage == StorageClassUniform && ssbo)) &&
               !is_hidden_variable(var)) {
      if (buffers_requiring_array_length.count(var.self)) {
        entry_func.fixup_hooks_in.push_back([this, &type, &var, var_id]() {
          ...
          if (descriptor_set_is_argument_buffer(desc_set)) {
            statement("constant uint", is_array_type ? "* " : "& ",
                      to_buffer_size_expression(var_id),
                      is_array_type ? " = &" : " = ",
                      to_name(argument_buffer_ids[desc_set]),
  ...
  // Builtin variables
  ir.for_each_typed_id<SPIRVariable>([this, &entry_func](uint32_t,
                                                         SPIRVariable &var) {
    uint32_t var_id = var.self;
    ...
    if (var.storage == StorageClassInput && is_builtin_variable(var) &&
        active_input_builtins.get(bi_type)) {
      switch (bi_type) {
      case BuiltInSamplePosition:
        entry_func.fixup_hooks_in.push_back([=]() {
      ...
      case BuiltInHelperInvocation:
        ...
        else if (msl_options.is_macos() &&
                 !msl_options.supports_msl_version(2, 1))
          SPIRV_CROSS_THROW(
      ...
      case BuiltInInvocationId:
        ...
        break;
      case BuiltInPrimitiveId:
        // This is natively supported by fragment and tessellation evaluation
        // shaders.
        // In tessellation control shaders, this is direct-mapped without
        // multi-patch workgroups.
        if (get_execution_model() != ExecutionModelTessellationControl ||
            !msl_options.multi_patch_workgroup)
          break;
        ...
        entry_func.fixup_hooks_in.push_back([=]() {
          statement(builtin_type_decl(bi_type), " ", to_expression(var_id),
                    " = min(", to_expression(builtin_invocation_id_id), ".x / ",
                    this->get_entry_point().output_vertices,
      ...
      case BuiltInPatchVertices:
        if (get_execution_model() == ExecutionModelTessellationEvaluation)
          entry_func.fixup_hooks_in.push_back([=]() {
            statement(builtin_type_decl(bi_type), " ", to_expression(var_id),
                      " = ", to_expression(patch_stage_in_var_id),
                      ".gl_in.size();");
          });
        else
          entry_func.fixup_hooks_in.push_back([=]() {
        ...
        break;
      case BuiltInTessCoord:
        // Emit a fixup to account for the shifted domain. Don't do this for
        // triangles;
        ...
        if (msl_options.tess_domain_origin_lower_left &&
          ...
          entry_func.fixup_hooks_in.push_back(
              [=]() { statement(tc, ".y = 1.0 - ", tc, ".y;"); });
        }
        break;
      case BuiltInSubgroupId:
        if (!msl_options.emulate_subgroups)
          break;
        // For subgroup emulation, this is the same as the local invocation
        // index.
        entry_func.fixup_hooks_in.push_back([=]() {
          statement(builtin_type_decl(bi_type), " ", to_expression(var_id),
      ...
      case BuiltInNumSubgroups:
        ...
        entry_func.fixup_hooks_in.push_back([=]() {
          ...
          if (type.vecsize >= 3)
            size_expr =
                join(size_expr, ".x * ", size_expr, ".y * ", size_expr, ".z");
          else if (type.vecsize == 2)
        ...
        entry_func.fixup_hooks_in.push_back([=]() {
          ...
          if (msl_options.fixed_subgroup_size > 32) {
            ...
            statement(builtin_type_decl(bi_type), " ", to_expression(var_id),
                      ...
                      " (uint)max((int)",
                      to_expression(builtin_subgroup_invocation_id_id),
                      " - 32, 0), ", msl_options.fixed_subgroup_size, " - max(",
                      to_expression(builtin_subgroup_invocation_id_id),
                      ", 32u)), uint2(0));");
          } else if (msl_options.fixed_subgroup_size != 0) {
            statement(builtin_type_decl(bi_type), " ", to_expression(var_id),
        ...
        entry_func.fixup_hooks_in.push_back([=]() {
          ...
          if (msl_options.fixed_subgroup_size > 32) {
            statement(builtin_type_decl(bi_type), " ", to_expression(var_id),
                      ...
                      to_expression(builtin_subgroup_invocation_id_id),
                      " + 1 - 32, 0), ", msl_options.fixed_subgroup_size,
                      " - max(",
                      to_expression(builtin_subgroup_invocation_id_id),
                      " + 1, 32u)), uint2(0));");
          } else if (msl_options.fixed_subgroup_size != 0) {
            statement(builtin_type_decl(bi_type), " ", to_expression(var_id),
                      " = uint4(insert_bits(0u, 0xFFFFFFFF, ",
                      to_expression(builtin_subgroup_invocation_id_id),
                      " + 1, ", msl_options.fixed_subgroup_size, " - ",
                      to_expression(builtin_subgroup_invocation_id_id),
                      " - 1), uint3(0));");
          } else if (msl_options.is_ios()) {
            statement(builtin_type_decl(bi_type), " ", to_expression(var_id),
                      " = uint4(insert_bits(0u, 0xFFFFFFFF, ",
                      to_expression(builtin_subgroup_invocation_id_id),
                      " + 1, ", to_expression(builtin_subgroup_size_id), " - ",
                      to_expression(builtin_subgroup_invocation_id_id),
                      " - 1), uint3(0));");
          } else {
            statement(builtin_type_decl(bi_type), " ", to_expression(var_id),
      ...
      case BuiltInSubgroupLeMask:
        ...
        entry_func.fixup_hooks_in.push_back([=]() {
          if (msl_options.is_ios()) {
            ...
            statement(
                builtin_type_decl(bi_type), " ", to_expression(var_id),
      ...
      case BuiltInViewIndex:
        if (!msl_options.multiview) {
          ...
          entry_func.fixup_hooks_in.push_back([=]() {
            statement("const ", builtin_type_decl(bi_type), " ",
                      to_expression(var_id), " = 0;");
          });
        } else if (msl_options.view_index_from_device_index) {
          // In this case, we take the view index from that of the device we're
          // running on.
          entry_func.fixup_hooks_in.push_back([=]() {
            statement("const ", builtin_type_decl(bi_type), " ",
                      to_expression(var_id), " = ", msl_options.device_index,
                      ";");
          });
          ...
          // In this case, the views are rendered one at a time. The view index,
          // then,
          ...
          entry_func.fixup_hooks_out.push_back([=]() {
            statement(to_expression(builtin_layer_id), " = ",
                      to_expression(var_id), " - ",
                      to_expression(view_mask_buffer_id), "[0];");
          });
        }
      ...
      case BuiltInGlobalInvocationId:
        ...
        // GlobalInvocationId is defined as LocalInvocationId + WorkgroupId *
        // WorkgroupSize.
        ...
        entry_func.fixup_hooks_in.push_back([=]() {
          ...
          if (workgroup_size_id)
            ...
            statement(to_expression(var_id), " += ",
                      to_dereferenced_expression(builtin_dispatch_base_id),
                      " * uint3(", execution.workgroup_size.x, ", ",
      ...
      // If the additional fixed sample mask was set, we need to adjust the
      // sample_mask
      // output to reflect that. If the shader outputs the sample_mask itself
      // too, we need
      // to AND the two masks to get the final one.
      string op_str = does_shader_write_sample_mask ? " &= " : " = ";
      entry_func.fixup_hooks_out.push_back([=]() {
        statement(to_expression(builtin_sample_mask_id), op_str,
                  additional_fixed_sample_mask_str(), ";");
      });
    }
  });
}

// Returns the Metal index of the resource of the specified type as used by the
// specified variable.
uint32_t CompilerMSL::get_metal_resource_index(SPIRVariable &var,
                                               SPIRType::BaseType basetype,
                                               uint32_t plane) {
  auto &execution = get_entry_point();
  auto &var_dec = ir.meta[var.self].decoration;
  auto &var_type = get<SPIRType>(var.basetype);
  uint32_t var_desc_set = (var.storage == StorageClassPushConstant)
                              ? kPushConstDescSet
                              : var_dec.set;
  uint32_t var_binding = (var.storage == StorageClassPushConstant)
                             ? kPushConstBinding
                             : var_dec.binding;

  // If a matching binding has been specified, find and use it.
  auto itr =
      resource_bindings.find({execution.model, var_desc_set, var_binding});

  // Atomic helper buffers for image atomics need to use secondary bindings as
  // well.
  bool use_secondary_binding = (var_type.basetype == SPIRType::SampledImage &&
                                basetype == SPIRType::Sampler) ||
  ...
  if (itr != end(resource_bindings)) {
    ...
    switch (basetype) {
    case SPIRType::Image:
      set_extended_decoration(var.self, resource_decoration,
                              remap.first.msl_texture + plane);
      return remap.first.msl_texture + plane;
    case SPIRType::Sampler:
      set_extended_decoration(var.self, resource_decoration,
                              remap.first.msl_sampler);
      return remap.first.msl_sampler;
    default:
      set_extended_decoration(var.self, resource_decoration,
                              remap.first.msl_buffer);
  ...
  if (type_is_msl_framebuffer_fetch(type)) {
    // Frame-buffer fetch gets its fallback resource index from the input
    // attachment index,
    ...
    // If there is no explicit mapping of bindings to MSL, use the declared
  ...
  // We cannot reliably use Binding decorations since SPIR-V and MSL's binding
  ...
  if (var.storage != StorageClassPushConstant)
    allocate_argument_buffer_ids =
        descriptor_set_is_argument_buffer(var_desc_set);
  ...
  // If a binding has not been specified, revert to incrementing resource
  // indices.
...
bool CompilerMSL::type_is_pointer(const SPIRType &type) const {
  ...
  // Safeguards when we forget to set pointer_depth (there is an assert for it
  // in type_to_glsl),
  // but the extra check shouldn't hurt.
  return (type.pointer_depth > parent_type.pointer_depth) ||
         !parent_type.pointer;
}
...
string CompilerMSL::argument_decl(const SPIRFunction::Parameter &arg) {
  ...
  // If we need to modify the name of the variable, make sure we use the
  // original variable.
  // Our alias is just a shadow variable.
  ...
  bool constref =
      !arg.alias_global_variable && is_pointer && arg.write_count == 0;
  // Framebuffer fetch is plain value, const looks out of place, but it is not
  // wrong.
  if (type_is_msl_framebuffer_fetch(type))
    constref = false;

  bool type_is_image = type.basetype == SPIRType::Image ||
  ...
  // If this is a combined image-sampler for a 2D image with floating-point
  // type,
  // we emitted the 'spvDynamicImageSampler' type, and this is *not* an alias
  // parameter
  ...
  bool is_dynamic_img_sampler =
      !arg.alias_global_variable && type.basetype == SPIRType::SampledImage &&
      type.image.dim == Dim2D &&
      type_is_floating_point(get<SPIRType>(type.image.type)) &&
  ...
  if (var.basevariable && (var.basevariable == stage_in_ptr_var_id ||
                           var.basevariable == stage_out_ptr_var_id))
    ...
    if (storage == StorageClassInput &&
        (builtin_type == BuiltInTessLevelInner ||
         builtin_type == BuiltInTessLevelOuter)) {
      is_using_builtin_array = false;
    } else if (builtin_type != BuiltInClipDistance &&
               builtin_type != BuiltInCullDistance) {
      is_using_builtin_array = true;
    }
    ...
    if (is_using_builtin_array)
      ...
      decl = join(cv_qualifier, type_to_glsl(type, arg.id));
  } else if ((type_storage == StorageClassUniform ||
              type_storage == StorageClassStorageBuffer) &&
             is_array(type)) {
    is_using_builtin_array = true;
    decl += join(cv_qualifier, type_to_glsl(type, arg.id), "*");
  } else if (is_dynamic_img_sampler) {
    decl = join(cv_qualifier, "spvDynamicImageSampler<",
                type_to_glsl(get<SPIRType>(type.image.type)), ">");
  ...
  if (!builtin && !opaque_handle && !is_pointer &&
      (type_storage == StorageClassFunction ||
       type_storage == StorageClassGeneric)) {
    // If the argument is a pure value and not an opaque type, we will pass by
    // value.
    if (msl_options.force_native_arrays && is_array(type)) {
      // We are receiving an array by value. This is problematic.
      // We cannot be sure of the target address space since we are supposed to
      // receive a copy,
      // but this is not possible with MSL without some extra work.
      // We will have to assume we're getting a reference in thread address
      // space.
      // If we happen to get a reference in constant address space, the caller
      // must emit a copy and pass that.
      // Thread const therefore becomes the only logical choice, since we cannot
      // "create" a constant array from
    ...
    if (msl_options.argument_buffers) {
      ...
      if ((type_storage == StorageClassUniform ||
           type_storage == StorageClassStorageBuffer) &&
          descriptor_set_is_argument_buffer(desc_set)) {
        // An awkward case where we need to emit *more* address space
        // declarations (yay!).
        // An example is where we pass down an array of buffer pointers to leaf
        // functions.
        ...
        // However, this only matters for argument buffers, since for MSL 1.0
        // style codegen,
        // we emit the buffer array on stack instead, and that seems to work
        // just fine apparently.

        // If the argument was marked as being in device address space, any
        // pointer to member would
    ...
    // Special case, need to override the array size here if we're using tess
  ...
  } else if (!opaque_handle && (!pull_model_inputs.count(var.basevariable) ||
                                type.basetype == SPIRType::Struct)) {
    // If this is going to be a reference to a variable pointer, the address
    // space
  ...
  if (backing_var && atomic_image_vars.count(backing_var->self)) {
    decl += ", device atomic_" +
            type_to_glsl(get<SPIRType>(var_type.image.type), 0);
...
// Returns a name that combines the name of the struct with the name of the
// member, except for Builtins
string CompilerMSL::to_qualified_member_name(const SPIRType &type,
                                             uint32_t index) {
  // Don't qualify Builtin names because they are unique and are treated as such
...
string CompilerMSL::ensure_valid_name(string name, string pfx) {
  return (name.size() >= 2 && name[0] == '_' && isdigit(name[1])) ? (pfx + name)
                                                                  : name;
}

const std::unordered_set<std::string> &CompilerMSL::get_reserved_keyword_set() {
  static const unordered_set<string> keywords = {
      "kernel", "vertex", "fragment", "compute", "bias", "level", "gradient2d",
      "gradientcube", "gradient3d", "min_lod_clamp", "assert",
      "VARIABLE_TRACEPOINT", "STATIC_DATA_TRACEPOINT",
      "STATIC_DATA_TRACEPOINT_V", "METAL_ALIGN", "METAL_ASM", "METAL_CONST",
      "METAL_DEPRECATED", "METAL_ENABLE_IF", "METAL_FUNC", "METAL_INTERNAL",
      "METAL_NON_NULL_RETURN", "METAL_NORETURN", "METAL_NOTHROW", "METAL_PURE",
      "METAL_UNAVAILABLE", "METAL_IMPLICIT", "METAL_EXPLICIT",
      "METAL_CONST_ARG", "METAL_ARG_UNIFORM", "METAL_ZERO_ARG",
      "METAL_VALID_LOD_ARG", "METAL_VALID_LEVEL_ARG", "METAL_VALID_STORE_ORDER",
      "METAL_VALID_LOAD_ORDER", "METAL_VALID_COMPARE_EXCHANGE_FAILURE_ORDER",
      "METAL_COMPATIBLE_COMPARE_EXCHANGE_ORDERS", "METAL_VALID_RENDER_TARGET",
      "is_function_constant_defined", "CHAR_BIT", "SCHAR_MAX", "SCHAR_MIN",
      "UCHAR_MAX", "CHAR_MAX", "CHAR_MIN", "USHRT_MAX", "SHRT_MAX", "SHRT_MIN",
      "UINT_MAX", "INT_MAX", "INT_MIN", "FLT_DIG", "FLT_MANT_DIG",
      "FLT_MAX_10_EXP", "FLT_MAX_EXP", "FLT_MIN_10_EXP", "FLT_MIN_EXP",
      "FLT_RADIX", "FLT_MAX", "FLT_MIN", "FLT_EPSILON", "FP_ILOGB0",
      "FP_ILOGBNAN", "MAXFLOAT", "HUGE_VALF", "INFINITY", "NAN", "M_E_F",
      "M_LOG2E_F", "M_LOG10E_F", "M_LN2_F", "M_LN10_F", "M_PI_F", "M_PI_2_F",
      "M_PI_4_F", "M_1_PI_F", "M_2_PI_F", "M_2_SQRTPI_F", "M_SQRT2_F",
      "M_SQRT1_2_F", "HALF_DIG", "HALF_MANT_DIG", "HALF_MAX_10_EXP",
      "HALF_MAX_EXP", "HALF_MIN_10_EXP", "HALF_MIN_EXP", "HALF_RADIX",
      "HALF_MAX", "HALF_MIN", "HALF_EPSILON", "MAXHALF", "HUGE_VALH", "M_E_H",
      "M_LOG2E_H", "M_LOG10E_H", "M_LN2_H", "M_LN10_H", "M_PI_H", "M_PI_2_H",
      "M_PI_4_H", "M_1_PI_H", "M_2_PI_H", "M_2_SQRTPI_H", "M_SQRT2_H",
      "M_SQRT1_2_H", "DBL_DIG", "DBL_MANT_DIG", "DBL_MAX_10_EXP", "DBL_MAX_EXP",
      "DBL_MIN_10_EXP", "DBL_MIN_EXP", "DBL_RADIX", "DBL_MAX", "DBL_MIN",
      "DBL_EPSILON", "HUGE_VAL", "M_E", "M_LOG2E", "M_LOG10E", "M_LN2",
      "M_LN10", "M_PI", "M_PI_2", "M_PI_4", "M_1_PI", "M_2_PI", "M_2_SQRTPI",
      "M_SQRT2", "M_SQRT1_2", "quad_broadcast",
  };
...
const std::unordered_set<std::string> &CompilerMSL::get_illegal_func_names() {
  static const unordered_set<string> illegal_func_names = {
      "main", "saturate", "assert", "fmin3", "fmax3", "VARIABLE_TRACEPOINT",
      "STATIC_DATA_TRACEPOINT", "STATIC_DATA_TRACEPOINT_V", "METAL_ALIGN",
      "METAL_ASM", "METAL_CONST", "METAL_DEPRECATED", "METAL_ENABLE_IF",
      "METAL_FUNC", "METAL_INTERNAL", "METAL_NON_NULL_RETURN", "METAL_NORETURN",
      "METAL_NOTHROW", "METAL_PURE", "METAL_UNAVAILABLE", "METAL_IMPLICIT",
      "METAL_EXPLICIT", "METAL_CONST_ARG", "METAL_ARG_UNIFORM",
      "METAL_ZERO_ARG", "METAL_VALID_LOD_ARG", "METAL_VALID_LEVEL_ARG",
      "METAL_VALID_STORE_ORDER", "METAL_VALID_LOAD_ORDER",
...
string CompilerMSL::to_member_reference(uint32_t base, const SPIRType &type,
                                        uint32_t index, bool ptr_chain) {
  ...
  if (var) {
    // Only allow -> dereference for block types. This is so we get expressions
    // like
    // buffer[i]->first_member.second_member, rather than
    // buffer[i]->first->second.
    bool is_block = has_decoration(type.self, DecorationBlock) ||
                    has_decoration(type.self, DecorationBufferBlock);

    bool is_buffer_variable =
        is_block && (var->storage == StorageClassUniform ||
                     var->storage == StorageClassStorageBuffer);
    declared_as_pointer =
        is_buffer_variable && is_array(get<SPIRType>(var->basetype));
  }
...
string CompilerMSL::type_to_glsl(const SPIRType &type, uint32_t id) {
  ...
  if (type.pointer) {
    ...
    // Work around C pointer qualifier rules. If glsl_type is a pointer type as
    // well
    // we'll need to emit the address space to the right.
    // We could always go this route, but it makes the code unnatural.
    // Prefer emitting thread T *foo over T thread* foo since it's more
    // readable,
  ...
  case SPIRType::ControlPointArray:
    return join("patch_control_point<",
                type_to_glsl(get<SPIRType>(type.parent_type), id), ">");

  case SPIRType::Interpolant:
    return join(
        "interpolant<", type_to_glsl(get<SPIRType>(type.parent_type), id),
        ", interpolation::",
        has_decoration(type.self, DecorationNoPerspective) ? "no_perspective"
                                                           : "perspective",
        ">");
  ...
  case SPIRType::Boolean: {
    ...
    // Need to special-case threadgroup booleans. They are supposed to be
    // logical
    // storage, but MSL compilers will sometimes crash if you use threadgroup
    // bool.
    // Workaround this by using 16-bit types instead and fixup on load-store to
    // this data.
    // FIXME: We have no sane way of working around this problem if a struct
    // member is boolean
    // and that struct is used as a threadgroup variable, but ... sigh.
    if ((var && var->storage == StorageClassWorkgroup) ||
...
bool CompilerMSL::variable_decl_is_remapped_storage(
    const SPIRVariable &variable, spv::StorageClass storage) const {
  ...
  if (storage == StorageClassWorkgroup) {
    ...
    if (model == ExecutionModelTessellationControl &&
        ...
        has_decoration(get<SPIRType>(variable.basetype).self,
                       DecorationBlock)) {
      return true;
    }
    ...
    // We won't be able to catch writes to control point outputs here since
    // variable
    // refers to a function local pointer.
    // This is fine, as there cannot be concurrent writers to that memory
    // anyways,
...
std::string CompilerMSL::variable_decl(const SPIRType &type,
...
std::string CompilerMSL::sampler_type(const SPIRType &type, uint32_t id) {
  auto *var = maybe_get<SPIRVariable>(id);
  if (var && var->basevariable) {
    // Check against the base variable, and not a fake ID which might have been
  ...
  if (!type.array.empty()) {
    ...
    // Arrays of samplers in MSL must be declared with a special array<T, N>
    // syntax ala C++11 std::array.
    // If we have a runtime array, it could be a variable-count descriptor set
    // binding.
    uint32_t array_size = to_array_size_literal(type);
    if (array_size == 0)
...
string CompilerMSL::image_type_glsl(const SPIRType &type, uint32_t id) {
  ...
  if (!type.array.empty()) {
    ...
    // Arrays of images in MSL must be declared with a special array<T, N>
    // syntax ala C++11 std::array.
    // If we have a runtime array, it could be a variable-count descriptor set
    // binding.
    uint32_t array_size = to_array_size_literal(type);
  ...
  if (is_depth_image(type, id)) {
    ...
    case DimSubpassData: {
      bool subpass_array =
          img_type.dim == DimSubpassData &&
          (msl_options.multiview || msl_options.arrayed_subpass_input);
      if (img_type.dim == Dim1D && !msl_options.texture_1D_as_2D) {
        // Use a native Metal 1D texture
    ...
    case DimCube:
      if (!msl_options.emulate_cube_array)
        img_type_name +=
            (img_type.arrayed ? "texturecube_array" : "texturecube");
      else
  ...
  // Otherwise it may be set based on whether the image is read from or written
...
void CompilerMSL::emit_subgroup_op(const Instruction &i) {
  ...
  if (msl_options.is_ios() && (!msl_options.supports_msl_version(2, 3) ||
                               !msl_options.ios_use_simdgroup_functions)) {
    switch (op) {
    default:
      SPIRV_CROSS_THROW("Subgroup ops beyond broadcast, ballot, and shuffle on "
                        "iOS require Metal 2.3 and up.");
  ...
  if (msl_options.is_macos() && !msl_options.supports_msl_version(2, 1)) {
    switch (op) {
    default:
      SPIRV_CROSS_THROW("Subgroup ops beyond broadcast and shuffle on macOS "
                        "require Metal 2.1 and up.");
    case OpGroupNonUniformBroadcast:
    case OpGroupNonUniformShuffle:
    case OpGroupNonUniformShuffleXor:
    case OpGroupNonUniformShuffleUp:
  ...
  case OpGroupNonUniformInverseBallot:
    emit_binary_func_op(result_type, id, ops[3],
  ...
  case OpGroupNonUniformBallotFindLSB:
    emit_binary_func_op(result_type, id, ops[3], builtin_subgroup_size_id,
  ...
  case OpGroupNonUniformBallotBitCount: {
    ...
    switch (operation) {
    case GroupOperationReduce:
      emit_binary_func_op(result_type, id, ops[4], builtin_subgroup_size_id,
                          "spvSubgroupBallotBitCount");
      break;
    case GroupOperationInclusiveScan:
      emit_binary_func_op(result_type, id, ops[4],
                          builtin_subgroup_invocation_id_id,
...
string CompilerMSL::bitcast_glsl_op(const SPIRType &out_type,
                                    const SPIRType &in_type) {
  ...
  bool integral_cast = type_is_integral(out_type) &&
                       type_is_integral(in_type) &&
                       (out_type.vecsize == in_type.vecsize);
  bool same_size_cast =
      (out_type.width * out_type.vecsize) == (in_type.width * in_type.vecsize);
  ...
  // because Metal can internally cast the results of some integer ops to a
  // larger
...
string CompilerMSL::builtin_to_glsl(BuiltIn builtin, StorageClass storage) {
  ...
  case BuiltInVertexId:
    ...
    if (msl_options.enable_base_index_zero &&
        msl_options.supports_msl_version(1, 1) &&
        (msl_options.ios_support_base_vertex_instance ||
  ...
  // When used in the entry function, output builtins are qualified with output
  // struct name.
  // Test storage class as NOT Input, as output builtins might be part of
  // generic type.
  ...
  case BuiltInBaryCoordNoPerspNV:
    if (storage == StorageClassInput && current_function &&
        (current_function->self == ir.default_entry_point))
      return stage_in_var_name + "." +
             CompilerGLSL::builtin_to_glsl(builtin, storage);
    break;

  case BuiltInTessLevelOuter:
    if (get_execution_model() == ExecutionModelTessellationControl &&
        storage != StorageClassInput && current_function &&
        (current_function->self == ir.default_entry_point)) {
      return join(tess_factor_buffer_var_name, "[",
                  to_expression(builtin_primitive_id_id),
...
string CompilerMSL::builtin_qualifier(BuiltIn builtin) {
  ...
  case BuiltInInvocationId:
    if (msl_options.multi_patch_workgroup) {
      ...
      SPIRV_CROSS_THROW("InvocationId is computed manually with multi-patch "
                        "workgroups in MSL.");
    }
  ...
  case BuiltInPrimitiveId:
    ...
    case ExecutionModelTessellationControl:
      if (msl_options.multi_patch_workgroup) {
        // Shouldn't be reached.
        SPIRV_CROSS_THROW("PrimitiveId is computed manually with multi-patch "
                          "workgroups in MSL.");
      }
    ...
    case ExecutionModelFragment:
      if (msl_options.is_ios() && !msl_options.supports_msl_version(2, 3))
        SPIRV_CROSS_THROW("PrimitiveId on iOS requires MSL 2.3.");
               ...
               !msl_options.supports_msl_version(2, 2))
  ...
  case BuiltInViewIndex:
    ...
    // The ViewIndex was implicitly used in the prior stages to set the
  ...
  case BuiltInSubgroupSize:
    if (msl_options.emulate_subgroups || msl_options.fixed_subgroup_size != 0)
      // Shouldn't be reached.
      SPIRV_CROSS_THROW("Emitting threads_per_simdgroup attribute with fixed "
                        "subgroup size??");
    if (execution.model == ExecutionModelFragment) {
      if (!msl_options.supports_msl_version(2, 2))
        SPIRV_CROSS_THROW(
      ...
      // thread_execution_width is an alias for threads_per_simdgroup, and it's
      // only available since 1.0,
      // but not in fragment.
  ...
  case BuiltInNumSubgroups:
    ...
    return msl_options.is_ios() ? "quadgroups_per_threadgroup"
  ...
  case BuiltInSubgroupId:
    ...
    if (!msl_options.supports_msl_version(2))
      SPIRV_CROSS_THROW("Subgroup builtins require Metal 2.0.");
    return msl_options.is_ios() ? "quadgroup_index_in_threadgroup"
  ...
  case BuiltInSubgroupLocalInvocationId:
    ...
    if (execution.model == ExecutionModelFragment) {
      if (!msl_options.supports_msl_version(2, 2))
        SPIRV_CROSS_THROW("thread_index_in_simdgroup requires Metal 2.2 in "
                          "fragment shaders.");
      return "thread_index_in_simdgroup";
    } else if (execution.model == ExecutionModelKernel ||
      ...
      return msl_options.is_ios() ? "thread_index_in_quadgroup"
  ...
  case BuiltInBaryCoordNV:
    // TODO: AMD barycentrics as well? Seem to have different swizzle and 2
    // components rather than 3.
    if (msl_options.is_ios() && !msl_options.supports_msl_version(2, 3))
      SPIRV_CROSS_THROW(
...
const SPIRType &CompilerMSL::get_physical_member_type(const SPIRType &type,
                                                      uint32_t index) const {
  if (member_is_remapped_physical_type(type, index))
    return get<SPIRType>(get_extended_member_decoration(
        type.self, index, SPIRVCrossDecorationPhysicalTypeID));
  else
    return get<SPIRType>(type.member_types[index]);
}

SPIRType CompilerMSL::get_presumed_input_type(const SPIRType &ib_type,
                                              uint32_t index) const {
  ...
  uint32_t cmp =
      get_member_decoration(ib_type.self, index, DecorationComponent);
...
uint32_t CompilerMSL::get_declared_type_array_stride_msl(const SPIRType &type,
  ...
  // We could use parent type here and recurse, but that makes creating physical
  // type remappings
  // far more complicated. We'd rather just create the final type, and ignore
  // having to create the entire type
  // hierarchy in order to compute this value, so make a temporary type on the
  // stack.
  ...
  uint32_t value_size =
      get_declared_type_size_msl(basic_type, is_packed, row_major);
...
uint32_t
CompilerMSL::get_declared_struct_member_array_stride_msl(const SPIRType &type,
                                                         uint32_t index) const {
  return get_declared_type_array_stride_msl(
      get_physical_member_type(type, index),
      member_is_packed_physical_type(type, index),
      has_member_decoration(type.self, index, DecorationRowMajor));
}

uint32_t
CompilerMSL::get_declared_input_array_stride_msl(const SPIRType &type,
                                                 uint32_t index) const {
  return get_declared_type_array_stride_msl(
      get_presumed_input_type(type, index), false,
      has_member_decoration(type.self, index, DecorationRowMajor));
}

uint32_t CompilerMSL::get_declared_type_matrix_stride_msl(
    const SPIRType &type, bool packed, bool row_major) const {
  // For packed matrices, we just use the size of the vector type.
  // Otherwise, MatrixStride == alignment, which is the size of the underlying
  // vector type.
  if (packed)
    return (type.width / 8) *
           ((row_major && type.columns > 1) ? type.columns : type.vecsize);
  else
    return get_declared_type_alignment_msl(type, false, row_major);
}

uint32_t CompilerMSL::get_declared_struct_member_matrix_stride_msl(
    const SPIRType &type, uint32_t index) const {
  return get_declared_type_matrix_stride_msl(
      get_physical_member_type(type, index),
      member_is_packed_physical_type(type, index),
      has_member_decoration(type.self, index, DecorationRowMajor));
}
...
uint32_t
CompilerMSL::get_declared_input_matrix_stride_msl(const SPIRType &type,
                                                  uint32_t index) const {
  return get_declared_type_matrix_stride_msl(
      get_presumed_input_type(type, index), false,
      has_member_decoration(type.self, index, DecorationRowMajor));
...
uint32_t CompilerMSL::get_declared_struct_size_msl(const SPIRType &struct_type,
                                                   bool ignore_alignment,
                                                   bool ignore_padding) const {
  // If we have a target size, that is the declared size as well.
  if (!ignore_padding &&
      has_extended_decoration(struct_type.self,
                              SPIRVCrossDecorationPaddingTarget))
    return get_extended_decoration(struct_type.self,
                                   SPIRVCrossDecorationPaddingTarget);
  ...
  // In MSL, a struct's alignment is equal to the maximum alignment of any of
  // its members.
  ...
  if (!ignore_alignment) {
    for (uint32_t i = 0; i < mbr_cnt; i++) {
      uint32_t mbr_alignment =
          get_declared_struct_member_alignment_msl(struct_type, i);
      alignment = max(alignment, mbr_alignment);
    }
  }

  // Last member will always be matched to the final Offset decoration, but size
  // of struct in MSL now depends
  // on physical size in MSL, and the size of the struct itself is then aligned
  // to struct alignment.
  uint32_t spirv_offset = type_struct_member_offset(struct_type, mbr_cnt - 1);
  uint32_t msl_size = spirv_offset + get_declared_struct_member_size_msl(
                                         struct_type, mbr_cnt - 1);
  msl_size = (msl_size + alignment - 1) & ~(alignment - 1);
  return msl_size;
}

// Returns the byte size of a struct member.
uint32_t CompilerMSL::get_declared_type_size_msl(const SPIRType &type,
                                                 bool is_packed,
                                                 bool row_major) const {
  switch (type.basetype) {
  ...
  default: {
    if (!type.array.empty()) {
      uint32_t array_size = to_array_size_literal(type);
      return get_declared_type_array_stride_msl(type, is_packed, row_major) *
             max(array_size, 1u);
    }
    ...
    if (is_packed) {
      ...
      // An unpacked 3-element vector or matrix column is the same memory size
      // as a 4-element.
      uint32_t vecsize = type.vecsize;
      uint32_t columns = type.columns;
...
uint32_t
CompilerMSL::get_declared_struct_member_size_msl(const SPIRType &type,
                                                 uint32_t index) const {
  return get_declared_type_size_msl(
      get_physical_member_type(type, index),
      member_is_packed_physical_type(type, index),
      has_member_decoration(type.self, index, DecorationRowMajor));
}

uint32_t CompilerMSL::get_declared_input_size_msl(const SPIRType &type,
                                                  uint32_t index) const {
  return get_declared_type_size_msl(
      get_presumed_input_type(type, index), false,
      has_member_decoration(type.self, index, DecorationRowMajor));
...
uint32_t CompilerMSL::get_declared_type_alignment_msl(const SPIRType &type,
                                                      bool is_packed,
                                                      bool row_major) const {
  switch (type.basetype) {
  case SPIRType::Unknown:
  ...
  case SPIRType::Struct: {
    ...
    for (uint32_t i = 0; i < type.member_types.size(); i++)
      alignment =
          max(alignment,
              uint32_t(get_declared_struct_member_alignment_msl(type, i)));
    return alignment;
  }

  default: {
    if (type.basetype == SPIRType::Int64 &&
        !msl_options.supports_msl_version(2, 3))
      SPIRV_CROSS_THROW(
          "long types in buffers are only supported in MSL 2.3 and above.");
    if (type.basetype == SPIRType::UInt64 &&
        !msl_options.supports_msl_version(2, 3))
      SPIRV_CROSS_THROW(
          "ulong types in buffers are only supported in MSL 2.3 and above.");
    // Alignment of packed type is the same as the underlying component or
    // column size.
    // Alignment of unpacked type is the same as the vector size.
    // Alignment of 3-elements vector is the same as 4-elements (including
    // packed using column).
    if (is_packed) {
...
uint32_t
CompilerMSL::get_declared_struct_member_alignment_msl(const SPIRType &type,
                                                      uint32_t index) const {
...
uint32_t CompilerMSL::get_declared_input_alignment_msl(const SPIRType &type,
                                                       uint32_t index) const {
  return get_declared_type_alignment_msl(
      get_presumed_input_type(type, index), false,
      has_member_decoration(type.self, index, DecorationRowMajor));
...
bool CompilerMSL::skip_argument(uint32_t) const { return false; }
...
bool CompilerMSL::SampledImageScanner::handle(spv::Op opcode,
                                              const uint32_t *args,
                                              uint32_t length) {
  switch (opcode) {
  case OpLoad:
  ...
  case OpImageDrefGather:
    ...
    compiler.needs_swizzle_buffer_def =
        compiler.needs_swizzle_buffer_def || compiler.has_sampled_images;
    break;
  default:
    break;
  }
...
// If a needed custom function wasn't added before, add it and force a
// recompile.
...
bool CompilerMSL::OpCodePreprocessor::handle(Op opcode, const uint32_t *args,
                                             uint32_t length) {
  // Since MSL exists in a single execution scope, function prototype
  // declarations are not
  // needed, and clutter the output. If secondary functions are output (either
  // as a SPIR-V
  // function implementation or as indicated by the presence of OpFunctionCall),
  // then set
  // suppress_missing_prototypes to suppress compiler warnings of missing
  // function prototypes.

  // Mark if the input requires the implementation of an SPIR-V function that
  // does not exist in Metal.
  ...
  case OpAccessChain:
  case OpPtrAccessChain: {
    // OpArrayLength might want to know if taking ArrayLength of an array of
    // SSBOs.
    uint32_t result_type = args[0];
    uint32_t id = args[1];
    uint32_t ptr = args[2];
  ...
  case OpExtInst: {
    ...
    if (compiler.get<SPIRExtension>(extension_set).ext == SPIRExtension::GLSL) {
      ...
      case GLSLstd450InterpolateAtOffset: {
        ...
        // Fragment varyings used with pull-model interpolation need special
        // handling,
        ...
        if (var) {
          ...
          // In addition, if this variable has a 'Sample' decoration, we need
          // the sample ID
          ...
          if (compiler.has_decoration(var->self, DecorationSample)) {
            ...
            // Now we need to check each member and see if it has this
            // decoration.
            for (uint32_t i = 0; i < var_type.member_types.size(); ++i) {
              if (compiler.has_member_decoration(var_type.self, i,
                                                 DecorationSample)) {
                needs_sample_id = true;
                break;
              }
  ...
  if (compiler.instruction_to_result_type(result_type, result_id, opcode, args,
                                          length))
...
// If the variable is a Uniform or StorageBuffer, mark that a resource has been
// written to.
...
// Returns an enumeration of a SPIR-V function that needs to be output for
// certain Op codes.
CompilerMSL::SPVFuncImpl
CompilerMSL::OpCodePreprocessor::get_spv_func_impl(Op opcode,
                                                   const uint32_t *args) {
  switch (opcode) {
  case OpFMod:
  ...
  case OpImageRead:
  case OpImageWrite: {
    // Retrieve the image type, and if it's a Buffer, emit a texel coordinate
    // function
    uint32_t tid = result_types[args[opcode == OpImageWrite ? 0 : 2]];
    if (tid && compiler.get<SPIRType>(tid).image.dim == DimBuffer &&
        !compiler.msl_options.texture_buffer_native)
...
// Sort both type and meta member content based on builtin status (put builtins
// at end),
// then by the required sorting aspect.
void CompilerMSL::MemberSorter::sort() {
  // Create a temporary array of consecutive member indices and sort it based on
  // how
  // the members should be reordered, based on builtin and sorting aspect meta
  // info.
  size_t mbr_cnt = type.member_types.size();
  SmallVector<uint32_t> mbr_idxs(mbr_cnt);
  std::iota(mbr_idxs.begin(), mbr_idxs.end(),
            0); // Fill with consecutive indices
  std::stable_sort(mbr_idxs.begin(), mbr_idxs.end(),
                   *this); // Sort member indices based on sorting aspect
  ...
  // Move type and meta member info to the order defined by the sorted member
  // indices.
  // This is done by creating temporary copies of both member types and meta,
  // and then
  ...
  // If we're sorting by Offset, this might affect user code which accesses a
  // buffer block.
  // We will need to redirect member indices from defined index to sorted index
...
bool CompilerMSL::MemberSorter::operator()(uint32_t mbr_idx1,
                                           uint32_t mbr_idx2) {
  auto &mbr_meta1 = meta.members[mbr_idx1];
  auto &mbr_meta2 = meta.members[mbr_idx2];

  if (sort_aspect == LocationThenBuiltInType) {
    // Sort first by builtin status (put builtins at end), then by the sorting
    // aspect.
...
void CompilerMSL::remap_constexpr_sampler_by_binding(
    uint32_t desc_set, uint32_t binding, const MSLConstexprSampler &sampler) {
  constexpr_samplers_by_binding[{desc_set, binding}] = sampler;
}

void CompilerMSL::cast_from_variable_load(uint32_t source_id, std::string &expr,
  ...
  auto builtin =
      static_cast<BuiltIn>(get_decoration(source_id, DecorationBuiltIn));
  ...
  if (expected_type != expr_type.basetype) {
    if (!expr_type.array.empty() && (builtin == BuiltInTessLevelInner ||
                                     builtin == BuiltInTessLevelOuter)) {
  ...
  if (builtin == BuiltInTessCoord &&
      get_entry_point().flags.get(ExecutionModeQuads) &&
      expr_type.vecsize == 3) {
    // In SPIR-V, this is always a vec3, even for quads. In Metal, though, it's
    // a float2 for quads.
...
void CompilerMSL::cast_to_variable_store(uint32_t target_id, std::string &expr,
                                         const SPIRType &expr_type) {
  ...
  auto builtin =
      static_cast<BuiltIn>(get_decoration(target_id, DecorationBuiltIn));
  auto expected_type = expr_type.basetype;
  auto expected_width = expr_type.width;
  switch (builtin) {
  case BuiltInLayer:
...
string CompilerMSL::to_initializer_expression(const SPIRVariable &var) {
  ...
  // If the initializer has more vector components than the variable, add a
  // swizzle.
  ...
  if (type.array.empty() && type.basetype != SPIRType::Struct &&
      init_type.vecsize > type.vecsize)
    expr = enclose_expression(expr + vector_swizzle(type.vecsize, 0));
  return expr;
}

string CompilerMSL::to_zero_initialized_expression(uint32_t) { return "{}"; }
...
bool CompilerMSL::is_supported_argument_buffer_type(
    const SPIRType &type) const {
  // Very specifically, image load-store in argument buffers are disallowed on
  // MSL on iOS.
  // But we won't know when the argument buffer is encoded whether this image
  // will have
  // a NonWritable decoration. So just use discrete arguments for all storage
  // images
  // on iOS.
  bool is_storage_image =
      type.basetype == SPIRType::Image && type.image.sampled == 2;
...
void CompilerMSL::analyze_argument_buffers() {
  ...
  // The [[id(N)]] values used correspond to the resource mapping we have for
  // MSL.
  // Otherwise, the binding number is used, but this is generally not safe some
  // types like
  // combined image samplers and arrays of resources. Metal needs different
  // indices here,
  // while SPIR-V can have one descriptor set binding. To use argument buffers
  // in practice,
      ...
      if (type.basetype == SPIRType::SampledImage) {
        ...
        for (uint32_t i = 0; i < plane_count; i++) {
          uint32_t image_resource_index =
              get_metal_resource_index(var, SPIRType::Image, i);
          resources_in_set[desc_set].push_back({&var, to_name(var_id),
                                                SPIRType::Image,
                                                image_resource_index, i});
        }
        ...
        if (type.image.dim != DimBuffer && !constexpr_sampler) {
          ...
          resources_in_set[desc_set].push_back(
              {&var, to_sampler_expression(var_id), SPIRType::Sampler,
               sampler_resource_index, 0});
        }
      } else if (inline_uniform_blocks.count(
                     SetBindingPair{desc_set, binding})) {
        inline_block_vars.push_back(var_id);
      } else if (!constexpr_sampler &&
                 is_supported_argument_buffer_type(type)) {
        // constexpr samplers are not declared as resources.
        // Inline uniform blocks are always emitted at the end.
        add_resource_name(var_id);
        resources_in_set[desc_set].push_back(
            {&var, to_name(var_id), type.basetype,
             get_metal_resource_index(var, type.basetype), 0});
        ...
        if (atomic_image_vars.count(var.self)) {
          uint32_t buffer_resource_index =
              get_metal_resource_index(var, SPIRType::AtomicCounter, 0);
          resources_in_set[desc_set].push_back(
              {&var, to_name(var_id) + "_atomic", SPIRType::Struct,
  ...
  if (needs_swizzle_buffer_def || needs_buffer_sizes) {
    ...
    for (uint32_t desc_set = 0; desc_set < kMaxArgumentBuffers; desc_set++) {
      if (!set_needs_swizzle_buffer[desc_set] &&
          !set_needs_buffer_sizes[desc_set])
        continue;
      ...
      if (set_needs_swizzle_buffer[desc_set]) {
        uint32_t var_id = ir.increase_bound_by(1);
        auto &var = set<SPIRVariable>(var_id, uint_ptr_type_id,
                                      StorageClassUniformConstant);
        set_name(var_id, "spvSwizzleConstants");
        set_decoration(var_id, DecorationDescriptorSet, desc_set);
        set_decoration(var_id, DecorationBinding, kSwizzleBufferBinding);
        resources_in_set[desc_set].push_back(
            {&var, to_name(var_id), SPIRType::UInt,
             get_metal_resource_index(var, SPIRType::UInt), 0});
      }
  ...
  for (uint32_t var_id : inline_block_vars) {
    ...
    add_resource_name(var_id);
    resources_in_set[desc_set].push_back(
        {&var, to_name(var_id), SPIRType::Struct,
         get_metal_resource_index(var, SPIRType::Struct), 0});
  }
  ...
  for (uint32_t desc_set = 0; desc_set < kMaxArgumentBuffers; desc_set++) {
    ...
    sort(begin(resources), end(resources),
         [&](const Resource &lhs, const Resource &rhs) -> bool {
    ...
    for (auto &resource : resources) {
      ...
      // member_index and next_arg_buff_index are incremented when padding
      // members are added.
      if (msl_options.pad_argument_buffer_resources) {
        while (resource.index > next_arg_buff_index) {
          auto &rez_bind =
              get_argument_buffer_resource(desc_set, next_arg_buff_index);
          ...
          case SPIRType::Double:
            add_argument_buffer_padding_buffer_type(
                buffer_type, member_index, next_arg_buff_index, rez_bind);
            break;
          case SPIRType::Image:
        ...
        uint32_t elem_cnt =
            type.array.empty() ? 1 : to_array_size_literal(type);
        if (elem_cnt == 0)
      ...
      if (resource.basetype == SPIRType::Sampler &&
          type.basetype != SPIRType::Sampler) {
        ...
        if (resource.basetype == SPIRType::Image ||
            resource.basetype == SPIRType::Sampler ||
            resource.basetype == SPIRType::SampledImage) {
          // Drop pointer information when we emit the resources into a struct.
          buffer_type.member_types.push_back(get_variable_data_type_id(var));
          if (resource.plane == 0)
            set_qualified_name(
                var.self, join(to_name(buffer_variable_id), ".", mbr_name));
        } else if (buffers_requiring_dynamic_offset.count(pair)) {
          // Don't set the qualified name here; we'll define a variable holding
          // the corrected buffer address later.
          ...
          buffer_type.member_types.push_back(get_variable_data_type_id(var));
          set_qualified_name(var.self,
                             join(to_name(buffer_variable_id), ".", mbr_name));
          ...
          // Resources will be declared as pointers not references, so
          // automatically dereference as appropriate.
          buffer_type.member_types.push_back(var.basetype);
          if (type.array.empty())
            set_qualified_name(var.self, join("(*", to_name(buffer_variable_id),
                                              ".", mbr_name, ")"));
          else
            set_qualified_name(
                var.self, join(to_name(buffer_variable_id), ".", mbr_name));
      ...
      set_extended_member_decoration(buffer_type.self, member_index,
                                     SPIRVCrossDecorationResourceIndexPrimary,
                                     resource.index);
      set_extended_member_decoration(buffer_type.self, member_index,
                                     SPIRVCrossDecorationInterfaceOrigID,
...
// Return the resource type of the app-provided resources for the descriptor
// set,
// that matches the resource index of the argument buffer index.
// This is a two-step lookup, first lookup the resource binding number from the
// argument buffer index,
// then lookup the resource binding using the binding number.
MSLResourceBinding &
CompilerMSL::get_argument_buffer_resource(uint32_t desc_set, uint32_t arg_idx) {
  ...
  SPIRV_CROSS_THROW("Argument buffer resource base type could not be "
                    "determined. When padding argument buffer "
                    "elements, all descriptor set resources must be supplied "
                    "with a base type by the app.");
}

// Adds an argument buffer padding argument buffer type as one or more members
// of the struct type at the member index.
// Metal does not support arrays of buffers, so these are emitted as multiple
// struct members.
void CompilerMSL::add_argument_buffer_padding_buffer_type(
    SPIRType &struct_type, uint32_t &mbr_idx, uint32_t &arg_buff_index,
  ...
  for (uint32_t rez_idx = 0; rez_idx < rez_bind.count; rez_idx++)
    add_argument_buffer_padding_type(argument_buffer_padding_buffer_type_id,
                                     struct_type, mbr_idx, arg_buff_index, 1);
}

// Adds an argument buffer padding argument image type as a member of the struct
// type at the member index.
void CompilerMSL::add_argument_buffer_padding_image_type(
    SPIRType &struct_type, uint32_t &mbr_idx, uint32_t &arg_buff_index,
    MSLResourceBinding &rez_bind) {
  ...
  add_argument_buffer_padding_type(argument_buffer_padding_image_type_id,
                                   struct_type, mbr_idx, arg_buff_index,
                                   rez_bind.count);
}

// Adds an argument buffer padding argument sampler type as a member of the
// struct type at the member index.
void CompilerMSL::add_argument_buffer_padding_sampler_type(
    SPIRType &struct_type, uint32_t &mbr_idx, uint32_t &arg_buff_index,
    MSLResourceBinding &rez_bind) {
  ...
  add_argument_buffer_padding_type(argument_buffer_padding_sampler_type_id,
                                   struct_type, mbr_idx, arg_buff_index,
                                   rez_bind.count);
}

// Adds the argument buffer padding argument type as a member of the struct type
// at the member index.
// Advances both arg_buff_index and mbr_idx to next argument slots.
void CompilerMSL::add_argument_buffer_padding_type(uint32_t mbr_type_id,
  ...
  set_member_name(struct_type.self, mbr_idx,
                  join("_m", arg_buff_index, "_pad"));
  set_extended_member_decoration(struct_type.self, mbr_idx,
                                 SPIRVCrossDecorationResourceIndexPrimary,
                                 arg_buff_index);
...
void CompilerMSL::activate_argument_buffer_resources() {
  // For ABI compatibility, force-enable all resources which are part of
  // argument buffers.
