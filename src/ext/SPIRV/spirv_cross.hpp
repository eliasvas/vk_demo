...
struct Resource {
  ...
  // The type ID of the variable which includes arrays and all type
  // modifications.
  // This type ID is not suitable for parsing OpMemberDecoration of a struct and
  // other decorations in general
  // since these modifications typically happen on the base_type_id.
  TypeID type_id;

  // The base type of the declared resource.
  // This type is the base type which ignores pointers and arrays of the
  // type_id.
  ...
  // get_name(id) or get_name(base_type_id) depending if it's a buffer block or
  // not.
  //
...
struct BuiltInResource {
  // This is mostly here to support reflection of builtins such as
  // Position/PointSize/CullDistance/ClipDistance.
  // This needs to be different from Resource since we can collect builtins from
  // blocks.
  // A builtin present here does not necessarily mean it's considered an active
  // builtin,
  // since variable ID "activeness" is only tracked on OpVariable level, not
  // Block members.
  // For that, update_active_builtins() -> has_active_builtin() can be used to
  // further refine the reflection.
  spv::BuiltIn builtin;

  // This is the actual value type of the builtin.
  // Typically float4, float, array<float, N> for the gl_PerVertex builtins.
  // If the builtin is a control point, the control point array type will be
  // stripped away here as appropriate.
  TypeID value_type_id;

  // This refers to the base resource which contains the builtin.
  // If resource is a Block, it can hold multiple builtins, or it might not be a
  // block.
  // For advanced reflection scenarios, all information in builtin/value_type_id
  // can be deduced,
...
public:
  ...
  // This is more modular. We can also consume a ParsedIR structure directly,
  // either as a move, or copy.
  ...
  // Gets the identifier (OpName) of an ID. If not defined, an empty string will
  // be returned.
  ...
  void set_decoration_string(ID id, spv::Decoration decoration,
                             const std::string &argument);

  // Overrides the identifier OpName of an ID.
  // Identifiers beginning with underscores or identifiers which contain double
  // underscores
  ...
  const std::string &get_decoration_string(ID id,
  ...
  // Mostly used with Resource::type_id and Resource::base_type_id to parse the
  // underlying type of a resource.
  ...
  // This needs to be per-variable as multiple variables can use the same block
  // type.
  virtual const std::string get_block_fallback_name(VariableID id) const;

  // Given an OpTypeStruct in ID, obtain the identifier for member number
  // "index".
  // This may be an empty string.
  const std::string &get_member_name(TypeID id, uint32_t index) const;

  // Given an OpTypeStruct in ID, obtain the OpMemberDecoration for member
  // number "index".
  uint32_t get_member_decoration(TypeID id, uint32_t index,
                                 spv::Decoration decoration) const;
  const std::string &
  get_member_decoration_string(TypeID id, uint32_t index,
                               spv::Decoration decoration) const;

  // Sets the member identifier for OpTypeStruct ID, member number "index".
  void set_member_name(TypeID id, uint32_t index, const std::string &name);

  // Returns the qualified member identifier for OpTypeStruct ID, member number
  // "index",
  // or an empty string if no qualified alias exists
  const std::string &get_member_qualified_name(TypeID type_id,
                                               uint32_t index) const;

  // Gets the decoration mask for a member of a struct, similar to
  // get_decoration_mask.
  ...
  bool has_member_decoration(TypeID id, uint32_t index,
                             spv::Decoration decoration) const;

  // Similar to set_decoration, but for struct members.
  void set_member_decoration(TypeID id, uint32_t index,
                             spv::Decoration decoration, uint32_t argument = 0);
  void set_member_decoration_string(TypeID id, uint32_t index,
                                    spv::Decoration decoration,
                                    const std::string &argument);

  // Unsets a member decoration, similar to unset_decoration.
  void unset_member_decoration(TypeID id, uint32_t index,
                               spv::Decoration decoration);
  ...
  // This can be used for Buffer (UBO), BufferBlock/StorageBuffer (SSBO) and
  // PushConstant blocks.
  ...
  // for a runtime array.
  // SSBOs are typically declared as runtime arrays. get_declared_struct_size()
  // will return 0 for the size.
  // This is not very helpful for applications which might need to know the
  // array stride of its last member.
  // This can be done through the API, but it is not very intuitive how to
  // accomplish this, so here we provide a helper function
  // to query the size of the buffer, assuming that the last member has a
  // certain size.
  // If the buffer does not contain a runtime array, array_size is ignored, and
  // the function will behave as
  // get_declared_struct_size().
  // To get the array stride of the last member, something like:
  // get_declared_struct_size_runtime_array(type, 1) -
  // get_declared_struct_size_runtime_array(type, 0) will work.
  size_t get_declared_struct_size_runtime_array(const SPIRType &struct_type,
                                                size_t array_size) const;

  // Returns the effective size of a buffer block struct member.
  size_t get_declared_struct_member_size(const SPIRType &struct_type,
                                         uint32_t index) const;

  // Returns a set of all global variables which are statically accessed
  // by the control flow graph from the current entry point.
  // Only variables which change the interface for a shader are returned, that
  // is,
  // variables with storage class of Input, Output, Uniform, UniformConstant,
  // PushConstant and AtomicCounter
  // storage classes are returned.
  //
  // To use the returned set as the filter for which variables are used during
  // compilation,
  ...
  // Query shader resources, use ids with reflection interface to modify or
  // query binding points, etc.
  ShaderResources get_shader_resources() const;

  // Query shader resources, but only return the variables which are part of
  // active_variables.
  // E.g.: get_shader_resources(get_active_variables()) to only return the
  // variables which are statically
  // accessed.
  ShaderResources get_shader_resources(
      const std::unordered_set<VariableID> &active_variables) const;

  // Remapped variables are considered built-in variables and a backend will
  // not emit a declaration for this variable.
  // This is mostly useful for making use of builtins which are dependent on
  // extensions.
  ...
  // variable must be specified as the backing type of subpass inputs are
  // opaque.
  void set_subpass_input_remapped_components(VariableID id,
                                             uint32_t components);
  ...
  // Entry points should be set right after the constructor completes as some
  // reflection functions traverse the graph from the entry point.
  // Resource reflection also depends on the entry point.
  // By default, the current entry point is set to the first OpEntryPoint which
  // appears in the SPIR-V module.

  // Some shader languages restrict the names that can be given to entry points,
  // and the
  // corresponding backend will automatically rename an entry point name, during
  // the call
  // to compile() if it is illegal. For example, the common entry point name
  // main() is
  // illegal in MSL, and is renamed to an alternate name by the MSL backend.
  // Given the original entry point name contained in the SPIR-V, this function
  // returns
  // the name, as updated by the backend during the call to compile(). If the
  // name is not
  // illegal, and has not been renamed, or if this function is called before
  // compile(),
  // this function will simply return the same name.

  // New variants of entry point query and reflection.
  // Names for entry points in the SPIR-V module may alias if they belong to
  // different execution models.
  // To disambiguate, we must pass along with the entry point names the
  // execution model.
  ...
  // If old_name is currently selected as the current entry point, it will
  // continue to be the current entry point,
  // albeit with a new name.
  // get_entry_points() is essentially invalidated at this point.
  void rename_entry_point(const std::string &old_name,
                          const std::string &new_name,
                          spv::ExecutionModel execution_model);
  const SPIREntryPoint &
  get_entry_point(const std::string &name,
                  spv::ExecutionModel execution_model) const;
  SPIREntryPoint &get_entry_point(const std::string &name,
                                  spv::ExecutionModel execution_model);
  const std::string &
  get_cleansed_entry_point_name(const std::string &name,
                                spv::ExecutionModel execution_model) const;

  // Traverses all reachable opcodes and sets active_builtins to a bitmask of
  // all builtin variables which are accessed in the shader.
  void update_active_builtins();
  bool has_active_builtin(spv::BuiltIn builtin,
  ...
  void set_execution_mode(spv::ExecutionMode mode, uint32_t arg0 = 0,
                          uint32_t arg1 = 0, uint32_t arg2 = 0);

  // Gets argument for an execution mode (LocalSize, Invocations,
  // OutputVertices).
  // For LocalSize, the index argument is used to select the dimension (X = 0, Y
  // = 1, Z = 2).
  // For execution modes which do not have arguments, 0 is returned.
  uint32_t get_execution_mode_argument(spv::ExecutionMode mode,
                                       uint32_t index = 0) const;
  spv::ExecutionModel get_execution_model() const;

  bool is_tessellation_shader() const;

  // In SPIR-V, the compute work group size can be represented by a constant
  // vector, in which case
  // the LocalSize execution mode is ignored.
  //
  // This constant vector can be a constant vector, specialization constant
  // vector, or partly specialized constant vector.
  // To modify and query work group dimensions which are specialization
  // constants, SPIRConstant values must be modified
  // directly via get_constant() rather than using LocalSize directly. This
  // function will return which constants should be modified.
  //
  // To modify dimensions which are *not* specialization constants,
  // set_execution_mode should be used directly.
  // Arguments to set_execution_mode which are specialization constants are
  // effectively ignored during compilation.
  // NOTE: This is somewhat different from how SPIR-V works. In SPIR-V, the
  // constant vector will completely replace LocalSize,
  // while in this interface, LocalSize is only ignored for specialization
  // constants.
  //
  // The specialization constant will be written to x, y and z arguments.
  // If the component is not a specialization constant, a zeroed out struct will
  // be written.
  // The return value is the constant ID of the builtin WorkGroupSize, but this
  // is not expected to be useful
  // for most use cases.
  uint32_t
  get_work_group_size_specialization_constants(SpecializationConstant &x,
                                               SpecializationConstant &y,
                                               SpecializationConstant &z) const;

  // Analyzes all OpImageFetch (texelFetch) opcodes and checks if there are
  // instances where
  ...
  // To workaround this, we must inject a dummy sampler which can be used to
  // form a sampler2D at the call-site of
  // texelFetch as necessary.
  //
  // This must be called before build_combined_image_samplers().
  // build_combined_image_samplers() may refer to the ID returned by this method
  // if the returned ID is non-zero.
  // The return value will be the ID of a sampler object if a dummy sampler is
  // necessary, or 0 if no sampler object
  // is required.
  //
  // If the returned ID is non-zero, it can be decorated with set/bindings as
  // desired before calling compile().
  // Calling this function also invalidates get_active_interface_variables(), so
  ...
  // This is required to "support" separate image samplers in targets which do
  ...
  // so it will appear in reflection if get_shader_resources() is called after
  // build_combined_image_samplers.
  //
  // If any image/sampler remapping was found, no separate image/samplers will
  // appear in the decompiled output,
  // but will still appear in reflection.
  //
  // The resulting samplers will be void of any decorations like name,
  // descriptor sets and binding points,
  // so this can be added before compile() if desired.
  //
  // Combined image samplers originating from this set are always considered
  // active variables.
  // Arrays of separate samplers are not supported, but arrays of separate
  ...
  // The type remapping is designed to allow global interface variable to assume
  // more special types.
  // A typical example here is to remap sampler2D into samplerExternalOES, which
  ...
  // and the API user has a chance to change the textual representation of the
  ...
  // To modify a specialization constant before compile(), use
  // get_constant(constant.id),
  ...
  // which can be queried to determine which fields in the unions should be
  // poked at.
  ...
  // The type passed in here must have these decorations set, or an exception is
  // raised.
  // Only UBOs and SSBOs or sub-structs which are part of these buffer types
  // will have these decorations set.
  uint32_t type_struct_member_offset(const SPIRType &type,
                                     uint32_t index) const;
  uint32_t type_struct_member_array_stride(const SPIRType &type,
                                           uint32_t index) const;
  uint32_t type_struct_member_matrix_stride(const SPIRType &type,
                                            uint32_t index) const;

  // Gets the offset in SPIR-V words (uint32_t) for a decoration which was
  // originally declared in the SPIR-V binary.
  // The offset will point to one or more uint32_t literals which can be
  // modified in-place before using the SPIR-V binary.
  // Note that adding or removing decorations using the reflection API will not
  // change the behavior of this function.
  // If the decoration was declared, sets the word_offset to an offset into the
  // provided SPIR-V binary buffer and returns true,
  // otherwise, returns false.
  // If the decoration does not have any value attached to it (e.g.
  // DecorationRelaxedPrecision), this function will also return false.
  bool get_binary_offset_for_decoration(VariableID id,
                                        spv::Decoration decoration,
                                        uint32_t &word_offset) const;

  // HLSL counter buffer reflection interface.
  // Append/Consume/Increment/Decrement in HLSL is implemented as two "neighbor"
  // buffer objects where
  // one buffer implements the storage, and a single buffer containing just a
  // lone "int" implements the counter.
  // To SPIR-V these will be exposed as two separate buffers, but glslang HLSL
  // frontend emits a special indentifier
  // which lets us link the two buffers together.

  // Queries if a variable ID is a counter buffer which "belongs" to a regular
  // buffer object.

  // If SPV_GOOGLE_hlsl_functionality1 is used, this can be used even with a
  // stripped SPIR-V module.
  // Otherwise, this query is purely based on OpName identifiers as found in the
  // SPIR-V module, and will
  // only return true if OpSource was reported HLSL.
  // To rely on this functionality, ensure that the SPIR-V module is not
  // stripped.
  ...
  // To rely on this functionality, ensure that the SPIR-V module is not
  // stripped.
  bool buffer_get_hlsl_counter_buffer(VariableID id,
                                      uint32_t &counter_id) const;

  // Gets the list of all SPIR-V Capabilities which were declared in the SPIR-V
  // module.
  const SmallVector<spv::Capability> &get_declared_capabilities() const;

  // Gets the list of all SPIR-V extensions which were declared in the SPIR-V
  // module.
  ...
  // might not be the same as the name declared in the SPIR-V module due to
  // naming conflicts.
  ...
  // This is particularly important for HLSL input and UAVs which tends to reuse
  // the same block type
  // for multiple distinct blocks. For these cases it is not possible to modify
  // the name of the type itself
  // because it might be unique. Instead, you can use this interface to check
  // after compilation which
  // name was actually used if your input SPIR-V tends to have this problem.
  // For other names like remapped names for variables, etc, it's generally
  // enough to query the name of the variables
  // after compiling, block names are an exception to this rule.
  // ID is the name of a variable as returned by Resource::id, and must be a
  // variable with a Block-like type.
  //
  ...
  // Sometimes, decorations for buffer blocks are found in member decorations
  // instead
...
protected:
  ...
  ParsedIR ir;
  // Marks variables which have global scope and variables which can alias with
  // other variables
  ...
  // Gets the id of SPIR-V type underlying the given type_id, which might be a
  // pointer.
  ...
  // Gets the SPIR-V type underlying the given type_id, which might be a
  // pointer.
  ...
  // Sets the qualified member identifier for OpTypeStruct ID, member number
  // "index".
  void set_member_qualified_name(uint32_t type_id, uint32_t index,
  ...
  bool is_hidden_variable(const SPIRVariable &var,
                          bool include_builtins = false) const;
  bool is_immutable(uint32_t id) const;
  bool is_member_builtin(const SPIRType &type, uint32_t index,
  ...
  inline bool is_single_block_loop(uint32_t next) const {
    ...
    return block.merge == SPIRBlock::MergeLoop &&
           block.continue_block == ID(next);
  }

  inline bool is_break(uint32_t next) const {
    return (ir.block_meta[next] &
            (ParsedIR::BLOCK_META_LOOP_MERGE_BIT |
             ParsedIR::BLOCK_META_MULTISELECT_MERGE_BIT)) != 0;
  }
  ...
  void update_name_cache(std::unordered_set<std::string> &cache,
                         std::string &name);

  // A variant which takes two sets of names. The secondary is only used to
  ...
  bool execution_is_branchless(const SPIRBlock &from,
                               const SPIRBlock &to) const;
  bool execution_is_direct_branch(const SPIRBlock &from,
                                  const SPIRBlock &to) const;
  bool execution_is_noop(const SPIRBlock &from, const SPIRBlock &to) const;
  SPIRBlock::ContinueBlockType
  continue_block_type(const SPIRBlock &continue_block) const;
  ...
  bool block_is_loop_candidate(const SPIRBlock &block,
                               SPIRBlock::Method method) const;

  bool types_are_logically_equivalent(const SPIRType &a,
  ...
  // For proper multiple entry point support, allow querying if an Input or
  // Output
  ...
  void remap_variable_type_name(const SPIRType &type,
  ...
  struct OpcodeHandler {
    ...
    virtual bool handle(spv::Op opcode, const uint32_t *args,
                        uint32_t length) = 0;
  ...
  struct BufferAccessHandler : OpcodeHandler {
    BufferAccessHandler(const Compiler &compiler_,
                        SmallVector<BufferRange> &ranges_, uint32_t id_)
  ...
  struct InterfaceVariableAccessHandler : OpcodeHandler {
    InterfaceVariableAccessHandler(const Compiler &compiler_,
                                   std::unordered_set<VariableID> &variables_)
        : compiler(compiler_), variables(variables_) {}
  ...
  struct CombinedImageSamplerHandler : OpcodeHandler {
    CombinedImageSamplerHandler(Compiler &compiler_) : compiler(compiler_) {}
    bool handle(spv::Op opcode, const uint32_t *args, uint32_t length) override;
    ...
    // Each function in the call stack needs its own remapping for parameters so
    // we can deduce which global variable each texture/sampler the parameter is
    // statically bound to.
    ...
    void push_remap_parameters(const SPIRFunction &func, const uint32_t *args,
                               uint32_t length);
    void pop_remap_parameters();
    void register_combined_image_sampler(SPIRFunction &caller,
                                         VariableID combined_id,
                                         VariableID texture_id,
  ...
  struct ActiveBuiltinHandler : OpcodeHandler {
    ActiveBuiltinHandler(Compiler &compiler_) : compiler(compiler_) {}
  ...
  bool traverse_all_reachable_opcodes(const SPIRBlock &block,
                                      OpcodeHandler &handler) const;
  bool traverse_all_reachable_opcodes(const SPIRFunction &block,
                                      OpcodeHandler &handler) const;
  // This must be an ordered data structure so we always pick the same type
  // aliases.
  ...
  bool get_common_basic_type(const SPIRType &type,
  ...
  // If a variable ID or parameter ID is found in this set, a sampler is
  // actually a shadow/comparison sampler.
  // SPIR-V does not support this distinction, so we must keep track of this
  // information outside the type system.
  // There might be unrelated IDs found in this set which do not correspond to
  // actual variables.
  // This set should only be queried for the existence of samplers which are
  // already known to be variables or parameter IDs.
  // Similar is implemented for images, as well as if subpass inputs are needed.
  std::unordered_set<uint32_t> comparison_ids;
  bool need_subpass_input = false;

  // In certain backends, we will need to use a dummy sampler to be able to emit
  // code.
  ...
  struct CombinedImageSamplerUsageHandler : OpcodeHandler {
    CombinedImageSamplerUsageHandler(
        ...
        : compiler(compiler_), dref_combined_samplers(dref_combined_samplers_) {
    }
  ...
  struct AnalyzeVariableScopeAccessHandler : OpcodeHandler {
    AnalyzeVariableScopeAccessHandler(Compiler &compiler_,
                                      SPIRFunction &entry_);
  ...
  struct PhysicalStorageBufferPointerHandler : OpcodeHandler {
    ...
    std::unordered_map<uint32_t, PhysicalBlockMeta *>
        access_chain_to_physical_block;

    void mark_aligned_access(uint32_t id, const uint32_t *args,
                             uint32_t length);
  ...
  std::unordered_map<uint32_t, PhysicalBlockMeta>
      physical_storage_type_to_alignment;

  void analyze_variable_scope(SPIRFunction &function,
                              AnalyzeVariableScopeAccessHandler &handler);
  void
  find_function_local_luts(SPIRFunction &function,
                           const AnalyzeVariableScopeAccessHandler &handler,
                           bool single_function);
  bool may_read_undefined_variable_in_block(const SPIRBlock &block,
                                            uint32_t var);

  // Finds all resources that are written to from inside the critical section,
  // if present.
  // The critical section is delimited by OpBeginInvocationInterlockEXT and
  // OpEndInvocationInterlockEXT instructions. In MSL and HLSL, any resources
  // written
  ...
  struct InterlockedResourceAccessPrepassHandler : OpcodeHandler {
    InterlockedResourceAccessPrepassHandler(Compiler &compiler_,
                                            uint32_t entry_point_id)
        : compiler(compiler_) {
      call_stack.push_back(entry_point_id);
    }
  ...
  // The set of all resources written while inside the critical section, if
  // present.
  ...
  bool instruction_to_result_type(uint32_t &result_type, uint32_t &result_id,
                                  spv::Op op, const uint32_t *args,
                                  uint32_t length);

  Bitset combined_decoration_for_member(const SPIRType &type,
                                        uint32_t index) const;
  static bool is_desktop_only_format(spv::ImageFormat format);

  bool is_depth_image(const SPIRType &type, uint32_t id) const;

  void set_extended_decoration(uint32_t id, ExtendedDecorations decoration,
                               uint32_t value = 0);
  uint32_t get_extended_decoration(uint32_t id,
                                   ExtendedDecorations decoration) const;
  bool has_extended_decoration(uint32_t id,
                               ExtendedDecorations decoration) const;
  void unset_extended_decoration(uint32_t id, ExtendedDecorations decoration);

  void set_extended_member_decoration(uint32_t type, uint32_t index,
                                      ExtendedDecorations decoration,
                                      uint32_t value = 0);
  uint32_t get_extended_member_decoration(uint32_t type, uint32_t index,
                                          ExtendedDecorations decoration) const;
  bool has_extended_member_decoration(uint32_t type, uint32_t index,
                                      ExtendedDecorations decoration) const;
  void unset_extended_member_decoration(uint32_t type, uint32_t index,
                                        ExtendedDecorations decoration);
  ...
  std::string
  get_remapped_declared_block_name(uint32_t id,
  ...
  get_case_list(const SPIRBlock &block) const;
