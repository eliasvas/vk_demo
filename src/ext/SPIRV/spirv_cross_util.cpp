...
void rename_interface_variable(Compiler &compiler,
  ...
  for (auto &v : resources) {
    ...
    // This is more of a friendly variant. If we need to rename interface
    // variables, we might have to rename
    // structs as well and make sure all the names match up.
    if (type.basetype == SPIRType::Struct) {
      compiler.set_name(v.base_type_id,
                        join("SPIRV_Cross_Interface_Location", location));
...
void inherit_combined_sampler_bindings(Compiler &compiler) {
  ...
  for (auto &s : samplers) {
    if (compiler.has_decoration(s.image_id, spv::DecorationDescriptorSet)) {
      uint32_t set =
          compiler.get_decoration(s.image_id, spv::DecorationDescriptorSet);
    ...
    if (compiler.has_decoration(s.image_id, spv::DecorationBinding)) {
      uint32_t binding =
          compiler.get_decoration(s.image_id, spv::DecorationBinding);
