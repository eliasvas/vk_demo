...
// This data structure holds all information needed to perform cross-compilation
// and reflection.
// It is the output of the Parser, but any implementation could create this
// structure.
// It is intentionally very "open" and struct-like with some helper functions to
// deal with decorations.
// Parser is the reference implementation of how this data structure should be
// filled in.
...
public:
  ...
  // Due to custom allocations from object pools, we cannot use a default copy
  // constructor.
  ParsedIR(const ParsedIR &other);
  ParsedIR &operator=(const ParsedIR &other);

  // Moves are unproblematic, but we need to implement it anyways, since MSVC
  // 2013 does not understand
  ...
  // This is needed so we can iterate through a specific kind of resource
  // quickly,
  // and in-order of module declaration.
  SmallVector<ID> ids_for_type[TypeCount];

  // Special purpose lists which contain a union of types.
  // This is needed so we can declare specialization constants and structs in an
  // interleaved fashion,
  // among other things.
  // Constants can be of struct type, and struct array sizes can use
  ...
  // Meta data about blocks. The cross-compiler needs to query if a block is
  ...
  // Normally, we'd stick SPIREntryPoint in ids array, but it conflicts with
  // SPIRFunction.
  ...
  // and might as well just have the whole suite of decoration/name handling in
  // one place.
  ...
  // It is undefined if someone calls set<>() while we're iterating over a data
  // structure, so we must
  // make sure that this case is avoided.

  // If we have a hard lock, it is an error to call set<>(), and an exception is
  // thrown.
  // If we have a soft lock, we silently ignore any additions to the typed
  // arrays.
  // This should only be used for physical ID remapping where we need to create
  // an ID, but we will never
  ...
  template <typename T> void reset_all_of_type() {
  ...
  static void sanitize_identifier(std::string &str, bool member,
                                  bool allow_reserved_prefixes);
  static bool is_globally_reserved_identifier(std::string &str,
                                              bool allow_reserved_prefixes);
