...
private:
  ...
  template <typename... Ts> void statement_no_return(Ts &&... ts) {
    statement_indent();
    statement_inner(std::forward<Ts>(ts)...);
  }
};
...
// Hackery to emit JSON without using nlohmann/json C++ library (which requires
// a
...
string CompilerReflection::compile() {
  ...
  json_stream->set_current_locale_radix_character(
...
static bool naturally_emit_type(const SPIRType &type) {
  return type.basetype == SPIRType::Struct && !type.pointer &&
         type.array.empty();
}

bool CompilerReflection::type_is_reference(const SPIRType &type) const {
  // Physical pointers and arrays of physical pointers need to refer to the
  // pointee's type.
  return type_is_top_level_physical_pointer(type) ||
         (!type.array.empty() &&
          type_is_top_level_physical_pointer(get<SPIRType>(type.parent_type)));
}
...
void CompilerReflection::emit_types() {
  ...
  // If we have physical pointers or arrays of physical pointers, it's also
  // helpful to emit the pointee type
  // and chain the type hierarchy. For POD, arrays can emit the entire type
  // in-place.
...
void CompilerReflection::emit_type(uint32_t type_id, bool &emitted_open_tag) {
  ...
  if (type_is_top_level_physical_pointer(type)) {
    json_stream->emit_json_key_value("type",
                                     "_" + std::to_string(type.parent_type));
    json_stream->emit_json_key_value("physical_pointer", true);
  } else if (!type.array.empty()) {
    emit_type_array(type);
    json_stream->emit_json_key_value("type",
                                     "_" + std::to_string(type.parent_type));
    json_stream->emit_json_key_value(
        "array_stride", get_decoration(type_id, DecorationArrayStride));
  } else {
...
void CompilerReflection::emit_type_member(const SPIRType &type,
                                          uint32_t index) {
  ...
  if (type_is_reference(membertype)) {
    json_stream->emit_json_key_value(
        "type", "_" + std::to_string(membertype.parent_type));
  } else if (membertype.basetype == SPIRType::Struct) {
    json_stream->emit_json_key_value("type",
                                     "_" + std::to_string(membertype.self));
  } else {
...
void CompilerReflection::emit_type_member_qualifiers(const SPIRType &type,
                                                     uint32_t index) {
  auto &membertype = get<SPIRType>(type.member_types[index]);
...
// FIXME include things like the local_size dimensions, geometry output vertex
// count, etc
void CompilerReflection::emit_entry_points() {
  ...
  if (!entries.empty()) {
    ...
    for (auto &e : entries) {
      ...
      json_stream->emit_json_key_value(
          "mode", execution_model_to_str(e.execution_model));
...
void CompilerReflection::emit_resources(
    const char *tag, const SmallVector<Resource> &resources) {
  if (resources.empty()) {
    return;
  }
  ...
  for (auto &res : resources) {
    ...
    // If we don't have a name, use the fallback for the type instead of the
    // variable
    // for SSBOs and UBOs since those are the only meaningful names to use
    // externally.
    // Push constant blocks are still accessed by name and not block name, even
    // though they are technically Blocks.
    bool is_push_constant =
        get_storage_class(res.id) == StorageClassPushConstant;
    ...
    ID fallback_id =
        !is_push_constant && is_block ? ID(res.base_type_id) : ID(res.id);

    json_stream->begin_json_object();

    if (type.basetype == SPIRType::Struct) {
      json_stream->emit_json_key_value("type",
                                       "_" + std::to_string(res.base_type_id));
    } else {
...

    {
      bool is_sized_block =
          is_block &&
          (get_storage_class(res.id) == StorageClassUniform ||
           get_storage_class(res.id) == StorageClassUniformConstant ||
           get_storage_class(res.id) == StorageClassStorageBuffer);
      if (is_sized_block) {
        uint32_t block_size =
            uint32_t(get_declared_struct_size(get_type(res.base_type_id)));
    ...
    if (mask.get(DecorationLocation))
      json_stream->emit_json_key_value(
          "location", get_decoration(res.id, DecorationLocation));
    ...
    if (mask.get(DecorationIndex))
      json_stream->emit_json_key_value("index",
                                       get_decoration(res.id, DecorationIndex));
    if (type.storage != StorageClassPushConstant &&
        mask.get(DecorationDescriptorSet))
      json_stream->emit_json_key_value(
          "set", get_decoration(res.id, DecorationDescriptorSet));
    if (mask.get(DecorationBinding))
      json_stream->emit_json_key_value(
          "binding", get_decoration(res.id, DecorationBinding));
    ...
    if (mask.get(DecorationOffset))
      json_stream->emit_json_key_value(
          "offset", get_decoration(res.id, DecorationOffset));
...
string CompilerReflection::to_member_name(const SPIRType &type,
