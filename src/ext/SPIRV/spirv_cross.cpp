...
Compiler::Compiler(ParsedIR &&ir_) { set_ir(move(ir_)); }
...
bool Compiler::variable_storage_is_aliased(const SPIRVariable &v) {
  ...
  bool ssbo =
      v.storage == StorageClassStorageBuffer ||
...
bool Compiler::block_is_pure(const SPIRBlock &block) {
  ...
  for (auto &i : block.ops) {
    ...
    // Barriers disallow any reordering, so we should treat blocks with barrier
    // as writing.
    ...
    // OpExtInst is potentially impure depending on extension, but GLSL builtins
    // are at least pure.
...
string Compiler::to_name(uint32_t id, bool allow_alias) const {
  if (allow_alias && ir.ids[id].get_type() == TypeType) {
    ...
    if (type.type_alias) {
      // If the alias master has been specially packed, we will have emitted a
      // clean variant as well,
      // so skip the name aliasing here.
      if (!has_extended_decoration(type.type_alias,
...
void Compiler::register_global_read_dependencies(const SPIRBlock &block,
                                                 uint32_t id) {
  for (auto &i : block.ops) {
    auto ops = stream(i);
    auto op = static_cast<Op>(i.op);
    ...
    case OpLoad:
    case OpImageRead: {
      // If we're in a storage class which does not get invalidated, adding
      // dependencies here is no big deal.
...
void Compiler::register_read(uint32_t expr, uint32_t chain, bool forwarded) {
  ...
  if (var) {
    ...
    // If the backing variable is immutable, we do not need to depend on the
    // variable.
    if (forwarded && !is_immutable(var->self))
      var->dependees.push_back(e.self);

    // If we load from a parameter, make sure we create "inout" if we also write
    // to the parameter.
    // The default is "in" however, so we never invalidate our compilation by
    // reading.
...
void Compiler::register_write(uint32_t chain) {
  auto *var = maybe_get<SPIRVariable>(chain);
  if (!var) {
    // If we're storing through an access chain, invalidate the backing variable
    // instead.
  ...
  if (var) {
    ...
    // invalidate all variables which depend on aliased variables. And if this
    // is a
    ...
    if (get_variable_data_type(*var).pointer) {
      ...
      if (type.pointer_depth == 1) {
        ...
        // We are storing some data through a pointer acquired through that
        // variable,
    ...
    // We tried to write to a parameter which is not marked with out qualifier,
    // force a recompile.
    if (check_argument_storage_qualifier && var->parameter &&
        var->parameter->write_count == 0) {
  ...
  // If chain_type.pointer is false, we're not writing to memory backed
  // variables, but temporaries instead.
  // This can happen in copy_logical_type where we unroll complex reads and
...
void Compiler::flush_all_active_variables() {
  // Invalidate all temporaries we read from variables in this block since they
  // were forwarded.
...
bool Compiler::is_immutable(uint32_t id) const {
  if (ir.ids[id].get_type() == TypeVariable) {
    ...
    // Anything we load from the UniformConstant address space is guaranteed to
    // be immutable.
  ...
  else if (ir.ids[id].get_type() == TypeConstant ||
           ir.ids[id].get_type() == TypeConstantOp ||
...
bool Compiler::is_hidden_variable(const SPIRVariable &var,
                                  bool include_builtins) const {
  if ((is_builtin_variable(var) && !include_builtins) || var.remapped_variable)
    return true;

  // Combined image samplers are always considered active as they are "magic"
  // variables.
  ...
  // In SPIR-V 1.4 and up we must also use the active variable interface to
  // disable global variables
  // which are not part of the entry point.
  if (ir.get_spirv_version() >= 0x10400 &&
      var.storage != spv::StorageClassGeneric &&
      var.storage != spv::StorageClassFunction &&
      !interface_variable_exists_in_entry_point(var.self)) {
    return true;
  }

  return check_active_interface_variables &&
         storage_class_is_interface(var.storage) &&
         active_interface_variables.find(var.self) ==
...
bool Compiler::is_builtin_type(const SPIRType &type) const {
  ...
  // We can have builtin structs as well. If one member of a struct is builtin,
  // the struct must also be builtin.
  if (type_meta)
...
bool Compiler::is_member_builtin(const SPIRType &type, uint32_t index,
...
bool Compiler::is_scalar(const SPIRType &type) const {
  return type.basetype != SPIRType::Struct && type.vecsize == 1 &&
         type.columns == 1;
}
...
ShaderResources Compiler::get_shader_resources(
    const unordered_set<VariableID> &active_variables) const {
  return get_shader_resources(&active_variables);
}

bool Compiler::InterfaceVariableAccessHandler::handle(Op opcode,
                                                      const uint32_t *args,
                                                      uint32_t length) {
  uint32_t variable = 0;
  switch (opcode) {
  // Need this first, otherwise, GCC complains about unhandled switch
  // statements.
  default:
    break;
...
unordered_set<VariableID> Compiler::get_active_interface_variables() const {
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable &var) {
    ...
    // An output variable which is just declared (but uninitialized) might be
    // read by subsequent stages
    // so we should force-enable these outputs,
    // since compilation will fail if a subsequent stage attempts to read from
    // the variable in question.
    // Also, make sure we preserve output variables which are only initialized,
    // but never accessed by any code.
...
void Compiler::set_enabled_interface_variables(
    std::unordered_set<VariableID> active_variables) {
...
ShaderResources Compiler::get_shader_resources(
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable &var) {
    ...
    // It is possible for uniform storage classes to be passed as function
    // parameters, so detect
    // that. To detect function parameters, check of StorageClass of variable is
    // function scope.
    if (var.storage == StorageClassFunction || !type.pointer)
      return;

    if (active_variables &&
        active_variables->find(var.self) == end(*active_variables))
      return;

    // In SPIR-V 1.4 and up, every global must be present in the entry point
    // interface list,
    ...
    if (is_builtin) {
      ...
      auto &list = var.storage == StorageClassInput ? res.builtin_inputs
                                                    : res.builtin_outputs;
    ...
    // Push constant blocks
      ...
      // There can only be one push constant block, but keep the vector in case
    ...
    // Acceleration structures
      ...
      res.acceleration_structures.push_back(
          {var.self, var.basetype, type.self, get_name(var.self)});
    }
  });
...
void Compiler::parse_fixup() {
  ...
  for (auto id_ : ir.ids_for_constant_or_variable) {
    ...
    if (id.get_type() == TypeConstant) {
      ...
      if (ir.meta[c.self].decoration.builtin &&
          ir.meta[c.self].decoration.builtin_type == BuiltInWorkgroupSize) {
...
void Compiler::update_name_cache(unordered_set<string> &cache_primary,
                                 const unordered_set<string> &cache_secondary,
                                 string &name) {
  if (name.empty())
    return;
  ...
  if (tmpname == "_") {
    // We cannot just append numbers, as we will end up creating internally
    // reserved names.
    // Make it like _0_<counter> instead.
    tmpname += "0";
  } else if (tmpname.back() == '_') {
    // The last_character is an underscore, so we don't need to link in
    // underscore.
  ...
  do {
    counter++;
    name = tmpname + (use_linked_underscore ? "_" : "") +
           convert_to_string(counter);
...
const SPIRType &
Compiler::get_variable_data_type(const SPIRVariable &var) const {
  return get<SPIRType>(get_variable_data_type_id(var));
}
...
const SPIRType &
Compiler::get_variable_element_type(const SPIRVariable &var) const {
...
bool Compiler::is_sampled_image_type(const SPIRType &type) {
  return (type.basetype == SPIRType::Image ||
          type.basetype == SPIRType::SampledImage) &&
         type.image.sampled == 1 && type.image.dim != DimBuffer;
}

void Compiler::set_member_decoration_string(TypeID id, uint32_t index,
                                            spv::Decoration decoration,
                                            const std::string &argument) {
  ir.set_member_decoration_string(id, index, decoration, argument);
}

void Compiler::set_member_decoration(TypeID id, uint32_t index,
                                     Decoration decoration, uint32_t argument) {
  ir.set_member_decoration(id, index, decoration, argument);
}

void Compiler::set_member_name(TypeID id, uint32_t index,
...
void Compiler::set_member_qualified_name(uint32_t type_id, uint32_t index,
                                         const std::string &name) {
  ir.meta[type_id].members.resize(
      max(ir.meta[type_id].members.size(), size_t(index) + 1));
  ir.meta[type_id].members[index].qualified_alias = name;
}

const string &Compiler::get_member_qualified_name(TypeID type_id,
...
uint32_t Compiler::get_member_decoration(TypeID id, uint32_t index,
                                         Decoration decoration) const {
  return ir.get_member_decoration(id, index, decoration);
}

const Bitset &Compiler::get_member_decoration_bitset(TypeID id,
                                                     uint32_t index) const {
  return ir.get_member_decoration_bitset(id, index);
}

bool Compiler::has_member_decoration(TypeID id, uint32_t index,
                                     Decoration decoration) const {
  return ir.has_member_decoration(id, index, decoration);
}

void Compiler::unset_member_decoration(TypeID id, uint32_t index,
                                       Decoration decoration) {
  ir.unset_member_decoration(id, index, decoration);
}

void Compiler::set_decoration_string(ID id, spv::Decoration decoration,
                                     const std::string &argument) {
...
void Compiler::set_extended_decoration(uint32_t id,
...
void Compiler::set_extended_member_decoration(uint32_t type, uint32_t index,
                                              ExtendedDecorations decoration,
                                              uint32_t value) {
  ir.meta[type].members.resize(
      max(ir.meta[type].members.size(), size_t(index) + 1));
...
static uint32_t
get_default_extended_decoration(ExtendedDecorations decoration) {
  switch (decoration) {
...
uint32_t
Compiler::get_extended_decoration(uint32_t id,
...
uint32_t
Compiler::get_extended_member_decoration(uint32_t type, uint32_t index,
                                         ExtendedDecorations decoration) const {
...
bool Compiler::has_extended_decoration(uint32_t id,
                                       ExtendedDecorations decoration) const {
  auto *m = ir.find_meta(id);
  if (!m)
    return false;
...
bool Compiler::has_extended_member_decoration(
    uint32_t type, uint32_t index, ExtendedDecorations decoration) const {
  auto *m = ir.find_meta(type);
  if (!m)
    return false;
...
void Compiler::unset_extended_decoration(uint32_t id,
                                         ExtendedDecorations decoration) {
  auto &dec = ir.meta[id].decoration;
...
void Compiler::unset_extended_member_decoration(
    uint32_t type, uint32_t index, ExtendedDecorations decoration) {
  ir.meta[type].members.resize(
      max(ir.meta[type].members.size(), size_t(index) + 1));
  auto &dec = ir.meta[type].members[index];
  dec.extended.flags.clear(decoration);
...
const std::string &Compiler::get_name(ID id) const { return ir.get_name(id); }
...
const string &Compiler::get_decoration_string(ID id,
                                              Decoration decoration) const {
  return ir.get_decoration_string(id, decoration);
}

const string &
Compiler::get_member_decoration_string(TypeID id, uint32_t index,
                                       Decoration decoration) const {
...
bool Compiler::get_binary_offset_for_decoration(VariableID id,
                                                spv::Decoration decoration,
  ...
  if (method == SPIRBlock::MergeToSelectForLoop ||
    ...
    bool false_block_is_merge = block.false_block == block.merge_block ||
                                (false_block && merge_block &&
                                 execution_is_noop(*false_block, *merge_block));

    bool true_block_is_merge = block.true_block == block.merge_block ||
                               (true_block && merge_block &&
                                execution_is_noop(*true_block, *merge_block));

    bool positive_candidate = block.true_block != block.merge_block &&
                              block.true_block != block.self &&
                              false_block_is_merge;

    bool negative_candidate = block.false_block != block.merge_block &&
                              block.false_block != block.self &&
                              true_block_is_merge;

    bool ret = block.terminator == SPIRBlock::Select &&
               block.merge == SPIRBlock::MergeLoop &&
               (positive_candidate || negative_candidate);

    if (ret && positive_candidate &&
        method == SPIRBlock::MergeToSelectContinueForLoop)
      ret = block.true_block == block.continue_block;
    else if (ret && negative_candidate &&
             method == SPIRBlock::MergeToSelectContinueForLoop)
    ...
    bool ret = block.terminator == SPIRBlock::Direct &&
               block.merge == SPIRBlock::MergeLoop && block.ops.empty();
    ...
    bool positive_candidate = child.true_block != block.merge_block &&
                              child.true_block != block.self &&
                              false_block_is_merge;

    bool negative_candidate = child.false_block != block.merge_block &&
                              child.false_block != block.self &&
                              true_block_is_merge;

    ret = child.terminator == SPIRBlock::Select &&
...
bool Compiler::execution_is_branchless(const SPIRBlock &from,
  ...
  for (;;) {
    ...
    if (start->terminator == SPIRBlock::Direct &&
...
bool Compiler::execution_is_direct_branch(const SPIRBlock &from,
                                          const SPIRBlock &to) const {
  return from.terminator == SPIRBlock::Direct &&
         from.merge == SPIRBlock::MergeNone && from.next_block == to.self;
}

SPIRBlock::ContinueBlockType
Compiler::continue_block_type(const SPIRBlock &block) const {
  // The block was deemed too complex during code emit, pick conservative
  // fallback paths.
  ...
  // In this case, execution is clearly branchless, so just assume a while loop
  // header here.
  ...
  if (execution_is_noop(block, dominator))
    ...
    bool flush_phi_to_false =
        false_block && flush_phi_required(block.self, block.false_block);
    bool flush_phi_to_true =
        true_block && flush_phi_required(block.self, block.true_block);
...
const SmallVector<SPIRBlock::Case> &
Compiler::get_case_list(const SPIRBlock &block) const {
  uint32_t width = 0;
...
bool Compiler::traverse_all_reachable_opcodes(const SPIRBlock &block,
  ...
  // Ideally, perhaps traverse the CFG instead of all blocks in order to
  // eliminate dead blocks,
  // but this shouldn't be a problem in practice unless the SPIR-V is doing
  // insane things like recursing
  // inside dead blocks ...
  for (auto &i : block.ops) {
    auto ops = stream(i);
    auto op = static_cast<Op>(i.op);
...
bool Compiler::traverse_all_reachable_opcodes(const SPIRFunction &func,
                                              OpcodeHandler &handler) const {
  for (auto block : func.blocks)
...
size_t
Compiler::get_declared_struct_size_runtime_array(const SPIRType &type,
  ...
  if (!last_type.array.empty() && last_type.array_size_literal[0] &&
      last_type.array[0] == 0) // Runtime array
    size += array_size * type_struct_member_array_stride(
                             type, uint32_t(type.member_types.size() - 1));

  return size;
}

uint32_t
Compiler::evaluate_spec_constant_u32(const SPIRConstantOp &spec) const {
  auto &result_type = get<SPIRType>(spec.basetype);
  if (result_type.basetype != SPIRType::UInt &&
      result_type.basetype != SPIRType::Int &&
      result_type.basetype != SPIRType::Boolean) {
    SPIRV_CROSS_THROW("Only 32-bit integers and booleans are currently "
                      "supported when evaluating specialization constants.\n");
  }
  ...
  const auto eval_u32 = [&](uint32_t id) -> uint32_t {
    auto &type = expression_type(id);
    if (type.basetype != SPIRType::UInt && type.basetype != SPIRType::Int &&
        type.basetype != SPIRType::Boolean) {
      SPIRV_CROSS_THROW("Only 32-bit integers and booleans are currently "
                        "supported when evaluating "
...
#define binary_spec_op_cast(op, binary_op, type)                               \
  case Op##op:                                                                 \
    value = uint32_t(type(eval_u32(spec.arguments[0]))                         \
                         binary_op type(eval_u32(spec.arguments[1])));         \
    break

  // Support the basic opcodes which are typically used when computing array
  // sizes.
  switch (spec.opcode) {
    binary_spec_op(IAdd, +);
    binary_spec_op(ISub, -);
...
size_t Compiler::get_declared_struct_member_size(const SPIRType &struct_type,
  ...
  case SPIRType::Void:
  case SPIRType::Boolean: // Bools are purely logical, and cannot be used for
  ...
  if (!type.array.empty()) {
    ...
    uint32_t array_size = array_size_literal
                              ? type.array.back()
                              : evaluate_constant_u32(type.array.back());
    ...
    if (columns == 1) {
      ...
      uint32_t matrix_stride =
          type_struct_member_matrix_stride(struct_type, index);

      // Per SPIR-V spec, matrices must be tightly packed and aligned up for
      // vec3 accesses.
...
bool Compiler::BufferAccessHandler::handle(Op opcode, const uint32_t *args,
                                           uint32_t length) {
  ...
  size_t range;
  // If we have another member in the struct, deduce the range by looking at the
  // next member.
  // This is okay since structs in SPIR-V can have padding, but Offset
  // decoration must be
  // monotonically increasing.
  // Of course, this doesn't take into account if the SPIR-V for some reason
  // decided to add
...
SmallVector<BufferRange>
Compiler::get_active_buffer_ranges(VariableID id) const {
...
bool Compiler::types_are_logically_equivalent(const SPIRType &a,
  ...
  if (array_count &&
      memcmp(a.array.data(), b.array.data(), array_count * sizeof(uint32_t)) !=
          0)
    return false;
  ...
  for (size_t i = 0; i < member_types; i++) {
    if (!types_are_logically_equivalent(get<SPIRType>(a.member_types[i]),
                                        get<SPIRType>(b.member_types[i])))
      return false;
  }
...
void Compiler::set_execution_mode(ExecutionMode mode, uint32_t arg0,
...
uint32_t Compiler::get_work_group_size_specialization_constants(
    SpecializationConstant &x, SpecializationConstant &y,
...
bool Compiler::is_tessellation_shader(ExecutionModel model) {
  return model == ExecutionModelTessellationControl ||
...
void Compiler::set_subpass_input_remapped_components(VariableID id,
                                                     uint32_t components) {
...
void Compiler::add_implied_read_expression(SPIRExpression &e, uint32_t source) {
  auto itr = find(begin(e.implied_read_expressions),
                  end(e.implied_read_expressions), ID(source));
  if (itr == end(e.implied_read_expressions))
    e.implied_read_expressions.push_back(source);
}

void Compiler::add_implied_read_expression(SPIRAccessChain &e,
                                           uint32_t source) {
  auto itr = find(begin(e.implied_read_expressions),
                  end(e.implied_read_expressions), ID(source));
  if (itr == end(e.implied_read_expressions))
...
void Compiler::inherit_expression_dependencies(uint32_t dst,
  ...
  // If we depend on a expression, we also depend on all sub-dependencies from
  // source.
...
SPIREntryPoint &Compiler::get_first_entry_point(const std::string &name) {
  auto itr =
      find_if(begin(ir.entry_points), end(ir.entry_points),
...
const SPIREntryPoint &
Compiler::get_first_entry_point(const std::string &name) const {
  auto itr =
      find_if(begin(ir.entry_points), end(ir.entry_points),
              [&](const std::pair<uint32_t, SPIREntryPoint> &entry) -> bool {
                return entry.second.orig_name == name;
...
const SPIREntryPoint &Compiler::get_entry_point(const std::string &name,
...
const string &
Compiler::get_cleansed_entry_point_name(const std::string &name,
                                        ExecutionModel model) const {
...
bool Compiler::interface_variable_exists_in_entry_point(uint32_t id) const {
  ...
  if (ir.get_spirv_version() < 0x10400) {
    if (var.storage != StorageClassInput && var.storage != StorageClassOutput &&
      ...
      SPIRV_CROSS_THROW("Only Input, Output variables and Uniform constants "
                        "are part of a shader linking interface.");

    // This is to avoid potential problems with very old glslang versions which
    // did
  ...
  auto &execution = get_entry_point();
  return find(begin(execution.interface_variables),
              end(execution.interface_variables),
              VariableID(id)) != end(execution.interface_variables);
}

void Compiler::CombinedImageSamplerHandler::push_remap_parameters(
    const SPIRFunction &func, const uint32_t *args, uint32_t length) {
...
bool Compiler::CombinedImageSamplerHandler::begin_function_scope(
...
bool Compiler::CombinedImageSamplerHandler::end_function_scope(
  ...
  if (caller.do_combined_parameters) {
    for (auto &param : params) {
      VariableID image_id = param.global_image
                                ? param.image_id
                                : VariableID(args[param.image_id]);
      VariableID sampler_id = param.global_sampler
                                  ? param.sampler_id
                                  : VariableID(args[param.sampler_id]);
      ...
      register_combined_image_sampler(caller, 0, image_id, sampler_id,
                                      param.depth);
    }
  }
...
void Compiler::CombinedImageSamplerHandler::register_combined_image_sampler(
    SPIRFunction &caller, VariableID combined_module_id, VariableID image_id,
    VariableID sampler_id, bool depth) {
  // We now have a texture ID and a sampler ID which will either be found as a
  // global
  // or a parameter in our own function. If both are global, they will not need
  // a parameter,
  ...
  auto texture_itr = find_if(begin(caller.arguments), end(caller.arguments),
                             [image_id](const SPIRFunction::Parameter &p) {
                               return p.id == image_id;
                             });
  auto sampler_itr = find_if(begin(caller.arguments), end(caller.arguments),
                             [sampler_id](const SPIRFunction::Parameter &p) {
                               return p.id == sampler_id;
                             });
  ...
  auto itr = find_if(
      begin(caller.combined_parameters), end(caller.combined_parameters),
      [&param](const SPIRFunction::CombinedImageSamplerParameter &p) {
        return param.image_id == p.image_id &&
  ...
  if (itr == end(caller.combined_parameters)) {
    ...
    // Build new variable.
    compiler.set<SPIRVariable>(combined_id, ptr_type_id, StorageClassFunction,
                               0);

    // Inherit RelaxedPrecision.
    // If any of OpSampledImage, underlying image or sampler are marked, inherit
    // the decoration.
    bool relaxed_precision =
        ...
        (combined_module_id &&
         compiler.has_decoration(combined_module_id,
    ...
    compiler.set_name(combined_id,
                      join("SPIRV_Cross_Combined", compiler.to_name(image_id),
                           compiler.to_name(sampler_id)));
...
bool Compiler::DummySamplerForCombinedImageHandler::handle(Op opcode,
                                                           const uint32_t *args,
                                                           uint32_t length) {
  ...
  case OpLoad: {
    ...
    bool separate_image = type.basetype == SPIRType::Image &&
  ...
  case OpImageQuerySamples: {
    // If we are fetching or querying LOD from a plain OpTypeImage, we must
    // pre-combine with our dummy sampler.
...
bool Compiler::CombinedImageSamplerHandler::handle(Op opcode,
                                                   const uint32_t *args,
                                                   uint32_t length) {
  // We need to figure out where samplers and images are loaded from, so do only
  // the bare bones compilation we need.
  bool is_fetch = false;
  ...
  case OpPtrAccessChain: {
    ...
    // Technically, it is possible to have arrays of textures and arrays of
    // samplers and combine them, but this becomes essentially
    // impossible to implement, since we don't know which concrete sampler we
    // are accessing.
    // One potential way is to create a combinatorial explosion where N textures
    // and M samplers are combined into N * M sampler2Ds,
    // but this seems ridiculously complicated for a problem which is easy to
    // work around.
    // Checking access chains like this assumes we don't have samplers or
    // textures inside uniform structs, but this makes no sense.
    ...
    bool separate_sampler = type.basetype == SPIRType::Sampler;
    if (separate_sampler)
      SPIRV_CROSS_THROW("Attempting to use arrays or structs of separate "
                        "samplers. This is not possible to statically "
                        "remap to plain GLSL.");
  ...
  case OpImageQueryLevels:
  case OpImageQuerySamples: {
    // If we are fetching from a plain OpTypeImage or querying LOD, we must
    // pre-combine with our dummy sampler.
    auto *var = compiler.maybe_get_backing_variable(args[2]);
    if (!var)
      return true;
    ...
    if (type.basetype == SPIRType::Image && type.image.sampled == 1 &&
      ...
      if (compiler.dummy_sampler_id == 0)
        SPIRV_CROSS_THROW("texelFetch without sampler was found, but no dummy "
                          "sampler has been created with "
  ...
  // that their callees know which combined image samplers to propagate down the
  // call stack.
  if (!functions.empty()) {
    ...
    if (callee.do_combined_parameters) {
      ...
      register_combined_image_sampler(callee, combined_id, image_id, sampler_id,
                                      combined_type.image.depth);
    }
  }

  // For function calls, we need to remap IDs which are function parameters into
  // global variables.
  // This information is statically known from the current place in the call
  // stack.
  // Function parameters are not necessarily pointers, so if we don't have a
  // backing variable, remapping will know
  // which backing variable the image/sample came from.
  VariableID image_id = remap_parameter(args[2]);
  VariableID sampler_id =
      is_fetch ? compiler.dummy_sampler_id : remap_parameter(args[3]);

  auto itr =
      find_if(begin(compiler.combined_image_samplers),
              end(compiler.combined_image_samplers),
              [image_id, sampler_id](const CombinedImageSampler &combined) {
                return combined.image_id == image_id &&
                       combined.sampler_id == sampler_id;
              });
  ...
  if (itr == end(compiler.combined_image_samplers)) {
    ...
    // Make a new type, pointer to OpTypeSampledImage, so we can make a variable
    // of this type.
    // We will probably have this type lying around, but it doesn't hurt to make
    // duplicates for internal purposes.
    auto &type = compiler.set<SPIRType>(type_id);
    ...
    // Build new variable.
    compiler.set<SPIRVariable>(combined_id, type_id,
                               StorageClassUniformConstant, 0);

    // Inherit RelaxedPrecision (and potentially other useful flags if deemed
    // relevant).
    // If any of OpSampledImage, underlying image or sampler are marked, inherit
    // the decoration.
    bool relaxed_precision =
    ...
    compiler.combined_image_samplers.push_back(
        {combined_id, image_id, sampler_id});
  }
...
static bool
exists_unaccessed_path_to_return(const CFG &cfg, uint32_t block,
  ...
  // If any of our successors have a path to the end, there exists a path from
  // block.
...
void Compiler::analyze_parameter_preservation(
    ...
    const unordered_map<uint32_t, unordered_set<uint32_t>>
  ...
  for (auto &arg : entry.arguments) {
    ...
    // We have accessed a variable, but there was no complete writes to that
    // variable.
    ...
    // If there is a path through the CFG where no block completely writes to
    // the variable, the variable will be in an undefined state
    // when the function returns. We therefore need to implicitly preserve the
    // variable in case there are writers in the function.
    // Major case here is if a function is
    // void foo(int &var) { if (cond) var = 10; }
    // Using read/write counts, we will think it's just an out variable, but it
    // really needs to be inout,
    // because if we don't write anything whatever we put into the function must
...
Compiler::AnalyzeVariableScopeAccessHandler::AnalyzeVariableScopeAccessHandler(
    Compiler &compiler_, SPIRFunction &entry_)
    : compiler(compiler_), entry(entry_) {}

bool Compiler::AnalyzeVariableScopeAccessHandler::follow_function_call(
    const SPIRFunction &) {
...
void Compiler::AnalyzeVariableScopeAccessHandler::notify_variable_access(
    uint32_t id, uint32_t block) {
  if (id == 0)
    return;

  // Access chains used in multiple blocks mean hoisting all the variables used
  // to construct the access chain as not all backends can use pointers.
...
bool Compiler::AnalyzeVariableScopeAccessHandler::id_is_potential_temporary(
  ...
  return compiler.ir.ids[id].empty() ||
         (compiler.ir.ids[id].get_type() == TypeExpression);
}

bool Compiler::AnalyzeVariableScopeAccessHandler::handle_terminator(
    const SPIRBlock &block) {
  switch (block.terminator) {
...
bool Compiler::AnalyzeVariableScopeAccessHandler::handle(spv::Op op,
                                                         const uint32_t *args,
                                                         uint32_t length) {
  // Keep track of the types of temporaries, so we can hoist them out as
  // necessary.
  uint32_t result_type, result_id;
  if (compiler.instruction_to_result_type(result_type, result_id, op, args,
                                          length))
  ...
  case OpStore: {
    ...
    if (var) {
      ...
      if (var->self == ptr)
        complete_write_variables_to_block[var->self].insert(
            current_block->self);
      else
  ...
  case OpPtrAccessChain: {
    ...
    // Access chains used in multiple blocks mean hoisting all the variables
    // used to construct the access chain as not all backends can use pointers.
    uint32_t ptr = args[2];
    ...
    // the access chain is generated in the loop body, but is consumed in
    // continue block.
    // This means we need complex loop workarounds, and we must detect this via
    // CFG analysis.
    notify_variable_access(args[1], current_block->self);

    // The result of an access chain is a fixed expression and is not really
    ...
    e.loaded_from =
        backing_variable ? VariableID(backing_variable->self) : VariableID(0);
  ...
  case OpFunctionCall: {
    ...
    for (uint32_t i = 0; i < length; i++) {
      ...
      // Cannot easily prove if argument we pass to a function is completely
      // written.
    ...
    for (uint32_t i = 1; i < length; i++) {
      if (i >= 3) {
        ...
        if (var) {
          accessed_variables_to_block[var->self].insert(current_block->self);
          // Assume we can get partial writes to this variable.
          partial_write_variables_to_block[var->self].insert(
              current_block->self);
        }
      }
  ...
  default: {
    ...
    // As long as only IDs are used, we can scan through instructions and try to
    // find any evidence that
    // the ID of a variable has been used.
    // There are potential false positives here where a literal is used in-place
    // of an ID,
...
Compiler::StaticExpressionAccessHandler::StaticExpressionAccessHandler(
    Compiler &compiler_, uint32_t variable_id_)
    : compiler(compiler_), variable_id(variable_id_) {}

bool Compiler::StaticExpressionAccessHandler::follow_function_call(
    const SPIRFunction &) {
  return false;
}
...
bool Compiler::StaticExpressionAccessHandler::handle(spv::Op op,
  ...
  case OpPtrAccessChain:
    if (length < 3)
      return false;
    if (args[2] == variable_id) // If we try to access chain our candidate
                                // variable before we store to it, bail.
      return false;
    break;
...
void Compiler::find_function_local_luts(
    SPIRFunction &entry, const AnalyzeVariableScopeAccessHandler &handler,
  ...
  for (auto &accessed_var : handler.accessed_variables_to_block) {
    ...
    // If we only have a single function in our CFG, private storage is also
    // fine,
    // since it behaves like a function local variable.
    bool allow_lut = var.storage == StorageClassFunction ||
    ...
    // If the variable has an initializer, make sure it is a constant
    // expression.
    ...
    if (var.initializer) {
      ...
      // There can be no stores to this variable, we have now proved we have a
      // LUT.
      if (handler.complete_write_variables_to_block.count(var.self) != 0 ||
          handler.partial_write_variables_to_block.count(var.self) != 0)
        continue;
    } else {
      // We can have one, and only one write to the variable, and that write
      ...
      // The complete write happened in a branch or similar, cannot deduce
      // static expression.
      ...
      traverse_all_reachable_opcodes(get<SPIRBlock>(dominator),
      ...
      if (ir.ids[static_expression_handler.static_expression].get_type() !=
          TypeConstant)
        continue;
...
void Compiler::analyze_variable_scope(
    SPIRFunction &entry, AnalyzeVariableScopeAccessHandler &handler) {
  ...
  // Analyze if there are parameters which need to be implicitly preserved with
  // an "in" qualifier.
  analyze_parameter_preservation(entry, cfg,
  ...
  for (auto &block_id : entry.blocks) {
    ...
    if (itr != end(ir.continue_block_to_loop_header) &&
        itr->second != block_id) {
      // Continue block might be unreachable in the CFG, but we still like to
      // know the loop dominator.
      // Edge case is when continue block is also the loop header, don't set the
      // dominator in this case.
  ...
  for (auto &var : handler.accessed_variables_to_block) {
    // Only deal with variables which are considered local variables in this
    // function.
    if (find(begin(entry.local_variables), end(entry.local_variables),
             VariableID(var.first)) == end(entry.local_variables))
      continue;
    ...
    for (auto &block : blocks) {
      ...
      // We can only use loop variables with scalars, as we cannot track static
      // expressions for vectors.
      if (is_continue(block)) {
        // Potentially awkward case to check for.
        // We might have a variable inside a loop, which is touched by the
        // continue block,
        // but is not actually a loop variable.
        // The continue block is dominated by the inner part of the loop, which
        // does not make sense in high-level
        // language output because it will be declared before the body,
        // so we will have to lift the dominator up to the relevant loop header
        // instead.
        ...
        if (type.vecsize == 1 && type.columns == 1 &&
          ...
          // The variable is used in multiple continue blocks, this is not a
          // loop
    ...
    // For variables whose dominating block is inside a loop, there is a risk
    // that these variables
    // actually need to be preserved across loop iterations. We can express this
    // by adding
    // a "read" access to the loop header.
    // In the dominating block, we must see an OpStore or equivalent as the
    // first access of an OpVariable.
    // Should that fail, we look for the outermost loop header and tack on an
    // access there.
    ...
    // If all blocks here are dead code, this will be 0, so the variable in
    // question
  ...
  for (auto &var : handler.accessed_temporaries_to_block) {
    ...
    for (auto &block : blocks) {
      ...
      if (blocks.size() != 1 && is_continue(block)) {
        ...
        // Any temporary we access in the continue block must be declared before
        // the loop.
        // This is moot for complex loops however.
        auto &loop_header_block =
            get<SPIRBlock>(ir.continue_block_to_loop_header[block]);
    ...
    if (dominating_block) {
      // If we touch a variable in the dominating block, this is the expected
      // setup.
      // SPIR-V normally mandates this, but we have extra cases for temporary
      // use inside loops.
      ...
      if (!first_use_is_dominator || force_temporary) {
        if (handler.access_chain_expressions.count(var.first)) {
          ...
          // We cannot declare temporaries of access chains (except on MSL
          // perhaps with pointers).
          // Rather than do that, we force the indexing expressions to be
          // declared in the right scope by
          // tracking their usage to that end. There is no temporary to hoist.
          // However, we still need to observe declaration order of the access
          // chain.

          if (used_in_header_hoisted_continue_block) {
            // For this scenario, we used an access chain inside a continue
            // block where we also registered an access to header block.
            // This is a problem as we need to declare an access chain properly
            // first with full definition.
            ...
            // TODO: Out-of-order declaring for loops where continue blocks are
            // emitted last might be another option.
            auto &loop_header_block = get<SPIRBlock>(dominating_block);
            assert(loop_header_block.merge == SPIRBlock::MergeLoop);
            loop_header_block.complex_continue = true;
          }
        } else {
          // This should be very rare, but if we try to declare a temporary
          // inside a loop,
          // and that temporary is used outside the loop as well (spirv-opt
          // inliner likes this)
          ...
          auto &block_temporaries =
              get<SPIRBlock>(dominating_block).declare_temporary;
          block_temporaries.emplace_back(handler.result_id_to_type[var.first],
                                         var.first);
        }
      } else if (blocks.size() > 1) {
        // Keep track of the temporary as we might have to declare this
        // temporary.
        // This can happen if the loop header dominates a temporary, but we have
        // a complex fallback loop.
        // In this case, the header is actually inside the for (;;) {} block,
        // and we have problems.
        // What we need to do is hoist the temporaries outside the for (;;) {}
        // block in case the header block
        // declares the temporary.
        auto &block_temporaries =
            get<SPIRBlock>(dominating_block).potential_declare_temporary;
        block_temporaries.emplace_back(handler.result_id_to_type[var.first],
                                       var.first);
      }
    }
  ...
  // Now, try to analyze whether or not these variables are actually loop
  // variables.
  for (auto &loop_variable : potential_loop_variables) {
    ...
    // If a loop variable is not used before the loop, it's probably not a loop
    // variable.
    bool has_accessed_variable = blocks.count(header) != 0;

    // Now, there are two conditions we need to meet for the variable to be a
    // loop variable.
    // 1. The dominating block must have a branch-free path to the loop header,
    // this way we statically know which expression should be part of the loop
    ...
    cfg.walk_from(seen_blocks, header_block.merge_block,
                  [&](uint32_t walk_block) -> bool {
                    // We found a block which accesses the variable outside the
                    // loop.
    ...
    // Need to sort here as variables come from an unordered container, and
    // pushing stuff in wrong order
...
bool Compiler::may_read_undefined_variable_in_block(const SPIRBlock &block,
                                                    uint32_t var) {
  for (auto &op : block.ops) {
    ...
    case OpInBoundsAccessChain:
    case OpPtrAccessChain:
      // Access chains are generally used to partially read and write. It's too
      // hard to analyze
      // if all constituents are written fully before continuing, so just assume
      // it's preserved.
...
void Compiler::ActiveBuiltinHandler::handle_builtin(
    const SPIRType &type, BuiltIn builtin, const Bitset &decoration_flags) {
  // If used, we will need to explicitly declare a new array size for these
  // builtins.
...
void Compiler::ActiveBuiltinHandler::add_if_builtin(uint32_t id,
  ...
  if (var && m) {
    ...
    auto &flags = type.storage == StorageClassInput
                      ? compiler.active_input_builtins
                      : compiler.active_output_builtins;
    if (decorations.builtin) {
      flags.set(decorations.builtin_type);
      handle_builtin(type, decorations.builtin_type,
                     decorations.decoration_flags);
    } else if (allow_blocks &&
               compiler.has_decoration(type.self, DecorationBlock)) {
      ...
      for (uint32_t i = 0; i < member_count; i++) {
        if (compiler.has_member_decoration(type.self, i, DecorationBuiltIn)) {
          ...
          BuiltIn builtin = BuiltIn(
              compiler.get_member_decoration(type.self, i, DecorationBuiltIn));
          flags.set(builtin);
          handle_builtin(member_type, builtin,
                         compiler.get_member_decoration_bitset(type.self, i));
        }
      }
    }
  }
...
bool Compiler::ActiveBuiltinHandler::handle(spv::Op opcode,
                                            const uint32_t *args,
                                            uint32_t length) {
  switch (opcode) {
  case OpStore:
  ...
  case OpPtrAccessChain: {
    ...
    // Only consider global variables, cannot consider variables in functions
    // yet, or other
    ...
    auto &flags = var->storage == StorageClassInput
                      ? compiler.active_input_builtins
                      : compiler.active_output_builtins;
    ...
    for (uint32_t i = 0; i < count; i++) {
      ...
      // Structs
        ...
        if (index < uint32_t(compiler.ir.meta[type->self].members.size())) {
          ...
          if (decorations.builtin) {
            flags.set(decorations.builtin_type);
            handle_builtin(compiler.get<SPIRType>(type->member_types[index]),
...
void Compiler::analyze_image_and_sampler_usage() {
  ...
  CombinedImageSamplerUsageHandler handler(*this,
                                           dref_handler.dref_combined_samplers);
  traverse_all_reachable_opcodes(get<SPIRFunction>(ir.default_entry_point),
                                 handler);

  // Need to run this traversal twice. First time, we propagate any comparison
  // sampler usage from leaf functions
  // down to main().
  // In the second pass, we can propagate up forced depth state coming from
  // main() up into leaf functions.
  ...
  // Forward information from separate images and samplers into combined image
  // samplers.
...
bool Compiler::CombinedImageSamplerDrefHandler::handle(spv::Op opcode,
                                                       const uint32_t *args,
                                                       uint32_t) {
...
void Compiler::build_function_control_flow_graphs_and_analyze() {
  ...
  handler.function_cfgs[ir.default_entry_point].reset(
      new CFG(*this, get<SPIRFunction>(ir.default_entry_point)));
  traverse_all_reachable_opcodes(get<SPIRFunction>(ir.default_entry_point),
                                 handler);
  ...
  for (auto &f : function_cfgs) {
    ...
    // Check if we can actually use the loop variables we found in
...
Compiler::CFGBuilder::CFGBuilder(Compiler &compiler_) : compiler(compiler_) {}
...
void Compiler::CombinedImageSamplerUsageHandler::add_dependency(uint32_t dst,
                                                                uint32_t src) {
...
bool Compiler::CombinedImageSamplerUsageHandler::begin_function_scope(
    const uint32_t *args, uint32_t length) {
  if (length < 3)
    return false;
...
void Compiler::CombinedImageSamplerUsageHandler::
    add_hierarchy_to_comparison_ids(uint32_t id) {
  // Traverse the variable dependency hierarchy and tag everything in its path
  // with comparison ids.
...
bool Compiler::CombinedImageSamplerUsageHandler::handle(Op opcode,
                                                        const uint32_t *args,
                                                        uint32_t length) {
  switch (opcode) {
  case OpAccessChain:
  ...
  case OpLoad: {
    ...
    // Ideally defer this to OpImageRead, but then we'd need to track loaded
    // IDs.
    // If we load an image, we're going to use it and there is little harm in
    // declaring an unused gl_FragCoord.
    auto &type = compiler.get<SPIRType>(args[0]);
    if (type.image.dim == DimSubpassData)
      need_subpass_input = true;

    // If we load a SampledImage and it will be used with Dref, propagate the
    // state up.
  ...
  case OpSampledImage: {
    ...
    // If the underlying resource has been used for comparison then duplicate
    // loads of that resource must be too.
    ...
    if (dref_combined_samplers.count(result_id) != 0) {
      ...
      // This sampler must be a SamplerComparisonState, and not a regular
      // SamplerState.
...
bool Compiler::buffer_get_hlsl_counter_buffer(VariableID id,
...
void Compiler::make_constant_null(uint32_t id, uint32_t type) {
  ...
  if (constant_type.pointer) {
    ...
    uint32_t member_ids =
        ir.increase_bound_by(uint32_t(constant_type.member_types.size()));
...
const SmallVector<spv::Capability> &
...
std::string Compiler::get_remapped_declared_block_name(
  ...
  if (itr != end(declared_block_names)) {
    ...
    if (fallback_prefer_instance_name) {
      ...
      return (!block_name || block_name->empty()) ? get_block_fallback_name(id)
                                                  : *block_name;
    }
  }
}
...
bool Compiler::reflection_ssbo_instance_name_is_significant() const {
  if (ir.source.known) {
    // UAVs from HLSL source tend to be declared in a way where the type is
    // reused
    // but the instance name is significant, and that's the name we should
    // report.
    // For GLSL, SSBOs each have their own block type as that's how GLSL is
    // written.
  ...
  // If we don't have any OpSource information, we need to perform some shaky
  // heuristics.
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable &var) {
    auto &type = this->get<SPIRType>(var.basetype);
...
bool Compiler::instruction_to_result_type(uint32_t &result_type,
                                          uint32_t &result_id, spv::Op op,
                                          const uint32_t *args,
                                          uint32_t length) {
  // Most instructions follow the pattern of <result-type> <result-id>
  // <arguments>.
...
Bitset Compiler::combined_decoration_for_member(const SPIRType &type,
  ...
  if (type_meta) {
    ...
    // If our member type is a struct, traverse all the child members as well
    // recursively.
...
// An image is determined to be a depth image if it is marked as a depth image
// and is not also
// explicitly marked with a color format, or if there are any sample/gather
// compare operations on it.
bool Compiler::is_depth_image(const SPIRType &type, uint32_t id) const {
  return (type.image.depth && type.image.format == ImageFormatUnknown) ||
         comparison_ids.count(id);
}

bool Compiler::type_is_opaque_value(const SPIRType &type) const {
  return !type.pointer && (type.basetype == SPIRType::SampledImage ||
                           type.basetype == SPIRType::Image ||
                           type.basetype == SPIRType::Sampler);
}

// Make these member functions so we can easily break on any force_recompile
// events.
void Compiler::force_recompile() { is_force_recompile = true; }

bool Compiler::is_forcing_recompilation() const { return is_force_recompile; }

void Compiler::clear_force_recompile() { is_force_recompile = false; }

Compiler::PhysicalStorageBufferPointerHandler::
    PhysicalStorageBufferPointerHandler(Compiler &compiler_)
    : compiler(compiler_) {}

Compiler::PhysicalBlockMeta *
Compiler::PhysicalStorageBufferPointerHandler::find_block_meta(
    uint32_t id) const {
...
void Compiler::PhysicalStorageBufferPointerHandler::mark_aligned_access(
    uint32_t id, const uint32_t *args, uint32_t length) {
  uint32_t mask = *args;
  args++;
  length--;
  ...
  if (length && (mask & MemoryAccessAlignedMask) != 0) {
    ...
    // This makes the assumption that the application does not rely on insane
    // edge cases like:
    // Bind buffer with ADDR = 8, use block offset of 8 bytes, load/store with
    // 16 byte alignment.
    // If we emit the buffer with alignment = 16 here, the first element at
    // offset = 0 should
    // actually have alignment of 8 bytes, but this is too theoretical and
    // awkward to support.
    // We could potentially keep track of any offset in the access chain, but
    // it's
    // practically impossible for high level compilers to emit code like that,
    // so deducing overall alignment requirement based on maximum observed
    // Alignment value is probably fine.
    if (meta && alignment > meta->alignment)
      meta->alignment = alignment;
  }
}

bool Compiler::PhysicalStorageBufferPointerHandler::type_is_bda_block_entry(
...
uint32_t
Compiler::PhysicalStorageBufferPointerHandler::get_minimum_scalar_alignment(
  ...
  if (type.storage == spv::StorageClassPhysicalStorageBufferEXT)
    ...
    for (auto &member_type : type.member_types) {
      uint32_t member_align =
          get_minimum_scalar_alignment(compiler.get<SPIRType>(member_type));
...
void Compiler::PhysicalStorageBufferPointerHandler::setup_meta_chain(
    uint32_t type_id, uint32_t var_id) {
  if (type_is_bda_block_entry(type_id)) {
    ...
    if (meta.alignment == 0)
      meta.alignment =
          get_minimum_scalar_alignment(compiler.get_pointee_type(type));
  }
}

bool Compiler::PhysicalStorageBufferPointerHandler::handle(Op op,
                                                           const uint32_t *args,
                                                           uint32_t length) {
  // When a BDA pointer comes to life, we need to keep a mapping of SSA ID ->
  // type ID for the pointer type.
  // For every load and store, we'll need to be able to look up the type ID
  // being accessed and mark any alignment
  // requirements.
  switch (op) {
  case OpConvertUToPtr:
  case OpBitcast:
  case OpCompositeExtract:
    // Extract can begin a new chain if we had a struct or array of pointers as
    // input.
...
uint32_t
Compiler::PhysicalStorageBufferPointerHandler::get_base_non_block_type_id(
    uint32_t type_id) const {
  auto *type = &compiler.get<SPIRType>(type_id);
  while (type->pointer &&
...
void Compiler::PhysicalStorageBufferPointerHandler::
    analyze_non_block_types_from_block(const SPIRType &type) {
...
void Compiler::analyze_non_block_pointer_types() {
  ...
  // physical pointers to POD types which we never used, and thus never added to
  // the list.
  ...
  physical_storage_non_block_pointer_types.reserve(
      handler.non_block_types.size());
  for (auto type : handler.non_block_types)
    physical_storage_non_block_pointer_types.push_back(type);
  sort(begin(physical_storage_non_block_pointer_types),
       end(physical_storage_non_block_pointer_types));
  physical_storage_type_to_alignment = move(handler.physical_block_type_meta);
}

bool Compiler::InterlockedResourceAccessPrepassHandler::handle(Op op,
                                                               const uint32_t *,
                                                               uint32_t) {
  if (op == OpBeginInvocationInterlockEXT ||
      op == OpEndInvocationInterlockEXT) {
    if (interlock_function_id != 0 &&
      ...
      uint32_t from_block_id =
          compiler.get<SPIRFunction>(interlock_function_id).entry_block;
      bool outside_control_flow = cfg.node_terminates_control_flow_in_sub_graph(
...
void Compiler::InterlockedResourceAccessPrepassHandler::rearm_current_block(
    const SPIRBlock &block) {
  current_block_id = block.self;
}

bool Compiler::InterlockedResourceAccessPrepassHandler::begin_function_scope(
    const uint32_t *args, uint32_t length) {
  if (length < 3)
    return false;
...
bool Compiler::InterlockedResourceAccessPrepassHandler::end_function_scope(
    const uint32_t *, uint32_t) {
  call_stack.pop_back();
  return true;
}

bool Compiler::InterlockedResourceAccessHandler::begin_function_scope(
    const uint32_t *args, uint32_t length) {
  if (length < 3)
    return false;
...
bool Compiler::InterlockedResourceAccessHandler::end_function_scope(
    const uint32_t *, uint32_t) {
...
void Compiler::InterlockedResourceAccessHandler::access_potential_resource(
    uint32_t id) {
...
bool Compiler::InterlockedResourceAccessHandler::handle(Op opcode,
                                                        const uint32_t *args,
                                                        uint32_t length) {
  ...
  // We need to figure out where images and buffers are loaded from, so do only
  // the bare bones compilation we need.
  switch (opcode) {
  case OpLoad: {
    if (length < 3)
    ...
    case StorageClassUniform:
      ...
      if (!compiler.has_decoration(compiler.get<SPIRType>(var->basetype).self,
  ...
  case OpPtrAccessChain: {
    ...
    auto &type = compiler.get<SPIRType>(result_type);
    if (type.storage == StorageClassUniform ||
        type.storage == StorageClassUniformConstant ||
  ...
  case OpCopyMemory: {
    ...
    if (dst_var && (dst_var->storage == StorageClassUniform ||
                    dst_var->storage == StorageClassStorageBuffer))
      access_potential_resource(dst_var->self);

    if (src_var) {
      if (src_var->storage != StorageClassUniform &&
          src_var->storage != StorageClassStorageBuffer)
        break;

      if (src_var->storage == StorageClassUniform &&
          !compiler.has_decoration(
              compiler.get<SPIRType>(src_var->basetype).self,
              DecorationBufferBlock)) {
        break;
      }
...
void Compiler::analyze_interlocked_resource_usage() {
  if (get_execution_model() == ExecutionModelFragment &&
    ...
    // For GLSL. If we hit any of these cases, we have to fall back to
...
void Compiler::add_loop_level() { current_loop_level++; }
