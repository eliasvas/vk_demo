...
enum class ImageFormatNormalizedState { None = 0, Unorm = 1, Snorm = 2 };

static ImageFormatNormalizedState
image_format_to_normalized_state(ImageFormat fmt) {
  switch (fmt) {
  case ImageFormatR8:
  case ImageFormatR16:
  case ImageFormatRg8:
...
string CompilerHLSL::image_type_hlsl_modern(const SPIRType &type, uint32_t id) {
  ...
  bool force_image_srv = hlsl_options.nonwritable_uav_texture_as_srv &&
                         has_decoration(id, DecorationNonWritable);
  ...
  case DimRect:
    SPIRV_CROSS_THROW(
        "Rectangle texture support is not yet implemented for HLSL."); // TODO
  case DimBuffer:
    if (type.image.sampled == 1)
      return join("Buffer<", type_to_glsl(imagetype), components, ">");
    else if (type.image.sampled == 2) {
      if (interlocked_resources.count(id))
        return join("RasterizerOrderedBuffer<",
                    image_format_to_type(type.image.format, imagetype.basetype),
                    ">");
      ...
      return join(rw, "Buffer<",
                  typed_load ? image_format_to_type(type.image.format,
                                                    imagetype.basetype)
                             : join(type_to_glsl(imagetype), components),
                  ">");
    } else
      SPIRV_CROSS_THROW("Sampler buffers must be either sampled or unsampled. "
                        "Cannot deduce in runtime.");
  case DimSubpassData:
    dim = "2D";
    typed_load = false;
    break;
...
string CompilerHLSL::type_to_glsl(const SPIRType &type, uint32_t id) {
  ...
  } else if (type.vecsize > 1 && type.columns == 1) // Vector builtin
  {
    ...
    case SPIRType::Half:
      return join(hlsl_options.enable_16bit_types ? "half" : "min16float",
                  type.vecsize);
    case SPIRType::Short:
      return join(hlsl_options.enable_16bit_types ? "int16_t" : "min16int",
                  type.vecsize);
    case SPIRType::UShort:
      return join(hlsl_options.enable_16bit_types ? "uint16_t" : "min16uint",
                  type.vecsize);
    case SPIRType::Float:
...
void CompilerHLSL::emit_interface_block_globally(const SPIRVariable &var) {
  ...
  // The global copies of I/O variables should not contain interpolation
  // qualifiers.
...
const char *CompilerHLSL::to_storage_qualifiers_glsl(const SPIRVariable &var) {
  ...
  // The variables are declared as global, private variables, and do not need
  // any qualifiers.
...
void CompilerHLSL::emit_builtin_outputs_in_struct() {
  ...
  active_output_builtins.for_each_bit([&](uint32_t i) {
    ...
    case BuiltInPosition:
      type = is_position_invariant() && backend.support_precise_qualifier
                 ? "precise float4"
                 : "float4";
    ...
    case BuiltInFragDepth:
      ...
      if (legacy) {
        ...
        if (hlsl_options.shader_model >= 50 &&
            execution.flags.get(ExecutionModeDepthGreater))
          semantic = "SV_DepthGreaterEqual";
        else if (hlsl_options.shader_model >= 50 &&
    ...
    case BuiltInClipDistance:
      // HLSL is a bit weird here, use SV_ClipDistance0, SV_ClipDistance1 and so
      // on with vectors.
      for (uint32_t clip = 0; clip < clip_distance_count; clip += 4) {
        ...
        statement(types[to_declare - 1], " ",
                  builtin_to_glsl(builtin, StorageClassOutput), semantic_index,
                  " : SV_ClipDistance", semantic_index, ";");
      }
      break;

    case BuiltInCullDistance:
      // HLSL is a bit weird here, use SV_CullDistance0, SV_CullDistance1 and so
      // on with vectors.
    ...
    case BuiltInPointSize:
      ...
      // PointSize does not exist in HLSL, but some code bases might want to be
      // able to use these shaders,
    ...
    if (type && semantic)
      statement(type, " ", builtin_to_glsl(builtin, StorageClassOutput), " : ",
                semantic, ";");
  });
}
  ...
  active_input_builtins.for_each_bit([&](uint32_t i) {
    ...
    case BuiltInClipDistance:
      ...
      for (uint32_t clip = 0; clip < clip_distance_count; clip += 4) {
        ...
        static const char *types[] = {"float", "float2", "float3", "float4"};
        statement(types[to_declare - 1], " ",
                  builtin_to_glsl(builtin, StorageClassInput), semantic_index,
                  " : SV_ClipDistance", semantic_index, ";");
      }
      break;
    ...
    case BuiltInPointCoord:
      // PointCoord is not supported, but provide a way to just ignore that,
      // similar to PointSize.
    ...
    if (type && semantic)
      statement(type, " ", builtin_to_glsl(builtin, StorageClassInput), " : ",
                semantic, ";");
  });
}
...
uint32_t CompilerHLSL::type_to_consumed_locations(const SPIRType &type) const {
  ...
  if (type.basetype == SPIRType::Struct) {
    for (uint32_t i = 0; i < uint32_t(type.member_types.size()); i++)
      elements +=
          type_to_consumed_locations(get<SPIRType>(type.member_types[i]));
  } else {
...
std::string CompilerHLSL::to_semantic(uint32_t location, ExecutionModel em,
                                      StorageClass sc) {
  if (em == ExecutionModelVertex && sc == StorageClassInput) {
    // We have a vertex attribute - we should look at remapping it if the user
    // provided
...
std::string CompilerHLSL::to_initializer_expression(const SPIRVariable &var) {
  // We cannot emit static const initializer for block constants for practical
  // reasons,
  // so just inline the initializer.
  // FIXME: There is a theoretical problem here if someone tries to composite
  // extract
  // into this initializer since we don't declare it properly, but that is
...
void CompilerHLSL::emit_interface_block_member_in_struct(
    const SPIRVariable &var, uint32_t member_index, uint32_t location,
    std::unordered_set<uint32_t> &active_locations) {
  auto &execution = get_entry_point();
  auto type = get<SPIRType>(var.basetype);
  auto semantic = to_semantic(location, execution.model, var.storage);
  auto mbr_name =
      join(to_name(type.self), "_", to_member_name(type, member_index));
  auto &mbr_type = get<SPIRType>(type.member_types[member_index]);

  statement(to_interpolation_qualifiers(
                get_member_decoration_bitset(type.self, member_index)),
            type_to_glsl(mbr_type), " ", mbr_name, type_to_array_glsl(mbr_type),
            " : ", semantic, ";");
...
void CompilerHLSL::emit_interface_block_in_struct(
  ...
  if (execution.model == ExecutionModelFragment &&
    ...
    if (index != 0 && location != 0)
      SPIRV_CROSS_THROW(
          "Dual-source blending is only supported on MRT #0 in HLSL.");

    binding = join(legacy ? "COLOR" : "SV_Target", location + index);
    use_location_number = false;
    if (legacy) // COLOR must be a four-component vector on legacy shader model
                // targets (HLSL ERR_COLOR_4COMP)
      type.vecsize = 4;
  }
  ...
  bool need_matrix_unroll = var.storage == StorageClassInput &&
...
std::string CompilerHLSL::builtin_to_glsl(spv::BuiltIn builtin,
  ...
  case BuiltInNumWorkgroups: {
    if (!num_workgroups_builtin)
      SPIRV_CROSS_THROW("NumWorkgroups builtin is used, but "
                        "remap_num_workgroups_builtin() was not called. "
                        "Cannot emit code for this builtin.");

    auto &var = get<SPIRVariable>(num_workgroups_builtin);
    auto &type = get<SPIRType>(var.basetype);
    auto ret = join(to_name(num_workgroups_builtin), "_",
                    get_member_name(type.self, 0));
    ParsedIR::sanitize_underscores(ret);
    return ret;
  }
  case BuiltInPointCoord:
    // Crude hack, but there is no real alternative. This path is only enabled
    // if point_coord_compat is set.
...
void CompilerHLSL::emit_builtin_variables() {
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
    if (!is_builtin_variable(var) || var.storage != StorageClassOutput ||
        !var.initializer)
      return;
    ...
    if (type.basetype == SPIRType::Struct) {
      ...
      for (uint32_t i = 0; i < member_count; i++) {
        if (has_member_decoration(type.self, i, DecorationBuiltIn)) {
          builtin_to_initializer[get_member_decoration(
              type.self, i, DecorationBuiltIn)] = c->subconstants[i];
        }
      }
    } else if (has_decoration(var.self, DecorationBuiltIn))
      builtin_to_initializer[get_decoration(var.self, DecorationBuiltIn)] =
          var.initializer;
  });

  // Emit global variables for the interface variables which are statically used
  // by the shader.
  builtins.for_each_bit([&](uint32_t i) {
    ...
    case BuiltInPointSize:
      if (hlsl_options.point_size_compat) {
        ...
        SPIRV_CROSS_THROW(
            join("Unsupported builtin in HLSL: ", unsigned(builtin)));
    ...
    StorageClass storage =
        active_input_builtins.get(i) ? StorageClassInput : StorageClassOutput;
    ...
    // SampleMask can be both in and out with sample builtin, in this case we
    // have already
    // declared the input variable and we need to add the output one now.
    if (builtin == BuiltInSampleMask && storage == StorageClassInput &&
        this->active_output_builtins.get(i)) {
      statement("static ", type, " ",
                this->builtin_to_glsl(builtin, StorageClassOutput), init_expr,
                ";");
    }
  });
...
void CompilerHLSL::emit_composite_constants() {
  ...
  ir.for_each_typed_id<SPIRConstant>([&](uint32_t, SPIRConstant &c) {
    ...
    if (type.basetype == SPIRType::Struct || !type.array.empty()) {
      ...
      statement("static const ", variable_decl(type, name), " = ",
  ...
  for (auto &id_ : ir.ids_for_constant_or_type) {
    ...
    if (id.get_type() == TypeConstant) {
      ...
      if (c.self == workgroup_size_id) {
        ...
        // HLSL does not support specialization constants, so fallback to
        // macros.
...
void CompilerHLSL::emit_resources() {
  ...
  if (execution.model == ExecutionModelVertex &&
      hlsl_options.shader_model <= 30) {
    statement("uniform float4 gl_HalfPixel;");
    emitted = true;
  }

  bool skip_separate_image_sampler =
      !combined_image_samplers.empty() || hlsl_options.shader_model <= 30;
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
    ...
    if (var.storage != StorageClassFunction && !is_builtin_variable(var) &&
  ...
  const auto variable_compare =
      [&](const IOVariable &a, const IOVariable &b) -> bool {
        // Sort input and output variables based on, from more robust to less
        // robust:
        // - Location
        ...
        // - Fallback: ID
        bool has_location_a =
            a.block || has_decoration(a.var->self, DecorationLocation);
        bool has_location_b =
            b.block || has_decoration(b.var->self, DecorationLocation);
  ...
  if (!input_variables.empty() || !input_builtins.empty()) {
    ...
    for (auto &var : input_variables) {
      if (var.block)
        emit_interface_block_member_in_struct(*var.var, var.block_member_index,
                                              var.location, active_inputs);
      else
  ...
  for (uint32_t norm = 0; norm < 3; norm++) {
    for (uint32_t comp = 0; comp < 4; comp++) {
      ...
      emit_texture_size_variants(required_texture_size_variants.uav[norm][comp],
                                 vecsizes[comp], true, qualifiers[norm]);
    }
  }
  ...
  // HLSL does not seem to have builtins for these operation, so roll them by
  // hand ...
  if (requires_unorm8_packing) {
    ...
    statement("return Packed.x | (Packed.y << 8) | (Packed.z << 16) | "
              "(Packed.w << 24);");
    end_scope();
    statement("");
    ...
    begin_scope();
    statement("uint4 Packed = uint4(value & 0xff, (value >> 8) & 0xff, (value "
              ">> 16) & 0xff, value >> 24);");
  ...
  if (requires_snorm8_packing) {
    ...
    statement("return uint(Packed.x | (Packed.y << 8) | (Packed.z << 16) | "
              "(Packed.w << 24));");
    end_scope();
    statement("");
    ...
    statement("int4 Packed = int4(SignedValue << 24, SignedValue << 16, "
              "SignedValue << 8, SignedValue) >> 24;");
  ...
  if (requires_snorm16_packing) {
    ...
    begin_scope();
    statement("int2 Packed = int2(round(clamp(value, -1.0, 1.0) * 32767.0)) & "
              "0xffff;");
  ...
  if (requires_bitfield_insert) {
    ...
    for (auto &type : types) {
      statement(type, " spvBitfieldInsert(", type, " Base, ", type,
                " Insert, uint Offset, uint Count)");
      begin_scope();
      statement("uint Mask = Count == 32 ? 0xffffffff : (((1u << Count) - 1) "
                "<< (Offset & 31));");
  ...
  if (requires_bitfield_extract) {
    ...
    for (auto &type : unsigned_types) {
      statement(type, " spvBitfieldUExtract(", type,
    ...
    // In this overload, we will have to do sign-extension, which we will
    // emulate by shifting up and down.
    static const char *signed_types[] = {"int", "int2", "int3", "int4"};
    for (auto &type : signed_types) {
      statement(type, " spvBitfieldSExtract(", type,
  ...
  if (requires_inverse_2x2) {
    ...
    statement("// adjoint and dividing by the determinant. The contents of the "
              "matrix are changed.");
    ...
    statement_no_indent("");
    statement("// Calculate the determinant as a combination of the cofactors "
              "of the first row.");
  ...
  if (requires_inverse_4x4) {
    ...
    statement("float spvDet3x3(float a1, float a2, float a3, float b1, float "
              ...
              "float c2, float c3)");
    begin_scope();
    statement("return a1 * spvDet2x2(b2, b3, c2, c3) - b1 * spvDet2x2(a2, a3, "
              "c2, c3) + c1 * "
              "spvDet2x2(a2, a3, "
              "b2, b3);");
    end_scope();
  ...
  for (TypeID type_id : composite_selection_workaround_types) {
    ...
    statement("void spvSelectComposite(out ", type_str, " out_value",
              type_arr_str, ", bool cond, ", type_str, " true_val",
              type_arr_str, ", ", type_str, " false_val", type_arr_str, ")");
    begin_scope();
    statement("if (cond)");
    begin_scope();
...
void CompilerHLSL::emit_texture_size_variants(uint64_t variant_mask,
                                              const char *vecsize_qualifier,
                                              bool uav,
  ...
  static const char *dims[QueryDimCount] = {
      "Texture1D",   "Texture1DArray",  "Texture2D",   "Texture2DArray",
      "Texture3D",   "Buffer",          "TextureCube", "TextureCubeArray",
      "Texture2DMS", "Texture2DMSArray"};

  static const bool has_lod[QueryDimCount] = {true,  true, true, true,  true,
                                              false, true, true, false, false};

  static const char *ret_types[QueryDimCount] = {
      "uint", "uint2", "uint2", "uint3", "uint3",
      "uint", "uint2", "uint3", "uint2", "uint3",
  };
  ...
  for (uint32_t index = 0; index < QueryDimCount; index++) {
    for (uint32_t type_index = 0; type_index < QueryTypeCount; type_index++) {
      ...
      statement(ret_types[index], " spv", (uav ? "Image" : "Texture"), "Size(",
                (uav ? "RW" : ""), dims[index], "<", type_qualifier,
                types[type_index], vecsize_qualifier, "> Tex, ",
...
string CompilerHLSL::layout_for_member(const SPIRType &type, uint32_t index) {
  ...
  // Flip the convention. HLSL is a bit odd in that the memory layout is column
  // major ... but the language API is "row-major".
  // The way to deal with this is to multiply everything in inverse order, and
  // reverse the memory layout.
...
void CompilerHLSL::emit_struct_member(const SPIRType &type,
  ...
  if ((has_extended_decoration(type.self, SPIRVCrossDecorationExplicitOffset) ||
    ...
    packing_offset = join(" : packoffset(c", offset / 16,
                          packing_swizzle[(offset & 15) >> 2], ")");
  }
...
void CompilerHLSL::emit_buffer_block(const SPIRVariable &var) {
  ...
  bool is_uav = var.storage == StorageClassStorageBuffer ||
                has_decoration(type.self, DecorationBufferBlock);

  if (is_uav) {
    Bitset flags = ir.get_buffer_block_flags(var);
    bool is_readonly = flags.get(DecorationNonWritable) &&
                       !is_hlsl_force_storage_buffer_as_uav(var.self);
    ...
    if (!is_readonly)
      type_name = is_interlocked ? "RasterizerOrderedByteAddressBuffer "
                                 : "RWByteAddressBuffer ";
    add_resource_name(var.self);
    statement(is_coherent ? "globallycoherent " : "", type_name,
              to_name(var.self), type_to_array_glsl(type),
              to_resource_binding(var), ";");
  } else {
    if (type.array.empty()) {
      // Flatten the top-level struct so we can use packoffset,
      // this restriction is similar to GLSL where layout(offset) is not
      // possible on sub-structs.
      ...
      if (buffer_name.empty())
        buffer_name =
            join("_", get<SPIRType>(var.basetype).self, "_", var.self);
      ...
      if (buffer_is_packing_standard(type, BufferPackingHLSLCbufferPackOffset,
        ...
        SPIRV_CROSS_THROW(join("cbuffer ID ", var.self, " (name: ", buffer_name,
                               "), member index ", failed_index, " (name: ",
                               to_member_name(type, failed_index),
                               ") cannot be expressed with either HLSL packing "
      ...
      if (hlsl_options.shader_model < 51)
        SPIRV_CROSS_THROW("Need ConstantBuffer<T> to use arrays of UBOs, but "
                          "this is only supported in SM 5.1.");

      add_resource_name(type.self);
      add_resource_name(var.self);

      // ConstantBuffer<T> does not support packoffset, so it is unuseable
      // unless everything aligns as we expect.
      uint32_t failed_index = 0;
      if (!buffer_is_packing_standard(type, BufferPackingHLSLCbuffer,
                                      &failed_index)) {
        SPIRV_CROSS_THROW(
            join("HLSL ConstantBuffer<T> ID ", var.self, " (name: ",
                 to_name(type.self), "), member index ", failed_index,
                 " (name: ", to_member_name(type, failed_index),
...
void CompilerHLSL::emit_push_constant_block(const SPIRVariable &var) {
  if (root_constants_layout.empty()) {
    ...
    for (const auto &layout : root_constants_layout) {
      ...
      uint32_t failed_index = 0;
      if (buffer_is_packing_standard(type, BufferPackingHLSLCbufferPackOffset,
                                     &failed_index, layout.start, layout.end))
        set_extended_decoration(type.self, SPIRVCrossDecorationExplicitOffset);
      else {
        SPIRV_CROSS_THROW(join("Root constant cbuffer ID ", var.self,
                               " (name: ", to_name(type.self), ")",
                               ", member index ", failed_index, " (name: ",
                               to_member_name(type, failed_index),
                               ") cannot be expressed with either HLSL packing "
                               "layout or packoffset."));
      ...
      statement("cbuffer SPIRV_CROSS_RootConstant_", to_name(var.self),
                to_resource_register(HLSL_BINDING_AUTO_PUSH_CONSTANT_BIT, 'b',
                                     layout.binding, layout.space));
      begin_scope();

      // Index of the next field in the generated root constant constant buffer
      auto constant_index = 0u;

      // Iterate over all member of the push constant and check which of the
      // fields
...
string CompilerHLSL::to_sampler_expression(uint32_t id) {
  ...
  if (index == string::npos) {
    ...
    // We have an expression like _ident[array], so we cannot tack on _sampler,
    // insert it inside the string instead.
...
string CompilerHLSL::to_func_call_arg(const SPIRFunction::Parameter &arg,
                                      uint32_t id) {
  ...
  // Manufacture automatic sampler arg if the arg is a SampledImage texture and
  // we're in modern HLSL.
  auto &type = expression_type(id);

  // We don't have to consider combined image samplers here via OpSampledImage
  // because
...
void CompilerHLSL::emit_function_prototype(SPIRFunction &func,
  ...
  for (auto &arg : func.arguments) {
    ...
    if (hlsl_options.shader_model > 30 &&
        arg_type.basetype == SPIRType::SampledImage &&
        arg_type.image.dim != DimBuffer) {
      // Manufacture automatic sampler arg for SampledImage texture
      arglist.push_back(
          join(is_depth_image(arg_type, arg.id) ? "SamplerComparisonState "
                                                : "SamplerState ",
...
void CompilerHLSL::emit_hlsl_entry_point() {
  ...
  case ExecutionModelGLCompute: {
    ...
    auto x_expr =
        wg_x.id ? get<SPIRConstant>(wg_x.id).specialization_constant_macro_name
                : to_string(x);
    auto y_expr =
        wg_y.id ? get<SPIRConstant>(wg_y.id).specialization_constant_macro_name
                : to_string(y);
    auto z_expr =
        wg_z.id ? get<SPIRConstant>(wg_z.id).specialization_constant_macro_name
                : to_string(z);
  ...
  active_input_builtins.for_each_bit([&](uint32_t i) {
    ...
    case BuiltInFragCoord:
      // VPOS in D3D9 is sampled at integer locations, apply half-pixel offset
      // to be consistent.
      // TODO: Do we need an option here? Any reason why a D3D9 shader would be
      // used
    ...
    case BuiltInSubgroupEqMask:
      ...
      statement("gl_SubgroupEqMask = 1u << (WaveGetLaneIndex() - uint4(0, 32, "
                "64, 96));");
      statement("if (WaveGetLaneIndex() >= 32) gl_SubgroupEqMask.x = 0;");
      statement("if (WaveGetLaneIndex() >= 64 || WaveGetLaneIndex() < 32) "
                "gl_SubgroupEqMask.y = 0;");
      statement("if (WaveGetLaneIndex() >= 96 || WaveGetLaneIndex() < 64) "
                "gl_SubgroupEqMask.z = 0;");
    ...
    case BuiltInSubgroupGeMask:
      // Emulate these ...
      // No 64-bit in HLSL, so have to do it in 32-bit and unroll.
      statement("gl_SubgroupGeMask = ~((1u << (WaveGetLaneIndex() - uint4(0, "
                "32, 64, 96))) - 1u);");
    ...
    case BuiltInSubgroupGtMask:
      ...
      statement("gl_SubgroupGtMask = ~((1u << (gt_lane_index - uint4(0, 32, "
                "64, 96))) - 1u);");
    ...
    case BuiltInSubgroupLeMask:
      ...
      statement("gl_SubgroupLeMask = (1u << (le_lane_index - uint4(0, 32, 64, "
                "96))) - 1u;");
    ...
    case BuiltInSubgroupLtMask:
      // Emulate these ...
      // No 64-bit in HLSL, so have to do it in 32-bit and unroll.
      statement("gl_SubgroupLtMask = (1u << (WaveGetLaneIndex() - uint4(0, 32, "
                "64, 96))) - 1u;");
    ...
    case BuiltInClipDistance:
      for (uint32_t clip = 0; clip < clip_distance_count; clip++)
        statement("gl_ClipDistance[", clip, "] = stage_input.gl_ClipDistance",
                  clip / 4, ".", "xyzw"[clip & 3], ";");
      break;

    case BuiltInCullDistance:
      for (uint32_t cull = 0; cull < cull_distance_count; cull++)
        statement("gl_CullDistance[", cull, "] = stage_input.gl_CullDistance",
                  cull / 4, ".", "xyzw"[cull & 3], ";");
      break;
  ...
  ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
    ...
    if (!var.remapped_variable && type.pointer && !is_builtin_variable(var) &&
      ...
      if (block) {
        ...
        for (uint32_t mbr_idx = 0; mbr_idx < uint32_t(type.member_types.size());
             mbr_idx++) {
  ...
  if (require_output) {
    ...
    active_output_builtins.for_each_bit([&](uint32_t i) {
      ...
      case BuiltInClipDistance:
        for (uint32_t clip = 0; clip < clip_distance_count; clip++)
          statement("stage_output.gl_ClipDistance", clip / 4, ".",
                    "xyzw"[clip & 3], " = gl_ClipDistance[", clip, "];");
        break;

      case BuiltInCullDistance:
        for (uint32_t cull = 0; cull < cull_distance_count; cull++)
          statement("stage_output.gl_CullDistance", cull / 4, ".",
                    "xyzw"[cull & 3], " = gl_CullDistance[", cull, "];");
        break;

      default: {
        auto builtin_expr =
            builtin_to_glsl(static_cast<BuiltIn>(i), StorageClassOutput);
      ...
      if (!var.remapped_variable && type.pointer && !is_builtin_variable(var) &&
        ...
        if (block) {
          ...
          for (uint32_t mbr_idx = 0;
               mbr_idx < uint32_t(type.member_types.size()); mbr_idx++) {
            auto mbr_name = to_member_name(type, mbr_idx);
            auto flat_name = join(type_name, "_", mbr_name);
            statement("stage_output.", flat_name, " = ", var_name, ".",
                      mbr_name, ";");
          }
        } else {
          auto name = to_name(var.self);
...
void CompilerHLSL::emit_texture_op(const Instruction &i, bool sparse) {
  ...
  auto img_expr = to_non_uniform_aware_expression(
      combined_image ? combined_image->image : img);
  ...
  if (op == OpImageFetch) {
    if (hlsl_options.shader_model < 40) {
      SPIRV_CROSS_THROW(
          "texelFetch is not supported in HLSL shader model 2/3.");
    }
    texop += img_expr;
    texop += ".Load";
    ...
    if (imgformat.basetype != SPIRType::Float) {
      SPIRV_CROSS_THROW(
          "Sampling non-float textures is not supported in HLSL.");
    }
    ...
    if (hlsl_options.shader_model >= 40) {
      ...
      if (is_depth_image(imgtype, img)) {
        if (gather) {
          ...
          // Assume we want a fixed level, and the only thing we can get in HLSL
          // is SampleCmpLevelZero.
      ...
      case DimSubpassData:
        SPIRV_CROSS_THROW(
            "Buffer texture support is not yet implemented for HLSL"); // TODO
      default:
        SPIRV_CROSS_THROW("Invalid dimension.");
      }
      ...
      if (gather)
        SPIRV_CROSS_THROW(
            "textureGather is not supported in HLSL shader model 2/3.");
      if (offset || coffset)
        SPIRV_CROSS_THROW(
            "textureOffset is not supported in HLSL shader model 2/3.");
  ...
  if (hlsl_options.shader_model < 40) {
    if (combined_image)
      SPIRV_CROSS_THROW("Separate images/samplers are not supported in HLSL "
                        "shader model 2/3.");
  ...
  if (coord_components != coord_type.vecsize)
    coord_expr = to_enclosed_expression(coord) +
                 swizzle(coord_components, expression_type(coord).vecsize);
  else
    coord_expr = to_expression(coord);

  if (proj &&
      hlsl_options.shader_model >=
          40) // Legacy HLSL has "proj" operations which do this for us.
  ...
  if (hlsl_options.shader_model < 40) {
    if (dref) {
      if (imgtype.image.dim != spv::Dim1D && imgtype.image.dim != spv::Dim2D) {
        SPIRV_CROSS_THROW("Depth comparison is only supported for 1D and 2D "
                          "textures in HLSL shader model 2/3.");
      }

      if (grad_x || grad_y)
        SPIRV_CROSS_THROW("Depth comparison is not supported for grad sampling "
                          "in HLSL shader model 2/3.");
    ...
    if (!!lod + !!bias + !!proj > 1)
      SPIRV_CROSS_THROW(
          "Legacy HLSL can only use one of lod/bias/proj modifiers.");
  }

  if (op == OpImageFetch) {
    if (imgtype.image.dim != DimBuffer && !imgtype.image.ms)
      coord_expr = join("int", coord_components + 1, "(", coord_expr, ", ",
                        lod ? to_expression(lod) : string("0"), ")");
  } else
    expr += ", ";
  expr += coord_expr;
  ...
  if (dref && hlsl_options.shader_model >= 40) {
    ...
    if (proj)
      expr += to_enclosed_expression(dref) + " / " +
              to_extract_component_expression(coord, coord_components);
    else
...
string CompilerHLSL::to_resource_binding(const SPIRVariable &var) {
  ...
  // We can remap push constant blocks, even if they don't have any binding
  // decoration.
  if (type.storage != StorageClassPushConstant &&
      !has_decoration(var.self, DecorationBinding))
    return "";
  ...
  case SPIRType::Image:
    if (type.image.sampled == 2 && type.image.dim != DimSubpassData) {
      if (has_decoration(var.self, DecorationNonWritable) &&
          hlsl_options.nonwritable_uav_texture_as_srv) {
        space = 't'; // SRV
        resource_flags = HLSL_BINDING_AUTO_SRV_BIT;
      } else {
        space = 'u'; // UAV
  ...
  uint32_t desc_set = resource_flags == HLSL_BINDING_AUTO_PUSH_CONSTANT_BIT
                          ? ResourceBindingPushConstantDescriptorSet
                          : 0u;
  uint32_t binding = resource_flags == HLSL_BINDING_AUTO_PUSH_CONSTANT_BIT
...
string CompilerHLSL::to_resource_binding_sampler(const SPIRVariable &var) {
  ...
  return to_resource_register(
      HLSL_BINDING_AUTO_SAMPLER_BIT, 's',
      get_decoration(var.self, DecorationBinding),
      get_decoration(var.self, DecorationDescriptorSet));
}

void CompilerHLSL::remap_hlsl_resource_binding(HLSLBindingFlagBits type,
                                               uint32_t &desc_set,
                                               uint32_t &binding) {
...
string CompilerHLSL::to_resource_register(HLSLBindingFlagBits flag, char space,
                                          uint32_t binding,
                                          uint32_t space_set) {
  if ((flag & resource_binding_flags) == 0) {
    remap_hlsl_resource_binding(flag, space_set, binding);

    // The push constant block did not have a binding, and there were no remap
    // for it,
...
void CompilerHLSL::emit_modern_uniform(const SPIRVariable &var) {
  ...
  case SPIRType::Image: {
    ...
    statement(is_coherent ? "globallycoherent " : "",
              image_type_hlsl_modern(type, var.self), " ", to_name(var.self),
              type_to_array_glsl(type), to_resource_binding(var), ";");
    ...
    if (type.basetype == SPIRType::SampledImage &&
      ...
      if (is_depth_image(type, var.self))
        statement("SamplerComparisonState ", to_sampler_expression(var.self),
                  type_to_array_glsl(type), to_resource_binding_sampler(var),
                  ";");
      else
  ...
  case SPIRType::Sampler:
    if (comparison_ids.count(var.self))
      ...
      statement("SamplerState ", to_name(var.self), type_to_array_glsl(type),
                to_resource_binding(var), ";");
    break;
...
void CompilerHLSL::emit_legacy_uniform(const SPIRVariable &var) {
  ...
  case SPIRType::Image:
    SPIRV_CROSS_THROW(
        "Separate image and samplers not supported in legacy HLSL.");
...
string CompilerHLSL::bitcast_glsl_op(const SPIRType &out_type,
...
void CompilerHLSL::emit_glsl_op(uint32_t result_type, uint32_t id, uint32_t eop,
  ...
  case GLSLstd450FindILsb: {
    ...
    emit_unary_func_op_cast(result_type, id, args[0], "firstbitlow", basetype,
                            basetype);
    break;
  }

  case GLSLstd450FindSMsb:
    emit_unary_func_op_cast(result_type, id, args[0], "firstbithigh", int_type,
                            int_type);
    break;

  case GLSLstd450FindUMsb:
    emit_unary_func_op_cast(result_type, id, args[0], "firstbithigh", uint_type,
                            uint_type);
    break;
...
void CompilerHLSL::read_access_chain_array(const string &lhs,
                                           const SPIRAccessChain &chain) {
  auto &type = get<SPIRType>(chain.basetype);

  // Need to use a reserved identifier here since it might shadow an identifier
  // in the access chain input or other loops.
  auto ident = get_unique_identifier();

  statement("[unroll]");
  statement("for (int ", ident, " = 0; ", ident, " < ",
            to_array_size(type, uint32_t(type.array.size() - 1)), "; ", ident,
            "++)");
  begin_scope();
  auto subchain = chain;
  ...
  if (!get<SPIRType>(subchain.basetype).array.empty())
    subchain.array_stride =
        get_decoration(subchain.basetype, DecorationArrayStride);
  read_access_chain(nullptr, join(lhs, "[", ident, "]"), subchain);
  end_scope();
}

void CompilerHLSL::read_access_chain_struct(const string &lhs,
                                            const SPIRAccessChain &chain) {
  auto &type = get<SPIRType>(chain.basetype);
  auto subchain = chain;
  uint32_t member_count = uint32_t(type.member_types.size());
  ...
  for (uint32_t i = 0; i < member_count; i++) {
    ...
    if (member_type.columns > 1) {
      ...
      subchain.row_major_matrix =
          has_member_decoration(type.self, i, DecorationRowMajor);
    }

    if (!member_type.array.empty())
      subchain.array_stride = type_struct_member_array_stride(type, i);

    read_access_chain(nullptr, join(lhs, ".", to_member_name(type, i)),
                      subchain);
  }
}

void CompilerHLSL::read_access_chain(string *expr, const string &lhs,
                                     const SPIRAccessChain &chain) {
  auto &type = get<SPIRType>(chain.basetype);
  ...
  if (!type.array.empty()) {
    ...
    SPIRV_CROSS_THROW("Reading types other than 32-bit from ByteAddressBuffer "
                      "not yet supported, unless SM 6.2 and "
  ...
  if (type.columns == 1 && !chain.row_major_matrix) {
    ...
    load_expr = join(base, ".", load_op, template_expr, "(",
    ...
    if (templated_load) {
      ...
      // Note, this loading style in HLSL is *actually* row-major, but we always
      // treat matrices as transposed in this backend,
    ...
    for (uint32_t c = 0; c < type.columns; c++) {
      load_expr +=
          join(base, ".", load_op, template_expr, "(", chain.dynamic_index,
    ...
    // Pick out elements one by one ... Hopefully compilers are smart enough to
    // recognize this pattern
    // considering HLSL is "row-major decl", but "column-major" memory layout
    // (basically implicit transpose model, ugh) ...
    ...
    for (uint32_t c = 0; c < type.columns; c++) {
      for (uint32_t r = 0; r < type.vecsize; r++) {
        load_expr += join(
            base, ".Load", template_expr, "(", chain.dynamic_index,
...
void CompilerHLSL::emit_load(const Instruction &instruction) {
  ...
  if (chain) {
    ...
    if (composite_load) {
      // We cannot make this work in one single expression as we might have
      ...
      // Do not forward complex load sequences like matrices, structs and
      // arrays.
      if (type.columns > 1)
        forward = false;
...
void CompilerHLSL::write_access_chain_array(
    const SPIRAccessChain &chain, uint32_t value,
...
void CompilerHLSL::write_access_chain_struct(
    const SPIRAccessChain &chain, uint32_t value,
    const SmallVector<uint32_t> &composite_chain) {
  auto &type = get<SPIRType>(chain.basetype);
  uint32_t member_count = uint32_t(type.member_types.size());
...
string CompilerHLSL::write_access_chain_value(
    uint32_t value, const SmallVector<uint32_t> &composite_chain,
    bool enclose) {
  string ret;
  if (composite_chain.empty())
    ...
    ret = access_chain_internal(
        value, composite_chain.data(), uint32_t(composite_chain.size()),
...
void CompilerHLSL::write_access_chain(
  ...
  if (!type.array.empty()) {
    ...
    SPIRV_CROSS_THROW("Writing types other than 32-bit to RWByteAddressBuffer "
                      "not yet supported, unless SM 6.2 and "
                      "native 16-bit types are enabled.");
  ...
  if (type.columns == 1 && !chain.row_major_matrix) {
    ...
    for (uint32_t c = 0; c < type.columns; c++) {
      auto store_expr = join(
          write_access_chain_value(value, composite_chain, true), "[", c, "]");
    ...
    for (uint32_t r = 0; r < type.vecsize; r++) {
      for (uint32_t c = 0; c < type.columns; c++) {
        auto store_expr =
            join(write_access_chain_value(value, composite_chain, true), "[", c,
                 "].", index_to_swizzle(r));
        remove_duplicate_swizzle(store_expr);
        auto bitcast_op = bitcast_glsl_op(target_type, type);
        if (!bitcast_op.empty())
          store_expr = join(bitcast_op, "(", store_expr, ")");
        statement(base, ".Store", template_expr, "(", chain.dynamic_index,
                  chain.static_index + c * (type.width / 8) +
                      r * chain.matrix_stride,
                  ", ", store_expr, ");");
      }
    }
  }
...
void CompilerHLSL::emit_access_chain(const Instruction &instruction) {
  ...
  if (chain) {
    ...
    need_byte_access_chain = true;
             ...
             has_decoration(type.self, DecorationBufferBlock)) {
    // If we are starting to poke into an SSBO, we are dealing with
    // ByteAddressBuffers, and we need
  ...
  if (need_byte_access_chain) {
    // If we have a chain variable, we are already inside the SSBO, and any
    // array type will refer to arrays within a block,
    // and not array of SSBO.
    uint32_t to_plain_buffer_length =
        chain ? 0u : static_cast<uint32_t>(type.array.size());

    auto *backing_variable = maybe_get_backing_variable(ops[2]);

    string base;
    if (to_plain_buffer_length != 0)
      base = access_chain(ops[2], &ops[3], to_plain_buffer_length,
                          get<SPIRType>(ops[0]));
    else if (chain)
      base = chain->base;
    else
    ...
    auto offsets = flattened_access_chain_offset(
        *basetype, &ops[3 + to_plain_buffer_length],
        length - 3 - to_plain_buffer_length, 0, 1, &row_major_matrix,
        &matrix_stride, &array_stride);

    auto &e = set<SPIRAccessChain>(ops[1], ops[0], type.storage, base,
...
void CompilerHLSL::emit_atomic(const uint32_t *ops, uint32_t length,
                               spv::Op op) {
  const char *atomic_op = nullptr;

  string value_expr;
  if (op != OpAtomicIDecrement && op != OpAtomicIIncrement &&
      op != OpAtomicLoad && op != OpAtomicStore)
  ...
  if (is_atomic_store) {
    ...
    if (!tmp_id) {
      ...
      emit_uninitialized_temporary_expression(get_pointee_type(data_type).self,
                                              tmp_id);
    }
    ...
    if (data_type.storage == StorageClassImage || !chain) {
      ...
      statement(base, ".", atomic_op, "(", chain->dynamic_index,
                chain->static_index, ", ", to_expression(ops[3]), ", ",
    ...
    if (data_type.storage == StorageClassImage || !chain) {
      statement(atomic_op, "(", to_non_uniform_aware_expression(ops[2]), ", ",
      ...
      statement(base, ".", atomic_op, "(", chain->dynamic_index,
                chain->static_index, ", ", value_expr, ", ", to_name(id), ");");
    }
...
#define make_inclusive_Max(expr) ""
  ...
  case OpGroupNonUniformBallotBitCount: {
    ...
    if (operation == GroupOperationReduce) {
      ...
      auto left =
          join("countbits(", to_enclosed_expression(ops[4]), ".x) + countbits(",
               to_enclosed_expression(ops[4]), ".y)");
      auto right =
          join("countbits(", to_enclosed_expression(ops[4]), ".z) + countbits(",
      ...
      SPIRV_CROSS_THROW(
          "Cannot trivially implement BallotBitCount Inclusive Scan in HLSL.");
    else if (operation == GroupOperationExclusiveScan)
      SPIRV_CROSS_THROW(
          "Cannot trivially implement BallotBitCount Exclusive Scan in HLSL.");
    else
  ...
  case OpGroupNonUniformShuffleXor: {
    ...
    emit_op(ops[0], ops[1],
            join("WaveReadLaneAt(", to_unpacked_expression(ops[3]), ", ",
                 "WaveGetLaneIndex() ^ ", to_enclosed_expression(ops[4]), ")"),
            forward);
...
#define HLSL_UFOP(op) emit_unary_func_op(ops[0], ops[1], ops[2], #op)
  ...
  case OpSelect: {
    ...
    if (value_type.basetype == SPIRType::Struct || is_array(value_type)) {
      ...
      auto itr =
          std::find(composite_selection_workaround_types.begin(),
      ...
      statement("spvSelectComposite(", to_expression(ops[1]), ", ",
  ...
  case OpMatrixTimesVector: {
    // Matrices are kept in a transposed state all the time, flip multiplication
    // order always.
  ...
  case OpFUnordEqual: {
    ...
    // We cannot implement UnordEqual as !(OrdNotEqual), as HLSL cannot express
    // OrdNotEqual.
  ...
  case OpFUnordNotEqual: {
    ...
    // We would need to do something like not(UnordEqual), but that cannot be
    // expressed either.
    // Adding a lot of NaN checks would be a breaking change from perspective of
    // performance.
  ...
  case OpImageQuerySize: {
    ...
    if (const auto *var = maybe_get_backing_variable(ops[2]))
      if (hlsl_options.nonwritable_uav_texture_as_srv &&
          has_decoration(var->self, DecorationNonWritable))
        uav = false;
    ...
    string expr;
    if (uav)
      expr = join("spvImageSize(", to_non_uniform_aware_expression(ops[2]),
                  ", ", dummy_samples_levels, ")");
    else
      expr = join("spvTextureSize(", to_non_uniform_aware_expression(ops[2]),
                  ", 0u, ", dummy_samples_levels, ")");
  ...
  case OpImageQueryLevels: {
    ...
    if (uav)
      statement("spvImageSize(", to_non_uniform_aware_expression(ops[2]), ", ",
                to_name(id), ");");
    else
      statement("spvTextureSize(", to_non_uniform_aware_expression(ops[2]),
                ", 0u, ", to_name(id), ");");
    ...
    if (subpass_data) {
      ...
      if (type.image.ms) {
        ...
        if (operands != ImageOperandsSampleMask || instruction.length != 6)
          SPIRV_CROSS_THROW("Multisampled image used in OpImageRead, but "
                            "unexpected operand mask was used.");
        uint32_t sample = ops[5];
        imgexpr =
            join(to_non_uniform_aware_expression(ops[2]),
                 ".Load(int2(gl_FragCoord.xy), ", to_expression(sample), ")");
      } else
        imgexpr = join(to_non_uniform_aware_expression(ops[2]),
      ...
      imgexpr = join(to_non_uniform_aware_expression(ops[2]), "[",
                     to_expression(ops[3]), "]");
      // The underlying image type in HLSL depends on the image format, unlike
      // GLSL, where all images are "vec4",
      // except that the underlying type changes how the data is interpreted.

      bool force_srv = hlsl_options.nonwritable_uav_texture_as_srv && var &&
                       has_decoration(var->self, DecorationNonWritable);
      pure = force_srv;
      ...
      if (var && !subpass_data && !force_srv)
        imgexpr = remap_swizzle(get<SPIRType>(result_type),
                                image_format_to_components(
                                    get<SPIRType>(var->basetype).image.format),
                                imgexpr);
    }
  ...
  case OpImageWrite: {
    ...
    if (var) {
      ...
      value_expr = remap_swizzle(narrowed_type, expression_type(ops[2]).vecsize,
                                 value_expr);
    }

    statement(to_non_uniform_aware_expression(ops[0]), "[",
              to_expression(ops[1]), "] = ", value_expr, ";");
  ...
  case OpMemoryBarrier: {
    ...
    if (opcode == OpMemoryBarrier) {
      ...
      if (next && next->op == OpControlBarrier) {
        ...
        // If there are no memory semantics for next instruction, we will imply
        // group shared memory is synced.
        if (next_semantics == 0)
    ...
    if (opcode == OpControlBarrier) {
      // We cannot emit just execution barrier, for no memory semantics pick the
      // cheapest option.
  ...
  case OpBitFieldInsert: {
    ...
    auto expr = join("spvBitfieldInsert(", to_expression(ops[2]), ", ",
  ...
  case OpBitCount: {
    ...
    emit_unary_func_op_cast(ops[0], ops[1], ops[2], "countbits", basetype,
                            basetype);
    break;
  }
  ...
  case OpArrayLength: {
    ...
    emit_uninitialized_temporary_expression(ops[0], ops[1]);
    statement(to_non_uniform_aware_expression(ops[2]), ".GetDimensions(",
              to_expression(ops[1]), ");");
    uint32_t offset = type_struct_member_offset(type, ops[3]);
    uint32_t stride = type_struct_member_array_stride(type, ops[3]);
    statement(to_expression(ops[1]), " = (", to_expression(ops[1]), " - ",
              offset, ") / ", stride, ";");
    break;
  }
...
void CompilerHLSL::require_texture_query_variant(uint32_t var_id) {
  ...
  if (hlsl_options.nonwritable_uav_texture_as_srv &&
      has_decoration(var_id, DecorationNonWritable))
    uav = false;
  ...
  auto &variant =
      uav
          ? required_texture_size_variants.uav[uint32_t(
                norm_state)][image_format_to_components(type.image.format) - 1]
...
void CompilerHLSL::set_root_constant_layouts(
    std::vector<RootConstants> layout) {
...
void CompilerHLSL::validate_shader_model() {
  ...
  for (auto &cap : ir.declared_capabilities) {
    ...
    case CapabilityRuntimeDescriptorArrayEXT:
      if (hlsl_options.shader_model < 51)
        SPIRV_CROSS_THROW("Shader model 5.1 or higher is required to use "
                          "bindless resources or NonUniformResourceIndex.");
      break;

    case CapabilityVariablePointers:
    case CapabilityVariablePointersStorageBuffer:
      SPIRV_CROSS_THROW(
          "VariablePointers capability is not supported in HLSL.");
  ...
  if (hlsl_options.enable_16bit_types && hlsl_options.shader_model < 62)
    SPIRV_CROSS_THROW("Need at least shader model 6.2 when enabling native "
                      "16-bit type support.");
}
...
string CompilerHLSL::compile() {
  ...
  backend.support_precise_qualifier =
      hlsl_options.shader_model >= 50 || hlsl_options.shader_model == 40;
...
void CompilerHLSL::add_hlsl_resource_binding(
    const HLSLResourceBinding &binding) {
  StageSetBinding tuple = {binding.stage, binding.desc_set, binding.binding};
  resource_bindings[tuple] = {binding, false};
}

bool CompilerHLSL::is_hlsl_resource_binding_used(ExecutionModel model,
                                                 uint32_t desc_set,
...
CompilerHLSL::BitcastType CompilerHLSL::get_bitcast_type(uint32_t result_type,
                                                         uint32_t op0) {
  auto &rslt_type = get<SPIRType>(result_type);
  auto &expr_type = expression_type(op0);

  if (rslt_type.basetype == SPIRType::BaseType::UInt64 &&
      expr_type.basetype == SPIRType::BaseType::UInt && expr_type.vecsize == 2)
...
bool CompilerHLSL::is_hlsl_force_storage_buffer_as_uav(ID id) const {
  ...
  return (force_uav_buffer_bindings.find({desc_set, binding}) !=
          force_uav_buffer_bindings.end());
}

void CompilerHLSL::set_hlsl_force_storage_buffer_as_uav(uint32_t desc_set,
                                                        uint32_t binding) {
