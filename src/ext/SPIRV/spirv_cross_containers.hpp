...
public:
  T *data() {
    ...
    // Only use this workaround on MSVC 2013 due to some confusion around
    // default initialized unions.
    // Spec seems to suggest the memory will be zero-initialized, which is *not*
    // what we want.
...
// An immutable version of SmallVector which erases type information about
// storage.
...
// Makes it easier to consume SmallVector.
  ...
  explicit operator std::vector<T>() const {
    // Another MSVC 2013 workaround. It does not understand lvalue/rvalue
  ...
  explicit operator std::vector<T>() && {
    return std::vector<T>(std::make_move_iterator(ptr),
                          std::make_move_iterator(ptr + buffer_size));
  }
#endif
...
// We use a lot of throwaway vectors all over the place which triggers
// allocations.
...
public:
  ...
  SmallVector(const T *arg_list_begin,
              const T *arg_list_end) SPIRV_CROSS_NOEXCEPT : SmallVector() {
  ...
  SmallVector(std::initializer_list<T> init) SPIRV_CROSS_NOEXCEPT
  ...
  void reserve(size_t count) SPIRV_CROSS_NOEXCEPT {
    ...
    if (count > buffer_capacity) {
      ...
      // Weird parens works around macro issues on Windows if NOMINMAX is not
      // used.
      target_capacity = (std::max)(target_capacity, N);

      // Need to ensure there is a POT value of target capacity which is larger
      // than count,
      ...
      T *new_buffer =
          target_capacity > N
              ? static_cast<T *>(malloc(target_capacity * sizeof(T)))
              : stack_storage.data();

      // If we actually fail this malloc, we are hosed anyways, there is no
      // reason to attempt recovery.
      if (!new_buffer)
        std::terminate();
  ...
  void insert(T *itr, const T *insert_begin,
              const T *insert_end) SPIRV_CROSS_NOEXCEPT {
    ...
    if (itr == this->end()) {
      ...
      if (this->buffer_size + count > buffer_capacity) {
        ...
        for (auto *source_itr = insert_begin; source_itr != insert_end;
             ++source_itr, ++target_itr)
        ...
        // Move in place, need to be a bit careful about which elements are
        ...
        // For the inserts which go to already constructed elements, we can do a
        // plain copy.
        while (itr != this->end() && insert_begin != insert_end)
          *itr++ = *insert_begin++;

        // For inserts into newly allocated memory, we must copy-construct
        // instead.
...
public:
  ...
  // Disable copies and moves. Makes it easier to implement, and we don't need
  // it.
  ...
  std::string str() const {
    ...
    ret.insert(ret.end(), current_buffer.buffer,
               current_buffer.buffer + current_buffer.offset);
    return ret;
  }
