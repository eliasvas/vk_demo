...
// Interface which remaps vertex inputs to a fixed semantic name to make linking
// easier.
...
// with unset_decoration().
enum HLSLBindingFlagBits {
  HLSL_BINDING_AUTO_NONE_BIT = 0,

  // Push constant (root constant) resources will be declared as CBVs (b-space)
  // without a register() declaration.
  // A register will be automatically assigned by the D3D compiler, but must
  // therefore be reflected in D3D-land.
  // Push constants do not normally have a DecorationBinding set, but if they
  // do, this can be used to ignore it.
  HLSL_BINDING_AUTO_PUSH_CONSTANT_BIT = 1 << 0,

  // cbuffer resources will be declared as CBVs (b-space) without a register()
  // declaration.
  // A register will be automatically assigned, but must be reflected in
  // D3D-land.
...
// resource types, e.g. COMBINED_IMAGE_SAMPLER, and SRV/Sampler bindings will be
// used respectively.
// On SM 5.0 and lower, register_space is ignored.
//
// To remap a push constant block which does not have any desc_set/binding
// associated with it,
// use ResourceBindingPushConstant{DescriptorSet,Binding} as values for
// desc_set/binding.
// For deeper control of push constants, set_root_constant_layouts() can be used
// instead.
...
public:
  struct Options {
    ...
    // Allows the PointSize builtin, and ignores it, as PointSize is not
    // supported in HLSL.
    bool point_size_compat = false;

    // Allows the PointCoord builtin, returns float2(0.5, 0.5), as PointCoord is
    // not supported in HLSL.
    bool point_coord_compat = false;

    // If true, the backend will assume that VertexIndex and InstanceIndex will
    // need to apply
    ...
    // Forces a storage buffer to always be declared as UAV, even if the
    // readonly decoration is used.
    // By default, a readonly storage buffer will be declared as
    // ByteAddressBuffer (SRV) instead.
    // Alternatively, use set_hlsl_force_storage_buffer_as_uav to specify
    // individually.
    bool force_storage_buffer_as_uav = false;

    // Forces any storage image type marked as NonWritable to be considered an
    // SRV instead.
    // For this to work with function call parameters, NonWritable must be
    // considered to be part of the type system
    // so that NonWritable image arguments are also translated to Texture rather
    // than RWTexture.
    ...
    // If matrices are used as IO variables, flatten the attribute declaration
    // to use
  ...
  CompilerHLSL(const uint32_t *ir_, size_t size) : CompilerGLSL(ir_, size) {}

  explicit CompilerHLSL(const ParsedIR &ir_) : CompilerGLSL(ir_) {}

  explicit CompilerHLSL(ParsedIR &&ir_) : CompilerGLSL(std::move(ir_)) {}

  const Options &get_hlsl_options() const { return hlsl_options; }

  void set_hlsl_options(const Options &opts) { hlsl_options = opts; }
  ...
  // Compiles and remaps vertex attributes at specific locations to a fixed
  // semantic.
  // The default is TEXCOORD# where # denotes location.
  // Matrices are unrolled to vectors with notation ${SEMANTIC}_#, where #
  // denotes row.
  // $SEMANTIC is either TEXCOORD# or a semantic name specified here.
  void
  add_vertex_attribute_remap(const HLSLVertexAttributeRemap &vertex_attributes);
  std::string compile() override;

  // This is a special HLSL workaround for the NumWorkGroups builtin.
  // This does not exist in HLSL, so the calling application must create a dummy
  // cbuffer in
  // which the application will store this builtin.
  // The cbuffer layout will be:
  // cbuffer SPIRV_Cross_NumWorkgroups : register(b#, space#) { uint3
  // SPIRV_Cross_NumWorkgroups_count; };
  // This must be called before compile().
  // The function returns 0 if NumWorkGroups builtin is not statically used in
  // the shader from the current entry point.
  // If non-zero, this returns the variable ID of a cbuffer which corresponds to
  // the cbuffer declared above. By default, no binding or descriptor set
  // decoration is set,
  // so the calling application should declare explicit bindings on this ID
  ...
  // resource is a resource binding to indicate the HLSL CBV, SRV, UAV or
  // sampler binding
  // to use for a particular SPIR-V description set
  // and binding. If resource bindings are provided,
  // is_hlsl_resource_binding_used() will return true after calling ::compile()
  // if
  // the set/binding combination was used by the HLSL code.
  void add_hlsl_resource_binding(const HLSLResourceBinding &resource);
  bool is_hlsl_resource_binding_used(spv::ExecutionModel model, uint32_t set,
                                     uint32_t binding) const;

  // Controls which storage buffer bindings will be forced to be declared as
  // UAVs.
  void set_hlsl_force_storage_buffer_as_uav(uint32_t desc_set,
                                            uint32_t binding);
...
private:
  ...
  void emit_interface_block_in_struct(
      const SPIRVariable &var, std::unordered_set<uint32_t> &active_locations);
  void emit_interface_block_member_in_struct(
  ...
  bool emit_complex_bitcast(uint32_t result_type, uint32_t id,
  ...
  std::string to_resource_register(HLSLBindingFlagBits flag, char space,
  ...
  void read_access_chain(std::string *expr, const std::string &lhs,
                         const SPIRAccessChain &chain);
  void read_access_chain_struct(const std::string &lhs,
                                const SPIRAccessChain &chain);
  void read_access_chain_array(const std::string &lhs,
                               const SPIRAccessChain &chain);
  ...
  void write_access_chain_array(const SPIRAccessChain &chain, uint32_t value,
                                const SmallVector<uint32_t> &composite_chain);
  std::string
  write_access_chain_value(uint32_t value,
                           const SmallVector<uint32_t> &composite_chain,
                           bool enclose);
  ...
  // TODO: Refactor this to be more similar to MSL, maybe have some common
  // system in place?
  ...
  void emit_texture_size_variants(uint64_t variant_mask,
                                  const char *vecsize_qualifier, bool uav,
  ...
  enum BitcastType { TypeNormal, TypePackUint2x32, TypeUnpackUint64 };
  ...
  std::string to_semantic(uint32_t location, spv::ExecutionModel em,
                          spv::StorageClass sc);
  ...
  std::unordered_map<StageSetBinding, std::pair<HLSLResourceBinding, bool>,
                     InternalHasher> resource_bindings;
  void remap_hlsl_resource_binding(HLSLBindingFlagBits type, uint32_t &desc_set,
                                   uint32_t &binding);
  ...
  // SPIR-V, but SV_Coverage is a scalar in HLSL.
